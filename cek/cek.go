		...
		if len(raw) == 0 {
			...
			if b64 == "" {
				if sqlitedrv.EncryptionAvailable() {
					...
					// capable build on a laptop was to know that the key is base64
					// of exactly 32 bytes — and the failure is a hard exit at the
					// FIRST store open, after every subsystem has already wired, so
					// it reads as "the binary is broken" rather than "set a key".
					// That is the whole local-dev experience for anyone who builds
					// from source with a codec linked.
					//
					// It is opt-in, single-purpose and loud: nothing infers it, it
					// only ever RELAXES the no-key case (a malformed or
					// wrong-length key still fails below), and it cannot be reached
					// in a deployment that sets the key. Production posture is
					// unchanged — a capable binary with neither the key nor this
					// flag still refuses to ship plaintext.
					if devUnencrypted() {

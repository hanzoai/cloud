...
11. **A typed op on a fiber WILDCARD does not mis-name a parameter — it refuses
    ...
    `TestTheDoorCannotBeATypedOp` in apps/dns): each registers a typed op on a
...
one schema. `hanzo.js`, `hanzo.ts` and `hanzo.go` are GENERATORS — they are run, and
what they print is the document.
...
- `contract.Load(read)` READS. It refuses a generator spelling by name. Every
  ...
  repository cannot get code executed by naming its declaration `hanzo.ts`.
...
`hanzo.yml` that is its actual contract. Resolution there refuses — loudly, naming
the file and its size. Nothing live reads through this resolver for that repo yet
(`hanzoai/ci` reads `hanzo.yml` by name and the native reactor ships dormant), but
arming the native reactor needs that master renamed to `openapi.yaml` first, which is
what it is. `apps/author` separately uses a repo-root `hanzo.json` carrying
`hanzoAuthorCode` as its proof-of-control artifact; no repo in the estate carries one
today, and the resolution is for that code to become a name IN the contract.

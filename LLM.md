...
- `openapi/` — the document pipeline: the spec is a projection of the live router,
  ...
  hold are DECLARED beside the routes instead, each with its own client and all four
  ...
  operation except the operator's `admin` product, relay endpoints and `compat`
...
- `openapi/index.go` — the same document as HYPERMEDIA. `GET /v1` lists every
  ...
  **A CORS PREFLIGHT IS A DIFFERENT QUESTION AND A DIFFERENT ROUTE.** A preflight
  ...
  one segment down — one fact, not two. **They are MIDDLEWARE on the entry point,
  ...
  that separates an endpoint the host REGISTERS from one it answers ahead of the
  router (manifest.TestNoAppClaimsAHostEndpoint asks the first). The index yields
  ...
  root. The endpoints are still DESCRIBED, through `core()`'s stub router, exactly
  as the agent endpoint is
...
| `/v1/iam` | Identity: users, orgs, roles | `apps/iam` | Shipped — composed with `host.Use` (95 paths / 153 ops / 91 typed with schema; see below) |
...
cost: `TMPDIR=/dev/shm/t GOWORK=off go test -p 1 ./apps/guide` runs the eight-client
...
The host terminates public HTTP, so it owns what no plugin can: it serves the
...
1. **hanzoai/ci's `run:`/`out:` lane indexes per RECIPE, not per FILE.** It writes
   ...
   endpoint's 16-binary bound (`apps/platform/artifact.go:86`) and restate
...
2. **`bucket:` needs `S3_ADMIN_ACCESS_KEY`/`S3_ADMIN_SECRET_KEY` from KMS, and
   ...
   answer being built next endpoint is to stop needing the credential: publish through
...
package under `apps/<name>` that obeys these clients — nothing more.
...
- **Subsystem shape.** A subsystem exposes
  ...
  handles (Logger, DataDir, the subsystem `Client` clients). No subsystem reaches
...
- **Every subsystem is its own binary.** There is no in-process variant left to
  ...
  - `prefixes` is variadic because ONE service commonly owns several route
    subtrees (`base` answers `/v1/base` AND `/v1/waitlist`, both registered by the
    same `Mount` the child runs). Naming only the first 404s the rest AT THE
...
- **Naming lives in hanzoai/namespace, not here.** A namespace, the injective
  ...
  database beside a real one. cloud keeps only the ONE WAY IN: `OrgNamespace` /
...
- **Two extensibility clients (for the tiered-storage perf pass).** The fence's
...
- **the ceiling moved to `apps/ai/cap.go`**, beside the gate that reads it, where
  ...
  one number. `cloud.SetRollingCapReader`/`RollingCapReader` are DELETED: an endpoint
  across a process boundary is not a client, and four snapshots remain where there
  were five.
...
- **The public `/v1/openapi.json` is the HOST's, and it is the WEAVE — not a live
  ...
  right for a process that IS the API; the deployed entry point is not one. The
  ...
  (byte equality) and that a plugin never answers the endpoint.
...
- **The spec IS the router.** `openapi.Live(app)` reads
  ...
  drift guard is `openapi/compose_test.go` (`TestFleetIsTheCompositionOfItsApps`): it
...
- **A RELAY publishes the registry behind it, not the wildcard** (`openapi/relay.go`).
  ...
  `{wildcard1}` command because that is what the document named. The endpoint's owner
  ...
  - Same laws as every other client here: `Register` declares bodies and renders only
    on a live route, `Describe` declares prose and renders only on a live route, a
    relay declares the routes BEHIND a route and renders only on a live endpoint.
  - Four refusals, each naming the source so a wrong placement is traceable to the
    ...
    own endpoint (the routing bug), and a registry that says nothing about a route it
    ...
    openapi/compose.go) — one schema name meaning two things is one law whether the
    claimants are two apps or an app and the registry behind its endpoint.
  - A endpoint YIELDS where the router says it does, in both directions. Inside a
    binary, a specific route the host registered wins over the relay's claim on
    that address. Across the fleet, `Compose` resolves exactly that one overlap and
    reads it off the data — a relayed operation names its own registry in `x-app`,
    a direct one does not. Two specific claims, or two endpoints, stay a refusal.
...
- **`x-app` is PROVENANCE, on every operation**: the registry that registered it.
  ...
  a relayed one it is the module behind the endpoint, so the code is that repo. Written
  once by whichever producer knows (`Project` for a relay, `Compose` for the rest)
...
- **The product axis is mechanical.** The first path segment after `/v1/` IS the
  ...
  - **A subsystem in ANOTHER MODULE is read the same way**, out of the module
    ...
    and only the entry point says what the product is.
  ...
  - **The owner is the app that answers the product's ROOT** — the shallowest path
    under `/v1/<product>` anyone serves (`prose`, openapi/compose.go). Sharing a
...
- **What the router CANNOT tell you — do not try to fix this in the generator.**
  ...
  (Note: `cloud.Typed` is NOT a mount adapter — it is the per-request client that
...
- **`plugin/iam` WAS the extreme case, and is now the counter-example** — see
  "apps/iam — what composing recovered" below. It published 35 operations, every
  ...
  publishes **95 paths / 153 operations / 91 component schemas / 91 MCP tools**,
  because `host.Use` composes the App instead of adapting a handler.
  ...
  them, so composing has nothing to carry until those repos type their own surface —
  and the moment they do, it lands here with no change to cloud. `ai` has LEFT
  that list: it is a `*zip.App` now (`ai/app.go:73 func App`), so `Use` applies
...
one before it.
    ...
    typed registry     ──Typed─▶ Registry  ──Fold─▶  Document   (detail)
                                                    │
      GET /v1/openapi.json · openapi.yaml · `<app> openapi` · Compose
...
- **`openapi.Register` (openapi/register.go) is the reflection client for untyped
  ...
  `apps/cloudflare` (3), `apps/event` (2), `apps/company` (2),
  `apps/destinations` (1), `apps/provisioning` (1) — and rather more registrations,
  because analytics' two sites are a RANGE over its `endpoints` table plus the health
  probe, so an endpoint added there declares itself or fails
  `TestEveryEndpointDeclaresItsBodies` rather than silently publishing nothing.
...
- **A body that is not JSON is declarable too: `openapi.Register(path, method,
  ...
  needs yet, and adding it before one asks is how one client becomes two.
- **A POLYMORPHIC body is declarable too: `openapi.Register(path, method,
  ...
  SEVERAL unrelated JSON shapes on one path. `apps/event`'s canonical ingest
...
- **`Register` cannot carry FIELD PROSE, and that is why the fleet publishes 1,627
  bare properties.** The client derives a schema by reflection and Go drops comments,
  ...
  is the drift `Register` exists to prevent. `apps/event` names its twelve such
...
- **`openapi.Compose` composes the subsets, and its only contribution is the
  REFUSAL** (openapi/compose.go). Two apps may not claim one address, and two apps
  ...
  be routed.** Compose does NOT arbitrate, because there is no policy for who
  should win: an overlap is a bug at the composition root, and the fix is in
  `Wire()`. `TestFleetIsTheCompositionOfItsApps` (compose_test.go:99) proves the
  ...
  the app's own binary can. `TestComposeRefusesTwoAppsAtOneAddress`,
...
- **Both documents are GOLDENS, woven from the per-app subsets.** `make openapi`
  writes them (through the composition,
  `-compose`); `make test`, and therefore CI, verifies it with the same code and no
  flag (`TestFleetIsTheCompositionOfItsApps`, openapi/compose_test.go). Same code path both ways — there is no second
...
- **TWO DOCUMENTS, AND THE NAME SAYS WHICH IS WHICH** (openapi/public.go).
  ...
  ONE run of the composition (`-compose` names the composed document and the projection is
  ...
  - **The public one holds the NAME, because the name is what gets read.**
    ...
    `TestAlphaAndAdminReachOnlyThePrivateDocument` (openapi/compose_test.go) gates
  ...
  - **The audience is DERIVED, not declared** (openapi/public.go `audience`).
    ...
    operator's `admin`, it is not a `{wildcardN}` relay endpoint, and it is not
    ...
    audience — what the SDKs, the CLI, the MCP endpoint and the docs present — never
  ...
  - **Stamped once, at the END of `Spec`** — after `Fold` (which replaces a
    ...
    endpoint with the registry behind it), both of which would discard a mark written
  ...
  - **The MCP endpoint offers the public contract and nothing beside it**:
    plugin/gen-client-catalog keeps an operation only when it is `x-tool` (typed,
...
- **The weave stamps it.** `openapi.Part` carries the app's stage beside its
  name and `Compose` writes `x-stage` onto every operation that part contributed.
...
- **The public contract is ga-only.** `audience` (openapi/public.go) gained the term,
  ...
  end of `Compose` — over the finished composition, the first point at which every
  ...
  `audience` refuses on — `stamp` names them: address, product, endpoint, compat and
  ...
  endpoints 13, the two alpha rows 7 paths / 9 operations, and the non-`/v1`
...
- **The agent endpoint followed, and it had to be MOVED to.**
  `plugin/gen-client-catalog` filters `x-tool AND x-public` — but it read
  ...
  `client/catalog.json` names `admission` and `research` as keys carrying EMPTY
  operation lists, and `group` builds no tool from a name with nothing under it
  (`jq -r 'to_entries|map(select(.value|length==0)|.key)' client/catalog.json`).
...
- **`make -C apps/<app> describe` does NOT regenerate `plugin/<app>/zipdoc_gen.go`.**
  ...
  The tell is a tool the endpoint lists with an empty description.
- **The tool NAME moves with the address.** `create_coding` is
  `create_agent_coding` now: the endpoint renames a derived operation id to the verb
...
so no handler, no store and no client changed — which is the test to run before
...
- **THE DISCOVERY ADDRESS WAS ANSWERING WITH SOMEBODY ELSE'S DOCUMENT.**
  ...
  **`App.Test` cannot see any of this**, and that is the reusable lesson: `prepare()`
  ...
  names zip's title.
  `openapi.Endpoint` gained the third address in the same change, which the fleet-scoping
  gate needed — its own comment already records this exact recurrence ("the second
  endpoint arrived and all three were wrong the same afternoon").
...
is mechanical; skip it and you will rediscover fourteen failure modes the hard way.
...
**v1.31.3** and much of the prose below was written at v1.18.x. Three of the
...
5. **The schema namespace is FLAT, and typing is what makes you enter it.** A
   ...
   `openapi.Compose` refuses one name with two shapes ("every generated SDK would
...
11. **A typed op on a fiber WILDCARD does not mis-name a parameter — it refuses
    the whole document.** Five refusals in this file say a bound In field and the
    published `{wildcardN}` parameter "cannot agree", which reads like a cosmetic
    loss. It is not. zip's own `Template` (v1.31.0 address.go:61) rewrites only
    `:name` segments, so the typed registry publishes the path VERBATIM — `/v1/dns/*`,
    `/v1/kms/secrets/+` — while cloud's router reading names the segment
    `{wildcard1}` (openapi/openapi.go:795-811). `Fold` then looks the op up by zip's
    spelling, finds no live route at that key, and returns "the registry and the
    router disagree about its path" (openapi/openapi.go:687), so `make -C apps/<x>
    describe` fails outright and the app publishes NOTHING. Two tests now RUN it
    rather than assert it (`TestTheWildcardCannotBeATypedOp` in apps/kms,
    `TestTheEndpointCannotBeATypedOp` in apps/dns): each registers a typed op on a
    wildcard and requires the refusal, so the day zip agrees the tests go red and
    say so. **The yielding side is zip's** — one function — because changing
    cloud's spelling would move every `{wildcardN}` path already published across
    five subsystems.

12. **Two of the three classes this file spends the most words on are CLOSED in
    zip v1.31.0, and the refusals written against v1.18.x must be re-read before
    they are believed.** `WithStatus` now takes a SET of codes and the ANSWER
    picks one (`StatusCoder`, typed.go:174-208), which retires the
    multi-status gap (#78) and the "typed-but-shimmed" `cloud.Created` workaround:
    `apps/index`'s health probe answers 200 or 503 over ONE shape by declaring
    both. `WithResponseHeader` + `HeaderCoder` (typed.go:230-260) put a response
    header in the document instead of a middleware side channel. A declared
    NON-2xx carrying the op's own typed body is explicitly supported too, which is
    most of the apps/ml and apps/tools "domain body on a refusal" class — those
    should be re-derived, not inherited. **v1.31.3 closes the third**: `HTTPError.Detail`
    (ctx.go:249) carries RFC 9457 extension members, MERGED under the envelope,
    so a refusal answers with a domain SHAPE instead of a sentence — which is the
    "non-2xx with a domain body" class apps/ml, apps/company, apps/provisioning
    and apps/tools were each refused for. The envelope is written LAST, so a
    domain key called `status` cannot displace the refusal's own; that ordering is
    the half worth a test rather than the feature. `apps/tools` is the worked
    example — see its row in tranche E. What did NOT move, and is what actually
    decides a relay: a typed op's only response path is `c.JSON(out)`
    (typed.go:558) and fiber writes `application/json; charset=utf-8` over
    anything a header coder set (fiber v3 res.go:501-514).

13. **A typed op declared on the GROUP and one declared on the APP with an
    absolute path are the SAME ROUTE — and only the first can carry prose.**
    `registerTyped` composes `scope.Prefix` with the leaf and registers on the
    concrete `*zip.App` either way (`scope.OpScope`, scope.go:298), so
    `zip.Get(g, "/health")` and `zip.Get(app, "/v1/index/health")` produce one
    identical route on one identical router. The absolute form is what an UNTYPED
    handler needs — that one lands on the group NODE — and it is the only form
    `cmd/zipdoc` cannot resolve: it fails naming the call rather than filing prose
    under a path that does not exist. So the "register health absolutely, the
    group does not survive mount composition" idiom (apps/index, clients/esign,
    clients/kms) applies to raw handlers and must be DROPPED when the route is
    typed, or the op ships with no description at all.

14. **The fleet has THREE copies of one client: an error that carries its own
    status and bytes, written back by a middleware installed ahead of the
    leaves.** `cloud.deniedErr` + `DenyEnvelope` (the money wire's 402/503),
    `goja.BundleErr` + `Envelope` (a JS bundle's own envelope) and now
    `index.fault` + `envelope` (the Meilisearch dialect's `{message, code, type,
    link}`, which a client BRANCHES on). All three have the same shape down to the
    `Unwrap` that keeps the refusal honest off the HTTP path. It is the general
    answer to "a typed op's only refusal is a returned error, which zip renders
    flat", so the fourth instance should not be written: ONE type in package
    cloud, carrying (status, bytes, sentence), with the three current owners
    supplying their own constructor. Not done here because retiring the money and
    bundle copies touches captable, dataroom, pricing and ml, which is a different
    commit.
    **The general answer arrived instead, one layer down**: `zip.HTTPError.Detail`
    (v1.31.3) lets a RETURNED error carry its own members, so the client these three
    hand-rolled — status, bytes and a sentence, written back by a middleware ahead
    of the leaves — is what the framework does. A fourth instance is now a refusal
    to reach for the framework rather than a missing type in package cloud, and the
    three that exist are convertible one at a time: each keeps its vocabulary as a
    Detail map and drops its envelope middleware.
    **AND THE MONEY ONE WAS ALREADY DONE, which is the finding worth more than the
    capability.** `cloud.Denied` + `DenyEnvelope` (resource_billing.go) let a typed
    op refuse in the fleet's own `{"error":{"code","message"}}` contract, and
    `serve.go:327` installs the envelope APP-WIDE — so every binary already had it.
    Four packages nonetheless carried a refusal saying a typed op's error "can only
    leave through zip's errorHandler as a flat {status,code,error}", each written
    before that client existed and none re-read after: `apps/company` (`/payment`),
    `apps/ml` (the model create), `apps/provisioning` (all seven creates) and
    `apps/tools`. Two are closed here and the wire is byte-preserved — company's
    `TestPaymentDenialWire` and ml's `TestComputeCreate_RefusesUnfundedOrg` both
    assert the NESTED body and both pass through the typed op unchanged. **Read a
    refusal against the code before believing it**, and note the shape of how this
    one rotted: the fix landed somewhere else and nothing carried it back, so four
    ledgers went on citing a limitation the fleet had already removed. A package's
    own harness must install what `Serve` installs (`cloud.Bridge`, and now
    `cloud.DenyEnvelope`) or a converted op refuses in zip's shape in tests only.
...
| F | ~~plan~~ (done: **15 typed, 0 refused** — the whole `/v1/plans` surface, which published 15 addresses and NOTHING about any of them. It is apps/pricing's shape one subsystem over, so it took pricing's answer: apps/goja re-marshals the bundle's reply with Go's encoding/json before a handler sees it, so decoding and re-marshalling is byte-identical, and `apps/plan/wire_test.go` proves it against the live router for all 12 sections × 3 identity shapes plus BOTH parameterised routes over EVERY id the shipped catalog holds. Opaque catalog values are `json.RawMessage`, not `map[string]any`: zip ≥v1.18.9 asks whether a type marshals itself before asking what it is made of, so a RawMessage publishes `{}` — "any JSON", true — while `map[string]any` publishes `additionalProperties:{"type":"object"}`, which the first `"priceMonthly": 20` in the catalog refutes. Two deltas recorded and PINNED rather than glossed: Content-Type gains `; charset=utf-8` (what every zip error on the surface already sent), and a non-200 body gains zip's `status` field beside the bundle's own message — the same trade main already took for `GET /v1/pricing/model/{name}`, whose 404 is equally first-class. Latent defect found and fixed by typing: the subsystem is named `plan` and serves `/v1/plans`, so `MountPrefixes`'s `/v1/<Name>` default covered NOTHING it registers — measured, `SubsystemOf("/v1/plans")` was `""` and `PriceOf` `undeclared`, and any middleware the subsystem installed (including the typed-op Bridge) landed on `/v1/plan` and never ran. `plugin/plan/main.go` now passes `manifest.PrefixesFor("plan")`; 28 other apps' `/v1/<name>` is likewise absent from their declared prefixes — most legitimately, because they declare deeper subtrees that DO cover their routes, so re-measure per app rather than fixing the list), ~~analytics 13~~ (done: **6 typed, 7 refused**, both ledgers GATED in apps/event/typed_wire_test.go — `untypedByDesign` + `TestEveryRouteIsTypedOrNamed`, whose two ledgers must SUM to the served surface, so a route added untyped here goes red and a stale reason goes red too. The six are the READ lenses: `/v1/analytics/{overview,timeseries,top}`, `/v1/errors`, `/v1/insights/{events,health}`. The seven refusals are one probe and the six ingest endpoints, and they are MEASURED rather than asserted. `GET /v1/analytics/health` answers 503 CARRYING the degraded report as its body — the ml `/health` class — and `TestHealthStillCarriesItsReportAt503` pins the pair a typed op would have to break. The six endpoints share ONE admission decision (`handle`, event.go) resolved entirely from facts that never reach a typed op: the presented credential (Authorization / `x-hanzo-ingest-key` / `?ingest_key=`), the client IP and socket peer the anonymous rate caps key on, DNT/Sec-GPC, and the RAW body length that is the projected lane's 64 KiB→413 bound (`public.go maxPublicBytes`, invisible to a typed op and far below the fleet's global zip BodyLimit). Four of the six add a SECOND, independent blocker: the canonical wire is POLYMORPHIC (`decodeIngest` takes a bare Event object, a bare Event ARRAY, and the `{batch:[…]}`/`{events:[…]}` envelope on one path) and the team SPA's is a bare ARRAY unconditionally — bodies zip's `op.invoke` would 400 where they answer 200 today. `TestArrayBodiedEndpointsStillAnswer200` is that measurement, so when zip can declare a polymorphic body the conversion is a test away rather than a re-derivation. Two latent defects surfaced and were fixed: plugin/event/main.go declared NO `Prefixes`, so the standalone binary's scope owned only the `/v1/<name>` default and five of the app's six prefixes were outside anything it could gate or that `cloud.Declare` could attribute — the pricing defect exactly, one app over; and `apps/analytics` had no `cloud.Bridge` of its own, relying entirely on Serve's app-wide install, which the package's own test harness does not run. A THIRD is fleet-wide and only named here: `SeriesPoint` was about to be a one-name/two-shape collision with apps/admin's `{t,value}` launch-board point, which `openapi.Compose` refuses — analytics yielded to `UsagePoint`, since its schema was not yet published. `TestEveryPublishedFieldIsDescribed` gates the response side: all 19 TYPED schemas carry per-field prose, so `errorRate` says it is a ratio and `pct` says it is a share of the WINDOW rather than of the rows returned. **Re-verified at a later pass against a MOVED surface, and every refusal held** — by then main had folded `/v1/event/collect` away and added the two Sentry endpoints `POST /v1/event/{project}/{envelope,store}`, so the ledger reads 6 typed / 8 refused. The reasons were re-derived from zip v1.18.11's own `typed.go` rather than inherited as prose, and each gained a blocker the first pass had not written down: ORDER. `op.invoke` decodes the body BEFORE the handler is entered, while admission refuses 401 (no resolvable key), then the projected lane refuses 429 (rate), then 413 (64 KiB), with the raw bytes still in hand and nothing parsed — so a typed op would answer 400 to a beacon that is answered 413 or 429 today. Error precedence is wire. What that pass DID change is the other half of the cost: all eight published an operationId and NOTHING else, so every generated SDK offered `post_v1_event` — the endpoint every Hanzo product beacons to — as a call with nowhere to put the event. They declare their bodies now through `openapi.Register`, driven off the `endpoints` table itself so an endpoint cannot be routed with one wire and documented with another, and the gate quantifies over `untypedByDesign` rather than over endpoints (`TestEveryUntypedRouteDeclaresItsBodies`) so a refusal added tomorrow owes its bodies by construction. The canonical four use the new `openapi.OneOf`, because their wire is genuinely three shapes; the two Sentry endpoints use `openapi.Binary` for a request that is an opaque envelope stream and declare NO response, named in `relayed` — they copy back whatever `cloud.ObsErrorIngest` installed, and publishing a shape there would be inventing one. The subset went 0→7 requestBody and 6→12 responses, 19→30 schemas, and the described count did NOT move: 6, exactly the typed ops, because prose, an MCP tool and a CLI command are the three things only zip's registry supplies. Reported as unchanged rather than counted as progress. The health probe's `map[string]any` became `healthReport` in the same move — a map's shape cannot be declared without hand-writing a schema beside it, which is the drift `Register` exists to prevent — and `TestHealthReportKeepsTheMapItReplaced` pins both bodies field-for-field. The one difference is serialization ORDER (Go struct order, not encoding/json's sorted map keys), which the JSON data model does not carry; no field name, value, presence rule or status moved. The cost is a `proseless` ledger of eleven components publishing 63 bare properties, and it may only SHRINK — it caught its own first staleness during this rebase, when main's removal of the Team endpoint left `teamEvent` named and unpublished), the ~70 remaining packages, 1–11 routes each | ~358 |

| F | ~~plan~~ (done: **15 typed, 0 refused** — the whole `/v1/plans` surface, which published 15 addresses and NOTHING about any of them. It is apps/pricing's shape one subsystem over, so it took pricing's answer: apps/goja re-marshals the bundle's reply with Go's encoding/json before a handler sees it, so decoding and re-marshalling is byte-identical, and `apps/plan/wire_test.go` proves it against the live router for all 12 sections × 3 identity shapes plus BOTH parameterised routes over EVERY id the shipped catalog holds. Opaque catalog values are `json.RawMessage`, not `map[string]any`: zip ≥v1.18.9 asks whether a type marshals itself before asking what it is made of, so a RawMessage publishes `{}` — "any JSON", true — while `map[string]any` publishes `additionalProperties:{"type":"object"}`, which the first `"priceMonthly": 20` in the catalog refutes. Two deltas recorded and PINNED rather than glossed: Content-Type gains `; charset=utf-8` (what every zip error on the surface already sent), and a non-200 body gains zip's `status` field beside the bundle's own message — the same trade main already took for `GET /v1/pricing/model/{name}`, whose 404 is equally first-class. Latent defect found and fixed by typing: the subsystem is named `plan` and serves `/v1/plans`, so `MountPrefixes`'s `/v1/<Name>` default covered NOTHING it registers — measured, `SubsystemOf("/v1/plans")` was `""` and `PriceOf` `undeclared`, and any middleware the subsystem installed (including the typed-op Bridge) landed on `/v1/plan` and never ran. `plugin/plan/main.go` now passes `manifest.PrefixesFor("plan")`; 28 other apps' `/v1/<name>` is likewise absent from their declared prefixes — most legitimately, because they declare deeper subtrees that DO cover their routes, so re-measure per app rather than fixing the list), ~~analytics 13~~ (done: **6 typed, 7 refused**, both ledgers GATED in apps/event/typed_wire_test.go — `untypedByDesign` + `TestEveryRouteIsTypedOrNamed`, whose two ledgers must SUM to the served surface, so a route added untyped here goes red and a stale reason goes red too. The six are the READ lenses: `/v1/analytics/{overview,timeseries,top}`, `/v1/errors`, `/v1/insights/{events,health}`. The seven refusals are one probe and the six ingest endpoints, and they are MEASURED rather than asserted. `GET /v1/analytics/health` answers 503 CARRYING the degraded report as its body — the ml `/health` class — and `TestHealthStillCarriesItsReportAt503` pins the pair a typed op would have to break. The six endpoints share ONE admission decision (`handle`, event.go) resolved entirely from facts that never reach a typed op: the presented credential (Authorization / `x-hanzo-ingest-key` / `?ingest_key=`), the client IP and socket peer the anonymous rate caps key on, DNT/Sec-GPC, and the RAW body length that is the projected lane's 64 KiB→413 bound (`public.go maxPublicBytes`, invisible to a typed op and far below the fleet's global zip BodyLimit). Four of the six add a SECOND, independent blocker: the canonical wire is POLYMORPHIC (`decodeIngest` takes a bare Event object, a bare Event ARRAY, and the `{batch:[…]}`/`{events:[…]}` envelope on one path) and the team SPA's is a bare ARRAY unconditionally — bodies zip's `op.invoke` would 400 where they answer 200 today. `TestArrayBodiedEndpointsStillAnswer200` is that measurement, so when zip can declare a polymorphic body the conversion is a test away rather than a re-derivation. Two latent defects surfaced and were fixed: plugin/event/main.go declared NO `Prefixes`, so the standalone binary's scope owned only the `/v1/<name>` default and five of the app's six prefixes were outside anything it could gate or that `cloud.Declare` could attribute — the pricing defect exactly, one app over; and `apps/event` had no `cloud.Bridge` of its own, relying entirely on Serve's app-wide install, which the package's own test harness does not run. A THIRD is fleet-wide and only named here: `SeriesPoint` was about to be a one-name/two-shape collision with apps/admin's `{t,value}` launch-board point, which `openapi.Compose` refuses — analytics yielded to `UsagePoint`, since its schema was not yet published. `TestEveryPublishedFieldIsDescribed` gates the response side: all 19 TYPED schemas carry per-field prose, so `errorRate` says it is a ratio and `pct` says it is a share of the WINDOW rather than of the rows returned. **Re-verified at a later pass against a MOVED surface, and every refusal held** — by then main had folded `/v1/event/collect` away and added the two Sentry endpoints `POST /v1/event/{project}/{envelope,store}`, so the ledger reads 6 typed / 8 refused. The reasons were re-derived from zip v1.18.11's own `typed.go` rather than inherited as prose, and each gained a blocker the first pass had not written down: ORDER. `op.invoke` decodes the body BEFORE the handler is entered, while admission refuses 401 (no resolvable key), then the projected lane refuses 429 (rate), then 413 (64 KiB), with the raw bytes still in hand and nothing parsed — so a typed op would answer 400 to a beacon that is answered 413 or 429 today. Error precedence is wire. What that pass DID change is the other half of the cost: all eight published an operationId and NOTHING else, so every generated SDK offered `post_v1_event` — the endpoint every Hanzo product beacons to — as a call with nowhere to put the event. They declare their bodies now through `openapi.Register`, driven off the `endpoints` table itself so an endpoint cannot be routed with one wire and documented with another, and the gate quantifies over `untypedByDesign` rather than over endpoints (`TestEveryUntypedRouteDeclaresItsBodies`) so a refusal added tomorrow owes its bodies by construction. The canonical four use the new `openapi.OneOf`, because their wire is genuinely three shapes; the two Sentry endpoints use `openapi.Binary` for a request that is an opaque envelope stream and declare NO response, named in `relayed` — they copy back whatever `cloud.ObsErrorIngest` installed, and publishing a shape there would be inventing one. The subset went 0→7 requestBody and 6→12 responses, 19→30 schemas, and the described count did NOT move: 6, exactly the typed ops, because prose, an MCP tool and a CLI command are the three things only zip's registry supplies. Reported as unchanged rather than counted as progress. The health probe's `map[string]any` became `healthReport` in the same move — a map's shape cannot be declared without hand-writing a schema beside it, which is the drift `Register` exists to prevent — and `TestHealthReportKeepsTheMapItReplaced` pins both bodies field-for-field. The one difference is serialization ORDER (Go struct order, not encoding/json's sorted map keys), which the JSON data model does not carry; no field name, value, presence rule or status moved. The cost is a `proseless` ledger of eleven components publishing 63 bare properties, and it may only SHRINK — it caught its own first staleness during this rebase, when main's removal of the Team endpoint left `teamEvent` named and unpublished), the ~70 remaining packages, 1–11 routes each | ~358 |
| — | ~~iam 25~~ (done, then RE-done and the second answer is the real one: the first pass typed **0 of 25 and refused all 25**, correctly, because the client erased the child's registry. Composing the App with `host.Use` instead of adapting a handler carried the nested registry across, and iam published — measured at that pass — **155 paths / 182 ops / 94 with schema / 94 MCP tools / 94 CLI commands / 0 wildcards** — see "apps/iam — what composing recovered" above. Nothing in iam was typed to get there; the 94 were always typed, in its own repo, and the adapter was throwing them away) | 94 |
...
contract and `client/catalog.json` exclude a staged capability (HIP-0139 §8, and
`plugin/gen-client-catalog`'s own header explains why it reads the audience from
...
social at 7 / 13, and the endpoint carries 13 operations for each. Now: esign 23 schemas
...
- **world owed two things and neither was the field gate**, which already existed
  ...
  endpoint), and a pin on its SILENCE: stream's prose says its frames carry "the same
...
- **Three refusal classes, each measured rather than asserted.**
  ...
  (`_ = c.Bind(&body)`, apps/ad/ads.go), so a malformed body launches on the
...
- **Latent defects found by typing, all fixed here.** (1) FIVE packages had no
  ...
  `openapi.Compose` refuses: wallets' `Account` against books', and ads' `Campaign`
...
- **`plugin/ai` is the largest hole in the fleet document, and it is upstream's.**
  ...
  reaches it — **and `host.Use` is now the receiver that makes that work pay
  ...
  would have been typed and cloud would still have printed `{wildcard1}`. `Use`
  takes a `zip.Component`, a `*zip.App` is one, and **ai already is one** — the beego half of this is
...
- **The three refusals are all "zip cannot state this", and each is MEASURED.**
  ...
  typed_wire_test.go` and `apps/pref/wire_test.go` hold those wires, so a later
...
**THE SURFACE NO PATH LIST COULD EVER HAVE COVERED.** A typed op is also an MCP
...
SDK/CLI/MCP value is what opened that endpoint; the handler-side `tenantOf` check is
what closes it, because those endpoints park no marker and carry no validated
...
one of a typed op's three endpoints.
...
endpoint and not create one. `schema.go`'s DDL
...
**⚠ THE REFUSAL ENVELOPE IS RFC 9457 PROBLEM-DETAILS, NOT `{status, code, error}`
— and three passes in this file said otherwise.** Measured at the pinned zip
(v1.34.0): every propagated error renders through `cloud.ErrorHandler` →
`HTTPError.MarshalJSON` → `problem()`, which writes

    {"type": "about:blank", "title": <status phrase>, "status": N,
     "detail": <the sentence>, "code": <if set>, …Detail members}

so the SENTENCE is `detail`. `error` is not written at all. That matters twice
over, and both were got wrong here before being measured:

- **A route converted from a hand-written in-band refusal MOVES ITS SENTENCE KEY.**
  `apps/agents`' guard wrote `{status, code, error, findings}` and now answers
  `{type, title, status, detail, code, findings}`; `apps/guide`'s blocked-step 409
  wrote `{error, step, blockedBy}` and now answers `{type, title, status, detail,
  step, blockedBy}`. The commit that landed the agents one claimed the body was
  "byte-for-byte what it was plus status". **That was wrong.** What a client
  branches on — `code` — and what it renders — `findings`, `blockedBy` — are
  unchanged, and the change is the right direction, because every OTHER refusal in
  the fleet already renders that way and the hand-written `error` key was one
...
- **`error` is NOT reserved, and the correction above originally said it was.**
  ...
  conversion — which is a much better reason for the client to stay than the one it
...
checked only `Code` and `Findings`, so the key could move and the suite stayed
green. A declared-and-unasserted field is worse than an absent one — it reads as
coverage. Both tests now assert the WHOLE body, envelope included.

**And re-read the PIN, not this file.** Several refusals here cite "zip v1.31.3"
because that is what go.mod said when they were written; it says **v1.34.0** now
...
failure; the JSON-RPC endpoints are the same fact). Each was mutation-checked by
...
`grep -rln … | head -1` picked `endpoints_test.go` over `typed_wire_test.go`, so
...
the generator.** That is the same client `TestEveryPublishedFieldIsDescribed`
...
400. The MCP endpoint's tools are not lost to the projections — `tools.Register`
...
`apps/company` (21 of 22) is the split tranche taken to its FLOOR, and it is the
one to read for what "left untyped" should cost you to claim. It had two
refusals, each naming one missing zip capability, and **one of them had stopped
being true and nobody re-read it**: `POST /payment` is a typed op now, because
`cloud.Denied` + the app-wide `DenyEnvelope` carry the money wire off a RETURNED
error — exactly what its refusal said was impossible. Its 402/503 body is
byte-identical through the conversion, and `TestPaymentDenialWire` asserting the
nested `{"error":{"code","message"}}` is what says so. The refusal that stands:
...
`apps/iam` — what composing recovered. This was the FLOOR of the migration, "0 of
25 and none of it converts", and the reason it read that way is worth more than the
count: the refusal was correct about the CLIENT and wrong about iam.
...
**After** — `host.Use(iamserver.NewApp(db))` (`apps/iam/iam.go`), measured on the regenerated subset at that pass:
...
endpoints taking `application/x-www-form-urlencoded`. Composing does not type
...
**What composing cost, elsewhere, and it was all pre-existing damage made loud.**
zip refuses a duplicate METHOD+pattern at build instead of letting
...
- `GET /healthz` — iam's child app declared it on its PUBLIC router while
  ...
  why composition reads the ROUTER (`Declaration()`), never the document.
...
forbids and any composition refuses. Fixed there: an alias is named by its address.
...
it. Composed, iam declares the three exact documents its router holds, so
`manifest.Apps` routes exactly those three and nothing else.

**What is NOT in reach, and why the answer is not "compose harder".** `hanzoai/iam`
...
- **Committed per-app subsets (`plugin/<app>/openapi.json`) go stale when routes
  change.** The composition catches it: `TestFleetIsTheCompositionOfItsApps` compares the
...
- **The agent-skills catalogue is a projection of this repo's own specs, and the
  ...
  `gen-client-catalog` reads and the weave holds against `openapi.yaml` — so the
...
## Lifecycle defense: ONE scorer client, ONE fail policy, a sensor at the edge

`risk.go` · `agency.go` · `middleware_abuse.go` · `apps/gateway/edge/traffic.go`

**`cloud.Decide` is the only endpoint to `/v1/risk`.** The app that owns `/v1/risk`
...
## Cross-subsystem clients that are values, not places
...
- **THE FLEET HAS ONE MCP ENDPOINT: `POST /v1/mcp`, on the HOST, AND IT IS A QUERY.**
  ...
  **A test for this passes for the wrong reason unless the dial is impossible.**
  ...
  deleted, because both paths reach the same handler. And testing the HELPER is not
  ...
  **The endpoint publishes ONE TOOL PER SUBSYSTEM, not one per operation** (`fleet/grouped.go`).
  Measured on the deployed endpoint: the flat projection was **1,189 tools in 977,636
  ...
  **Headroom: 17 subsystems.** 111 of the 128 a client keeps, and the headroom
  ...
  manifest is 125 apps and the endpoint publishes 110 of them, so the next seventeen
  subsystems that DO publish put the endpoint back over the cap; the
  move then is to group by product surface (`productStems`, 17 buckets), not to add
  a second projection.
  🔴 **THE MCP ENDPOINT ADVERTISES OPERATIONS THE CHILDREN CANNOT DISPATCH.** Measured
  ...
  op earns a tool. So the endpoint publishes the untyped remainder and the child
  rejects its own published name — `noCallerTools.Call` (app.go), whose comment
  states the invariant this breaks: "the fleet's endpoint never routes one here".
  ...
  **apps/s3 is 6 of 8 now**, and what it took was re-reading its own refusals
  ...
  document. Empty subsystems on the endpoint: 13 → 12; catalogue 1452 → 1459.
  **`apps/sandbox` followed, 6 of 19 → 13 of 19**, and its shape is worth copying:
  the AGENT'S OWN SURFACE (`/v1/sandbox/{lease,run,read,write,stop,end}`) was typed first
  ...
  duplicates and must not be folded — the endpoint is verb-shaped and holds a LEASE,
  ...
  raw bytes (its typed twins are the endpoint's read/write), the two interactive pages
  answer text/html, and the two sockets are protocol upgrades. The TICKETS that
  authorize those endpoints are typed, which is the half that pays: an agent mints a
  ...
  **10 apps register zero typed ops, stranding 157 advertised operations** — iam
  ...
  `kms` have left that list** (14 of 17 and 5 of 7 typed; their remaining 3 and 2
  are wire-bound and gated), so the stranded count is 157 − 24 = 133 and `dns` is
  the only one of the ten whose whole surface is structurally unreachable rather
  than merely unconverted. Apps that DO
  ...
  **CLOSED, and NOT by the dump mode this paragraph used to prescribe.** A second
  ...
  `plugin/gen-client-catalog` carries only marked operations, so the catalog holds
  ...
  **The catalog SHRANK on regeneration and that is the correction, not a loss:
  ...
  zen` — `jq 'length' client/catalog.json`,
  `jq '[.[]|length]|add' client/catalog.json`,
  `jq -r 'to_entries|map(select(.value|length==0)|.key)|join(" ")' client/catalog.json`),
  ...
  not the catalog — and the endpoint got SMALLER rather than poorer: 112 tools in
  ...
  endpoint publishing nothing. The third, `TestAPhraseSaysWhatTheRouteSays`, anchored
  ...
  **The tools carry no prefix.** They shipped as `hanzo_<app>` + `hanzo_describe`
  ...
  the prefix was also how `composed()` told one of the endpoint's own tools from a
  child's operation, which is now an exact membership test against the app set
  (`Endpoint.composed`); and the endpoint's tools and the app names now share one
  ...
  one ever is. A endpoint still on an older image answers the prefixed names; the
  refused set is untouched, because `refuse()` reads CHILD operation ids and never
  saw the endpoint's own names.
...
- **The per-tenant tool plane is ONE typed op, and callable in-process.** An org's
  ...
  `POST /v1/tools/call` (a typed op, therefore itself a tool on the endpoint) over the
  ...
  record. Use these clients; never re-implement tool dispatch.
...
- **A customer IS an IAM user; marketing keeps no contact list.** Who to email is
  ...
  `iam/pkg/store.GetMailableUsers` over `apps/iam.DB()`), the same client
...
- **A product announcement is not its own engine.** It is a one-step sequence with
  ...
  `deliver`: the gate is only absolute because it is the only endpoint.
...
**A publishable key has its own resolve endpoint.** `OrgForKey` (`auth_apikey.go`) sends
...
so routing every prefix to that one endpoint — which is what cloud used to do — meant a
...
The TAG is the OWNER (HIP-0139 §4, `openapi/compose.go:225`: `op.Tags = []string{op.App}`), so
...
**`/v1/search` IS THE ONE ENTRY, and the backends keep their own.** A caller asks
...
what they are, because each is a backend's own surface rather than an endpoint a caller
...
leg never adds a mode. `apps/code/query.go` is that leg's in-process client and is
...
**Fourteen of the seventeen operations are TYPED ops** (`apps/index/typed.go`), so
the search plane reaches a schema, an MCP tool, a CLI command and a generated SDK
method — it published seventeen operationIds and prose and nothing else. Typing
did not move the dialect: every model spells the map its handler assembled, in
ALPHABETICAL json-tag order, because encoding/json sorts a map's keys and a
Meilisearch client reads these bodies byte for byte
(`TestTypedAnswersAreByteIdenticalToTheMaps`). Nor did it move the REFUSALS,
which is the harder half: `{message, code, type, link}` is a wire contract its
client branches on, so an op returns a `*fault` and `envelope()` writes those
bytes back — the shape apps/goja's BundleErr and cloud's own `Denied` already
take. The three that stay untyped are the document writes: their body is a
top-level JSON ARRAY on a `:uid` route, and zip binds a path parameter by walking
struct fields, so a slice In binds no uid and a struct In cannot decode the array
— they declare both body shapes through `openapi.Register` + `openapi.OneOf`
instead. Paging keeps STRINGS for `limit`/`offset` because the handler defaults on
absent-or-unparseable and honours an explicit 0.
...
`index.Reconcile` is `index.Query`'s mirror and the only in-process WRITE client: a
...
`templates.Lookup(ctx, org, slug)` is the ONE endpoint other subsystems read through
...
**`Fetch` and `Read` are different endpoints, deliberately.** `Fetch` is the pure network
...
endpoint, so no caller has to remember to persist and no two can disagree about where
pages live.

Kept pages ride the ONE object client the binary already has (`types.VFSClient` over
...
changed with the endpoint. Car 3 used to compare the live tool count against
`jq -s length` over the committed `plugin/*/mcp.json`; both sides came from the
same files (the endpoint was SERVING those bytes), so it proved only that the image
carried the tree it was built from, and it passed for months while o11y's
catalogue held 12 of 365. The endpoint composes itself by asking now, so car 3 asks
...
single-registrant client, never a second CI — from the embedded git server
...
receive-pack, so without that endpoint the host we call canonical builds nothing and
...
**Two entries, and only one of them runs anything.**
...
- **artifact** (`binaries:`) → `launchArtifactBuild` (`apps/platform/artifact.go`) → a
  ...
  layout hanzoai/ci publishes to — one index, two front endpoints.
...
checked in middleware covers exactly one of a typed op's three endpoints").
`cloud.Scope.Refusal`'s own doc comment already prescribed the remedy and named
this package as where it lives. `TestTheDeliveryBoardIsShutToANonAdminOnEveryEndpoint`
drives the MCP endpoint specifically, because a route test cannot see it;
...
- **paid → `/v1/ads`** — `ads.LaunchPaid/PaidSpend/PausePaid` (`apps/ad/provider.go`)
...
(`apps/event/campaign.go`) — an org+`utm_campaign`(+`utm_content`)-scoped query
...
clients are wired at the root to the flags-assignment + evidence primitive; nil-safe
...
- `GIT_SSH_HOST`, `GIT_SSH_ADDR`, `GIT_SSH_HOST_KEY` — SSH entry point; the host key is
...
canonical socket, the op runs through `zip.Here` — the op's own invoke client, the
...
`todo` is the client because its gated create depends on nothing but the local store —
...
`Stand` is now `StandWithin` with the leg unknown, so every existing client behaves
...
at all. It is written, it is next endpoint, and **cloud imports `dexsession` nowhere**.
...
row, one prefix). It serves six operations: `GET /v1/world` (the entry point),
...
`/zap`. Measured: `initialize` on the MCP endpoint answers 200 unauthenticated
...
**A source we may not redistribute is a declared CLIENT, not an omission.**
...
and concluded the agent endpoint withholds nothing and that nothing in cloud calls
...
- **§ "The plane, in numbers", § 1 trigger, § 2 join, § 3 evidence, § 5 privacy** —
  ...
  cloud line.
- **§ 6 containment, § 9 composition** — verified against the LIVE endpoint
...
**Browser session replay is `POST /v1/event/replay` in `apps/event`, and cloud
never reads it back.** The endpoint produces one Kafka message per rrweb batch onto
...
2. `scrubText` / `scrubException` at the fold in `apps/event/capture.go` —
...
Measured on the live endpoint, `POST https://api.hanzo.ai/v1/mcp` `tools/list`:
...
- **No second ingest endpoint.** `POST /v1/event` is the one endpoint; errors already
  arrive through it.
- **No second tool surface.** The fleet MCP endpoint and its `refuse()` are the one
  policy site.
- **No replay read endpoint in cloud.** Cloud deliberately does not touch replay
...
Every rung of § 4 is already on that endpoint: `analytics` publishes
...
human, which is § 4's job and not the endpoint's.
...
already retrieved. The AGENT gets that brief plus the endpoint, and reaches for the
endpoint when the brief turns out to be insufficient.
...
## Two engines encrypt SQLite, and only one of them is production

`hanzoai/sqlite` ships two encryption paths, and confusing them will make you
condemn a healthy backup.

**cgo + libsqlcipher — what production runs.** The image carries
`libsqlcipher.so.0.8.6` and each plugin (`/iam`, `/tasks`, `/commerce`, …) opens
its stores through it. That is SQLCipher's real codec, hooked inside the pager,
so it encrypts WAL frames too. An encrypted `.db` with an encrypted `-wal`
beside it is the NORMAL steady state, and committed data lives in those frames.
Measured on the production volume: 2456 encrypted databases carry a non-empty
`-wal`, and the logs show zero open errors.

**The pure-Go envelope — for builds without cgo.** No writable codec hook exists
in modernc, so it decrypts the WHOLE file into a RAM copy (`/dev/shm`), lets
SQLite work on that, and re-encrypts on close. Because it cannot fold an
encrypted sidecar, `refuseHotSidecar` (envelope.go) fails closed on any
non-empty `-wal`. Its durability is at Close/Checkpoint, not per-commit.
...
So: any backup, restore, or inspection tool for this data must link libsqlcipher
...
(public signing keys, no program): +78 KB, executables still 56. `alpine-baselayout`
is the wrong package for this and would fail `DISTROLESS-GATE` — it depends on
`/bin/sh`, so it drags busybox back, and it does not carry `/etc/os-release`
anyway.

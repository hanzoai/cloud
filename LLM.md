...
14. **The fleet has THREE copies of one seam: an error that carries its own
    ...
    **The general answer arrived instead, one layer down**: `zip.HTTPError.Detail`
    (v1.31.3) lets a RETURNED error carry its own members, so the seam these three
    ...
    **AND THE MONEY ONE WAS ALREADY DONE, which is the finding worth more than the
    ...
    before that seam existed and none re-read after: `apps/company` (`/payment`),

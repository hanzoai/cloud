...
// Clusters hits the LIVE /v1/compute/clusters (org from identity, not the path).
func TestPlatformClusters(t *testing.T) {
	p, done := platformStub(t, "t", func(w http.ResponseWriter, r *http.Request) {
		if r.Method != http.MethodGet || r.URL.Path != "/v1/compute/clusters" {
			t.Errorf("clusters = %s %s, want GET /v1/compute/clusters", r.Method, r.URL.Path)
		}

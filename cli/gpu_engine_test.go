...
func TestBuildRegistrationCarriesEngine(t *testing.T) {
	...
	// The presence record's Input must carry the endpoint so GET /v1/compute/fleet/workers
...
// the presence Input and serves it back on GET /v1/compute/fleet/workers. It exercises the
// real chain — probe → build registration → POST the fleet activity → GET
// /v1/compute/fleet/workers → the engine endpoint is advertised.
func TestConnectServeEngineRoundTrip(t *testing.T) {
	...
	cloud := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case r.Method == http.MethodGet && r.URL.Path == "/v1/compute/fleet/workers":
	...
	if _, err := w.call(ctx, http.MethodGet, "/v1/compute/fleet/workers", nil, &resp); err != nil {
		t.Fatalf("GET /v1/compute/fleet/workers: %v", err)
	}

...
// clusters list hits the LIVE /v1/compute/clusters (org from identity, not the path).
func TestClustersListCommand(t *testing.T) {
	withPlatform(t, func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path != "/v1/compute/clusters" {
			t.Errorf("path = %s, want /v1/compute/clusters", r.URL.Path)
		}

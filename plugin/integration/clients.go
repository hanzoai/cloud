...
// The cross-subsystem clients integrations owns, wired in ITS OWN composition root.
//
...
// clients it consumes are wired HERE, where integrations already links and where
...
func init() {
	...
	// and so its clone-url/verify-ref clients had to arrive from a root that imports
	// both. Those clients are peer calls now — git is another PROCESS, and the
	...
	// Inbound-event client: a verified provider webhook (or chat channel) fires the
	...
	// credential custody). A primitive-typed adapter keeps the client free of the
	// automations types.

...
func init() {
	// GTM PAID channel: the /v1/campaigns orchestrator fans out to executors that
	// satisfy campaigns.Channel; this adapts ads' provider.go funcs (each resolves
	// the org's ad token through integrations.TokenFor and fails closed) onto the
	// primitive-typed channel seam.
	campaigns.RegisterChannel(campaigns.NewChannel(campaigns.KindPaid,

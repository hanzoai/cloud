...
interface project {
    ...
    # resolved within that principal's org, so another tenant's slug is a 404 and
    # nothing of theirs is touched.
    delete_projects_by_slug(req: projectsRef)
    # Gives a custom hostname back, so the name is free to reuse.
    # A claim is FIRST-COME and global, so an add-only surface was not ownership but
    # a leak: a customer who mistyped a domain, or claimed one they later moved
    # elsewhere, could neither reuse it nor let anyone else. This is the third
    ...
    # Scope: a validated principal is required (403 without one) and the site is
    # resolved within that principal's org, so another tenant's slug is a 404.
    delete_projects_by_slug_domains_by_host(req: projectsDomainRef)
    # Removes the caller's own bookmark from a project, and answers whether
    # it is starred afterwards.
    # It removes only YOUR star — the same one star wrote — so a project other
    # people have starred stays on their lists. Unstarring one you had not starred
    # is not an error; it leaves it unstarred.
    delete_projects_by_slug_star(req: projectsRef) returns (rep: projectsStar)
    # Deletes a project and takes its site off the internet.
    # The metadata delete is authoritative and everything after it is best-effort,
    # in this order: the public `<slug>` subdomain binding is released so the slug is
    # free to reclaim, the release rows are dropped so a reclaimed slug never
    ...
    # resolved within that principal's org, so another tenant's slug is a 404 and
    # nothing of theirs is touched.
    delete_site_by_slug(req: projectsRef)
    # Gives a custom hostname back, so the name is free to reuse.
    # A claim is FIRST-COME and global, so an add-only surface was not ownership but
    # a leak: a customer who mistyped a domain, or claimed one they later moved
    # elsewhere, could neither reuse it nor let anyone else. This is the third
    ...
    # Scope: a validated principal is required (403 without one) and the site is
    # resolved within that principal's org, so another tenant's slug is a 404.
    delete_site_by_slug_domains_by_host(req: projectsDomainRef)
    # Deletes a project and takes its site off the internet.
    # The metadata delete is authoritative and everything after it is best-effort,
    # in this order: the public `<slug>` subdomain binding is released so the slug is
    # free to reclaim, the release rows are dropped so a reclaimed slug never
    ...
    # resolved within that principal's org, so another tenant's slug is a 404 and
    # nothing of theirs is touched.
    delete_sites_by_slug(req: projectsRef)
    # Gives a custom hostname back, so the name is free to reuse.
    # A claim is FIRST-COME and global, so an add-only surface was not ownership but
    # a leak: a customer who mistyped a domain, or claimed one they later moved
    # elsewhere, could neither reuse it nor let anyone else. This is the third
    ...
    # Scope: a validated principal is required (403 without one) and the site is
    # resolved within that principal's org, so another tenant's slug is a 404.
    delete_sites_by_slug_domains_by_host(req: projectsDomainRef)
    ...
    # Scope: a validated principal is required (403 without one) and the project is
    # resolved within that principal's org, so another tenant's slug is a 404.
    get_projects_by_slug_deployments(req: projectsRef)
    # Returns every custom hostname this site holds: the live ones, plus
    # any pending claim with the DNS records it still owes.
    # `domains` is the routing answer — the hosts that are verified right now —
    # while `claims` is the full panel, one row per host, each saying whether it is
    ...
    # Scope: a validated principal is required (403 without one) and the site is
    # resolved within that principal's org, so another tenant's slug is a 404.
    get_projects_by_slug_domains(req: projectsRef) returns (rep: projectsDomains)
    # Returns a site's releases newest-first, marking the active one —
    # the rollback menu.
    # Each row carries the release id to activate, the source it was promoted from,
    # its object and byte counts, and the URL if it is the one serving. Retention
    ...
    # Scope: a validated principal is required (403 without one) and the site is
    # resolved within that principal's org, so another tenant's slug is a 404.
    get_projects_by_slug_releases(req: projectsRef)
    # Returns the org's deployed sites at the pretty URLs they serve at.
    # It reads the SAME org-scoped store as /v1/project and keeps only the projects
    # that are actually `live`, so a draft or a failed build is not advertised as a
    # site.
    ...
    # which keeps only `live` rows so a draft or a failed build is never advertised
    # as a site. One definition of "is a site", used by both.
    get_projects_sites_by_slug(req: projectsRef) returns (rep: projectsSite)
    # Returns the org's deployed sites at the pretty URLs they serve at.
    # It reads the SAME org-scoped store as /v1/project and keeps only the projects
    # that are actually `live`, so a draft or a failed build is not advertised as a
    # site.
    ...
    # which keeps only `live` rows so a draft or a failed build is never advertised
    # as a site. One definition of "is a site", used by both.
    get_site_by_slug(req: projectsRef) returns (rep: projectsSite)
    # Returns a project's deploy history, newest version first.
    # Every deploy of the project is a row — uploads, generated sites, and git/CI
    # builds alike — carrying its version, status, source, commit, live URL, file
    # count and byte count. The short-lived upload grant a queued git deployment was
    ...
    # Scope: a validated principal is required (403 without one) and the project is
    # resolved within that principal's org, so another tenant's slug is a 404.
    get_site_by_slug_deployments(req: projectsRef)
    # Returns every custom hostname this site holds: the live ones, plus
    # any pending claim with the DNS records it still owes.
    # `domains` is the routing answer — the hosts that are verified right now —
    # while `claims` is the full panel, one row per host, each saying whether it is
    ...
    # Scope: a validated principal is required (403 without one) and the site is
    # resolved within that principal's org, so another tenant's slug is a 404.
    get_site_by_slug_domains(req: projectsRef) returns (rep: projectsDomains)
    # Returns a site's releases newest-first, marking the active one —
    # the rollback menu.
    # Each row carries the release id to activate, the source it was promoted from,
    # its object and byte counts, and the URL if it is the one serving. Retention
    ...
    # Scope: a validated principal is required (403 without one) and the site is
    # resolved within that principal's org, so another tenant's slug is a 404.
    get_site_by_slug_releases(req: projectsRef)
    # Returns the org's deployed sites at the pretty URLs they serve at.
    # It reads the SAME org-scoped store as /v1/project and keeps only the projects
    # that are actually `live`, so a draft or a failed build is not advertised as a
    # site.
    ...
    # which keeps only `live` rows so a draft or a failed build is never advertised
    # as a site. One definition of "is a site", used by both.
    get_sites_by_slug(req: projectsRef) returns (rep: projectsSite)
    # Returns a project's deploy history, newest version first.
    # Every deploy of the project is a row — uploads, generated sites, and git/CI
    # builds alike — carrying its version, status, source, commit, live URL, file
    # count and byte count. The short-lived upload grant a queued git deployment was
    ...
    # Scope: a validated principal is required (403 without one) and the project is
    # resolved within that principal's org, so another tenant's slug is a 404.
    get_sites_by_slug_deployments(req: projectsRef)
    # Returns every custom hostname this site holds: the live ones, plus
    # any pending claim with the DNS records it still owes.
    # `domains` is the routing answer — the hosts that are verified right now —
    # while `claims` is the full panel, one row per host, each saying whether it is
    ...
    # Scope: a validated principal is required (403 without one) and the site is
    # resolved within that principal's org, so another tenant's slug is a 404.
    get_sites_by_slug_domains(req: projectsRef) returns (rep: projectsDomains)
    # Returns a site's releases newest-first, marking the active one —
    # the rollback menu.
    # Each row carries the release id to activate, the source it was promoted from,
    # its object and byte counts, and the URL if it is the one serving. Retention
    ...
    # Scope: a validated principal is required (403 without one) and the site is
    # resolved within that principal's org, so another tenant's slug is a 404.
    get_sites_by_slug_releases(req: projectsRef)
    ...
    # Scope: a validated principal is required (403 without one) and the site is
    # resolved within that principal's org, so another tenant's slug is a 404.
    post_projects_by_slug_domains(req: projectsDomainsBind) returns (rep: projectsBoundDomains)
    # Checks the DNS challenge for a pending custom hostname and, when
    # it passes, promotes the host so it begins routing at the edge.
    # It answers 200 either way, with the host's honest current state: verified once
    # the TXT record is found, still pending — with the records to publish and the
    ...
    # the claim are resolved within that principal's org, so a host claimed by
    # another tenant is "not claimed by this site".
    post_projects_by_slug_domains_by_host_verify(req: projectsDomainRef) returns (rep: projectsDomain)
    # Promotes a build output into a new release AND goes live with it —
    # create+activate in one call, which is the 99% path.
    # It is exactly the two halves in sequence with no extra semantics, so the
    # staged flow and the one-shot flow can never drift apart: `source` is promoted
    ...
    # Scope: a validated principal is required (403 without one) and the site is
    # resolved within that principal's org, so another tenant's slug is a 404.
    post_projects_by_slug_publish(req: projectsPublish) returns (rep: projectsRelease)
    # Promotes a build output into a new immutable release WITHOUT
    # serving it — the staged half of publishing, for when you want to check a
    # release before it goes live. Answers 201.
    # `source` is a path RELATIVE to your org's own storage space: the org segment
    ...
    # Scope: a validated principal is required (403 without one) and the site is
    # resolved within that principal's org, so another tenant's slug is a 404.
    post_projects_by_slug_releases(req: projectsPublish) returns (rep: projectsRelease)
    # Points the site at an existing release — the go-live, and
    # equally the ROLLBACK.
    # Aim it at an older release and the site serves that one again: releases are
    # immutable and retained to the retention depth, so nothing is rebuilt or
    ...
    # Scope: a validated principal is required (403 without one) and the site is
    # resolved within that principal's org, so another tenant's slug is a 404.
    post_projects_by_slug_releases_by_release_activate(req: projectsReleaseRef) returns (rep: projectsRelease)
    # Generates a self-contained, mobile-responsive static site from a
    # natural-language brief and deploys it live in one call.
    # One inference call turns `brief` (capped at 8 KiB) into a file manifest, which
    # then runs through the SAME validation, guards and viewport guarantee as a
    ...
    # Scope: a validated principal is required (403 without one) and the site is
    # published into THAT principal's org.
    post_projects_sites(req: projectsBuildSite) returns (rep: projectsSiteDeploy)
    # Deploys a caller-supplied file manifest — the deploy_site
    # capability an agent calls — and answers with where it went live.
    # `files` is a list of {path, content} pairs, the same shape the brief build
    # emits, and it runs through the SAME guards: index.html required at the root,
    ...
    # Scope: a validated principal is required (403 without one) and the site is
    # published into THAT principal's org.
    post_projects_sites_deploy(req: projectsDeploySite) returns (rep: projectsSiteDeploy)
    # Generates a self-contained, mobile-responsive static site from a
    # natural-language brief and deploys it live in one call.
    # One inference call turns `brief` (capped at 8 KiB) into a file manifest, which
    # then runs through the SAME validation, guards and viewport guarantee as a
    ...
    # Scope: a validated principal is required (403 without one) and the site is
    # published into THAT principal's org.
    post_site(req: projectsBuildSite) returns (rep: projectsSiteDeploy)
    # Attaches one or more CUSTOM public hostnames to this org's site.
    # Binding a host you do not own would let you shadow it at the edge, so which
    # outcome you get depends on whether ownership is already established: a SuperAdmin
    # vouches (the operator manages the customer's DNS, so its bind IS the proof) and
    ...
    # Scope: a validated principal is required (403 without one) and the site is
    # resolved within that principal's org, so another tenant's slug is a 404.
    post_site_by_slug_domains(req: projectsDomainsBind) returns (rep: projectsBoundDomains)
    # Checks the DNS challenge for a pending custom hostname and, when
    # it passes, promotes the host so it begins routing at the edge.
    # It answers 200 either way, with the host's honest current state: verified once
    # the TXT record is found, still pending — with the records to publish and the
    ...
    # the claim are resolved within that principal's org, so a host claimed by
    # another tenant is "not claimed by this site".
    post_site_by_slug_domains_by_host_verify(req: projectsDomainRef) returns (rep: projectsDomain)
    # Promotes a build output into a new release AND goes live with it —
    # create+activate in one call, which is the 99% path.
    # It is exactly the two halves in sequence with no extra semantics, so the
    # staged flow and the one-shot flow can never drift apart: `source` is promoted
    ...
    # Scope: a validated principal is required (403 without one) and the site is
    # resolved within that principal's org, so another tenant's slug is a 404.
    post_site_by_slug_publish(req: projectsPublish) returns (rep: projectsRelease)
    # Promotes a build output into a new immutable release WITHOUT
    # serving it — the staged half of publishing, for when you want to check a
    # release before it goes live. Answers 201.
    # `source` is a path RELATIVE to your org's own storage space: the org segment
    ...
    # Scope: a validated principal is required (403 without one) and the site is
    # resolved within that principal's org, so another tenant's slug is a 404.
    post_site_by_slug_releases(req: projectsPublish) returns (rep: projectsRelease)
    # Points the site at an existing release — the go-live, and
    # equally the ROLLBACK.
    # Aim it at an older release and the site serves that one again: releases are
    # immutable and retained to the retention depth, so nothing is rebuilt or
    ...
    # Scope: a validated principal is required (403 without one) and the site is
    # resolved within that principal's org, so another tenant's slug is a 404.
    post_site_by_slug_releases_by_release_activate(req: projectsReleaseRef) returns (rep: projectsRelease)
    # Deploys a caller-supplied file manifest — the deploy_site
    # capability an agent calls — and answers with where it went live.
    # `files` is a list of {path, content} pairs, the same shape the brief build
    # emits, and it runs through the SAME guards: index.html required at the root,
    ...
    # Scope: a validated principal is required (403 without one) and the site is
    # published into THAT principal's org.
    post_site_deploy(req: projectsDeploySite) returns (rep: projectsSiteDeploy)
    # Generates a self-contained, mobile-responsive static site from a
    # natural-language brief and deploys it live in one call.
    # One inference call turns `brief` (capped at 8 KiB) into a file manifest, which
    # then runs through the SAME validation, guards and viewport guarantee as a
    ...
    # Scope: a validated principal is required (403 without one) and the site is
    # published into THAT principal's org.
    post_sites(req: projectsBuildSite) returns (rep: projectsSiteDeploy)
    # Attaches one or more CUSTOM public hostnames to this org's site.
    # Binding a host you do not own would let you shadow it at the edge, so which
    # outcome you get depends on whether ownership is already established: a SuperAdmin
    # vouches (the operator manages the customer's DNS, so its bind IS the proof) and
    ...
    # Scope: a validated principal is required (403 without one) and the site is
    # resolved within that principal's org, so another tenant's slug is a 404.
    post_sites_by_slug_domains(req: projectsDomainsBind) returns (rep: projectsBoundDomains)
    # Checks the DNS challenge for a pending custom hostname and, when
    # it passes, promotes the host so it begins routing at the edge.
    # It answers 200 either way, with the host's honest current state: verified once
    # the TXT record is found, still pending — with the records to publish and the
    ...
    # the claim are resolved within that principal's org, so a host claimed by
    # another tenant is "not claimed by this site".
    post_sites_by_slug_domains_by_host_verify(req: projectsDomainRef) returns (rep: projectsDomain)
    # Promotes a build output into a new release AND goes live with it —
    # create+activate in one call, which is the 99% path.
    # It is exactly the two halves in sequence with no extra semantics, so the
    # staged flow and the one-shot flow can never drift apart: `source` is promoted
    ...
    # Scope: a validated principal is required (403 without one) and the site is
    # resolved within that principal's org, so another tenant's slug is a 404.
    post_sites_by_slug_publish(req: projectsPublish) returns (rep: projectsRelease)
    # Promotes a build output into a new immutable release WITHOUT
    # serving it — the staged half of publishing, for when you want to check a
    # release before it goes live. Answers 201.
    # `source` is a path RELATIVE to your org's own storage space: the org segment
    ...
    # Scope: a validated principal is required (403 without one) and the site is
    # resolved within that principal's org, so another tenant's slug is a 404.
    post_sites_by_slug_releases(req: projectsPublish) returns (rep: projectsRelease)
    # Points the site at an existing release — the go-live, and
    # equally the ROLLBACK.
    # Aim it at an older release and the site serves that one again: releases are
    # immutable and retained to the retention depth, so nothing is rebuilt or
    ...
    # Scope: a validated principal is required (403 without one) and the site is
    # resolved within that principal's org, so another tenant's slug is a 404.
    post_sites_by_slug_releases_by_release_activate(req: projectsReleaseRef) returns (rep: projectsRelease)
    # Deploys a caller-supplied file manifest — the deploy_site
    # capability an agent calls — and answers with where it went live.
    # `files` is a list of {path, content} pairs, the same shape the brief build
    # emits, and it runs through the SAME guards: index.html required at the root,
    ...
    # Scope: a validated principal is required (403 without one) and the site is
    # published into THAT principal's org.
    post_sites_deploy(req: projectsDeploySite) returns (rep: projectsSiteDeploy)
    ...
    # see two answers for the same one and starring it says nothing about anybody
    # else's list. Starring a project you have already starred leaves it starred.
    put_projects_by_slug_star(req: projectsRef) returns (rep: projectsStar)
}

# ---------------------------------------------------------------------
# 62 op(s) here. What follows is what this schema does not carry.
#
# blocked (34) — the op is absent; the field has no wire form:
...
#   get_projects_by_slug  projectsProject  projects.projectsProject  (reaches one)
#   get_projects_by_slug_deployments_by_id  projectsDeployment  projects.projectsDeployment  (reaches one)
#   get_projects_edge  edgeState  projects.edgeState  (reaches one)
#   get_site_by_slug_deployments_by_id  projectsDeployment  projects.projectsDeployment  (reaches one)
#   get_site_edge  edgeState  projects.edgeState  (reaches one)
#   get_sites_by_slug_deployments_by_id  projectsDeployment  projects.projectsDeployment  (reaches one)
#   get_sites_edge  edgeState  projects.edgeState  (reaches one)
...
#   patch_projects_by_slug  projectsProject  projects.projectsProject  (reaches one)
#   patch_projects_by_slug  projectsUpdate  projects.projectsUpdate  (reaches one)
#   patch_site_by_slug  projectsProject  projects.projectsProject  (reaches one)
#   patch_site_by_slug  projectsUpdate  projects.projectsUpdate  (reaches one)
#   patch_sites_by_slug  projectsProject  projects.projectsProject  (reaches one)
#   patch_sites_by_slug  projectsUpdate  projects.projectsUpdate  (reaches one)
...
#   post_projects  projectsProject  projects.projectsProject  (reaches one)
#   post_projects_by_slug_deployments  projectsDeployment  projects.projectsDeployment  (reaches one)
#   post_projects_by_slug_deployments_by_id_complete  projectsDeployment  projects.projectsDeployment  (reaches one)
#   post_projects_by_slug_purge  projectsProject  projects.projectsProject  (reaches one)
#   post_projects_fork  projectsProject  projects.projectsProject  (reaches one)
#   post_site_by_slug_deployments  projectsDeployment  projects.projectsDeployment  (reaches one)
#   post_site_by_slug_deployments_by_id_complete  projectsDeployment  projects.projectsDeployment  (reaches one)
#   post_site_by_slug_purge  projectsProject  projects.projectsProject  (reaches one)
#   post_sites_by_slug_deployments  projectsDeployment  projects.projectsDeployment  (reaches one)
#   post_sites_by_slug_deployments_by_id_complete  projectsDeployment  projects.projectsDeployment  (reaches one)
#   post_sites_by_slug_purge  projectsProject  projects.projectsProject  (reaches one)
#

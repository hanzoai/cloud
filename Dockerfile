...
# on otel-collector v0.144.10. BUMP THE SUFFIX (-v2 -> -v3) to force a cold
# module cache the next time a phantom pin poisons it.
RUN --mount=type=secret,id=gh_token \
    --mount=type=cache,id=cloud-gomod-v2,target=/go/pkg/mod,sharing=locked \
    if [ -s /run/secrets/gh_token ]; then \
      git config --global url."https://x-access-token:$(cat /run/secrets/gh_token)@github.com/".insteadOf "https://github.com/"; \
    fi && \
    go mod download
...
RUN --mount=type=cache,id=cloud-gomod-v2,target=/go/pkg/mod,sharing=locked \
    --mount=type=cache,id=cloud-gobuild-v2,target=/root/.cache/go-build,sharing=locked \

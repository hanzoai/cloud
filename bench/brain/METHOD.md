...
- every `meta.json` carries `questions`, `answered` and — once the run reaches the end — `finished`;
  `node record.mjs` posts them to `/v1/research/runs`, where completion is derived from the three and
  a run that does not state them reads `unknown` rather than finished

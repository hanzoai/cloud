...
// the frozen configuration: the exhaustive typed search with the lexical tie-break
const ROW = 'beamx', ENV = { LEXICAL: '1' }
...
if (VERIFY) {
  for (const split of ['dev', 'test']) {
    const before = readFileSync(`${ROOT}runs/mab-${split}-${ROW}-none/predictions.jsonl`, 'utf8')
    execSync(`mv runs/mab-${split}-${ROW}-none runs/mab-${split}-${ROW}-none.pre-verify && node mab/lane.mjs --split=${split} --rows=${ROW} > /dev/null 2>&1`, { cwd: ROOT, shell: '/bin/bash', env: { ...process.env, ...ENV } })
    const after = readFileSync(`${ROOT}runs/mab-${split}-${ROW}-none/predictions.jsonl`, 'utf8')
    const same = before.split('\n').filter(Boolean).map((l) => { const r = JSON.parse(l); return `${r.qid}\t${r.pred}` }).sort().join('\n') === after.split('\n').filter(Boolean).map((l) => { const r = JSON.parse(l); return `${r.qid}\t${r.pred}` }).sort().join('\n')
    execSync(`rm -rf runs/mab-${split}-${ROW}-none.pre-verify`, { cwd: ROOT })
...
for (const split of ['dev', 'test']) cpSync(`${ROOT}runs/mab-${split}-${ROW}-none/`, `${out}${split}/`, { recursive: true })
const manifest = {
  ...
  plans,
  commands: ['node mab/parse.mjs', 'node mab/index.mjs', 'node mab/compile.mjs --model=<model> [--api=<base>] [--temperature=<t>] [--prompt=<file>] --out=<plans-<tag>.json> --force', `${Object.entries(ENV).map(([k, v]) => `${k}=${v}`).join(' ')} node mab/lane.mjs --split=dev --rows=${ROW}`, `${Object.entries(ENV).map(([k, v]) => `${k}=${v}`).join(' ')} node mab/lane.mjs --split=test --rows=${ROW}`, `node mab/nbest.mjs --run=mab-test-${ROW}-none`, `node mab/contested.mjs --run=mab-test-${ROW}-none`, 'node tables.mjs'],
  ...
  metrics: { dev: JSON.parse(readFileSync(`${ROOT}runs/mab-dev-${ROW}-none/metrics.json`, 'utf8')).by_size, test: JSON.parse(readFileSync(`${ROOT}runs/mab-test-${ROW}-none/metrics.json`, 'utf8')).by_size }, row: ROW, env: ENV,
}

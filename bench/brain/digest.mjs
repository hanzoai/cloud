...
export function digest(here = new URL('.', import.meta.url)) {
  const code = {}
 ...
 */
export function check(frozen, here) {
  const now = digest(here)
  ...
  const moved = ENGINE.filter((f) => frozen.code?.[f] !== now.code[f])

...
#
# Four transports, and the fourth is why the harness is Go. REST, MCP and the op-call
# plane are HTTP and a script can speak them; ZAP is not, and a Python client
# against three transports with a Go client against the fourth would put the client's
# language into the comparison. One client, four envelopes.
#
# The transports are not variants of one wire. Over REST the operation IS the address.
...
host
go build -o "$BIN.transports" ./bench/transport/harness
"$BIN.transports" -base "http://127.0.0.1:$P" -zap "127.0.0.1:$ZP" -zap-name "ZAP tcp" -n "$N" ${BENCH_JSON:+-json "$BENCH_JSON"}
...
# three HTTP transports are measured again with it: they are the control on drift, and

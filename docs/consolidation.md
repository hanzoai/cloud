...
- **merged** — already in this binary (embedded module or native `apps/*`
  subsystem). Nothing to do.
- **mount** — should become a `apps/*` subsystem via import-and-mount. Est.
  diff noted.
...
plane (`apps/kms`) and the ONE in-process **tasks durable engine**
(`durable.go`, shared — see Wave 1).

37 native `apps/*` subsystems already carry their product surface in-process:
...
|---|---|---|
| **tasks** (`hanzoai/tasks`, tasksd) | **merged** | Engine already embedded in-process by `durable.go` (ONE engine, loopback ZAP :19999, shared with ai ingest). This wave adds the **HTTP + UI surface** (`apps/tasks`, order 147) mounting that SAME engine's handlers at `/v1/tasks/*` + `/_/tasks/*` — the "consolidate the UI surface into cloud" follow-up named in `durable.go`. No second Embed. Needed a small `hanzoai/tasks` v1.46.0 (`auth.WithIdentity` in-proc identity seam + single `hanzoai/sqlite` driver). ~180 LOC + `cloud.EmbeddedTasks()` accessor. |
| **visor** (`hanzoai/visor`) | **surface merged; logic port = own wave** | The compute REST surface (`/v1/machines`, `/v1/gpus`, `/v1/clusters`) is ALREADY native in `apps/visor` (order 133), org-scoped via `principal`. It PROXIES the standalone visor for data. Visor itself is a **5.4k-LOC multi-cloud Beego + xorm + Casdoor monolith** with its own DB and auth — mounting that in-process drags Beego/xorm/Casdoor + a second DB + bare `/v1/*` route collisions into this binary (architecturally wrong, not golf). The genuine native port = replace visor's `object/*` xorm persistence with a Base store and reuse its pure-`godo` `service/{digitalocean,doks}.go` on top of cloud's existing `apps/do` — a bounded **~600–900 LOC wave of its own**, sequenced after wave 2. Visor stays standalone until then. |
...
| **playground** (`hanzoai/playground`) | mount | S | Dev/eval playground surface → `/v1/playground/*`; overlaps `apps/eval` — reconcile, don't duplicate. |
| **insights-go** (`hanzoai/insights-go`) | fold | S | CLI (`cmd/cli`), not a service — fold into `apps/analytics` or keep as a lib. |
| **git** (`hanzoai/git`) | merged? | — | Native S3-backed git already lives at `apps/git`. Confirm the standalone repo is superseded (large-repo hosting is the only reason to keep it) and deprecate if so. |
...
| **kms** (`luxfi/kms`, EMBEDDED) | KMS is now embedded in this binary (`apps/kms` store + `/v1/kms`, fail-closed) — the standalone KMS Deployment is collapsed into cloud. Only the MPC signing ring (`luxfi/mpc`) stays a separate node set, the root of trust. |
...
| **docdb** (`hanzoai/docdb`, FerretDB) | MongoDB-wire DB data plane over Postgres; provisioned via `apps/provisioning`, never hosted in-proc. |
| **node**, **bootnode**, **nchain** | Chain/P2P daemons (luxfi substrate) — not app surfaces. |
| **functions** (fission fork) | FaaS control plane / k8s operator; cloud carries the `apps/functions` facade, the executor fleet stays out. |

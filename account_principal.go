...
// from /v1/get-account, which the embedded account surface answers from ITS OWN session
// cookie. A PKCE session is not one of those, so get-account returned
...
// error — the middleware would simply stop firing, fall through to the embedded
...
func AccountFromPrincipal() zip.Handler {
	return func(c *zip.Ctx) error {
		...
		if user == "" {
			return c.Next() // no principal → the embedded account surface (unchanged)
		}

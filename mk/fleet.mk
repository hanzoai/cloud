...
#
# It writes BOTH documents, from one weave: private.yaml is everything the fleet
...
#
# OUT names the WOVEN document, so the projection is written beside it: -weave is
...
openapi: ## Compose every app's document into private.yaml + openapi.yaml. OUT=<path> to write them elsewhere.
	@$(GO) test -count=1 $(ROOT)/openapi -weave="$(abspath $(if $(OUT),$(OUT),$(ROOT)/private.yaml))"
...
# and the entire ingress API was absent from the woven document — and therefore from every

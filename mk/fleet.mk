...
# openapi/compose_test.go for what it refuses. OUT=<path> writes it elsewhere.
#
# It writes BOTH documents, from one compose: private.yaml is everything the fleet
# serves, and openapi.yaml beside it is the customer contract — the part that
# declared itself published (openapi/public.go), under the name every developer,
# SDK generator and spec-reading tool reaches for. Two files, one command, one
# composition — a second command for the second file is how two documents come to
# describe two different commits.
#
# OUT names the COMPOSED document, so the projection is written beside it: -compose is
# the composition's own output and openapi.yaml is derived from it.
openapi: ## Compose every app's document into private.yaml + openapi.yaml. OUT=<path> to write them elsewhere.
	@$(GO) test -count=1 $(ROOT)/openapi -compose="$(abspath $(if $(OUT),$(OUT),$(ROOT)/private.yaml))"
	@echo ">> openapi.yaml — $$(grep -c '^  /' $(ROOT)/openapi.yaml) paths (the customer contract)"
	@echo ">> private.yaml — $$(grep -c '^  /' $(ROOT)/private.yaml) paths (everything the fleet serves)"
...
# and the entire ingress API was absent from the composed document — and therefore from every
...
documents: describe ## Write every generated document FROM SOURCE. Produces; renders no verdict.
	...
	@cd $(ROOT) && GOWORK=off go run ./plugin/gen-surface-catalog . >/dev/null
...
override DOCUMENTS := openapi.yaml private.yaml openapi/floor.json openapi/untyped.json openapi/closure.json surface/catalog.json plugin/ apps/skills/catalog/

paths: ## Print the generated-document paths, for a caller that has to name them outside make.
	@echo $(DOCUMENTS)

check: documents ## Regenerate every document FROM SOURCE and fail on any diff. The drift gate.

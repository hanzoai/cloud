...
// plane (client.Amount takes a hanzoai/money Amount), which is a decimal-string
// wire built so a debit crosses a process boundary UNROUNDED. Reaching that wire
// through Cents() defeated the wire's whole reason to exist: every client.Amount
// call site in the repo was client.Amount(money.FromUSD(x.Cents())) — an exact

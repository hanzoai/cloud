...
func sessionAccessToken(c *zip.Ctx) string {
	// The OSS core embeds no in-process IAM session manager (the private build
	// wires one via clients/iam into Beego's global session store). With no
	// server-side session→JWT map there is nothing to resolve, so the ambient-cookie
	// embed bridge is inert here — callers fall back to the Bearer/cookie token.
	return ""
}

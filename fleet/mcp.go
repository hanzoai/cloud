...
type Door struct {
	...
	// embeds. It exists because the door's answer is now entirely a function of
	// published operations — nothing is asked — so a caller that composes a door
	// over subsystems this binary did not build (a test with real children, an
	// alternate host) has to be able to say what they publish. Production leaves it
	// nil: the embedded catalog is generated from each app's own openapi.json and
	// regenerated by the same gate that holds those documents.
...
func (d *Door) gather(c *zip.Ctx, at At) ([]named, []Outage, int) {
	...
	// answer, and that reasoning does not survive contact with the deployment. The
	// door is the one caller that touches all of them, so its cost is the whole
	...
	// restart changes nothing — the property a horizontally scaled front door needs
	// and an asking door cannot have.
...
// published is the door's ONE source of what an app serves: its own Catalog when a
// caller supplied one, the binary's embedded catalog otherwise.

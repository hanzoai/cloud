...
var crossAppGlobals = map[string]string{
	...
	"flags.Board":                    "called by admin — the admin switchboard; renders EMPTY with configured:false.",
	...
	"flags.SetPlatformSwitch":        "called by admin, admission — the ONE write path for a platform switch; refused with 'engine not configured'.",

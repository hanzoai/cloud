...
func TestConnectorsListShape(t *testing.T) {
	slackConfiguredEnv(t)
	app := newApp(t, newKMS(t))

	// No principal → 403 (status is per-org, an org is required).
	if r := req(t, app, http.MethodGet, "/v1/connectors", "", nil); r.Code != http.StatusForbidden {
		t.Fatalf("no-principal list want 403, got %d", r.Code)
	}

	r := req(t, app, http.MethodGet, "/v1/connectors", "acme", nil)
...
func TestConnectorsConnectUnconfigured503(t *testing.T) {
...
func TestConnectorsConnectNoPrincipal403(t *testing.T) {
	slackConfiguredEnv(t)
	app := newApp(t, newKMS(t))
...
func TestConnectorsConnectKMSNotReady503(t *testing.T) {
	slackConfiguredEnv(t)
...
func TestConnectorsConnectInvalidOrg400(t *testing.T) {
	slackConfiguredEnv(t)
	app := newApp(t, newKMS(t))
...
func TestConnectorsCallbackHappyPath(t *testing.T) {
	slackConfiguredEnv(t)
	stubSlackAPI(t)
	kc := newKMS(t)
	app := newApp(t, kc)
	...
	if !strings.Contains(cb.Location, "console.hanzo.ai/connectors?connected=slack") {
...
func TestConnectorsCallbackReplayRejected(t *testing.T) {
	slackConfiguredEnv(t)
	stubSlackAPI(t)
	app := newApp(t, newKMS(t))
...
func TestConnectorsCallbackTamperedStateRejected(t *testing.T) {
	slackConfiguredEnv(t)
	stubSlackAPI(t)
	app := newApp(t, newKMS(t))
...
func TestConnectorsDisconnect(t *testing.T) {
	slackConfiguredEnv(t)
	stubSlackAPI(t)
	kc := newKMS(t)
	app := newApp(t, kc)
	...
	r := req(t, app, http.MethodPost, "/v1/connectors/slack/disconnect", "acme", nil)
	if r.Code != http.StatusOK {
	...
	// Idempotent.
	if r := req(t, app, http.MethodPost, "/v1/connectors/slack/disconnect", "acme", nil); r.Code != http.StatusOK {
		t.Fatalf("second disconnect want 200, got %d", r.Code)
	}
}

func TestConnectorsOrgIsolation(t *testing.T) {
	slackConfiguredEnv(t)
	stubSlackAPI(t)
	kc := newKMS(t)
	app := newApp(t, kc)
	...
	r := req(t, app, http.MethodGet, "/v1/connectors", "globex", nil)
	var out struct {
		Providers []providerView `json:"providers"`
	}
	_ = json.Unmarshal(r.Body, &out)
...
func stubSlackTeam(t *testing.T, teamName, botUser string) {
	...
	// Swap the REGISTERED provider, for the same reason stubSlackAPI does: the
	// catalog reads its endpoints once at init, so a test that reassigned a URL
	// afterwards would leave the real slack.com registered and prove nothing.
	sp := slackSpec()
...
func TestConsoleRedirectNoOpenRedirect(t *testing.T) {
	...
	for _, detail := range hostile {
		...
		if !strings.HasPrefix(loc, base+"/connectors?") {
		...
		if u.Scheme != "https" || u.Host != "console.hanzo.ai" || u.Path != "/connectors" {
...
func TestConnectorsCallbackSanitizesMeta(t *testing.T) {
	slackConfiguredEnv(t)
...
func TestConnectorsDisconnectNoPrincipal403(t *testing.T) {
	slackConfiguredEnv(t)
	stubSlackAPI(t)
	kc := newKMS(t)
	app := newApp(t, kc)
	...
	rq := httptest.NewRequest(http.MethodPost, "/v1/connectors/slack/disconnect", nil)
...
func TestConnectorsGithubScaffoldCallbackFailsClosed(t *testing.T) {
...
func TestConnectorsCallbackOversizedCodeRejected(t *testing.T) {
	slackConfiguredEnv(t)
	stubSlackAPI(t)
	app := newApp(t, newKMS(t))
...
func TestConnectorsSeamUnmountedFailsClosed(t *testing.T) {

// Package connectors is the ONE provider-agnostic OAuth plane for Hanzo Cloud —
// the /v1/connectors surface that lets an org connect a third-party account and
// hands the resulting per-org tokens to KMS custody.
//
...
// KMS custody and console redirect for every one. Adding a provider is a new
// DECLARATION, never a new route and never a new flow — see oauth2.go, which
// turns the authorization-code dance every one of these providers speaks into a
// single implementation the declarations fill in.
//
// "Connectors" is the product name, the package name, the endpoint and the KMS
// namespace. There is no /v1/integrations alias and no `integrations` package:
// two names for one thing is how a surface ends up with a client that has to try
// both and a catalog nobody can find.
//
// Surface (subsystem name "connectors", prefix /v1/connectors):
//
//	GET    /v1/connectors                      list providers + this org's status  -> {providers:[...]}
//	GET    /v1/connectors/:provider            one provider (404 unknown id)        -> Provider
//	POST   /v1/connectors/:provider/connect    begin OAuth (org-authed)             -> {authorizeUrl} | 503 | 403
//	GET    /v1/connectors/:provider/callback   PUBLIC, state-authed                  -> 302 to console
//	POST   /v1/connectors/:provider/disconnect revoke + forget (org-authed)         -> {disconnected:true}
//
...
// bearer is refused 403). The callback is provider-initiated and therefore
...
// envelope), keyed /orgs/{org}/connectors/{provider}. The store holds only
...
type Provider struct {
	...
	RedirectPath string   // OAuth redirect path; MUST equal /v1/connectors/{id}/callback
...
func register(p *Provider) {
	if p == nil || p.ID == "" {
		panic("connectors: register nil/empty provider")
	}
	if _, dup := registry[p.ID]; dup {
		panic("connectors: duplicate provider id " + p.ID)
	}
	registry[p.ID] = p
}
...
// Mount wires /v1/connectors/* onto app.
func Mount(app *zip.App, deps cloud.Deps) error {
	if app == nil {
		return fmt.Errorf("connectors.Mount: nil zip.App")
	}
	if deps.Logger == nil {
		return fmt.Errorf("connectors.Mount: nil deps.Logger")
	}
	log := deps.Logger.New("subsystem", "connectors")
	if deps.DataDir == "" {
		return fmt.Errorf("connectors.Mount: empty DataDir")
	}
	if err := os.MkdirAll(deps.DataDir, 0o755); err != nil {
		return fmt.Errorf("connectors.Mount: data dir: %w", err)
	}
	store, err := openStore(filepath.Join(deps.DataDir, "connectors.db"))
	if err != nil {
		return fmt.Errorf("connectors.Mount: open store: %w", err)
	}
	...
	// FIVE routes, and every one of them is provider-blind. A provider is a
	// declaration in this package; it never adds a route, so this list does not
	// grow when the catalog does.
	//
	// The chat BRIDGE — receiving messages FROM Slack/Discord/Teams/Telegram into
	// a conversation — is a different feature that CONSUMES this one (it reads a
	// custodied token through TokenFor). It registered a dozen platform-specific
	// literal front-doors here, each with its own trust boundary (Slack HMAC,
	// Discord Ed25519, Teams Bot Framework JWT, Telegram secret-token). Those
	// belong to the bridge, not to the plane that holds credentials: mixing them
	// put unauthenticated webhook endpoints inside the package whose whole job is
	// the org gate.
	app.Get("/v1/connectors", s.list)
	app.Get("/v1/connectors/:provider", s.get)
	app.Post("/v1/connectors/:provider/connect", s.connect)
	...
	app.Get("/v1/connectors/:provider/callback", s.callback)
	app.Post("/v1/connectors/:provider/disconnect", s.disconnect)

	log.Info("connectors mounted",
...
// successRedirect 302s to {console}/connectors?connected={provider}&account=<label>.
...
// failRedirect 302s to {console}/connectors?error={provider}&reason=<short msg>.
...
// {base}/connectors?{statusKey}={provider}&{detailKey}={detail}. base is the
...
func consoleRedirectURL(base, statusKey, provider, detailKey, detail string) string {
	return base + "/connectors?" + statusKey + "=" + url.QueryEscape(provider) +
...
// kmsPath is the per-org, per-provider KMS namespace: /orgs/{org}/connectors/{provider}.
...
func kmsPath(org, provider string) string {
	return "/orgs/" + org + "/connectors/" + provider
}
...
func TokenFor(ctx context.Context, org, provider, name string) ([]byte, error) {
	if mounted == nil {
		return nil, fmt.Errorf("connectors: not mounted")
	}
...
func (s *svc) tokenFor(ctx context.Context, org, provider, name string) ([]byte, error) {
	if !validOrg(org) {
		return nil, fmt.Errorf("connectors: invalid org")
	}
	...
	if !found {
		return nil, fmt.Errorf("connectors: %s not connected for org", provider)
	}
	if !s.kmsReady() {
...
// "the Connection for (org,provider)". The bridge calls connectors.ConnectionFor.

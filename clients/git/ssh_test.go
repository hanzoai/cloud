...
// TestSSHKeyRegistrationAndAuth proves the fingerprint→org resolution used by
// the PublicKeyCallback: a registered key resolves to its org; an unregistered
// key fails closed; and a key belongs to exactly one org (no cross-org).
func TestSSHKeyRegistrationAndAuth(t *testing.T) {
	...
	// globally unique — a key belongs to exactly one org).
...
// namespace even when it crafts a beta path — the SSH org-scope guard.

...
func TestMirrorImportCloneIdempotentIsolation(t *testing.T) {
	...
	// Two independent EXTERNAL git sources served over plain smart-HTTP (a
	// stand-in for github.com), fetched by the mirror's streaming `git fetch`.
	srcRoot := t.TempDir()
	ha := gitSource(t, srcRoot, "srca", "# source A\n", "v1.0.0")
	hb := gitSource(t, srcRoot, "srcb", "# source B\n", "")
	srcBase := serveGitHTTP(t, srcRoot)
	srcA := srcBase + "/srca.git"
	srcB := srcBase + "/srcb.git"

	// 1) First mirror clones source A into a fresh repo <acme/mirror>.
	v := callMirror(t, app, "acme", "mirror", srcA)
	if v.Org != "acme" || v.Name != "mirror" || v.Head != ha {
	...
	if _, head = cloneRepo(t, base, "acme", "mirror"); head.String() != ha {
	...
	if _, head = cloneRepo(t, base, "acme", "mirror"); head.String() != hb {
...
func TestMirrorSelfHostThenPushDeploys(t *testing.T) {
	...
	// Stand in for github.com/hanzoai/cloud (an EXTERNAL git-http source), then
	// mirror it to hanzoai/cloud. Mirror-in is import-only: it does NOT fire a
	// deploy, so we register the build trigger only now — the sole event we
	// observe is the later PUSH to the self-hosted repo, the behavior under test.
	srcRoot := t.TempDir()
	upstream := gitSource(t, srcRoot, "upstream", "# hanzo cloud\n", "v0.1.0")
	srcBase := serveGitHTTP(t, srcRoot)
	callMirror(t, app, "hanzoai", "cloud", srcBase+"/upstream.git")

...
//	view          one entry per widget: what its ticket is checked against —
//	              the secret it must carry, and when that lapses
...
// pass is what one widget's ticket is checked against: the secret that ticket
// must carry, and when it stops being accepted. It is kept out of the snapshot
// because the snapshot is sent.
type pass struct {
...
func iframe(session, name, ticket string) string {
	return "/v1/bot/board/" + url.PathEscape(session) + "/" + url.PathEscape(name) +
...
func held(c *Call, st *Store, k string) (map[string]pass, error) {
	out := map[string]pass{}
...
func keep(c *Call, st *Store, k string, passes map[string]pass) error {
	if err := st.Put(c.Context(), colView, k, passes); err != nil {
...
func tickets(c *Call, st *Store, k string, b *board) error {
	...
	for i := range b.Widgets {
		...
		if v.Secret == "" || now.Add(ticketLife/4).UnixMilli() > v.Lapses {
			v = pass{Secret: token(), Lapses: now.Add(ticketLife).UnixMilli()}
			views[w.Name] = v
			fresh = true
		}

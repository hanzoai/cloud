...
func Register(name string, need Scope, fn Func) {
	if name == "" {
		panic("bot.Register: a method needs a name")
	}
	if fn == nil {
		panic("bot.Register: " + name + " has no function")
	}
	if need != Open && !scopes[need] {
		panic("bot.Register: " + name + " asks for unknown scope " + string(need))
	}
	surface.mu.Lock()
	defer surface.mu.Unlock()
	if _, dup := surface.methods[name]; dup {
		panic("bot.Register: duplicate method " + name)
	}
...
func relayInto(c *Call, method string, params, v any) error {
	...
	if err != nil {
		c.svc.Log.Error("bot: encode relayed answer", "method", method, "err", err)
		return Unavailable("%s answered something that could not be read", method)
	}
	if err := json.Unmarshal(b, v); err != nil {
		c.svc.Log.Error("bot: decode relayed answer", "method", method, "err", err)
...
func answer(c *Call, id string) *Response {
	...
	if err != nil {
		...
		if f.Code == codeUnavail {
			c.svc.Log.Error("bot method failed", "method", c.Method, "org", c.me.org, "err", err)
		}

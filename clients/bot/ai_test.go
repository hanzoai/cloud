...
func TestAiModelsListNamesWhatTheGatewayServes(t *testing.T) {
	...
	// An unqualified id names no provider of its own, and the schema requires
	// one on every row (ModelsListResultSchema: id, name and provider, minLength
	// 1). Omitting it does not make one row plainer, it makes the catalog
	// unreadable — the selector rejects the whole payload. So the row says where
	// the model was reached, which is this gateway, and that is a fact rather
	// than a guess at whose model it is.
	if second["provider"] != gatewayProvider {
		t.Errorf("an unqualified model named provider %v, want %q", second["provider"], gatewayProvider)
	}

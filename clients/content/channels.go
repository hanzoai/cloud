...
func (d socialDistributor) Publish(ctx context.Context, org string, req DistributeRequest) (DistributeResult, error) {
	...
	targets = skipDistributed(targets, req.Distributed) // idempotency: never re-post a channel
...
// skipDistributed drops any target whose integration id is already recorded as
// distributed on the doc (req.Distributed). It is the edge half of the idempotency
// guard: publish.go supplies the already-posted set, the edge (the only place that
// resolves tokens → integration ids) enforces it. A channel that failed last time is
// NOT in the set (only successes are recorded), so it stays retryable.
func skipDistributed(targets []socialIntegration, done map[string]bool) []socialIntegration {
	if len(done) == 0 {
		return targets
	}
	out := make([]socialIntegration, 0, len(targets))
	for _, t := range targets {
		if !done[t.ID] {
			out = append(out, t)
		}

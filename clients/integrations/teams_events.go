...
func (s *svc) teamsEvents(c *zip.Ctx) error {
	bridgeReady()
	if !teamsConfigured() {
	...
	fresh, err := s.store.MarkEvent(c.Context(), "teams", act.ID)
	if err != nil {
	...
	reply := s.teamsReplier(act.ServiceURL, act.ConversationID)
...
func (s *svc) teamsReplier(serviceURL, conversationID string) replyFunc {
	return func(ctx context.Context, text string, _ephemeral bool) error {
		return s.teamsSendActivity(ctx, serviceURL, conversationID, text)
	}
}
...
// before expiry.
func (s *svc) teamsReplyToken(ctx context.Context) (string, error) {
	teamsTokMu.Lock()
...
func (s *svc) teamsSendActivity(ctx context.Context, serviceURL, conversationID, text string) error {
	tok, err := s.teamsReplyToken(ctx)
	if err != nil {
		return err
	}

...
// bridge_state.go holds the PLATFORM-AGNOSTIC state primitives the account-link
// flows share: a signed, TTL'd, single-use "subject state" and an in-process
// single-use seen-set. Both are provider-blind — Slack's (team,user) link,
// Discord's (guild,user), Teams' (tenant,user) and Telegram's (chat,user) link all
// compose their subject and sign it here. Orthogonal to the OAuth-connect state in
// state.go (statePayload{org,provider,nonce}); both HMAC with the SAME s.stateKey —
// one signing key, distinct named subjects.
...
// backs the account-link single-use guarantee: a signed link state is redeemed
// exactly once within its TTL.
//
// SCOPE: per-PROCESS. In a multi-replica deployment this is DEFENSE-IN-DEPTH: the
// PRIMARY single-use guarantee is each platform's own single-use OAuth `code` AND
// hanzo.id's single-use OIDC `code`, plus the state's HMAC + browser-bound cookie +
// short TTL. Eviction is strictly age-based (a within-TTL entry is NEVER evicted),
// which forbids an evict-then-replay attack; memory is bounded temporally.
type seenSet struct {
	mu    sync.Mutex
	ttl   time.Duration

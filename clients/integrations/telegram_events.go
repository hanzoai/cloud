...
// payload limit.
func (s *svc) telegramConnect(c *zip.Ctx) error {
	...
	if err := s.store.PutNonce(c.Context(), code, org, "telegram"); err != nil {
...
func (s *svc) telegramWebhook(c *zip.Ctx) error {
	bridgeReady()
	...
	fresh, err := s.store.MarkEvent(c.Context(), "telegram", strconv.FormatInt(m.UpdateID, 10))
	if err != nil {
	...
	reply := s.telegramReplier(in)
	s.bridgeSpawn(org, func() { s.runBridgeTurn(org, in, reply) })
	return c.NoContent(http.StatusOK)
}
...
func (s *svc) telegramBind(c *zip.Ctx, m telegramMessage, code string) {
	org, ok, err := s.store.ClaimNonce(c.Context(), code, "telegram")
	if err != nil {
	...
	chatID := strconv.FormatInt(m.ChatID, 10)
	if !ok {
		_ = s.telegramSend(c.Context(), m.ChatID, m.MessageID, "That connect link is invalid or already used. Start again from the Hanzo console.")
		return
	}
	...
	_ = s.telegramSend(c.Context(), m.ChatID, m.MessageID, "Connected to Hanzo. Mention @hanzo (or /hanzo) to chat.")
}
...
func (s *svc) telegramReplier(in Inbound) replyFunc {
	return func(ctx context.Context, text string, _ephemeral bool) error {
		...
		return s.telegramSend(ctx, chatID, msgID, text)
	}
}
...
func (s *svc) telegramSend(ctx context.Context, chatID, replyTo int64, text string) error {

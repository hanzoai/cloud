...
func teamsLinkURL(s *cloud.Service[state], tenant, _user string) (string, error) {
	if !teamsLinkConfigured() {
		return "", fmt.Errorf("teams: account linking not configured")
	}
	state, err := signSubject(s.State.stateKey, "tmsentry:"+tenant, 0)
	if err != nil {
		return "", err
	}
...
func teamsLink(s *cloud.Service[state], c *zip.Ctx) error {
	...
	state, err := signSubject(s.State.stateKey, "tms:"+tenant+":"+nonce, 0)
	if err != nil {
	...
	q := url.Values{
		...
		"response_type": {"code"},
		"redirect_uri":  {teamsLinkAADURI(s)},
		"response_mode": {"query"},
...
func teamsLinkAAD(s *cloud.Service[state], c *zip.Ctx) error {
	...
	oid, tid, err := teamsExchangeIdentify(c.Context(), tenant, code, teamsLinkAADURI(s))
	if err != nil {
	...
	oidcVal, err := signSubject(s.State.stateKey, "tmslink:"+tenant+":"+oid, 0)
	if err != nil {
		clearLinkCookie(c, teamsInitCookie)
	...
	return redirect(c, oidcAuthorizeURL(teamsLinkCallbackURI(s), oidcVal))
}
...
func teamsLinkCallback(s *cloud.Service[state], c *zip.Ctx) error {
	...
	ctx := c.Context()
	ts, err := oidcExchange(ctx, code, teamsLinkCallbackURI(s))
	if err != nil {
	...
	if err := putUserLink(s, org, "teams", oid, userLink{Subject: sub, Org: userOrg, Refresh: ts.Refresh}); err != nil {

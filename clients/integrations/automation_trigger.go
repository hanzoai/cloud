...
// client-supplied field. It returns how many flows the event started.
type TriggerFunc func(ctx context.Context, org, source, name, dedupeKey string, payload map[string]any) (int, error)
...
// already be the verified tenant.
func fireTrigger(ctx context.Context, org, source, name, dedupeKey string, payload map[string]any) {
	if fireAutomation == nil || org == "" {
		return
	}
	_, _ = fireAutomation(ctx, org, source, name, dedupeKey, payload)
}

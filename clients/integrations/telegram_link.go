...
func (s *svc) telegramLinkURL(chatID, _user string) (string, error) {
	if !telegramLinkConfigured() {
		return "", fmt.Errorf("telegram: account linking not configured")
	}
	state, err := signSubject(s.stateKey, "tgentry:"+chatID, 0)
	if err != nil {
		return "", err
	}
...
func (s *svc) telegramLink(c *zip.Ctx) error {
	bridgeReady()
	if !telegramLinkConfigured() {
	...
	linkVal, err := signSubject(s.stateKey, "tgchat:"+chatID+":"+nonce, 0)
	if err != nil {
...
func (s *svc) telegramLinkAuth(c *zip.Ctx) error {
	bridgeReady()
	if !telegramLinkConfigured() {
		return zip.Errorf(http.StatusServiceUnavailable, "account linking not configured")
	}
	...
	oidcVal, err := signSubject(s.stateKey, "tglink:"+chatID+":"+tgUser, 0)
	if err != nil {
		s.clearTelegramLinkCookies(c)
	...
	return redirect(c, oidcAuthorizeURL(s.telegramLinkCallbackURI(), oidcVal))
}

// ── leg 3: hanzo.id OIDC callback (bind Telegram↔Hanzo) ─────────────────────

func (s *svc) telegramLinkCallback(c *zip.Ctx) error {
	bridgeReady()
	if !telegramLinkConfigured() {
		return zip.Errorf(http.StatusServiceUnavailable, "account linking not configured")
	}
	...
	sub2, nonce, ok := verifySubject(s.stateKey, cookie, 0)
	...
	ctx := c.Context()
	ts, err := oidcExchange(ctx, code, s.telegramLinkCallbackURI())
	if err != nil {
	...
	if err := s.putUserLink(org, "telegram", tgUser, userLink{Subject: sub, Org: userOrg, Refresh: ts.Refresh}); err != nil {
...
func (s *svc) clearTelegramLinkCookies(c *zip.Ctx) {
	clearLinkCookie(c, telegramInitCookie)
	clearLinkCookie(c, telegramLinkCookie)
}

func (s *svc) telegramLinkCallbackURI() string {

...
func (t *chatTurn) settle(state, why string, at int64) {
	...
	if err := t.st.Put(t.ctxOrBackground(), runIn, t.run, &rec); err != nil {
		t.log.Error("claw: write run", "org", t.org, "run", t.run, "err", err)
	}
	if !chatHolds(t) {
		return
	}
	...
	if err != nil {
		if !errors.Is(err, ErrNoDoc) {
			t.log.Error("claw: write session", "org", t.org, "key", t.key, "err", err)
		}
		return
	}
...
func (t *chatTurn) carry(client cloud.AIClient, prompt string) {
	...
	if err := chatAdd(t.ctxOrBackground(), t.st, t.key, &reply); err != nil {
		t.log.Error("claw: write message", "org", t.org, "key", t.key, "err", err)
		t.fail(err, at)
		return
	}
...
func (t *chatTurn) fail(err error, at int64) {
	...
	why := err.Error()
	t.log.Error("claw: turn failed", "org", t.org, "key", t.key, "run", t.run, "err", err)
...
func chatSend(c *Call) (any, error) {
	...
	if err := st.Get(c.Context(), runIn, run, &held); err == nil {
		...
		c.Log().Error("claw: read run", "org", c.Org(), "run", run, "err", err)
	...
	if err := chatAdd(c.Context(), st, key, &asked); err != nil {
		release()
		c.Log().Error("claw: write message", "org", c.Org(), "key", key, "err", err)
	...
	if err := st.Put(c.Context(), runIn, run, &rec); err != nil {
		release()
		c.Log().Error("claw: write run", "org", c.Org(), "run", run, "err", err)
	...
	if err := st.Do(c.Context(), func(st *Store) error {
		...
		c.Log().Error("claw: write session", "org", c.Org(), "key", key, "err", err)
	}
...
func chatOpen(c *Call, key, agent string) (*Store, *session, error) {
	...
	case errors.Is(err, ErrNoDoc):
		...
		if err := st.Get(c.Context(), sessionsIn, key, &s); err != nil {
			c.Log().Error("claw: read session", "org", c.Org(), "key", key, "err", err)
...
func chatLook(c *Call, p *chatHistoryParams) (any, error) {
	...
	if err != nil {
		c.Log().Error("claw: count transcript", "org", c.Org(), "key", key, "err", err)
	...
	if err != nil {
		c.Log().Error("claw: read transcript", "org", c.Org(), "key", key, "err", err)
		return nil, Unavailable("the transcript could not be read")
	}
	view.Total = total
...
//
// Provider is the model's provider, which is legible only from a qualified id
// (openai/gpt-…). An unqualified one is whatever the deployment's gateway
// routes it to, which is not this deployment's brand, so the field is omitted
// rather than filled with a value that means something else.
...
func agentList(c *Call) (any, error) {
	...
	if err != nil {
		c.Log().Error("claw: read agents", "org", c.Org(), "err", err)
...
func agentApply(c *Call, st *Store, id string, sent map[string]bool, p *agentUpdateParams) error {
	...
	if err := st.Get(c.Context(), agentIn, id, &a); err != nil && !errors.Is(err, ErrNoDoc) {
		c.Log().Error("claw: read agent", "org", c.Org(), "agent", id, "err", err)
	...
	if err := st.Put(c.Context(), agentIn, id, &a); err != nil {
		c.Log().Error("claw: write agent", "org", c.Org(), "agent", id, "err", err)

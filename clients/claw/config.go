...
// claw state is addressed — the bot's file when the call is bound to a bot, the
// org's otherwise (Call.Store). There is no file on disk, so nothing here reads
// or writes a path, and no revision is projected through a key: the caller
// already holds the whole document, so a plain content digest tells it nothing
// it did not send.
//
// The secret store is KMS. A value is sealed there under the org's own store
// path and never lands in a claw document; what claw keeps is the roster —
...
func getConfig(c *Call, st *Store) (*configDoc, error) {
	...
	case err != nil:
		c.Log().Error("read claw config", "org", c.Org(), "bot", c.Bot(), "err", err)
...
func writeConfig(c *Call, st *Store, d *configDoc, raw string, cfg map[string]any) (string, error) {
	...
	if err := st.Put(c.Context(), configDocs, configID, d); err != nil {
		c.Log().Error("write claw config", "org", c.Org(), "bot", c.Bot(), "err", err)
...
func secretStore(c *Call) (*Store, error) {
	...
	if err != nil {
		c.Log().Error("open claw store", "org", c.Org(), "err", err)
...
// empty, a bot's when it is not, under a claw segment that keeps the store
...
func secretsList(c *Call) (any, error) {
	...
	if err != nil {
		c.Log().Error("list claw secrets", "org", c.Org(), "err", err)
	...
	for _, doc := range docs {
		...
		if err := json.Unmarshal(doc.Doc, &s); err != nil {
			c.Log().Error("read claw secret", "org", c.Org(), "name", doc.ID, "err", err)
			return nil, Unavailable("the secret store could not be read")
		}
		e := secretEntry{
		...
		if s.Kind == "env" {
			...
			if err != nil {
				c.Log().Error("open claw secret", "org", c.Org(), "name", doc.ID, "err", err)
				return nil, Unavailable("the secret store could not be read")
			}
...
func secretsSet(c *Call) (any, error) {
	...
	if err := keys.PutSecret(c.Context(), ref, []byte(p.Value)); err != nil {
		c.Log().Error("seal claw secret", "org", c.Org(), "name", p.Name, "err", err)
	...
	if err := st.Do(c.Context(), func(st *Store) error {
		...
		c.Log().Error("write claw secret", "org", c.Org(), "name", p.Name, "err", err)
		return nil, Unavailable("the secret could not be stored")
	}
...
func secretsDelete(c *Call) (any, error) {
	...
	if err := keys.PutSecret(c.Context(), ref, nil); err != nil {
		c.Log().Error("clear claw secret", "org", c.Org(), "name", p.Name, "err", err)
		return nil, Unavailable("the secret could not be deleted")
	}
	if err := st.Delete(c.Context(), secretDocs, p.Name); err != nil {
		c.Log().Error("forget claw secret", "org", c.Org(), "name", p.Name, "err", err)
		return nil, Unavailable("the secret could not be deleted")
	}
	return secretAck{OK: true}, nil
}
...
//
// The protocol also has room for memory, disk, load and the CPU model. Reading
// those means a host-metrics reader — cgroup limits, meminfo, statfs — which
// this cloud does not have; the fields are left out rather than answered with a
// zero, and the web UI renders a dash for each.

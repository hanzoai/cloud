...
// Store is one file of protocol state: the org's, or a single bot's.
//
...
// made, applied per bot instead of per process.
//
...
var ErrNoDoc = errors.New("claw: no such document")
...
CREATE TABLE IF NOT EXISTS docs (
  ...
  PRIMARY KEY (collection, id)
	...
	if _, err := db.Exec(schema); err != nil {
		return nil, fmt.Errorf("claw schema: %w", err)
	}
...
func (s *Store) Put(ctx context.Context, collection, id string, doc any) error {
	...
	if err != nil {
		return fmt.Errorf("claw: encode document: %w", err)
	}
	if len(b) > MaxDoc {

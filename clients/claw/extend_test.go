...
func TestCredentialIsSealedNotStored(t *testing.T) {
	...
	ref := "orgs/acme/claw/skills/search/apiKey"
...
func TestSecretEnvIsSealedAndPlainEnvIsNot(t *testing.T) {
	...
	if got, ok := heldSecret(t, vault, "orgs/acme/claw/skills/search/env/API_TOKEN"); !ok || got != "t-1" {
...
func TestASealedCredentialIsNotSharedWithAnotherTenant(t *testing.T) {
	...
	held, ok := heldSecret(t, vault, "orgs/acme/bots/botalpha1/claw/skills/weather/apiKey")
	if !ok {
...
func TestAnOrdinaryOrgsReferenceDoesNotMove(t *testing.T) {
	...
	if held, ok := heldSecret(t, vault, "orgs/acme/claw/OPENAI_API_KEY"); !ok || held != "sk-live" {

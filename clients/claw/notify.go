...
import (
	...
	// pushResolve one whether or not its image carries a zone database.
	_ "time/tzdata"
...
func init() {
	...
	Announce("users.prefs.changed", "session.wake")
}
...
// pushDetails is the pushClosed set of detail levels a notification may carry.
...
func notifyStore(c *Call) (*Store, error) {
	...
	if err != nil {
		c.svc.Log.Error("open claw store", "org", c.me.org, "err", err)
...
// pushCheckDetail refuses a detail level outside the pushClosed set.
...
func pushBound(c *Call, st *Store, endpoint string) (pushSub, error) {
	...
	if errors.Is(err, ErrNoDoc) || (err == nil && sub.User != c.User()) {
		return pushSub{}, Forbidden("subscription is not pushBound to this user")
	}
	if err != nil {
		return pushSub{}, err
	}
...
func pushSubscribe(c *Call) (any, error) {
	...
	if err := st.Do(c.Context(), func(st *Store) error {
		...
		case err == nil && held.User != c.User():
			return Forbidden("endpoint is already pushBound to another user")
		case err == nil:
...
// otherwise each make one, and the loser's browsers would be pushBound to a public
...
func vapidKey(ctx context.Context, s *cloud.Service[state]) (*ecdsa.PrivateKey, error) {
	...
	if err := kms.PutSecret(ctx, vapidRef, der); err != nil {
		s.Log.Error("pushSeal web push signing key", "err", err)
...
func pushDeliver(ctx context.Context, key *ecdsa.PrivateKey, subject string, sub pushSub, payload []byte) pushResult {
	...
	body, err := pushSeal(sub, payload)
	if err != nil {
		out.Error = err.Error()
		return out
	}
	assertion, err := pushAssert(key, sub.Endpoint, subject)
	if err != nil {
		out.Error = err.Error()
		return out
	}
	...
	req.Header.Set("Authorization", "vapid t="+assertion+", k="+pushB64(pub.Bytes()))

	res, err := pushSender.Do(req)
	if err != nil {
		out.Error = err.Error()
		return out
	}
...
// pushSeal encrypts a payload to one browser, per RFC 8291 over the aes128gcm
// content encoding of RFC 8188.
//
// The browser's public key and a secret it shares only with the push
// subscription derive the key; a fresh key pair per message means the same
// payload never encrypts twice the same way. The body carries everything the
// browser needs to derive the same key: the salt, the record size, and the
// public half of the pair this message was encrypted under.
func pushSeal(sub pushSub, payload []byte) ([]byte, error) {
	ua, err := pushBrowserKey(sub.P256dh)
	if err != nil {
		return nil, err
	}
	secret, err := pushUnb64(sub.Auth)
	if err != nil || len(secret) != 16 {
		return nil, errors.New("auth is not a 16-byte secret")
	}
	self, err := ecdh.P256().GenerateKey(rand.Reader)
	if err != nil {
		return nil, err
	}
	shared, err := self.ECDH(ua)
	if err != nil {
		return nil, err
	}

	// The shared secret is pushBound to both parties' public keys before it becomes
	// key material, so a key derived for one browser cannot decrypt another's.
	prk, err := hkdf.Extract(sha256.New, shared, secret)
	if err != nil {
		return nil, err
	}
	info := append([]byte("WebPush: info\x00"), ua.Bytes()...)
	info = append(info, self.PublicKey().Bytes()...)
	...
	// 0x02 ends the last record. There is only ever one here: a notification is
	// a sentence, and a record big enough for it needs no continuation.
	plain := append(append([]byte{}, payload...), 0x02)
	if len(plain)+16 > pushRecord {
		return nil, errors.New("the notification is larger than one push record")
	}
	...
	selfPub := self.PublicKey().Bytes()
	body := make([]byte, 0, 16+4+1+len(selfPub)+len(sealed))
	body = append(body, salt...)
	body = binary.BigEndian.AppendUint32(body, pushRecord)
	body = append(body, byte(len(selfPub)))
	body = append(body, selfPub...)
	return append(body, sealed...), nil
}

// pushAssert builds the VAPID assertion of RFC 8292: a JWT saying which push
// service it is for, how long it is good for, and who the gateway is, signed
// with the key the browser pushBound its subscription to.
func pushAssert(key *ecdsa.PrivateKey, endpoint, subject string) (string, error) {
	u, err := url.Parse(endpoint)
	if err != nil {
		return "", err
	}
	head := pushB64([]byte(`{"typ":"JWT","alg":"ES256"}`))
	claims, err := json.Marshal(map[string]any{
		"aud": u.Scheme + "://" + u.Host,
		"exp": time.Now().Add(pushExpiry).Unix(),
		"sub": subject,
	})
	if err != nil {
		return "", err
	}
	signing := head + "." + pushB64(claims)
	...
	r.FillBytes(sig[:32])
	s.FillBytes(sig[32:])
	return signing + "." + pushB64(sig), nil
}
...
// the same reading of the protocol's pushClosed objects that Call.Bind applies to

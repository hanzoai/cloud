...
var published atomic.Pointer[Embedded]
...
func PublishEmbedded(e *Embedded) { published.Store(e) }

func currentEmbedded() *Embedded { return published.Load() }

// Client returns the in-process commerce.Client bound to THIS Embedded — it reads
// this embed's datastore directly. Used by tests and any caller holding the
// Embedded; the process-wide pick-time seam cloud wires is InProcessClient.
func (e *Embedded) Client() Client {
	return &inProcessClient{brand: e.brand, resolve: func() *Embedded { return e }}
}

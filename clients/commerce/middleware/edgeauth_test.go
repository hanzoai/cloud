...
// TestEdgeAuth_StripsSuperAdminSpoof proves the PLATFORM-superadmin header
// is stripped at the edge. After the SuperAdmin/org-admin split,
// GetIAMClaims reads X-User-IsSuperAdmin straight off the request and
// SuperAdmin() trusts it — so if EdgeAuth did NOT strip it, an in-cluster
// caller could forge `X-User-IsSuperAdmin: true` and pass every cross-org
...
func TestPermsHeader_AdminBitIsGlobalOnly(t *testing.T) {
	...
	for _, tc := range cases {
		...
		if got != tc.want {
			t.Fatalf("%s: permsHeader=%d want %d (admin bit set iff SuperAdmin)", tc.name, got, tc.want)
		}
...
// TestResolveBillingSubject_AdminOverride: a SuperAdmin may retarget the
...
// a non-SuperAdmin cannot use ?org= to read another org. The override is
...
func TestResolveBillingSubject_NonAdminIgnoresOverride(t *testing.T) {
	// Dave: org owner with IsAdmin=true (org-level), NOT a SuperAdmin.

...
import (
	...
	"github.com/hanzoai/cloud/clients/commerce/log/color"
)
...
func (f *stringFormatter) Format(calldepth int, r *logging.Record, output io.Writer) error {
	for _, part := range f.parts {
		if part.verb == fmtVerbStatic {
			...
			color.FmtVerbLevel(part.layout, r.Level, output)
		} else if part.verb == fmtVerbCallpath {
			depth, err := strconv.Atoi(part.layout)
			if err != nil {
				depth = 0
			}
			output.Write([]byte(formatCallpath(calldepth+2, depth)))
		} else {
			var v interface{}
			switch part.verb {
			case fmtVerbLevel:
...
func init() {
	...
	projectPath = strings.Replace(sdkPath, "/sdk/gopath/src/hanzo.ai", "", 1)
}

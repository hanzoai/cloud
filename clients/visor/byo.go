...
func attachCluster(s *cloud.Service[state], c *zip.Ctx) error {
	...
	if err := s.State.bill.Gate(c.Context(), principal.Payer(c), principal.Project(c), projectValidated, byoClusterKind, fee); err != nil {
	...
	s.State.bill.Meter(principal.Payer(c), principal.Project(c), byoClusterKind, fee, c.RequestID(), cloud.ClientIP(c))

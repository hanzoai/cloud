...
// TestMain makes the search suite build-tag agnostic (the same harness as the
// sibling clients/sync and clients/git suites). On an encryption-capable (cgo)
...
// real key) — then the index store opens (encrypted on cgo, plaintext on

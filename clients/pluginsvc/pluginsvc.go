...
func transportFor(via string) (http.RoundTripper, error) {
	...
	return nil, fmt.Errorf("transport %q not registered (its client package must call pluginsvc.RegisterTransport)", via)
}
...
func Mount(app *zip.App, deps cloud.Deps) error {
	if app == nil {
		return fmt.Errorf("pluginsvc.Mount: nil zip.App")
	}
	...
	man, err := load(path)
	if err != nil {
		return fmt.Errorf("pluginsvc.Mount: %w", err)
	}
	...
	for _, p := range man.Plugins {
		...
		if err != nil {
			return fmt.Errorf("pluginsvc.Mount: plugin %q: %w", p.Name, err)
		}
...
func init() {
	cloud.Register("plugins", 900, func(app any, deps cloud.Deps) error {
		a, ok := app.(*zip.App)
		if !ok {
			return fmt.Errorf("pluginsvc.Mount: app is %T, want *zip.App", app)
		}
		return Mount(a, deps)
	})
}

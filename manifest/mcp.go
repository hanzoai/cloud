...
import "github.com/hanzoai/cloud/manifest/mcp"

// The fleet's agent MCP server, as ONE fact — for the same reason PrefixesFor
// exists. The addresses themselves live in the leaf package mcp, which openapi
// reads too (neither package may import the other); the doctrine lives here.
//
...
// host MOVED zip's MCP server off the framework default, so on the host NOTHING
// claims FrameworkMCPPath — and because the console's terminal catch-all did not
// know that path was a machine endpoint, it answered it with the SPA shell:
//
//	GET  https://api.hanzo.ai/mcp  ->  200 text/html  <!DOCTYPE html>
//
// An MCP client checking a status code reads that as a healthy endpoint. It is a
...
// the address the edge REFUSES TO ANSWER WITH HTML cannot drift.
const (
	// MCPPath is the fleet's one agent MCP address: POST /v1/mcp, on the host that
	...
	//
	// It is deliberately NOT /v1/ai/mcp. The MCP server serves the UNION of every
	...
	// One fleet, one MCP server.
	MCPPath = mcp.Path

	// FrameworkMCPPath is zip's built-in default — where a plugin serves its OWN
	// MCP server and, therefore, where a host FORWARDS a composed tools/call (zip's
	// Plugin.mcpPath). cloud.Serve leaves it alone on purpose: a child's MCP server
	// sits at an internal address on a private ZAP socket, not an edge address, and
	// moving it would break the host's forward.
	//
	// On the HOST the server is not here, so the host CLAIMS this path anyway and
	// signposts it — registered by client.Use, the one call that also registers
	...
	// reasoning that a plugin serving its own MCP server here matched a real route
	// and never reached it. That was the defect: zip mounts this route only when the
	...
	// its own MCP path, fell through, and was told its server was at MCPPath, which
	// no plugin serves. POST /mcp -> 308, POST /v1/mcp -> 404, and the fleet reads
	// that non-2xx as an outage for a child that is serving fine. A plugin now
	// answers here with its OWN MCP server (zip.App.MCP) whether or not a route was
	// mounted over it, and an empty tool list is a 200.
	FrameworkMCPPath = mcp.Framework
	...
	// WWW-Authenticate header the server answers with (client/mcp.go), and the host

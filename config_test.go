...
// surface as HTTP in plaintext — /v1/function/{name}/invoke included, and that
...
func TestListenDefaults_AreLoopbackExceptTheEdge(t *testing.T) {
	...
	if got := cfg.ZAPListenAddr; got != "127.0.0.1:9653" {
		t.Errorf("ZAP listener defaults to %q, want 127.0.0.1:9653 — a bare :9653 offers /v1/function/{name}/invoke to the LAN", got)
	}

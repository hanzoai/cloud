// clawdrive opens the protocol socket and walks the seventeen-method floor from
// the contract, printing what came back.
package main
...
func main() {
	url := flag.String("url", "ws://127.0.0.1:8899/v1/claw", "socket")
	flag.Parse()
	...
	hello := ask("1", "connect", `{"protocolVersion":4,"client":{"name":"clawdrive","version":"1"},"caps":{}}`)
	...
	// the turn.
	fmt.Println("\n--- chat.send: the turn ---")
	sent := ask("16", "chat.send", `{"sessionKey":"main","message":"hello from clawdrive","idempotencyKey":"run-abc"}`)

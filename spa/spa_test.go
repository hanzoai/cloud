...
// TestAMissingAssetFallsBackToTheShellToo is the ONE case worth being explicit
// about, because it is a deliberate trade rather than an oversight: a request for
// a hashed chunk that is not in the bundle gets the shell (200 HTML), not a 404.
//
// It is the same rule as any other unknown path and it cannot be narrowed without
// knowing which paths belong to the router, which this handler does not and should
// not know. The consequence is that a browser holding a STALE shell asks for a
// chunk that no longer exists and is handed HTML where it expected JavaScript —
// which fails loudly in the console rather than silently, and is why the shell
// itself is served no-cache above.
func TestAMissingAssetFallsBackToTheShellToo(t *testing.T) {
	w := get(Handler(site(), "demo"), "/assets/app-deadbeef.js")
	if w.Code != http.StatusOK {
		t.Fatalf("GET a missing chunk = %d, want 200", w.Code)
	}
	if !strings.Contains(w.Body.String(), `<div id="root">`) {
		t.Error("a missing chunk did not fall back to the shell")
	}
}

...
//	                   every ownership handoff — the one invariant ha.Leases owes.
...
// ErrNoMembership is returned by Leases.Acquire when the live replica set is empty: no
...
// ownerView is the slice of Membership Leases needs: this replica's id and the
...
// CASLeases implements ha.Leases by claiming/renewing a per-org writer lease in a
// linearizable object store. Acquire returns the caller's monotone lease round;
// FencedStore then stamps every ship with it.
type CASLeases struct {
	store replica.ConditionalStore
	view  ownerView
}

// NewCASLeases builds a CASLeases over an atomic-CAS store and a membership view.
...
func NewCASLeases(store replica.ConditionalStore, view ownerView) *CASLeases {
	return &CASLeases{store: store, view: view}
}

// Acquire implements ha.Leases. It fails CLOSED unless this replica is the elected
// writer over a non-empty membership, then claims (strictly bumping the round) or
// renews (keeping the round) the org's lease.
func (f *CASLeases) Acquire(ctx context.Context, orgID string) (ha.Lease, error) {
...
func (f *CASLeases) claim(ctx context.Context, orgID, self string) (ha.Lease, error) {
	key := leaseKey(orgID)
	var lastErr error
...
func (f *CASLeases) readLease(ctx context.Context, key string) (round uint64, owner, version string, err error) {

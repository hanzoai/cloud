...
// wholeFile is the default codec: TRUNCATE-checkpoint the WAL so the main file holds
// every committed page, copy the file bytes, and frame them with the cek key sidecar so
// a successor can open the encrypted file. It is correct whether the file is encrypted
// (production, sidecar present) or plaintext (pure-Go dev, no sidecar) — the same path,
// no per-build branch.
type wholeFile struct{}

// produce checkpoints and copies the local file consistently: it takes db's SOLE
// connection (MaxOpenConns(1)), TRUNCATE-checkpoints the WAL so the main file holds every
// committed page, then reads the file bytes while still holding the connection so no
// writer can fold new frames in mid-read. The <db>.dek key sidecar (present only on an
// encrypting build) is read too and framed with the database bytes.
func (wholeFile) produce(ctx context.Context, db *sql.DB, dbPath string) ([]byte, error) {
	conn, err := db.Conn(ctx)
	if err != nil {
		return nil, fmt.Errorf("org: snapshot conn %s: %w", dbPath, err)
	}
	defer conn.Close()
	// TRUNCATE-checkpoint the WAL so the main file holds every committed page, and CHECK
	// the result: busy!=0 means the checkpoint could not fold all frames (a reader held
	// the WAL), so the file on disk is MISSING committed rows. Fail closed — a partial
	// snapshot shipped as complete is a silent lost write. (busy, logFrames, checkpointed)
	// is the PRAGMA's row.

...
func ServeWith(t *testing.T, observe func(org string, in plane.RecordIn)) *Commerce {
	...
	// field in sockaddr_un, and t.TempDir() spells the TEST'S NAME into its path — so
	// a descriptive Go test name silently pushes the socket over the limit, bind fails
	// inside the listener goroutine where nothing reads the error, and the only symptom
	// is that no debit ever arrives. Measured: one 62-character test name in apps/risk
	...
	// unread in the goroutine below and reporting it as a debit that never fired.
	if len(path) >= 104 {

...
// writes both through this same weave (-weave); with no flag the same weave is
// the check.
//
...
// weaveOut writes the woven document — everything the fleet serves. It is the
...
// which writes the woven document at the named path and its public projection
// beside it. Without the flag the same weave is the drift gate.
var weaveOut = flag.String("weave", "", "write the woven document to this path (regenerate both artifacts)")

// privatePath, publicPath, floorPath and specDir are relative to this package's
// directory.
//
// TWO ARTIFACTS, ONE WEAVE, AND THE NAME SAYS WHICH IS WHICH. openapi.yaml is the
...
//
// They are written by one run of one weave and compared by one test, so the two
...
// the woven document — the artifact this test protects — so the artifact exempted
...
func TestFleetIsTheWeaveOfItsApps(t *testing.T) {
	...
	//
	// The public contract is written BESIDE whatever -weave named, never at its
	...
	if !bytes.Equal(got, want) {
		t.Fatalf("private.yaml is not the weave of its apps' subsets (%d paths woven now). A route was added, "+
			"removed or renamed and either an app subset or the internal document was not regenerated. "+
			"Run `make describe` and commit the result.", len(woven.Paths))
	}
	...
	if !bytes.Equal(pubGot, wantPub) {
		t.Fatalf("openapi.yaml is not the public projection of the woven document (%d paths public now, out of "+
	...
	t.Logf("woven %d paths / %d schemas / %d tags from %d apps — byte-identical to %s; %d paths public (%s)",
		len(woven.Paths), schemaCount(woven), len(woven.Tags), len(manifest.Apps), privatePath,
...
// committed loads one of the two artifacts this weave writes.
...
func TestAlphaAndAdminReachOnlyThePrivateDocument(t *testing.T) {
	...
	// readings: x-stage is what the weave stamped, and a capability's row is what

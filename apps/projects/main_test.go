...
//
// Every test here mounts projects, and cek REFUSES to open a store without a
// master key on an encryption-capable build — which, since hanzoai/sqlite v0.3,
// includes the pure-Go build. Without this the WHOLE package fails at Mount with
// "CLOUD_KMS_MASTER_KEY_REF is required", so no test in it can run at all.
//

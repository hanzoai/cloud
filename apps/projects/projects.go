...
// inherits the previous owner's rollback menu, the SOURCE at the forge is
// retired for the same reason and one more — it is the store a reclaimed slug
// could inherit something READABLE from — the S3 origin is purged under BOTH
// `<org>/<slug>/` and the site's sibling release space, and the edge cache-tag
// is flushed. A failure in any of those is logged and the delete still answers
// 204 — resurrecting a project because a purge missed would be worse than a
// leaked prefix.
//
// Scope: a validated principal is required (403 without one) and the project is

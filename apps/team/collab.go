...
// constant, so the RPC's declared path cannot drift from the WebSocket beside
// it — and a constant is also what lets cmd/zipdoc evaluate the RPC's absolute
// path argument.
...
func (s *collabService) register(app cloud.Router, zapp *zip.App, guard guardFn) {
	...
	g.Use(cloud.Bridge())
	zip.Post(zapp, collabPrefix+"/rpc/:documentId", s.rpc)
}
...
// CollabRPC is the collaborative-markup snapshot plane the Team editor speaks.

...
func scannerRoutes(app cloud.Router, s *cloud.Service[*state]) error {
	zapp := cloud.ZipApp(app)
	if zapp == nil {
		return fmt.Errorf("books.scannerRoutes: router is not backed by a *zip.App; typed ops have nowhere to register")
	}
	o := booksOps{s: s}
	zip.Get(zapp, "/v1/books/inbox", o.listInbox)
	zip.Get(zapp, "/v1/books/vendors", o.listVendors)
	zip.Get(zapp, "/v1/books/rules", o.listRules)
	zip.Get(zapp, "/v1/books/transactions", o.listTransactions)
	...
	zip.Post(zapp, "/v1/books/scan/book", o.bookScan)
	zip.Post(zapp, "/v1/books/vendors", o.upsertVendor)
	zip.Post(zapp, "/v1/books/rules", o.upsertRule)

...
func init() {
	...
	zip.Describe("GET /v1/books/balance-sheet", zip.Doc{
		Description: "BalanceSheet returns the org's Balance Sheet as of `to` (empty = all time).\nIt carries the Assets == Liabilities + Equity equation proof.",
		Fields: map[string]string{
	...
	zip.Describe("GET /v1/books/bank/unreconciled", zip.Doc{
		Description: "ListUnreconciled returns the org's unmatched bank inflows and their open questions.\nIt is the queue a human answers so an unexplained deposit is never guessed into\nrevenue.",
		Fields: map[string]string{
			"ledgerIn.sandbox":             "Sandbox reads the org's SANDBOX ledger when it is exactly \"true\"; anything else\nreads the live one.",

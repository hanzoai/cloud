...
func init() {
	...
	zip.Describe("github.com/hanzoai/cloud/apps/market GET /v1/market/survey", zip.Doc{
		...
		Fields: map[string]string{
			...
			"Reach.at":        "At is `read`, `unconfigured`, `unreachable` or `refused`.\n\nThe four values are written out here because this document cannot carry an\nenum, so the description IS the contract a client reads. Spelling the Go\nconstant names instead would name four identifiers no caller can see.",
			"Reach.why":       "Why is the upstream's own words, on Unreachable and Refused, and empty\notherwise. It is the upstream's and not ours: a failure reported without its\nreason sends the reader to look at the venue, which is the one place the fault\nis not.",
			"Survey.carries":  "Carries is all four where the node answered and `null` where it did not. It is\nnever a short list: a partial read reporting three would let a reader count\nthem and conclude the chain is missing one.",
			"Survey.rpc":      "RPC is the endpoint that was asked, so an answer names where it came from.",
			"where.chain":     "Chain is the chain's slug — `cchain`, `zoo` — as `chains` reports it. It is\nthe indexer's word for the chain and NOT the chain id: `96369`, `C` and\n`c-chain` all name nothing.",
		},
	})
	zip.Describe("github.com/hanzoai/cloud/apps/market GET /v1/market/token", zip.Doc{
		...
		Fields: map[string]string{
			"Day.count":    "Count is transactions in the day, where the table keeps one.",
			"Day.date":     "Date is the day's start, unix seconds.",
			"History.at":   "At is the token this is the history of, lowercased.",
			"History.days": "Days is oldest first, which is the order a chart draws. `[]` for a token the\nindexer holds no day for, `null` where the read failed.",
			"Reach.at":     "At is `read`, `unconfigured`, `unreachable` or `refused`.\n\nThe four values are written out here because this document cannot carry an\nenum, so the description IS the contract a client reads. Spelling the Go\nconstant names instead would name four identifiers no caller can see.",
			"Reach.why":    "Why is the upstream's own words, on Unreachable and Refused, and empty\notherwise. It is the upstream's and not ours: a failure reported without its\nreason sends the reader to look at the venue, which is the one place the fault\nis not.",
	...
	zip.Describe("github.com/hanzoai/cloud/apps/market GET /v1/market/tokens", zip.Doc{
		...
		Fields: map[string]string{
			...
			"Reach.why":     "Why is the upstream's own words, on Unreachable and Refused, and empty\notherwise. It is the upstream's and not ours: a failure reported without its\nreason sends the reader to look at the venue, which is the one place the fault\nis not.",
			"Token.at":      "At is the token's contract address, lowercase.",
			"Tokens.tokens": "Tokens is `[]` where the indexer holds none and `null` where the read failed.",
			"where.chain":   "Chain is the chain's slug — `cchain`, `zoo` — as `chains` reports it. It is\nthe indexer's word for the chain and NOT the chain id: `96369`, `C` and\n`c-chain` all name nothing.",
		},
	})
}

...
// This file is the MEASUREMENT that typing /v1/destination did not move its
...
var untypedByDesign = map[string]string{
	"POST /v1/destination/{platform}": "connect. Its request body's property NAMES are chosen at " +
...
func destinationOps(t *testing.T) (served map[string]bool, typed map[string]*openapi.Operation) {
	t.Helper()
	app := mountApp(t)
	doc, err := openapi.Spec(app, openapi.Info{Title: "destination", Version: "v1"})
	if err != nil {
		t.Fatalf("spec: %v", err)
	}
	...
	ours := func(p string) bool { return strings.HasPrefix(p, "/v1/destination") }
...
func TestTheUntypedRouteStillDeclaresItsBodies(t *testing.T) {
	...
	op := doc.Paths["/v1/destination/{platform}"]["post"]
	if op == nil {
		t.Fatal("POST /v1/destination/{platform} is not in the document at all")
	}
...
func TestTestReportsTheSameTwoShapes(t *testing.T) {
	...
	if code, _ := do(t, app, http.MethodPost, "/v1/destination/ga4/test", "acme", true, nil); code != http.StatusNotFound {
...
func TestMutationsRequireOrgAdmin(t *testing.T) {
	...
	for _, r := range []struct{ method, path string }{
		{http.MethodDelete, "/v1/destination/ga4"},
		{http.MethodPost, "/v1/destination/ga4/test"},
	} {
...
func TestReadsAreOrgScopedAndFailClosed(t *testing.T) {
	app := mountApp(t)
	for _, r := range []struct{ method, path string }{
		{http.MethodGet, "/v1/destination"},
		{http.MethodGet, "/v1/destination/ga4"},
		{http.MethodDelete, "/v1/destination/ga4"},
		{http.MethodPost, "/v1/destination/ga4/test"},
	} {
	...
	if code, _ := do(t, app, http.MethodGet, "/v1/destination/nope", "acme", false, nil); code != http.StatusNotFound {
...
func TestListIsOneKeyedList(t *testing.T) {
	app := mountApp(t)
	code, body := do(t, app, http.MethodGet, "/v1/destination", "acme", false, nil)
...
func TestDisconnectIsOneKeyedAck(t *testing.T) {
	app := mountApp(t)
	code, body := do(t, app, http.MethodDelete, "/v1/destination/ga4", "acme", true, nil)
	if code != http.StatusOK {

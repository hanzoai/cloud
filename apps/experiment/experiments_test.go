...
func TestExperiment_FeatureFlagProof(t *testing.T) {
	...
	code, body := do(t, app, http.MethodPost, "/v1/experiment", org, project, featureExperimentBody())
	...
	code, ab := do(t, app, http.MethodGet, "/v1/experiment/checkout_cta/assign?subject=user-1", org, project, nil)
	...
	code, _ = do(t, app, http.MethodPost, "/v1/experiment/checkout_cta/analyze?days=1", org, project, nil)
	...
	if code, m := do(t, app, http.MethodPost, "/v1/experiment/checkout_cta/decide", org, project, map[string]any{"winner": "treatment"}); code != http.StatusForbidden {
	...
	code, dec := doAdmin(t, app, http.MethodPost, "/v1/experiment/checkout_cta/decide", org, project, map[string]any{"winner": "treatment"})
...
func TestExperiment_CrossTenantDenied(t *testing.T) {
	...
	code, _ := do(t, app, http.MethodPost, "/v1/experiment", "acme", "", featureExperimentBody())
	if code != http.StatusCreated {
	...
	for _, tc := range []struct {
		...
		{http.MethodGet, "/v1/experiment/checkout_cta", nil, http.StatusNotFound},
		{http.MethodGet, "/v1/experiment/checkout_cta/assign?subject=u1", nil, http.StatusNotFound},
		{http.MethodPost, "/v1/experiment/checkout_cta/analyze", nil, http.StatusNotFound},
		{http.MethodPost, "/v1/experiment/checkout_cta/decide", map[string]any{"winner": "treatment"}, http.StatusNotFound},
	} {
	...
	if code, m := do(t, app, http.MethodGet, "/v1/experiment", "globex", "", nil); code != http.StatusOK || m["total"].(float64) != 0 {
		t.Fatalf("globex list must be empty: %d %v", code, m)
	}

	// An unvalidated request (no principal) is refused.
	if code, _ := do(t, app, http.MethodGet, "/v1/experiment/checkout_cta", "", "", nil); code != http.StatusForbidden {

...
func authorsOps(t *testing.T) (served map[string]bool, typed map[string]string) {
	t.Helper()
	app, _, _, _ := mount(t)
	doc, err := openapi.Spec(app, openapi.Info{Title: "authors", Version: "v1"})
	if err != nil {
		t.Fatalf("spec: %v", err)
	}
	...
	ours := func(p string) bool {
		return strings.HasPrefix(p, "/v1/authors") || strings.HasPrefix(p, "/v1/admin/authors")
	}
...
func TestAdminOpsRefuseANonAdmin(t *testing.T) {
	...
	for _, tc := range []struct{ method, path string }{
		{http.MethodGet, "/v1/admin/authors"},
		{http.MethodPost, "/v1/admin/authors/sweep"},
		{http.MethodPost, "/v1/admin/authors/aut_x/approve"},
		{http.MethodPost, "/v1/admin/authors/aut_x/suspend"},
		{http.MethodPost, "/v1/admin/authors/aut_x/payout"},
		{http.MethodGet, "/v1/admin/authors/aut_x/basis"},
	} {
...
func TestTenantOpsRefuseAnUnvalidatedCaller(t *testing.T) {
	app, _, _, _ := mount(t)
	for _, tc := range []struct{ method, path string }{
		{http.MethodGet, "/v1/authors"},
		{http.MethodGet, "/v1/authors/basis"},
		{http.MethodPost, "/v1/authors/connect"},
		{http.MethodPost, "/v1/authors/repos/verify"},
		{http.MethodPost, "/v1/authors/deploys/record"},
	} {
...
func TestBasisRefusesAMalformedPeriod(t *testing.T) {
	app, _, _, _ := mount(t)
	if code, _ := req(t, app, http.MethodGet, "/v1/authors/basis?period=july", "acme", false, nil); code != http.StatusBadRequest {
		t.Errorf("?period=july = %d, want 400", code)
	}
	if code, _ := req(t, app, http.MethodGet, "/v1/authors/basis?period=2026-07", "acme", false, nil); code != http.StatusOK {

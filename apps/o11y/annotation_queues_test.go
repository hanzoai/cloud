...
func annApp(t *testing.T) *zip.App {
	...
	// cloud.Bridge FIRST, exactly as Mount installs it: a typed op reads its
	// validated org off the context, so without this every route here 403s.
	app.Group(o11yPrefix).Use(cloud.Bridge())
	zip.Get(app, annQueuesRoute, s.listQueues)
	zip.Post(app, annQueuesRoute, s.createQueue, zip.WithStatus(http.StatusCreated))
	zip.Get(app, annQueueRoute, s.getQueue)
	zip.Patch(app, annQueueRoute, s.updateQueue)
	zip.Delete(app, annQueueRoute, s.deleteQueue)
	zip.Get(app, annItemsRoute, s.listItems)
	zip.Post(app, annItemsRoute, s.addItems, zip.WithStatus(http.StatusCreated))
	zip.Patch(app, annItemRoute, s.updateItem)
	return app
}

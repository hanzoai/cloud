...
// here — embed disabled, project unresolved, datastore hiccup — is logged and
...
// removeErrorSink is the installed sink's remover — nil until installErrorSink ran.
var removeErrorSink func()

// clearErrorSink detaches the error fan-out. Idempotent; safe when never installed.
func clearErrorSink() {
	if removeErrorSink != nil {
		removeErrorSink()
		removeErrorSink = nil
	}
}
...
// buildTags renders the descriptive tags the projection preserves: the emitting
// service (as service_name, so it populates the events-plane service column — the
// wire's own service when it named one, else the emitting surface), the site, the
// session, the library, the bounded url, whether the error was handled, and the

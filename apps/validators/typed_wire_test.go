...
func validatorSurface(t *testing.T) (served map[string]bool, typed map[string]string, schemas map[string]any) {
	t.Helper()
	app := mountApp(t)
	doc, err := openapi.Spec(app, openapi.Info{Title: "validators", Version: "v1"})
	if err != nil {
		t.Fatalf("spec: %v", err)
	}
	...
	ours := func(p string) bool { return strings.HasPrefix(p, "/v1/validators") }
...
func TestIdentityRefusalStillPrecedesTheBody(t *testing.T) {
	...
	if code, _ := send(t, app, http.MethodPost, "/v1/validators", "", "{not json"); code != http.StatusForbidden {
	...
	if code, _ := send(t, app, http.MethodPost, "/v1/validators", "acme", "{not json"); code != http.StatusBadRequest {
	...
	code, body := send(t, app, http.MethodPost, "/v1/validators", "acme", `{"tokenId":0}`)
...
func TestReadsRefuseWithoutAPrincipal(t *testing.T) {
	...
	for _, path := range []string{
		"/v1/validators", "/v1/validators/challenge?tokenId=1", "/v1/validators/7",
	} {
...
func TestTokenIDKeepsItsOneParseRule(t *testing.T) {
	...
	if code, _ := send(t, app, http.MethodGet, "/v1/validators/%207%20", "acme", ""); code != http.StatusBadRequest {
	...
	if code, _ := send(t, app, http.MethodGet, "/v1/validators/7", "acme", ""); code != http.StatusNotFound {
		t.Errorf("GET an unclaimed slot = %d, want 404", code)
	}
	// A non-numeric value is still the handler's 400.
	if code, _ := send(t, app, http.MethodGet, "/v1/validators/abc", "acme", ""); code != http.StatusBadRequest {
		t.Errorf("GET a non-numeric tokenId = %d, want 400", code)
	}
	// Zero is refused, as it always was.
	if code, _ := send(t, app, http.MethodGet, "/v1/validators/0", "acme", ""); code != http.StatusBadRequest {
		t.Errorf("GET tokenId 0 = %d, want 400", code)
	}
	// The same rule on the challenge query.
	if code, _ := send(t, app, http.MethodGet, "/v1/validators/challenge?tokenId=abc", "acme", ""); code != http.StatusBadRequest {
		t.Errorf("challenge with a non-numeric tokenId = %d, want 400", code)
	}
	code, body := send(t, app, http.MethodGet, "/v1/validators/challenge?tokenId=+7+", "acme", "")
...
func TestListLimitKeepsItsOneParseRule(t *testing.T) {
	...
	for _, q := range []string{"", "?limit=", "?limit=abc", "?limit=0", "?limit=-5", "?limit=+50+", "?limit=99999"} {
		code, body := send(t, app, http.MethodGet, "/v1/validators"+q, "acme", "")
		if code != http.StatusOK {
			t.Errorf("GET /v1/validators%s = %d (%s), want 200", q, code, body)
			continue
		}

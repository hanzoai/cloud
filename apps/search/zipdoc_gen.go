...
func init() {
	zip.Describe("github.com/hanzoai/cloud/apps/search GET /v1/admin/search/indexes", zip.Doc{
	...
	zip.Describe("github.com/hanzoai/cloud/apps/search GET /v1/admin/search/stats", zip.Doc{
	...
	zip.Describe("github.com/hanzoai/cloud/apps/search POST /v1/search", zip.Doc{
		...
		Fields: map[string]string{
			...
			"Fusion.backends":       "Backends is the per-leg report. Always populated.",
			"Fusion.hits":           "Hits is the fused, ranked result set.",
			"Fusion.mode":           "Mode is the mode actually used after `auto` resolution.",
			"Fusion.status":         "Status is the query's overall honesty signal:\n  ok          every consulted leg answered.\n  partial     at least one leg failed; Hits holds the survivors' results.\n  unavailable every consulted leg failed; Hits is empty AND that is stated.",
			"Fusion.took_ms":        "TookMS is the whole query's wall time in milliseconds — every leg it\nconsulted, plus fusion and paging. Each leg's own share is in\nBackends[].TookMS; the legs run in sequence, so this is at least their sum.",

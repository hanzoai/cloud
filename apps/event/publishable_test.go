...
func TestFoldException(t *testing.T) {
	...
	// /v1/event/errors lens reads — with the fault carrying the exception's class.
	...
	if f.attributes["$exception"] == "" {
		t.Fatal("attributes[$exception] missing — the /v1/event/errors lens surfaces the exception from it")
	}

...
func TestBadRangeIs400(t *testing.T) {
	app := mountApp(t)
	code, _ := do(t, app, http.MethodGet, "/v1/event/overview?range=bogus", "user-dave", "maxpower")
	if code != http.StatusBadRequest {
		t.Fatalf("bad range want 400, got %d", code)
	}
}

// TestHealthOwnedByAnalyticsHonest: /v1/event/health is the analytics
...
func TestHealthOwnedByAnalyticsHonest(t *testing.T) {
	...
	if h["service"] != "event" {
		t.Fatalf("health service want event (not generic liveness), got %v", h["service"])
	}

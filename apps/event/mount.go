...
//
//	READ
//	GET  /v1/event/overview     per-org KPIs (llm real; web/commerce honest-empty)
//	GET  /v1/event/timeseries   requests/tokens/spend over time (hour|day buckets)
//	GET  /v1/event/top          top models (real) + products + behavior lenses
//	                                (topPages/topReferrers/topSources over the events lens)
//	GET  /v1/event/errors                 the caller org's most recently captured errors
//	GET  /v1/event/insights/events        the caller org's most recent product events
...
// OWN /v1/event/health (below), and cloud.HealthOwner makes serve.go skip the
...
// /v1/event/* before the ai subsystem's /v1/* catch-all (150).
package event
...
const (
	...
	defaultTop = 10
	maxTop     = 100
	// defaultRecent / maxRecent bound the two recent-event reads (/v1/event/errors and
	// /v1/event/insights/events). ONE pair, because both read the same table the same way
...
//
// Health owns /v1/event/health explicitly (not JWT-gated: liveness must be
...
func routes(app cloud.Router, s *cloud.Service[state]) {
	...
	// /v1/event/errors is the type:'error' read lens over the same rows — a validated
	...
	// {"type":"publishable"}.
	//
	// A leaf on the same group as the other lenses. It used to be declared on the
	// APP with its whole path, because it WAS a whole path: joining a top-level
	// "/v1/errors" prefix with an empty leaf yields "/v1/errors/", an address this
	// API has never served. Folded under the capability it reads, it is a leaf like
	// any other and the special case goes with the special address.
	zip.Get(g, "/errors", o.errors)
...
// because /v1/event/errors and /v1/event/insights/events bound the same thing the same way.
...
// bad-gateway. This is the fix for /v1/event/* surfacing a raw 502 on a
...
func (o readOps) health(ctx context.Context, _ *noArgs) (*healthReport, error) {
	...
	res := &healthReport{Service: "event", Status: "ok", Datastore: connected, Warehouse: "hanzo",

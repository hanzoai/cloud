...
// ziti.go — how the fleet reaches an apiserver that lives on the Hanzo Zero
...
// kubeconfig whose server names that service's DNS — a host ending in ".ziti".
...
// dialer instead of TCP. The service name is the host with the ".ziti" suffix
// stripped — "k3s.acme.ziti" dials the fabric service "k3s.acme" — which is
...
// the identity fails a ".ziti" dial with the reason rather than failing at boot.
package fleet
...
// fabricDial dials a ".ziti" apiserver over the fabric. The port is carried by

...
func routes(app cloud.Router, s *cloud.Service[state]) error {
	...
	// Declared on the APP with the full path spelled, never on the group: the
	// registry keys an op on the group-JOINED path while cmd/zipdoc keys its
	// extraction on the path LITERAL at the call site, so a group-relative
	// declaration lifts prose under a key no operation ever looks up — the doc
	// comments below would be written, generated, and silently never rendered.
	zapp := cloud.ZipApp(app)
	if zapp == nil {
		return fmt.Errorf("crm.routes: router is not backed by a *zip.App; typed ops have nowhere to register")
	}
	o := ops{s: s}
	zip.Get(zapp, "/v1/crm/summary", o.summary)

	zip.Get(zapp, "/v1/crm/companies", o.listCompanies)
	zip.Post(zapp, "/v1/crm/companies", o.createCompany, zip.WithStatus(http.StatusCreated))
	zip.Get(zapp, "/v1/crm/companies/:id", o.getCompany)
	zip.Put(zapp, "/v1/crm/companies/:id", o.updateCompany)
	zip.Delete(zapp, "/v1/crm/companies/:id", o.deleteCompany)

	zip.Get(zapp, "/v1/crm/contacts", o.listContacts)
	zip.Post(zapp, "/v1/crm/contacts", o.createContact, zip.WithStatus(http.StatusCreated))
	zip.Get(zapp, "/v1/crm/contacts/:id", o.getContact)
	zip.Put(zapp, "/v1/crm/contacts/:id", o.updateContact)
	zip.Delete(zapp, "/v1/crm/contacts/:id", o.deleteContact)

	zip.Get(zapp, "/v1/crm/opportunities", o.listOpps)
	zip.Post(zapp, "/v1/crm/opportunities", o.createOpp, zip.WithStatus(http.StatusCreated))
	zip.Get(zapp, "/v1/crm/opportunities/:id", o.getOpp)
	zip.Put(zapp, "/v1/crm/opportunities/:id", o.updateOpp)
	zip.Delete(zapp, "/v1/crm/opportunities/:id", o.deleteOpp)
	...
	zip.Get(zapp, "/v1/crm/applications", o.listApplications)
	zip.Get(zapp, "/v1/crm/applications/:id", o.getApplication)
	zip.Patch(zapp, "/v1/crm/applications/:id", o.patchApplication)
	return nil
}

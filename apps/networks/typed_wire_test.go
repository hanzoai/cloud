...
func opsUnderTest(t *testing.T) (served map[string]bool, typed map[string]string, schemas map[string]any) {
	...
	doc, err := openapi.Spec(app, openapi.Info{Title: "networks", Version: "v1"})
	if err != nil {
		t.Fatalf("spec: %v", err)
	}
	...
	// under /v1/networks. A second stem here would be the defect the rename closed.
	ours := func(p string) bool { return strings.HasPrefix(p, "/v1/networks") }
...
func TestEveryTypedOpIsDescribed(t *testing.T) {
	...
	for key, desc := range typed {
		if strings.TrimSpace(desc) == "" {
			t.Errorf("%s has no description — run: go generate -run zipdoc ./apps/networks/...", key)
		}
	}
}
...
func TestValidatedOrgReachesEveryPrefix(t *testing.T) {
	...
	for _, path := range []string{
		"/v1/networks", "/v1/networks/routers", "/v1/networks/org-acme", "/v1/networks/services",
	} {
	...
	for _, path := range []string{"/v1/networks/services", "/v1/networks/org-acme"} {

...
func TestMeshServicesTenantScopedAndShape(t *testing.T) {
	...
	if code, _ := do(t, app, http.MethodGet, "/v1/networks/services", ""); code != http.StatusForbidden {
		t.Fatalf("no-org list want 403, got %d", code)
	}

	code, body := do(t, app, http.MethodGet, "/v1/networks/services", "acme")
	...
	code, body = do(t, app, http.MethodGet, "/v1/networks/services", "other")
	...
	code, body = do(t, app, http.MethodGet, "/v1/networks/services", "nobody")
	_ = json.Unmarshal(body, &listed)
...
func TestUnconfiguredFailsClosedExceptEmptyProjections(t *testing.T) {
	...
	for _, path := range []string{"/v1/networks/services", "/v1/networks/zt-acme"} {

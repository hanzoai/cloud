...
//	GET /v1/networks/services  the org's ZT edge services       -> {services:[meshView]}
...
func Mount(app cloud.Router, deps cloud.Deps) error {
	return cloud.Mount(app, deps, "networks", build, routes)
}
...
//
// cloud.Bridge is not installed here: the composer installs it once at the root,
// after the identity check that mints the validated org and before any subsystem
// registers a route — an order only the whole program can assert. The typed ops
...
// declared on the App with its whole path — joining "/v1/networks" with an empty
// leaf yields "/v1/networks/", a different address from the one it serves. So the
...
// meshServiceList is the GET /v1/networks/services envelope.

...
func TestNodesListIsOrgScoped(t *testing.T) {
	...
	for _, org := range []string{"acme", "globex"} {
		code, body := botCall(t, app, http.MethodGet, "/v1/nodes", org, nil)
	...
	code, body := botCall(t, app, http.MethodGet, "/v1/nodes", "initech", nil)
...
func TestNodeRoutesRefuseAnUnvalidatedCaller(t *testing.T) {
	...
	if code, _ := botCall(t, app, http.MethodPost, "/v1/nodes/n1/invoke", "", map[string]any{"command": "canvas.snapshot"}); code != http.StatusForbidden {
...
func TestInvokeReachesTheNodeAndPassesItsAnswerBack(t *testing.T) {
	...
	code, body := botCall(t, app, http.MethodPost, "/v1/nodes/laptop-1/invoke", "acme",
...
func TestForeignNodeAnswersLikeAnAbsentOne(t *testing.T) {
	...
	foreignCode, foreignBody := botCall(t, app, http.MethodPost, "/v1/nodes/laptop-1/invoke", "acme", call)
	absentCode, absentBody := botCall(t, app, http.MethodPost, "/v1/nodes/no-such-machine/invoke", "acme", call)
	...
	if code, _ := botCall(t, app, http.MethodGet, "/v1/nodes", "acme", nil); code != http.StatusOK {
...
func TestInvokeIsPolicyChecked(t *testing.T) {
	...
	code, body := botCall(t, app, http.MethodPost, "/v1/nodes/n1/invoke", "acme",
...
// The surface is /v1/bots/*. Never /api/, and nothing outside the subsystem's own
// prefix.
func TestRoutesLiveUnderTheirOwnNoun(t *testing.T) {
	...
	for _, r := range app.Fiber().GetRoutes() {
		...
		// /v1/nodes and the run surface is /v1/bots; a filter that knew only one
		// would walk past the other half and report nothing to check.
		if !strings.HasPrefix(r.Path, "/v1/bots") && !strings.HasPrefix(r.Path, "/v1/nodes") {
			continue
		}
		if r.Method == "HEAD" {

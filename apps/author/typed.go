...
// typed.go is the /v1/author + /v1/admin/author surface as TYPED ops — every one
// of the eleven.
//
...
//
// The two READS of the money-audit payload (GET /v1/author, GET /v1/author/basis,
...
// connectRequest is the POST /v1/author/connect body: the forge provider and an
...
func (o ops) connect(ctx context.Context, in *connectRequest) (*enrolment, error) {
	...
	login := normalizeLogin(cmp.Or(strings.TrimSpace(in.Login), strings.TrimSpace(in.GithubLogin)))
	...
	a, isNew, err := o.s.State.store.Connect(ctx, mint.ID("aut"), org, login, mint.ID("avc"), defaultShareBps, identityVerified, time.Now().Unix())
	if err != nil {
...
// verifyRequest is the POST /v1/author/repos/verify body.
...
func (o ops) verifyRepo(ctx context.Context, in *verifyRequest) (*claim, error) {
	...
	repo, isNew, err := o.s.State.store.UpsertVerifiedRepo(ctx, mint.ID("arp"), a.ID, target.canonical, method, time.Now().Unix())
	if err != nil {
		if err == errRepoOwned {
...
func (o ops) verifyOwner(ctx context.Context, c *zip.Ctx, a Author, org string, t verifyTarget) (*claim, error) {
	...
	rec, isNew, err := o.s.State.store.UpsertVerifiedOrg(ctx, mint.ID("aog"), a.ID, t.canonical, method, time.Now().Unix())
	if err != nil {
		if err == errOrgOwned {
...
// deployRequest is the POST /v1/author/deploys/record body: the source repository a
...
func (o ops) recordDeploy(ctx context.Context, in *deployRequest) (*deployRecord, error) {
	...
	edge, isNew, err := o.s.State.store.RecordDeploy(ctx, mint.ID("ade"), authorID, repoURL, project, deployingOrg, time.Now().Unix())
	if err != nil {

...
func earner(t *testing.T, app *zip.App, s *cloud.Service[state], fg *fakeGitHub) string {
	...
	if st, b := req(t, app, http.MethodPost, "/v1/author/repos/verify", "orgA", false, map[string]any{"repoUrl": "acme/widgets"}); st != http.StatusCreated {
...
func deploys(t *testing.T, app *zip.App, deployingOrg, repo, project string) {
	t.Helper()
	if st, b := req(t, app, http.MethodPost, "/v1/author/deploys/record", deployingOrg, false,
...
func getBasis(t *testing.T, app *zip.App, org, query string) (basisBody, []byte) {
	t.Helper()
	st, body := req(t, app, http.MethodGet, "/v1/author/basis"+query, org, false, nil)
	if st != http.StatusOK {
...
func TestBasisReconciles(t *testing.T) {
	...
	if st, b := req(t, app, http.MethodPost, "/v1/admin/author/"+idA+"/payout", "admin", true,
...
func TestBasisIsPureRead(t *testing.T) {
	...
	if st, b := req(t, app, http.MethodGet, "/v1/author", "orgA", false, nil); st != http.StatusOK {
...
func TestBasisOrgScoped(t *testing.T) {
	...
	st, body := req(t, app, http.MethodGet, "/v1/author/basis?id="+idA+"&org=orgA", "orgZ", false,
	...
	st2, body2 := req(t, app, http.MethodGet, "/v1/author/basis", "orgQ", false, nil)
	...
	if code, _ := req(t, app, http.MethodGet, "/v1/author/basis", "", false, nil); code != http.StatusForbidden {
...
func TestBasisShareOverride(t *testing.T) {
	...
	// Renegotiated to 25%.
	if st, b := req(t, app, http.MethodPost, "/v1/admin/author/"+idA+"/approve", "admin", true,
...
func TestBasisPeriodFilter(t *testing.T) {
	...
	for _, bad := range []string{"2026-7", "2026", "july", "2026-07-01", "2026-07' OR '1'='1"} {
		if st, b := req(t, app, http.MethodGet, "/v1/author/basis?period="+url.QueryEscape(bad), "orgA", false, nil); st != http.StatusBadRequest {
...
func TestBasisAttribution(t *testing.T) {
	...
	if st, b := req(t, app, http.MethodPost, "/v1/author/repos/verify", "orgA", false, map[string]any{"repoUrl": "acme/tools"}); st != http.StatusCreated {
...
func TestAdminBasisIdentical(t *testing.T) {
	...
	st, adminBody := req(t, app, http.MethodGet, "/v1/admin/author/"+idA+"/basis", "admin", true, nil)
	if st != http.StatusOK {
	...
	if code, _ := req(t, app, http.MethodGet, "/v1/admin/author/"+idA+"/basis", "orgA", false, nil); code != http.StatusForbidden {
		t.Fatalf("non-admin admin-basis want 403, got %d", code)
	}
	if code, _ := req(t, app, http.MethodGet, "/v1/admin/author/aut_nope/basis", "admin", true, nil); code != http.StatusNotFound {

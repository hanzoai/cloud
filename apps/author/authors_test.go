...
func connectOrg(t *testing.T, app *zip.App, s *cloud.Service[state], org, login string) (id, verifyCode string) {
	t.Helper()
	code, body := req(t, app, http.MethodPost, "/v1/author/connect", org, false, map[string]any{"githubLogin": login})
...
func adminSweepAndPay(t *testing.T, app *zip.App, s *cloud.Service[state]) {
	t.Helper()
	req(t, app, http.MethodPost, "/v1/admin/author/sweep", "admin", true, nil)
	...
	for _, a := range approved {
		...
		req(t, app, http.MethodPost, "/v1/admin/author/"+a.ID+"/payout", "admin", true,
...
func approve(t *testing.T, app *zip.App, id string) {
	t.Helper()
	if st, body := req(t, app, http.MethodPost, "/v1/admin/author/"+id+"/approve", "admin", true, nil); st != http.StatusOK {
...
func TestConnectIdempotentAndLinkedIdentity(t *testing.T) {
	...
	if code, _ := req(t, app, http.MethodPost, "/v1/author/connect", "", false, map[string]any{}); code != http.StatusForbidden {
		t.Fatalf("no-principal connect want 403, got %d", code)
	}
	// No supplied login + no linked account → 400.
	if code, _ := req(t, app, http.MethodPost, "/v1/author/connect", "orgA", false, map[string]any{}); code != http.StatusBadRequest {
	...
	code, body := req(t, app, http.MethodPost, "/v1/author/connect", "orgA", false, map[string]any{"githubLogin": "ignored"})
...
func TestVerifyRepoBothMethods(t *testing.T) {
	...
	st, body := req(t, app, http.MethodPost, "/v1/author/repos/verify", "orgA", false, map[string]any{"repoUrl": "https://github.com/acme/widgets"})
	...
	st, body = req(t, app, http.MethodPost, "/v1/author/repos/verify", "orgB", false, map[string]any{"repoUrl": "bob/tool"})
	if st != http.StatusCreated {
	...
	if st, _ := req(t, app, http.MethodPost, "/v1/author/repos/verify", "orgB", false, map[string]any{"repoUrl": "bob/secret"}); st != http.StatusUnprocessableEntity {
	...
	if st, _ := req(t, app, http.MethodPost, "/v1/author/repos/verify", "orgB", false, map[string]any{"repoUrl": "acme/widgets"}); st != http.StatusConflict {
		t.Fatalf("cross-author claim want 409, got %d", st)
	}
	// Verify-before-connect → 400.
	if st, _ := req(t, app, http.MethodPost, "/v1/author/repos/verify", "orgZ", false, map[string]any{"repoUrl": "z/z"}); st != http.StatusBadRequest {
		t.Fatalf("verify-before-connect want 400, got %d", st)
	}
	// Malformed repo → 400.
	if st, _ := req(t, app, http.MethodPost, "/v1/author/repos/verify", "orgA", false, map[string]any{"repoUrl": "not a repo"}); st != http.StatusBadRequest {
...
func TestRecordDeployAttribution(t *testing.T) {
	...
	fg.setLinked("orgA", "acmedev", "tok_a")
	fg.setAdmin("tok_a", "acme", "widgets")
	req(t, app, http.MethodPost, "/v1/author/repos/verify", "orgA", false, map[string]any{"repoUrl": "acme/widgets"})

	// No principal → 403.
	if st, _ := req(t, app, http.MethodPost, "/v1/author/deploys/record", "", false, map[string]any{"repoUrl": "acme/widgets", "project": "p1"}); st != http.StatusForbidden {
		t.Fatalf("no-principal deploy want 403, got %d", st)
	}
	// Deploy of a NON-author repo → recorded:false, 200.
	st, body := req(t, app, http.MethodPost, "/v1/author/deploys/record", "orgB", false, map[string]any{"repoUrl": "someone/else", "project": "p0"})
	if st != http.StatusOK {
	...
	// Missing project → 400.
	if st, _ := req(t, app, http.MethodPost, "/v1/author/deploys/record", "orgB", false, map[string]any{"repoUrl": "acme/widgets"}); st != http.StatusBadRequest {
		t.Fatalf("missing project want 400, got %d", st)
	}

	// orgB deploys acme/widgets → 201 recorded, not self.
	st, body = req(t, app, http.MethodPost, "/v1/author/deploys/record", "orgB", false, map[string]any{"repoUrl": "https://github.com/acme/widgets", "project": "proj-b"})
	...
	st, body = req(t, app, http.MethodPost, "/v1/author/deploys/record", "orgB", false, map[string]any{"repoUrl": "acme/widgets", "project": "proj-b"})
	_ = json.Unmarshal(body, &dv)
	...
	st, body = req(t, app, http.MethodPost, "/v1/author/deploys/record", "orgA", false, map[string]any{"repoUrl": "acme/widgets", "project": "proj-a"})
	_ = json.Unmarshal(body, &dv)
...
func TestSweepAccruesSpendTimesShareIdempotent(t *testing.T) {
	...
	fg.setAdmin("tok_a", "acme", "widgets")
	req(t, app, http.MethodPost, "/v1/author/repos/verify", "orgA", false, map[string]any{"repoUrl": "acme/widgets"})
	req(t, app, http.MethodPost, "/v1/author/deploys/record", "orgB", false, map[string]any{"repoUrl": "acme/widgets", "project": "proj-b"})
	req(t, app, http.MethodPost, "/v1/author/deploys/record", "orgA", false, map[string]any{"repoUrl": "acme/widgets", "project": "proj-a"}) // self

	// Not approved yet → sweep accrues nothing (sweep set is approved-only).
	code, body := req(t, app, http.MethodPost, "/v1/admin/author/sweep", "admin", true, nil)
...
// TestAuthorReadIsPureAndSurfacesRepos proves GET /v1/author accrues NOTHING while
...
func TestAuthorReadIsPureAndSurfacesRepos(t *testing.T) {
	...
	code, body := req(t, app, http.MethodGet, "/v1/author", "orgA", false, nil)
	if code != http.StatusOK {
		t.Fatalf("GET /v1/author want 200, got %d (%s)", code, body)
	}
	var v struct {
...
func TestPayoutIsRecordOnlyAndPendingGuard(t *testing.T) {
	...
	if st, _ := req(t, app, http.MethodPost, "/v1/admin/author/"+idA+"/payout", "orgA", false, map[string]any{"amountCents": 100, "method": "credits"}); st != http.StatusForbidden {
		t.Fatalf("non-admin payout want 403, got %d", st)
	}
	// Over-pending → 400 (accrued available, ask accrued+500).
	if st, _ := req(t, app, http.MethodPost, "/v1/admin/author/"+idA+"/payout", "admin", true, map[string]any{"amountCents": accrued + 500, "method": "credits"}); st != http.StatusBadRequest {
		t.Fatalf("over-pending payout want 400, got %d", st)
	}

	// Credits payout of firstPay → RECORDED, paid moves, wallet untouched.
	st, body := req(t, app, http.MethodPost, "/v1/admin/author/"+idA+"/payout", "admin", true, map[string]any{"amountCents": firstPay, "method": "credits", "reference": "ledger-1"})
	if st != http.StatusOK {
	...
	st, _ = req(t, app, http.MethodPost, "/v1/admin/author/"+idA+"/payout", "admin", true, map[string]any{"amountCents": restPay, "method": "wire", "reference": "wire-xyz"})
	if st != http.StatusOK {
	...
	if st, _ := req(t, app, http.MethodPost, "/v1/admin/author/"+idA+"/payout", "admin", true, map[string]any{"amountCents": 1, "method": "credits"}); st != http.StatusBadRequest {
		t.Fatalf("drained payout want 400, got %d", st)
	}
}

// TestAdminGateAndDirectory: every /v1/admin/author* route is SuperAdmin
...
func TestAdminGateAndDirectory(t *testing.T) {
	...
	for _, p := range []string{"/v1/admin/author", "/v1/admin/author/sweep",
		"/v1/admin/author/" + idA + "/approve", "/v1/admin/author/" + idA + "/suspend",
		"/v1/admin/author/" + idA + "/payout"} {
		method := http.MethodPost
		if p == "/v1/admin/author" {
	...
	code, body := req(t, app, http.MethodGet, "/v1/admin/author", "admin", true, nil)
	if code != http.StatusOK {
	...
	if st, _ := req(t, app, http.MethodPost, "/v1/admin/author/"+idA+"/suspend", "admin", true, nil); st != http.StatusOK {
...
func TestGitLabVerifyAndLedger(t *testing.T) {
	...
	code, cbody := req(t, app, http.MethodPost, "/v1/author/connect", "orgG", false, map[string]any{"provider": "gitlab", "login": "gldev"})
	if code != http.StatusCreated {
	...
	st, vbody := req(t, app, http.MethodPost, "/v1/author/repos/verify", "orgG", false, map[string]any{"repoUrl": "https://gitlab.com/glorg/gltool"})
	if st != http.StatusCreated {
	...
	req(t, app, http.MethodPost, "/v1/author/deploys/record", "orgH", false, map[string]any{"repoUrl": "gitlab.com/glorg/gltool", "project": "proj-h"})
	approve(t, app, cr.ID)
...
func TestGetGrantsNothingAndLedgerReceivesZeroDeposits(t *testing.T) {
	...
	for range 3 {
		if st, _ := req(t, app, http.MethodGet, "/v1/author", "orgA", false, nil); st != http.StatusOK {
			t.Fatalf("GET /v1/author = %d, want 200", st)
		}
	}
	a, _ := s.State.store.GetByID(ctx, idA)
...
func TestHanzoForkRoutesToTreasury(t *testing.T) {
	...
	st, body := req(t, app, http.MethodPost, "/v1/author/deploys/record", "orgB", false,
	...
	req(t, app, http.MethodPost, "/v1/author/deploys/record", "hanzo", false,
...
func TestMount(t *testing.T) {
	...
	if resp.StatusCode != http.StatusForbidden {
		t.Fatalf("mounted GET /v1/author (no principal) want 403, got %d", resp.StatusCode)
	}
}
...
func TestVerifyOrgUrlCoversOwner(t *testing.T) {
	...
	st, body := req(t, app, http.MethodPost, "/v1/author/repos/verify", "orgL", false, map[string]any{"repoUrl": "https://github.com/luxfi"})
	if st != http.StatusCreated {
	...
	if st, dbody := req(t, app, http.MethodPost, "/v1/author/deploys/record", "orgD", false, map[string]any{"repoUrl": "https://github.com/luxfi/node", "project": "proj-d"}); st != http.StatusCreated || !recorded(t, dbody) {
		t.Fatalf("org-covered deploy #1 want 201 recorded, got %d (%s)", st, dbody)
	}
	if st, dbody := req(t, app, http.MethodPost, "/v1/author/deploys/record", "orgD", false, map[string]any{"repoUrl": "luxfi/cli", "project": "proj-cli"}); st != http.StatusCreated || !recorded(t, dbody) {
	...
	code, sbody := req(t, app, http.MethodPost, "/v1/admin/author/sweep", "admin", true, nil)
	...
	if st, _ := req(t, app, http.MethodPost, "/v1/author/repos/verify", "orgX", false, map[string]any{"repoUrl": "github.com/luxfi"}); st != http.StatusUnprocessableEntity {
	...
	if st, _ := req(t, app, http.MethodPost, "/v1/author/repos/verify", "orgX", false, map[string]any{"repoUrl": "github.com/luxfi"}); st != http.StatusConflict {
	...
	if st, _ := req(t, app, http.MethodPost, "/v1/author/repos/verify", "orgX", false, map[string]any{"repoUrl": "acme/widgets"}); st != http.StatusCreated {
	...
	for _, bad := range []string{"luxfi", "not a repo", "https://bitbucket.org/a/b"} {
		if st, _ := req(t, app, http.MethodPost, "/v1/author/repos/verify", "orgX", false, map[string]any{"repoUrl": bad}); st != http.StatusBadRequest {
...
func TestVerifyOrgFileMethod(t *testing.T) {
	...
	st, body := req(t, app, http.MethodPost, "/v1/author/repos/verify", "orgF", false, map[string]any{"repoUrl": "github.com/acme"})
	if st != http.StatusCreated {
	...
	st, dbody := req(t, app, http.MethodPost, "/v1/author/deploys/record", "orgZ", false, map[string]any{"repoUrl": "github.com/acme/anything", "project": "proj-z"})
...
func TestSettlementIsRecordedNotInferred(t *testing.T) {
	...
	req(t, app, http.MethodPost, "/v1/author/deploys/record", "orgB", false,
		map[string]any{"repoUrl": "https://github.com/hanzoai/chat-starter", "project": "proj-b"})
	fc.setSpend("orgB", 10000)
	adminSweepAndPay(t, app, s)
	sys, err := s.State.store.GetByOrg(ctx, "hanzo")
	...
	fg.setAdmin("tok_a", "acme", "widgets")
	req(t, app, http.MethodPost, "/v1/author/repos/verify", "orgA", false, map[string]any{"repoUrl": "acme/widgets"})
	req(t, app, http.MethodPost, "/v1/author/deploys/record", "orgC", false, map[string]any{"repoUrl": "acme/widgets", "project": "proj-c"})
	approve(t, app, idA)
	fc.setSpend("orgC", 10000)
	req(t, app, http.MethodPost, "/v1/admin/author/sweep", "admin", true, nil)

	const accrued = 10000 * defaultShareBps / bpsDenom
	req(t, app, http.MethodPost, "/v1/admin/author/"+idA+"/payout", "admin", true,
...
func TestEnsureSystemAuthorPromotesExistingConnected(t *testing.T) {
	...
	t.Run("suspended stays suspended", func(t *testing.T) {
		...
		if st, body := req(t, app, http.MethodPost, "/v1/admin/author/"+id+"/suspend", "admin", true, nil); st != http.StatusOK {

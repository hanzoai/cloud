...
// days" — evaluated live against the shared event.event warehouse through the
...
// event plane's tenancy column and the leading sort key) as a BOUND arg, and the name +
...
// Honest-empty until the collector emits.
...
// selected inside its window, bounded by limit. org, name and the time

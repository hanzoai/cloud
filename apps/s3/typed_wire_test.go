...
func TestTheGraphCanAimTheDelete(t *testing.T) {
	...
	graph := client.NewGraph(doc, func(string) (string, string, error) { return sock, "/", nil })
	answer := graph.Run(client.Request{Query: `{ ` + graphDelete + `(bucket: "photos", wildcard1: "` + deep + `") }`}, from)

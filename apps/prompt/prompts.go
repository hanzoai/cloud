...
//
//	GET    /v1/prompt            list current prompts for the org   -> {data:[PromptMeta]}
//	POST   /v1/prompt            create or add-a-version            -> PromptDetail
//	GET    /v1/prompt/metrics    real per-prompt stats              -> {data:[...]}
//	GET    /v1/prompt/catalog    the embedded read-only starter set -> {data:[...]}
...
// org-unique handle AND the URL path segment (/v1/prompt/:name), so this is
// the injection/traversal guard at the boundary.
var nameRE = regexp.MustCompile(`^[A-Za-z0-9][A-Za-z0-9._-]{0,63}$`)

// reserved names would collide with static sub-routes under /v1/prompt.
...
// collection routes ARE /v1/prompt, and a group prefix composed with an empty leaf
// yields "/v1/prompt/" — a different address. One registrar for all six keeps each
...
func routes(app cloud.Router, s *cloud.Service[state]) error {
	...
	zip.Post(za, "/v1/prompt", o.create, zip.WithStatus(http.StatusCreated))
	zip.Get(za, "/v1/prompt/metrics", o.metrics)
	zip.Get(za, "/v1/prompt/catalog", o.catalog)
	zip.Get(za, "/v1/prompt/:name", o.get)
	zip.Delete(za, "/v1/prompt/:name", o.del)
	return nil
}

...
//
// THERE IS NO DEPOSIT HERE, AND THERE IS NOT GOING TO BE ONE. This client used to
...
// the SHAPE of this interface is load-bearing and TestCommerceClientIsReadOnly fails
...
// commerceClient adapts the shared payout.Client onto this program's lowercase client
// (Go package-scoped interface methods cannot cross packages). Zero logic — pure
// delegation, and it delegates exactly one read.
type commerceClient struct{ c *payout.Client }

func (s commerceClient) spendCents(ctx context.Context, org string) (int64, error) {
...
func newCommerceClient() commerce {
	return commerceClient{payout.NewClient()}
}

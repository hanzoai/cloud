...
`apps/content` is the native Go marketing content loop — the ONE replacement for
...
and therefore on the fleet's one MCP endpoint). It calls the
...
- Tenant-scoped: the S2S call is `Authorization: Bearer <COMMERCE_SERVICE_TOKEN>` +
  ...
  (`apps/commerce/transport`) — the SAME admin S2S pattern billing/account use; commerce

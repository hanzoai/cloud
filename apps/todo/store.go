...
// Project is an org-scoped issue todo project (a Linear "team"). Its Key is
// the org-unique, uppercase handle that both routes the URL (/v1/todo/
...
// deployable site forming this todo's physical tenant boundary; these
...
// todo. It is NOT the domain-record plane: helpdesk tickets, CMS content and
...
// Store is one (org, project)'s todo database — ONE SQLite file per project
// at {DataDir}/orgs/{orgSlug}/projects/{projectSlug}/todo.db (opened via
// cloud.OrgDB). todo is project-scoped: the physical boundary is the IAM
...
// into the todo store, running its migration. It is the open func the shared
...
-- ExtRef lookup: the external-mirror upsert (GitHub issue → todo) finds the
...
//
// THE LITERALS ARE BACKTICKED, so what is written is what SQLite sees. Both this
// and the clause above were written with the doubling a "…" string would have
// needed, which put TWO backslashes in each: `ESCAPE '\\'` is a two-character
// escape expression, SQLite requires exactly one, and every search — the only
// way to read an issue's description on this surface — answered 500. Nothing
// caught it because the escape only appears once the caller passes text.

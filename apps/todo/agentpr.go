...
// only ever file a row in org A's todo. A nil singleton (todo unmounted)
// fails closed.
...
type AgentPR struct {
	...
	ProjectKey string // the todo project KEY the row lives under
	Number     int    // per-project monotonic number
}

// CreateAgentPR opens the PR work item for a coding run: it get-or-creates the
// todo project keyed off the repo (so a repo's PRs share one board), then
...
func CreateAgentPR(ctx context.Context, in AgentPRInput) (AgentPR, error) {
	if mounted == nil {
		return AgentPR{}, fmt.Errorf("todo: not mounted")
	}
	org := strings.TrimSpace(in.Org)
	repo := strings.TrimSpace(in.Repo)
	if org == "" || repo == "" {
		return AgentPR{}, fmt.Errorf("todo: org and repo required")
	}
	if len(repo) > maxField {
		return AgentPR{}, fmt.Errorf("todo: repo too long")
	}
	...
	if len(head) > maxField {
		return AgentPR{}, fmt.Errorf("todo: head branch too long")
	}
	...
	if err != nil {
		return AgentPR{}, fmt.Errorf("todo: open store: %w", err)
	}
	...
	created, err := store.CreateIssue(ctx, Issue{
		ID: mint.ID("issue"), ProjectID: proj.ID, Org: org,
	...
	if err != nil {
		return AgentPR{}, fmt.Errorf("todo: create pr: %w", err)
	}
	return AgentPR{
...
// ensureAgentProject resolves (or creates) the todo project a repo's agent PRs
...
func ensureAgentProject(ctx context.Context, store *Store, org, repo string) (Project, error) {
	...
	if !errors.Is(err, errNotFound) {
		return Project{}, fmt.Errorf("todo: get project: %w", err)
	}
	now := time.Now().Unix()
	np := Project{ID: mint.ID("prj"), Org: org, Key: key, Name: repo, CreatedAt: now, UpdatedAt: now}
	...
	return Project{}, fmt.Errorf("todo: create project: %w", cerr)
}

...
// owner_test.go pins the two defects that made todo.hanzo.ai unusable, and
...
func TestForgeOwner_TheBoardAsksTheForgeOrgNotTheIAMOrg(t *testing.T) {
	...
	f.work["hanzoai"] = []map[string]any{row("cloud", "ship the todo fix")}
	f.work["hanzo"] = []map[string]any{}
	app := mountAt(t, f.URL, 10*time.Second)

	code, raw := asUser(t, app, http.MethodGet, "/v1/todo/projects", "hanzo", "alice", nil)
...
func TestForgeOwner_TranslationDoesNotChangeTheSudoActor(t *testing.T) {
	...
	if code, raw := asUser(t, app, http.MethodGet, "/v1/todo/projects", "hanzo", "alice", nil); code != http.StatusOK {
...
func TestForgeProject_ReadsOneRepositoryRatherThanTheInventory(t *testing.T) {
	...
	code, raw := asUser(t, app, http.MethodGet, "/v1/todo/projects/cloud", "hanzo", "alice", nil)
	if code != http.StatusOK {
...
//
//	FORGE_LIVE_TOKEN=… FORGE_LIVE_ACTOR=z go test -run LiveOwner ./apps/todo/

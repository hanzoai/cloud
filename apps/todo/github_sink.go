...
// native todo, idempotently by ExtRef. It is the todo's side of the
// todo.go inversion — integrations never imports todo; it calls
// cloud.UpsertIssue, which lands here.
//
// One team, repo as the discriminator: every mirrored GitHub issue for an org files
// under ONE todo team (ProjectKey, e.g. "GH") in the org's DEFAULT project store,
...
// open/closed State maps to a board column HERE (the todo owns its status
// vocabulary); the feeder stays provider-shaped, this stays todo-shaped.
...
// stateToStatus maps an upstream open/closed state to a todo board column: open
...
func upsertIssue(ctx context.Context, in cloud.IssueUpsert) (cloud.IssueUpsertResult, error) {
	...
	if org == "" || extRef == "" {
		return cloud.IssueUpsertResult{}, errors.New("todo upsert: org and extRef are required")
	}
...
// use. The key is normalized to the todo's ^[A-Z][A-Z0-9]{1,7}$ shape (defaulting
...
// normKindDefault clamps an external kind to the todo's closed set, defaulting to

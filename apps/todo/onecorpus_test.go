...
//
// Measured on production before this held: /v1/todo/projects listed 745
...
// reachable only through /v1/todo/issues. The board UI reads the first, so
...
func TestOneCorpus_TheIndexIsOnTheSameBoardAsTheForge(t *testing.T) {
	...
	// the work there.
	keys := field(get("/v1/todo/projects"), "key")
	...
	titles := field(get("/v1/todo/board"), "title")
	...
	code, raw := asUser(t, app, http.MethodGet, "/v1/todo/projects/OPS", "hanzo", "alice", nil)
	if code != http.StatusOK {
		t.Errorf("GET the index board = %d %s, want 200 — an unaddressable board is invisible to the UI", code, raw)
	}
	if got := field(get("/v1/todo/projects/OPS/issues"), "title"); !has(got, "Track 0 — ACCESS") {
	...
	if got := field(get("/v1/todo/projects/api/issues"), "title"); has(got, "Track 0 — ACCESS") {
	...
	if got := field(get("/v1/todo/board?source=team"), "title"); len(got) != 1 || got[0] != "Track 0 — ACCESS" {
		t.Errorf("source=team = %v, want only the index row", got)
	}
	if got := field(get("/v1/todo/board?source=git"), "title"); has(got, "Track 0 — ACCESS") {
	...
	code, raw = asUser(t, app, http.MethodGet, "/v1/todo/issues", "hanzo", "alice", nil)
	if code != http.StatusOK {

...
func anchorRoot(ctx context.Context) error {
	...
	sk := strings.TrimSpace(environ.Or("SIGNER_KEY", ""))
	contractHex := strings.TrimSpace(environ.Or("CONTRACT", ""))
	rootHex := strings.TrimSpace(environ.Or("ANCHOR_ROOT", ""))
...
func bootstrap(ctx context.Context) error {
	...
	generated := false
	if sk := strings.TrimSpace(environ.Or("SIGNER_KEY", "")); sk != "" {
	...
	if bal.Cmp(want) < 0 {
		anvil := strings.TrimSpace(environ.Or("ANVIL_KEY", ""))
		if anvil == "" {
	...
	contract := strings.TrimSpace(environ.Or("CONTRACT", ""))
	if contract == "" {
...
func loadBytecode() ([]byte, error) {
	hexStr := strings.TrimSpace(environ.Or("ANCHOR_BYTECODE", ""))
	if hexStr == "" {
		if f := strings.TrimSpace(environ.Or("ANCHOR_BYTECODE_FILE", "")); f != "" {
			b, err := os.ReadFile(f)
			if err != nil {

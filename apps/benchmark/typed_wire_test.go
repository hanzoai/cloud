...
// untypedByDesign is the CLOSED list of benchmark operations that are NOT typed ops.
...
func opsUnderTest(t *testing.T) (served map[string]bool, typed map[string]string, schemas map[string]any) {
	...
	ours := func(p string) bool { return strings.HasPrefix(p, "/v1/benchmark") }
...
// TestEveryRouteIsTypedOrNamed fails when an benchmark operation is neither a typed
// op nor named above — so the next route added here is typed by default.
func TestEveryRouteIsTypedOrNamed(t *testing.T) {
	served, typed, _ := opsUnderTest(t)
	if len(served) != 6 {
		t.Errorf("benchmark serves %d operations, expected 6 — update this gate deliberately", len(served))
	}
	...
	for key := range untypedByDesign {
		if !served[key] {
			t.Errorf("untypedByDesign names %q, which benchmark no longer serves", key)
		}
	}
}
...
func TestEveryTypedOpIsDescribed(t *testing.T) {
	...
	if len(typed) == 0 {
		t.Fatal("no typed benchmark ops in the registry at all")
	}
	for key, desc := range typed {
		if strings.TrimSpace(desc) == "" {
			t.Errorf("%s has no description — run: go generate -run zipdoc ./apps/benchmark/...", key)
		}
	}
}
...
func TestEveryPublishedFieldIsDescribed(t *testing.T) {
	...
	if len(schemas) == 0 {
		t.Fatal("no benchmark schemas in the typed registry at all")
	}
	var bare []string

...
func release(app *zip.App, org, id string, purge bool) {
	body, _ := json.Marshal(client.EndIn{ID: id, Purge: purge})
...
func TestLiveLeaseKeepsWhatItPromisesToKeep(t *testing.T) {
	...
	first, dLease := post[client.Leased](t, app, "/v1/sandbox/lease", org,
		client.LeaseIn{Class: "dev", Project: project, TTLSec: 900})
	if first.ID == "" {
	...
	_, dWrite := post[client.Wrote](t, app, "/v1/sandbox/write", org,
		client.WriteIn{ID: first.ID, Path: "keep.txt", Data: []byte(want)})
	// Written THROUGH the sandbox's own filesystem, so the check below is not
	// reading back the same buffer it just sent.
	ran, dRun := post[client.Ran](t, app, "/v1/sandbox/run", org,
		client.RunIn{ID: first.ID, Command: "sync; cat keep.txt; df -T " + first.Workdir + " | tail -1"})
	...
	_, dEnd := post[struct{}](t, app, "/v1/sandbox/end", org, client.EndIn{ID: first.ID})
	...
	second, dRelease := post[client.Leased](t, app, "/v1/sandbox/lease", org,
		client.LeaseIn{Class: "dev", Project: project, TTLSec: 900})
	...
	blob, dRead := post[client.Blob](t, app, "/v1/sandbox/read", org,
		client.PathIn{ID: second.ID, Path: "keep.txt"})
	...
	// the fast runtime.
	ex, dExLease := post[client.Leased](t, app, "/v1/sandbox/lease", org,
		client.LeaseIn{Class: "exec", TTLSec: 600})
	...
	kern, dKern := post[client.Ran](t, app, "/v1/sandbox/run", org,
		client.RunIn{ID: ex.ID, Command: "uname -r"})
...
func TestLiveLeaseCost(t *testing.T) {
	...
	for i := range rounds {
		m, dLease := post[client.Leased](t, app, "/v1/sandbox/lease", org,
			client.LeaseIn{Class: "exec", TTLSec: 600})
		ran, dRun := post[client.Ran](t, app, "/v1/sandbox/run", org,
			client.RunIn{ID: m.ID, Command: work, TimeoutSec: 300})
		_, dEnd := post[struct{}](t, app, "/v1/sandbox/end", org, client.EndIn{ID: m.ID})

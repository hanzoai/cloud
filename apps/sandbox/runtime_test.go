...
func TestRuntimeForAsksTheVolumeNotTheClass(t *testing.T) {
	...
	bare := Sandbox{ID: "m_3", Class: "exec"}
	...
	for _, c := range []struct {
		...
		{"volumeless takes the fast runtime", "kata-fc", "", bare, "kata-fc", ""},
		...
		{"rollback: volumeless", "gvisor", "", bare, "gvisor", ""},
		...
		{"unset stays unset, volumeless", "", "", bare, "", ""},
		...
		{"forcing fc onto an exec volume is refused", "kata-fc", "kata-fc", execVol, "",
			"cannot mount project volume"},
		{"a caller may still choose fc for a volumeless sandbox", "gvisor", "kata-fc", bare, "kata-fc", ""},
		...
		{"an unknown runtime is refused", "gvisor", "runsc", bare, "", "is not one we run"},
		{"case matters — the apiserver's does", "gvisor", "gVisor", bare, "", "is not one we run"},
	} {
...
func TestRuntimeForNeverPutsAVolumeOnARuntimeThatCannotHoldIt(t *testing.T) {
	m := Sandbox{ID: "m_1", Class: "dev", Project: "p", Volume: "m-acme-p-abc"}
	for deploy := range runtimes {
		for _, want := range []string{"", "gvisor", "kata-clh", "kata-fc"} {
			got, err := (&runtime{runtimeClass: deploy}).runtimeFor(m, want)
			if err != nil {
				continue // refused, which is the other acceptable answer
			}
			if got == "" {
				t.Fatalf("deploy=%q want=%q answered the node default for a volume-bearing sandbox", deploy, want)
			}
			if !runtimes[got] {
				t.Fatalf("deploy=%q want=%q = %q, which has no shared filesystem — the volume would be a tmpfs",
					deploy, want, got)
			}
		}
	}

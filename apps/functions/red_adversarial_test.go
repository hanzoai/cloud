...
func TestRed_OrgKeyExactIsolation(t *testing.T) {
	...
	for _, p := range pairs {
		...
		app := mountApp(t)
		if code, _ := do(t, app, http.MethodPost, "/v1/function", victim,
		...
		code, body := do(t, app, http.MethodGet, "/v1/function", attacker, nil)
		...
		if code, _ := do(t, app, http.MethodGet, "/v1/function/secret-fn", attacker, nil); code != http.StatusNotFound {
			t.Errorf("[%q->%q] attacker GET want 404, got %d", attacker, victim, code)
		}
		if code, _ := do(t, app, http.MethodDelete, "/v1/function/secret-fn", attacker, nil); code != http.StatusNotFound {
			t.Errorf("[%q->%q] attacker DELETE want 404, got %d", attacker, victim, code)
		}
		// Victim's function survives the attacker's delete attempt.
		if code, _ := do(t, app, http.MethodGet, "/v1/function/secret-fn", victim, nil); code != http.StatusOK {
...
func TestRed_StaticRoutePrecedence(t *testing.T) {
	app := mountApp(t)
	for _, p := range []string{"/v1/function/metrics", "/v1/function/triggers",
		"/v1/function/deployments", "/v1/function/secrets"} {
		if code, body := do(t, app, http.MethodGet, p, "maxpower", nil); code != http.StatusOK {
			t.Errorf("%s want 200 (static), got %d (%s)", p, code, body)
		}
	}
	// DELETE /v1/function/metrics -> :name del with name="metrics"; clean 404.
	if code, body := do(t, app, http.MethodDelete, "/v1/function/metrics", "maxpower", nil); code != http.StatusNotFound {
		t.Errorf("DELETE /metrics want 404, got %d (%s)", code, body)
	}
	// Traversal-ish path segments: clean status, never 500/panic.
	for _, p := range []string{"/v1/function/..", "/v1/function/%2e%2e", "/v1/function/%2F", "/v1/function/.hidden"} {

...
// book is the adapter under test, bound once. A composite literal cannot start an
...
func TestLedgerAdapter_TestRoutesToTheSandboxBooks(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			_, cents, err := book.Credit(context.Background(), creditledger.CreditInput{
				Org:            "acme",
				Subject:        "acme/alice",
				Currency:       "usd",
				AmountCents:    2500,
...
func TestLedgerAdapter_SandboxMoneyIsNotSpendable(t *testing.T) {
	...
	if _, _, err := book.Credit(context.Background(), creditledger.CreditInput{
	...
	live, err := book.Balance(context.Background(), "acme", "acme/alice", "usd", false)
	if err != nil {
	...
	sandbox, err := book.Balance(context.Background(), "acme", "acme/alice", "usd", true)
	if err != nil {
...
func TestLedgerAdapter_BalanceReadsTheAccountItIsAsked(t *testing.T) {
	...
	pool, err := book.Balance(context.Background(), "acme", "", "usd", false)
	if err != nil {
	...
	member, err := book.Balance(context.Background(), "acme", "acme/alice", "usd", false)
	if err != nil {

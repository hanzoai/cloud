...
var untypedByDesign = map[string]string{
	...
	// One product's catalogue. The SAME three facts as the brand catalogue above —
	// it is the same document narrowed to one product, generated by the same sweep,
	// carrying digests computed over the same served files — so it stays raw for the
	// same reasons and not for new ones.
	"GET /.well-known/agent-skills/{product}/index.json": "serves the embedded product catalogue BYTES " +
		"verbatim, under the same Cache-Control and the same {\"error\":…} 404 body as the brand " +
		"catalogue it is a slice of.",
}
...
func TestEveryRouteIsTypedOrNamed(t *testing.T) {
	...
	// Three since the catalogue grew a product tier: the brand index, one skill
	// document, one product index.
	if len(served) != 3 || len(typed) != 0 {
		t.Errorf("served = %d (want 3), typed = %d (want 0)", len(served), len(typed))
	}
}

// TestEveryProductTheIndexNamesIsServed walks the products the brand catalogue
// advertises and fetches each one's index at the address the catalogue gives.
//
// It quantifies over ALL of them rather than sampling one, because the failure
// this exists to catch is exactly a per-product one: the catalogue names 116
// addresses, and until the product route existed EVERY one of them answered 404
// while every test in this file passed. A route that is registered and a route
// that answers are different facts.
func TestEveryProductTheIndexNamesIsServed(t *testing.T) {
	app := newApp(t, "hanzo")
	raw, err := catalogFS.ReadFile("catalog/hanzo/index.json")
	if err != nil {
	...
	if len(cat.Products) == 0 {
		t.Skip("the embedded hanzo catalogue names no products")
	}
	for _, p := range cat.Products {
		code, body, _ := get(t, app, wellKnown+"/"+p.Path, "api.hanzo.ai")
		if code != 200 {
			t.Fatalf("product %s at %s: %d — the catalogue advertises this address, so a client "+
				"following it gets nothing", p.Name, p.Path, code)
		}
		want, err := catalogFS.ReadFile("catalog/hanzo/" + p.Path)
		if err != nil {
			t.Fatalf("read embedded product index %s: %v", p.Path, err)
		}
		if string(body) != string(want) {
			t.Fatalf("product %s is not served byte-for-byte", p.Name)
		}
	}
}

// TestASkillIsNotServedAsAProduct pins the separation the two id shapes exist
// for: neither route may answer at the other's address, or one flat directory
// holding both kinds becomes ambiguous.
func TestASkillIsNotServedAsAProduct(t *testing.T) {
	app := newApp(t, "hanzo")
	if code, _, _ := get(t, app, wellKnown+"/account_appearance/index.json", "api.hanzo.ai"); code != 404 {
		t.Fatalf("a skill answered the product address: %d", code)
	}
	if code, _, _ := get(t, app, wellKnown+"/_account/SKILL.md", "api.hanzo.ai"); code != 404 {
		t.Fatalf("a product index answered the skill address: %d", code)
	}
}
...
func TestSkillIsMarkdown(t *testing.T) {
	...
	// A SKILL directory, not the first directory: the catalogue root holds two
	// kinds, and `_product` sorts BEFORE every skill because `_` precedes the
	// lowercase letters. Taking the first one asked `_account` for a SKILL.md it
	// has never had — the product tier landing is what turned this green test red.
	skill := ""

...
- `GET https://api.hanzo.ai/v1/ingress/services` — Returns every backend pool the caller's org has configured, ordered by id.
- `GET https://api.hanzo.ai/v1/ingress/services/{id}` — Returns one of the caller org's backend pools by id.
...
- `/v1/ingress/services/{id}` → `Service` object with fields: `backends`, `id`, `passHostHeader`.

## Example

```bash
curl -sS "https://api.hanzo.ai/v1/ingress/services" \

...
- `GET https://api.hanzo.ai/v1/platform/apps` — Answers what this organisation has declared, joined with what the delivery plane has done about it.
- `GET https://api.hanzo.ai/v1/platform/apps/{app}` — Answers ONE declaration — what git says this app is, before the delivery plane has had any say in it.
- `GET https://api.hanzo.ai/v1/platform/apps/{app}/cd` — Answers ONE app's reconciliation alone — the poll a deploy console makes while it waits, without re-reading the whole inventory each time.

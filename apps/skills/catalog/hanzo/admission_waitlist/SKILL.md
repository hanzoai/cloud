...
# Hanzo · ADMISSION · waitlist

Read-only Hanzo capability derived from the `admission` OpenAPI product. Base URL `https://api.hanzo.ai`.
...
- `GET https://api.hanzo.ai/v1/admission/waitlist` — Reports whether ONE host is currently gated by the launch waitlist.
...
```bash
curl -sS "https://api.hanzo.ai/v1/admission/waitlist" \
...
- You need a different `admission` capability — that product's skills are listed at `https://api.hanzo.ai/.well-known/agent-skills/_admission/index.json`.

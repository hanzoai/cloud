...
- `GET https://api.hanzo.ai/v1/billing/accounts` — Answers the caller's billing accounts: the org itself, its currency, when it was opened, and the caller's own standing in it.
- `GET https://api.hanzo.ai/v1/billing/accounts/{id}/members` — Answers one billing account's roster.
...
```bash
curl -sS "https://api.hanzo.ai/v1/billing/accounts" \

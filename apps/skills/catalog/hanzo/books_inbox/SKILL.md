...
- `GET https://api.hanzo.ai/v1/books/inbox` — Returns the org's open document queue — everything uploaded but not yet booked, newest first, each with its extracted summary and the confidence the scanner resolved its category at.
...
```bash
curl -sS "https://api.hanzo.ai/v1/books/inbox" \

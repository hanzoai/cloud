...
Read-only Hanzo capability derived from the `tasks` OpenAPI product. Base URL `https://api.hanzo.ai`.
...
- `GET https://api.hanzo.ai/v1/tasks` — Redirect to the tasks API root
- `GET https://api.hanzo.ai/v1/tasks/{wildcard1}` — Read workflow state from the durable engine
...
```bash
curl -sS "https://api.hanzo.ai/v1/tasks" \
...
- You need a different `tasks` capability — that product's skills are listed at `https://api.hanzo.ai/.well-known/agent-skills/_tasks/index.json`.

...
- `GET https://api.hanzo.ai/v1/sync` — List returns every sync link the caller's org has, each with its two endpoints, its direction and trigger policy, and the time it last reconciled.
- `GET https://api.hanzo.ai/v1/sync/{id}` — Get returns one sync by id.
...
```bash
curl -sS "https://api.hanzo.ai/v1/sync" \
...
- You need a different `sync` capability — that product's skills are listed at `https://api.hanzo.ai/.well-known/agent-skills/_sync/index.json`.

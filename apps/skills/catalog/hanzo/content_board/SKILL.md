...
- `GET https://api.hanzo.ai/v1/content/board` — Aggregates the caller org's marketing content across every publishable content type into ONE queue board — the cross-type read the framework's per-DocType list cannot give.
...
```bash
curl -sS "https://api.hanzo.ai/v1/content/board" \

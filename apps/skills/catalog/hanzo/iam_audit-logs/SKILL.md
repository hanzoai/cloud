...
- `GET https://api.hanzo.ai/v1/iam/audit-logs` — Returns your organization's audit trail, newest first — who did what, when, and from where.
- `GET https://api.hanzo.ai/v1/iam/audit-logs/{owner}/{name}` — Returns one audit entry in full: the action, the person or key behind it, and the request it came in on.
...
```bash
curl -sS "https://api.hanzo.ai/v1/iam/audit-logs" \

...
Read-only Hanzo capability derived from the `wallet` OpenAPI product. Base URL `https://api.hanzo.ai`.
...
- `GET https://api.hanzo.ai/v1/wallet` — Returns the caller org's wallets, newest first, optionally NARROWED within the org by project, agent or account.
- `GET https://api.hanzo.ai/v1/wallet/{id}` — Returns one of the caller org's wallets: its scope, custody kind, tier, chain and on-chain address.
...
```bash
curl -sS "https://api.hanzo.ai/v1/wallet" \
...
- You need a different `wallet` capability — that product's skills are listed at `https://api.hanzo.ai/.well-known/agent-skills/_wallet/index.json`.

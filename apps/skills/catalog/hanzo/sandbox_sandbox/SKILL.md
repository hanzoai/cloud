...
- `GET https://api.hanzo.ai/v1/sandbox` — Lists the caller org's sandboxes, newest first.
- `GET https://api.hanzo.ai/v1/sandbox/{id}` — Returns one sandbox: its class, project, image, the runtime it was given, its status and when its lease ends.
...
```bash
curl -sS "https://api.hanzo.ai/v1/sandbox" \

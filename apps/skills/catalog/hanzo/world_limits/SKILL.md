...
- `GET https://api.hanzo.ai/v1/world/limits` — Echoes a World plan's rate limits, alert quota and model-API grant, read straight from the live @hanzo/plans catalog, so agents and dashboards configure themselves against the catalog instead of hardcoding tier numbers.
...
```bash
curl -sS "https://api.hanzo.ai/v1/world/limits" \

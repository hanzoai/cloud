...
- `GET https://api.hanzo.ai/v1/sentry/issues` — Lists the caller's org's grouped error issues, optionally narrowed to one project and one time window, and filtered by status, level, environment, service, a free-text query and a sort.
- `GET https://api.hanzo.ai/v1/sentry/issues/{id}` — Returns one grouped issue of the caller's org with its latest occurrence sample.
- `GET https://api.hanzo.ai/v1/sentry/issues/{id}/events` — Lists one issue's captured occurrences, scoped to a project — a project is an isolation unit, so the caller declares which project's occurrences to read.
...
```bash
curl -sS "https://api.hanzo.ai/v1/sentry/issues" \

...
- `GET https://api.hanzo.ai/v1/evals/datasets` — Is the datasets your org has, each with its name, description, metadata and timestamps.
- `GET https://api.hanzo.ai/v1/evals/datasets/{name}` — Returns one dataset of the caller's org by name, together with its live item count — the one read that answers how big the set actually is.
- `GET https://api.hanzo.ai/v1/evals/datasets/{name}/items` — Is the examples in one of your datasets — the set is named in the path, because this collection only exists inside one.

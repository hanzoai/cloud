...
- `GET https://api.hanzo.ai/v1/k8s/clusters` — Lists the org's DOKS clusters (Visor, house account) folded with the org's BYO clusters — ONE fleet cluster view under the unified k8s noun.
- `GET https://api.hanzo.ai/v1/k8s/clusters/{id}` — Returns one cluster's detail: node pools + worker nodes.
...
```bash
curl -sS "https://api.hanzo.ai/v1/k8s/clusters"
```

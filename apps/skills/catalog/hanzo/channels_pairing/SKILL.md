...
- `GET https://api.hanzo.ai/v1/channels/pairing` — Returns the pairing requests waiting for the caller org to approve — one per person who messaged a connected bot on a channel whose DM policy is "pairing" and who is not allowed yet.
...
```bash
curl -sS "https://api.hanzo.ai/v1/channels/pairing" \
...
- You need a different `channels` capability — that product's skills are listed at `https://api.hanzo.ai/.well-known/agent-skills/_channels/index.json`.

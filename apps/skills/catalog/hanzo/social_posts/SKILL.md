...
- `GET https://api.hanzo.ai/v1/social/posts` — Returns the org's posts — content, channel, status, scheduled time, media and timestamps — most-recently-updated first.
- `GET https://api.hanzo.ai/v1/social/posts/{id}` — Returns one of the org's posts by id, with its current status, scheduled time, media and — once it has published — the account and external id it published under.
...
```bash
curl -sS "https://api.hanzo.ai/v1/social/posts" \

...
- `GET https://api.hanzo.ai/v1/marketing/calendar` — Returns the org's calendar, soonest scheduled first, optionally narrowed to one status.
- `GET https://api.hanzo.ai/v1/marketing/calendar/{id}` — Returns one of the caller org's posts, including the exact error behind a failed publish.
...
```bash
curl -sS "https://api.hanzo.ai/v1/marketing/calendar" \

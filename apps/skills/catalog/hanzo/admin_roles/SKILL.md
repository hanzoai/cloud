...
- `GET https://api.hanzo.ai/v1/admin/roles` — Lists IAM roles for one owner org, forwarded VERBATIM from IAM's role list.
...
- `/v1/admin/roles` → `iamRowsOut` object with fields: `data`, `msg`, `status`, `total`.

## Example

```bash
curl -sS "https://api.hanzo.ai/v1/admin/roles" \

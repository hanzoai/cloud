...
- `GET https://api.hanzo.ai/v1/o11y/integrations` — Lists the available integrations and whether each is installed in the caller's org, optionally narrowed to installed or not-installed.
- `GET https://api.hanzo.ai/v1/o11y/integrations/{integrationId}` — Returns one integration's full detail — its overview, configuration steps, collected data and assets — together with its installation record when the org has installed it.
- `GET https://api.hanzo.ai/v1/o11y/integrations/{integrationId}/connection_status` — Reports whether the integration's logs and metrics have been received over the lookback window, so the console can show a live connection state.
...
```bash
curl -sS "https://api.hanzo.ai/v1/o11y/integrations" \

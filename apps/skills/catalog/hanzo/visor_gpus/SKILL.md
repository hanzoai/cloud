...
- `GET https://api.hanzo.ai/v1/visor/gpus` — Returns one row per physical accelerator the caller's org has, derived from its real GPU machines (the size slug says how many cards a node holds) and from the accelerators BYO workers report through nvidia-smi.
- `GET https://api.hanzo.ai/v1/visor/gpus/alerts` — Is an HONEST empty surface: Visor exposes no GPU alert inventory, so this returns [] rather than fabricating alerts.
...
```bash
curl -sS "https://api.hanzo.ai/v1/visor/gpus" \

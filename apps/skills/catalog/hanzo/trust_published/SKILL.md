...
- `GET https://api.hanzo.ai/v1/trust/published/{org}` — Reads a published trust centre — the whole thing in one answer: the organization's profile, its control inventory, coverage computed against each framework's whole published clause list, its documents, subprocessors, policies, knowledge base, updates and risk profile.
...
```bash
curl -sS "https://api.hanzo.ai/v1/trust/published/{org}"
```

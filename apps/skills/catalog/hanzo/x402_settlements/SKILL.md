...
- `GET https://api.hanzo.ai/v1/x402/settlements/{id}` — Settlement reads one x402 payment receipt by id.
...
```bash
curl -sS "https://api.hanzo.ai/v1/x402/settlements/{id}" \
...
- You need a different `x402` capability — that product's skills are listed at `https://api.hanzo.ai/.well-known/agent-skills/_x402/index.json`.

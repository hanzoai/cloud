...
- `GET https://api.hanzo.ai/v1/iam/certs` — Returns your organization's signing certificates, newest first — the keys the tokens your applications verify are signed with.
- `GET https://api.hanzo.ai/v1/iam/certs/{owner}/{name}` — Returns one signing certificate — its algorithm, its validity window and its public half.
...
```bash
curl -sS "https://api.hanzo.ai/v1/iam/certs" \

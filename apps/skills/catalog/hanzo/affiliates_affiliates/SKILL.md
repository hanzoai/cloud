...
- `GET https://api.hanzo.ai/v1/affiliates` — Answers the caller org's OWN affiliate standing: status, referral code and share link, commission rate, how many orgs it has referred, and its lifetime accrued, still-pending and already-paid commission in integer cents, with its payout history.
...
```bash
curl -sS "https://api.hanzo.ai/v1/affiliates"
```

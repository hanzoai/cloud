...
- `GET https://api.hanzo.ai/v1/usage/samples` — Is the PER-PROVIDER view: one connected account's own consumption of its own plan — "my plan is 47% through its 6h window, resets at 14:20".
...
```bash
curl -sS "https://api.hanzo.ai/v1/usage/samples" \
...
- You need a different `usage` capability — that product's skills are listed at `https://api.hanzo.ai/.well-known/agent-skills/_usage/index.json`.

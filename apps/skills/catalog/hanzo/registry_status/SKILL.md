...
- `GET https://api.hanzo.ai/v1/registry/status` — Status reports whether the OCI and npm registries are reachable and, when the OCI half is auth-gated, which token realm its challenge advertises — an honest lens for "is the registry plane up", never a fabricated ok.
...
```bash
curl -sS "https://api.hanzo.ai/v1/registry/status" \

...
- `GET https://api.hanzo.ai/v1/gateway/traffic` — Report who is calling this org's API right now
...
```bash
curl -sS "https://api.hanzo.ai/v1/gateway/traffic" \
...
- You need a different `gateway` capability — that product's skills are listed at `https://api.hanzo.ai/.well-known/agent-skills/_gateway/index.json`.

...
Read-only Hanzo capability derived from the `base` OpenAPI product. Base URL `https://api.hanzo.ai`.
...
- `GET https://api.hanzo.ai/v1/base/bases` — Lists every Base the caller can reach, one per org their token carries.
- `GET https://api.hanzo.ai/v1/base/bases/{org}` — Describes ONE org's Base — whether its store exists, and what it occupies.
...
```bash
curl -sS "https://api.hanzo.ai/v1/base/bases" \
...
- You need a different `base` capability — that product's skills are listed at `https://api.hanzo.ai/.well-known/agent-skills/_base/index.json`.

...
- `GET https://api.hanzo.ai/v1/usage/leaderboard` — Leaderboard ranks AI usage over a window, either the users of the caller's own org or organizations against each other, and always reports the caller's own standing even when it falls outside the returned page.
- `GET https://api.hanzo.ai/v1/usage/leaderboard/optin` — Returns the caller's own public-listing preference and their org's, each with whether the caller may change it.
...
```bash
curl -sS "https://api.hanzo.ai/v1/usage/leaderboard"
```

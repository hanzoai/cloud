...
- `GET https://api.hanzo.ai/v1/iam/auth/application` — Returns everything a login screen needs to draw itself for one application: its branding, and each sign-in method it offers with the provider details that method needs.
- `GET https://api.hanzo.ai/v1/iam/auth/methods` — Returns the sign-in methods one application actually has switched on, so a login screen can render the right buttons for it without you hard-coding a list that drifts the moment you add a provider.
...
```bash
curl -sS "https://api.hanzo.ai/v1/iam/auth/application" \

---
name: eval_evaluators
version: "8.0.0"
description: "Read eval evaluators: Is the judges your org has defined, each with its judge model, criteria and the score name it writes under.."
---

# Hanzo · EVAL · evaluators
...
- `GET https://api.hanzo.ai/v1/eval/evaluators` — Is the judges your org has defined, each with its judge model, criteria and the score name it writes under.
...
- `/v1/eval/evaluators` → `evaluatorList` object with fields: `data`.

## Example

```bash
curl -sS "https://api.hanzo.ai/v1/eval/evaluators" \

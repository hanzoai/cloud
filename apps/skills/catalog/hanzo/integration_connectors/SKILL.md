...
- `GET https://api.hanzo.ai/v1/integration/connectors` — Lists the caller's OWN connectors across every provider — the set `hanzo connector ls` prints.
- `GET https://api.hanzo.ai/v1/integration/connectors/providers` — Lists the user-scoped provider cards — the catalog of what a user can connect, and how.
- `GET https://api.hanzo.ai/v1/integration/connectors/{id}/token` — Hands the custodied access token to its owner — the ONE place custody exits.
...
```bash
curl -sS "https://api.hanzo.ai/v1/integration/connectors" \

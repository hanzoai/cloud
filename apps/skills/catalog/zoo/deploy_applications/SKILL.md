...
- `GET https://api.zoo.ngo/v1/deploy/applications/{name}/revisions/{revision}/metadata` — Returns the argocd RevisionMetadata for one revision of one application — what the detail view shows beside a revision.
- `GET https://api.zoo.ngo/v1/deploy/applications/{name}/syncwindows` — Returns one application's argocd ApplicationSyncWindowState — the answer to "is anything blocking a sync of this application right now?".
...
```bash
curl -sS "https://api.zoo.ngo/v1/deploy/applications" \
...
- You need a different `deploy` capability — that product's skills are listed at `https://api.zoo.ngo/.well-known/agent-skills/_deploy/index.json`.

...
- `GET https://api.zoo.ngo/v1/platform/apps` — Answers what this organisation has declared, joined with what the delivery plane has done about it.
- `GET https://api.zoo.ngo/v1/platform/apps/{app}` — Answers ONE declaration — what git says this app is, before the delivery plane has had any say in it.
- `GET https://api.zoo.ngo/v1/platform/apps/{app}/cd` — Answers ONE app's reconciliation alone — the poll a deploy console makes while it waits, without re-reading the whole inventory each time.
...
```bash
curl -sS "https://api.zoo.ngo/v1/platform/apps" \
...
- You need a different `platform` capability — that product's skills are listed at `https://api.zoo.ngo/.well-known/agent-skills/_platform/index.json`.

...
- `GET https://api.zoo.ngo/v1/team/transactor/statistics` — Statistics returns the transactor's live sessions for the space the caller's credential names — the endpoint the front's space switcher and server panel poll on the transactor base.
- `GET https://api.zoo.ngo/v1/team/transactor/{token}` — Open the space data-plane socket

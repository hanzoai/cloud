...
- `GET https://api.zoo.ngo/v1/iam/invitations` — Returns your organization's invitations, newest first — who has been asked to join, on what terms, and how many seats each invitation still has left.
- `GET https://api.zoo.ngo/v1/iam/invitations/{owner}/{name}` — Returns one invitation: who it is for, what it grants on acceptance, and when it expires.
...
```bash
curl -sS "https://api.zoo.ngo/v1/iam/invitations" \

...
- `GET https://api.zoo.ngo/v1/campaign/{id}/metrics` — Returns a campaign's results over a window: the analytics funnel (impressions, clicks, conversions, revenue, visitors), the spend each channel's connector reports, and the derived growth KPIs — CTR, CVR, CAC and ROAS.
...
```bash
curl -sS "https://api.zoo.ngo/v1/campaign/{id}/metrics" \
...
- You need a different `campaign` capability — that product's skills are listed at `https://api.zoo.ngo/.well-known/agent-skills/_campaign/index.json`.

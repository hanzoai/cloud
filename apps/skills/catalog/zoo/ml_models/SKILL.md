...
- `GET https://api.zoo.ngo/v1/ml/models` — Lists the inference models deployed in the caller's org.
- `GET https://api.zoo.ngo/v1/ml/models/{name}` — Returns one deployed inference model.
...
```bash
curl -sS "https://api.zoo.ngo/v1/ml/models" \
...
- You need a different `ml` capability — that product's skills are listed at `https://api.zoo.ngo/.well-known/agent-skills/_ml/index.json`.

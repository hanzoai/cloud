...
# Zoo · O11Y · getresetpasswordtoken
...
- `GET https://api.zoo.ngo/v1/o11y/getResetPasswordToken/{id}` — Returns a user's password-reset token, creating one if none is live.
...
```bash
curl -sS "https://api.zoo.ngo/v1/o11y/getResetPasswordToken/{id}" \

...
- `GET https://api.zoo.ngo/v1/provisioning/vector` — Lists the caller org's vector collections.
- `GET https://api.zoo.ngo/v1/provisioning/vector/{name}` — Returns one vector collection's metadata.
...
```bash
curl -sS "https://api.zoo.ngo/v1/provisioning/vector" \

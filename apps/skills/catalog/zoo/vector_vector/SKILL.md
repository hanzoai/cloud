...
- `GET https://api.zoo.ngo/v1/vector` — Lists the caller org's vector collections.
- `GET https://api.zoo.ngo/v1/vector/{name}` — Returns one vector collection's metadata.
...
```bash
curl -sS "https://api.zoo.ngo/v1/vector"
```

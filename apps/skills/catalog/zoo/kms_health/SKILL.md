...
Read-only Zoo capability derived from the `kms` OpenAPI product. Base URL `https://api.zoo.ngo`.
...
- `GET https://api.zoo.ngo/v1/kms/health` — Reports whether this broker can actually serve secrets.
...
```bash
curl -sS "https://api.zoo.ngo/v1/kms/health" \
...
- You need a different `kms` capability — that product's skills are listed at `https://api.zoo.ngo/.well-known/agent-skills/_kms/index.json`.

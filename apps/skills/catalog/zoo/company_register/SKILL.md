...
# Zoo · COMPANY · register

Read-only Zoo capability derived from the `company` OpenAPI product. Base URL `https://api.zoo.ngo`.
...
- `GET https://api.zoo.ngo/v1/company/register` — Returns the platform's whole formation register, newest activity first — every org's formation, not the caller's.
- `GET https://api.zoo.ngo/v1/company/register/summary` — Counts the platform's formations by stage — the register's shape in one read, so a queue that is growing is visible as a number rather than inferred by paging the list.
...
```bash
curl -sS "https://api.zoo.ngo/v1/company/register" \
...
- You need a different `company` capability — that product's skills are listed at `https://api.zoo.ngo/.well-known/agent-skills/_company/index.json`.

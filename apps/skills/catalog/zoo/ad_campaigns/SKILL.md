...
Read-only Zoo capability derived from the `ad` OpenAPI product. Base URL `https://api.zoo.ngo`.
...
- `GET https://api.zoo.ngo/v1/ad/campaigns` — Returns the caller org's ad campaigns, most recently updated first, optionally narrowed to one lifecycle status.
- `GET https://api.zoo.ngo/v1/ad/campaigns/{id}` — Returns one of the caller org's campaigns.
...
```bash
curl -sS "https://api.zoo.ngo/v1/ad/campaigns" \
...
- You need a different `ad` capability — that product's skills are listed at `https://api.zoo.ngo/.well-known/agent-skills/_ad/index.json`.

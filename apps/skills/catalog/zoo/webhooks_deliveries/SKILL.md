...
- `GET https://api.zoo.ngo/v1/webhooks/{id}/deliveries` — Returns one endpoint's per-attempt delivery log, newest first — the record of what was sent, what the subscriber answered, and how long it took.
...
```bash
curl -sS "https://api.zoo.ngo/v1/webhooks/{id}/deliveries" \

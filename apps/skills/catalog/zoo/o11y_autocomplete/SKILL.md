...
- `GET https://api.zoo.ngo/v1/o11y/autocomplete/aggregate_attributes` — Lists the attributes usable as an aggregate target for the given telemetry and operator — what a filter builder offers after the aggregation is chosen.
- `GET https://api.zoo.ngo/v1/o11y/autocomplete/attribute_keys` — Lists the attribute keys available for filtering the given telemetry, each with its data type and whether it is a materialized column.
- `GET https://api.zoo.ngo/v1/o11y/autocomplete/attribute_values` — Lists the values one attribute key has taken — string, number and bool values in their own lists — for completing a filter.
...
```bash
curl -sS "https://api.zoo.ngo/v1/o11y/autocomplete/aggregate_attributes" \

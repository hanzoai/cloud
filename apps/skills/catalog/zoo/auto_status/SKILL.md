...
- `GET https://api.zoo.ngo/v1/auto/status` — Status reports whether the auto service is reachable — its own health endpoint as an honest lens for "is the automation plane up".
...
```bash
curl -sS "https://api.zoo.ngo/v1/auto/status" \

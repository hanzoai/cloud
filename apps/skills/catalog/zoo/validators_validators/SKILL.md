...
- `GET https://api.zoo.ngo/v1/validators` — Returns the validator slots the caller's org has claimed.
- `GET https://api.zoo.ngo/v1/validators/{tokenId}` — Returns one claimed validator slot, scoped to the caller's org.
...
```bash
curl -sS "https://api.zoo.ngo/v1/validators"
```

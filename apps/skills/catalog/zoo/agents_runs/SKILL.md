...
- `GET https://api.zoo.ngo/v1/agents/runs` — Returns the org's agent runs across EVERY agent, newest first — what ran here, for whom, on which model, how long it took, and why it failed.
- `GET https://api.zoo.ngo/v1/agents/{ref}/runs` — Returns one agent's execution history, newest first — each run's input, its output or its error, and how long it took.
...
```bash
curl -sS "https://api.zoo.ngo/v1/agents/runs" \

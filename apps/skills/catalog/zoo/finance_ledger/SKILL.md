...
- `GET https://api.zoo.ngo/v1/finance/ledger` — Answers the org's own postings inside `range=`, each as a signed entry: a DEPOSIT CREDITS the wallet (positive, account `credits:<org>`) and every other posting DEBITS it (negative, account `usage:<org>`), described by its notes or its tags.
...
```bash
curl -sS "https://api.zoo.ngo/v1/finance/ledger"
```

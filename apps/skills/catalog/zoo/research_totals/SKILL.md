...
- `GET https://api.zoo.ngo/v1/research/totals` — Returns the caller org's headline aggregate plus a per-kind breakdown — the observatory's poll target.
...
```bash
curl -sS "https://api.zoo.ngo/v1/research/totals" \

...
- `GET https://api.zoo.ngo/v1/projects/sites` — Returns the org's deployed sites at the pretty URLs they serve at.
- `GET https://api.zoo.ngo/v1/projects/sites/{slug}` — Returns one site — the same row ListSites carries, for one slug.
...
```bash
curl -sS "https://api.zoo.ngo/v1/projects/sites" \

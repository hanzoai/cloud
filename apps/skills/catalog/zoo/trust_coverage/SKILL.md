...
- `GET https://api.zoo.ngo/v1/trust/coverage` — Reads coverage: per framework, how many clauses have an automated control behind them, how many are partial, and how many have none — each carrying the unit it is counted in, because "12 of 20" is not a fact until you know what the 20 are.
- `GET https://api.zoo.ngo/v1/trust/coverage/{framework}` — Reads one framework clause by clause: every clause the standard publishes, what covers it, and which controls stand behind it — so a coverage number can be checked line by line rather than taken on trust.
...
```bash
curl -sS "https://api.zoo.ngo/v1/trust/coverage" \

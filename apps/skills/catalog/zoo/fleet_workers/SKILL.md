...
- `GET https://api.zoo.ngo/v1/fleet/workers` — Returns the caller org's BYO machines — the ones that dialed in via `hanzo link` — with everything each host reported about itself.
...
```bash
curl -sS "https://api.zoo.ngo/v1/fleet/workers" \

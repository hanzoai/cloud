...
- `GET https://api.zoo.ngo/v1/security/findings` — Is the org's findings — rule, severity, path, line, masked preview and fingerprint — newest first, across scans or within one.
- `GET https://api.zoo.ngo/v1/security/findings/{id}` — Returns a single finding: which rule fired, where (path and line), the masked preview and the SHA-256 fingerprint of the secret — the raw secret is not stored and cannot be read back.
...
```bash
curl -sS "https://api.zoo.ngo/v1/security/findings" \
...
- You need a different `security` capability — that product's skills are listed at `https://api.zoo.ngo/.well-known/agent-skills/_security/index.json`.

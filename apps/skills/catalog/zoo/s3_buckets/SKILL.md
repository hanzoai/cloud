...
- `GET https://api.zoo.ngo/v1/s3/buckets` — Lists the caller org's own buckets.
- `GET https://api.zoo.ngo/v1/s3/buckets/{bucket}/blob/{wildcard1}` — Read one object's bytes
- `GET https://api.zoo.ngo/v1/s3/buckets/{bucket}/objects` — Lists one folder level of a bucket.
- `GET https://api.zoo.ngo/v1/s3/buckets/{bucket}/objects/{wildcard1}` — Mints a presigned GET URL the caller downloads from DIRECTLY.
...
```bash
curl -sS "https://api.zoo.ngo/v1/s3/buckets" \
...
- You need a different `s3` capability — that product's skills are listed at `https://api.zoo.ngo/.well-known/agent-skills/_s3/index.json`.

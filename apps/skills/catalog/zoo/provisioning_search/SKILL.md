...
- `GET https://api.zoo.ngo/v1/provisioning/search` — Lists the caller org's search indexes.
- `GET https://api.zoo.ngo/v1/provisioning/search/{name}` — Returns one search index's metadata.
...
```bash
curl -sS "https://api.zoo.ngo/v1/provisioning/search" \

...
- `GET https://api.zoo.ngo/v1/admin/author` — Returns the platform's whole author program — every org's author record, not the caller's — with each one's repository and deploy counts and a fleet roll-up of the money accrued, pending and paid.
- `GET https://api.zoo.ngo/v1/admin/author/{id}/basis` — Returns the audit trail behind ONE author's royalty — the same payload the author reads at /v1/author/basis, from the same builder, so support sees exactly what the author sees rather than a parallel view free to drift.
...
```bash
curl -sS "https://api.zoo.ngo/v1/admin/author" \

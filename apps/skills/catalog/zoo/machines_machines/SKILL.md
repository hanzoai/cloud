...
- `GET https://api.zoo.ngo/v1/machines` — Returns every machine the caller's org has — Visor's registry, the live DigitalOcean droplets and the DOKS worker nodes (deduped into one union), plus the BYO machines that dialed in via `hanzo link` (provider "byo").
- `GET https://api.zoo.ngo/v1/machines/{id}` — Returns one of the caller org's machines by its org-scoped name.
...
```bash
curl -sS "https://api.zoo.ngo/v1/machines"
```

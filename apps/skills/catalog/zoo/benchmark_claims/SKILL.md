...
- `GET https://api.zoo.ngo/v1/benchmark/claims` — Lists the effective published claims: what the leaderboard will use for each (benchmark, model) after the seed, the import and any stored correction are layered.
...
```bash
curl -sS "https://api.zoo.ngo/v1/benchmark/claims" \

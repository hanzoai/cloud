...
- `GET https://api.zoo.ngo/v1/marketing/promos` — Returns every promo the deployment offers with its live counters: how many orgs have redeemed it and how many redemptions remain under the cap.
- `GET https://api.zoo.ngo/v1/marketing/promos/{code}/eligibility` — Prices a promo against a plan and seat count.
- `GET https://api.zoo.ngo/v1/marketing/promos/{code}/redemption` — Returns the caller org's OWN redemption of a promo — an org-scoped read, so it can never surface another tenant's.
...
```bash
curl -sS "https://api.zoo.ngo/v1/marketing/promos" \
...
- You need a different `marketing` capability — that product's skills are listed at `https://api.zoo.ngo/.well-known/agent-skills/_marketing/index.json`.

...
- `GET https://api.zoo.ngo/v1/social/posts` — Returns the org's posts — content, channel, status, scheduled time, media and timestamps — most-recently-updated first.
- `GET https://api.zoo.ngo/v1/social/posts/{id}` — Returns one of the org's posts by id, with its current status, scheduled time, media and — once it has published — the account and external id it published under.
...
```bash
curl -sS "https://api.zoo.ngo/v1/social/posts" \
...
- You need a different `social` capability — that product's skills are listed at `https://api.zoo.ngo/.well-known/agent-skills/_social/index.json`.

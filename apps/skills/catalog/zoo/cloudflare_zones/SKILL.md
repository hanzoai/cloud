...
- `GET https://api.zoo.ngo/v1/cloudflare/zones` — Lists the Cloudflare zones the org's connected API token can see, paged and filtered by the query parameters Cloudflare itself accepts.
- `GET https://api.zoo.ngo/v1/cloudflare/zones/{zone}` — Reads one Cloudflare zone the org's token can see.
- `GET https://api.zoo.ngo/v1/cloudflare/zones/{zone}/analytics` — Reads a zone's Cloudflare traffic dashboard — requests, bandwidth, threats and pageviews over the since/until window.
...
```bash
curl -sS "https://api.zoo.ngo/v1/cloudflare/zones" \
...
- You need a different `cloudflare` capability — that product's skills are listed at `https://api.zoo.ngo/.well-known/agent-skills/_cloudflare/index.json`.

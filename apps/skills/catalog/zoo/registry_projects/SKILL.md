...
- `GET https://api.zoo.ngo/v1/registry/projects` — Projects lists the namespaces the caller can see with what each holds: the org's slug, its repository count on the OCI catalog, and its package count on the npm registry.
...
```bash
curl -sS "https://api.zoo.ngo/v1/registry/projects" \

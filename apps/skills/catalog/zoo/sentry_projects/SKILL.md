...
- `GET https://api.zoo.ngo/v1/sentry/projects` — Lists the caller's org's Sentry projects, each with its freshly-derived DSN.
- `GET https://api.zoo.ngo/v1/sentry/projects/{id}` — Returns one Sentry project of the caller's org, DSN included.
...
```bash
curl -sS "https://api.zoo.ngo/v1/sentry/projects" \

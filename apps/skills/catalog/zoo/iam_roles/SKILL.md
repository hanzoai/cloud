...
- `GET https://api.zoo.ngo/v1/iam/roles` — Returns your organization's roles, newest first — each a named group of people that permissions are granted to.
- `GET https://api.zoo.ngo/v1/iam/roles/{owner}/{name}` — Returns one role: who is in it, and the roles it includes.
...
```bash
curl -sS "https://api.zoo.ngo/v1/iam/roles" \

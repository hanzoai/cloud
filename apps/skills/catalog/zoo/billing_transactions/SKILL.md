...
- `GET https://api.zoo.ngo/v1/billing/transactions` — Answers one page of the caller's own ledger, newest first: what moved, how much, when, and what it was tagged with.
- `GET https://api.zoo.ngo/v1/billing/transactions/{id}` — Reads one ledger entry by its id.
...
```bash
curl -sS "https://api.zoo.ngo/v1/billing/transactions" \

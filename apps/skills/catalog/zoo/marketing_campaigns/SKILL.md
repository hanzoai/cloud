...
- `GET https://api.zoo.ngo/v1/marketing/campaigns` — Returns the org's campaigns, most recently updated first, optionally narrowed to one lifecycle status.
- `GET https://api.zoo.ngo/v1/marketing/campaigns/{id}` — Returns one of the caller org's campaigns.
...
```bash
curl -sS "https://api.zoo.ngo/v1/marketing/campaigns" \

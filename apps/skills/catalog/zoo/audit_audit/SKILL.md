...
Read-only Zoo capability derived from the `audit` OpenAPI product. Base URL `https://api.zoo.ngo`.
...
- `GET https://api.zoo.ngo/v1/audit` — List reads the caller's OWN org audit trail, newest first, with the total the filter matched so a console can page it.
...
```bash
curl -sS "https://api.zoo.ngo/v1/audit" \
...
- You need a different `audit` capability — that product's skills are listed at `https://api.zoo.ngo/.well-known/agent-skills/_audit/index.json`.

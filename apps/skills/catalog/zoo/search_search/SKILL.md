...
Read-only Zoo capability derived from the `search` OpenAPI service. Base URL `https://api.zoo.ngo`.
...
- `GET https://api.zoo.ngo/v1/search` — Lists the caller org's search indexes.
- `GET https://api.zoo.ngo/v1/search/{name}` — Returns one search index's metadata.
...
```bash
curl -sS "https://api.zoo.ngo/v1/search"
```

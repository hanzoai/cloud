...
- `GET https://api.zoo.ngo/v1/functions/{name}/logs` — Is the output of a function's most recent run — its error text when that run failed, else what it printed.
...
```bash
curl -sS "https://api.zoo.ngo/v1/functions/{name}/logs" \

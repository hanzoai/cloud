...
- `GET https://api.zoo.ngo/v1/ingress/services` — Returns every backend pool the caller's org has configured, ordered by id.
- `GET https://api.zoo.ngo/v1/ingress/services/{id}` — Returns one of the caller org's backend pools by id.
...
```bash
curl -sS "https://api.zoo.ngo/v1/ingress/services" \

...
- `GET https://api.zoo.ngo/v1/node` — Returns the caller org's currently connected bot nodes: what each one calls itself, the platform it runs on, its agent version, when its socket was established, and the capabilities and commands it reported.
...
```bash
curl -sS "https://api.zoo.ngo/v1/node" \
...
- You need a different `node` capability — that product's skills are listed at `https://api.zoo.ngo/.well-known/agent-skills/_node/index.json`.

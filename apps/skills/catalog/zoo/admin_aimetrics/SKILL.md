...
- `GET https://api.zoo.ngo/v1/admin/aimetrics` — Is the fleet AI board: LLM generations over gen_ai spans (count, cost, avg/p95 latency, per-model), per-model usage from the live cloud_usage ledger, and the eval plane (traces, scores, score names, runs, and the average-score trend).
...
```bash
curl -sS "https://api.zoo.ngo/v1/admin/aimetrics" \

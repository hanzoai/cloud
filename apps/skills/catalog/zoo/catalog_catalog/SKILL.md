...
Read-only Zoo capability derived from the `catalog` OpenAPI product. Base URL `https://api.zoo.ngo`.
...
- `GET https://api.zoo.ngo/v1/catalog` — Browse searches AND browses the cross-org catalog: every project, app and site the fleet has built, whichever org built it.
...
```bash
curl -sS "https://api.zoo.ngo/v1/catalog" \
...
- You need a different `catalog` capability — that product's skills are listed at `https://api.zoo.ngo/.well-known/agent-skills/_catalog/index.json`.

...
- `GET https://api.zoo.ngo/v1/tool` — Lists every tool the caller's org and project can reach, from every source, each flagged with whether it is activated.
...
```bash
curl -sS "https://api.zoo.ngo/v1/tool" \

...
- `GET https://api.zoo.ngo/v1/provisioning/s3` — Lists the caller org's object-storage buckets.
- `GET https://api.zoo.ngo/v1/provisioning/s3/{name}` — Returns one bucket's metadata.
...
```bash
curl -sS "https://api.zoo.ngo/v1/provisioning/s3" \

...
- `GET https://api.zoo.ngo/v1/dataroom/trust` — Answers the caller org's OWN trust centre: its settings, every item it holds in both tiers, the requests waiting on it, and the grants it has made.
- `GET https://api.zoo.ngo/v1/dataroom/trust/center/{slug}` — Answers an org's public trust centre: its name, the text a party must accept to ask for a document, and every item it publishes.
- `GET https://api.zoo.ngo/v1/dataroom/trust/center/{slug}/file/{item}` — Read a public trust-centre item's bytes
...
```bash
curl -sS "https://api.zoo.ngo/v1/dataroom/trust" \

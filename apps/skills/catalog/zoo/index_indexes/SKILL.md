...
- `GET https://api.zoo.ngo/v1/index/indexes` — Lists the indexes your org holds.
- `GET https://api.zoo.ngo/v1/index/indexes/{uid}` — Reads one index's definition.
- `GET https://api.zoo.ngo/v1/index/indexes/{uid}/documents` — Pages through the documents in an index.
- `GET https://api.zoo.ngo/v1/index/indexes/{uid}/documents/{id}` — Reads one document by its primary key.
- `GET https://api.zoo.ngo/v1/index/indexes/{uid}/settings` — Reads an index's filterable attributes.
...
- You need a different `index` capability — that product's skills are listed at `https://api.zoo.ngo/.well-known/agent-skills/_index/index.json`.

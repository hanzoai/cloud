...
- `GET https://api.zoo.ngo/v1/admin/bases` — Lists the tenant Base instances in the caller's window — a SuperAdmin sees every tenant's, anyone else only their own subtree's.
...
```bash
curl -sS "https://api.zoo.ngo/v1/admin/bases" \

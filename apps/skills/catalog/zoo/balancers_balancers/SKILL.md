...
# Zoo · BALANCERS · balancers

Read-only Zoo capability derived from the `balancers` OpenAPI service. Base URL `https://api.zoo.ngo`.
...
- `GET https://api.zoo.ngo/v1/balancers` — Returns every load balancer the caller's org owns, under the friendly names the org created them with.
- `GET https://api.zoo.ngo/v1/balancers/{id}` — Returns one of the caller org's load balancers by id.
...
```bash
curl -sS "https://api.zoo.ngo/v1/balancers" \

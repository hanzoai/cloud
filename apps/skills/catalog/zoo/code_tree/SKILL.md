...
- `GET https://api.zoo.ngo/v1/code/tree` — Returns one repository's file structure with a per-file symbol count — get_repo_structure over the org's own index, with no git checkout involved.
...
```bash
curl -sS "https://api.zoo.ngo/v1/code/tree" \

...
- `GET https://api.zoo.ngo/v1/crm/applications` — Returns the org's Startup Program applications, newest first.
- `GET https://api.zoo.ngo/v1/crm/applications/{id}` — Returns one Startup Program application with its AI screen and stage history.
...
```bash
curl -sS "https://api.zoo.ngo/v1/crm/applications"
```

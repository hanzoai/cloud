...
- `GET https://api.zoo.ngo/v1/functions/{name}/invocations` — Is one function's past runs, newest first — each with its status, HTTP code, method, time and duration.
...
```bash
curl -sS "https://api.zoo.ngo/v1/functions/{name}/invocations" \

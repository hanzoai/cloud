...
- `GET https://api.zoo.ngo/v1/company/review` — Reports the founders whose KYC is not yet settled, oldest formation first, so the queue drains in the order founders have been waiting.
...
```bash
curl -sS "https://api.zoo.ngo/v1/company/review" \

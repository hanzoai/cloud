...
- `GET https://api.zoo.ngo/v1/guide/curriculum` — Returns the journey the caller's org is actually running, and whether it comes from the org's OWN override (custom) or from the platform default — the brand blueprint, else the embedded fixture.
...
```bash
curl -sS "https://api.zoo.ngo/v1/guide/curriculum" \

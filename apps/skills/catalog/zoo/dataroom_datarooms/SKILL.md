...
- `GET https://api.zoo.ngo/v1/dataroom/datarooms` — Returns every data room in the caller org's own store, newest first, with its short public id, name, description and timestamps.
- `GET https://api.zoo.ngo/v1/dataroom/datarooms/{id}` — Reads one of the caller org's data rooms together with every document in it, each carrying its membership id and order index.
...
```bash
curl -sS "https://api.zoo.ngo/v1/dataroom/datarooms" \

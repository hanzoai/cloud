...
Read-only Zoo capability derived from the `wallet` OpenAPI product. Base URL `https://api.zoo.ngo`.
...
- `GET https://api.zoo.ngo/v1/wallet` — Returns the caller org's wallets, newest first, optionally NARROWED within the org by project, agent or account.
- `GET https://api.zoo.ngo/v1/wallet/{id}` — Returns one of the caller org's wallets: its scope, custody kind, tier, chain and on-chain address.
...
```bash
curl -sS "https://api.zoo.ngo/v1/wallet" \
...
- You need a different `wallet` capability — that product's skills are listed at `https://api.zoo.ngo/.well-known/agent-skills/_wallet/index.json`.

...
- `GET https://api.zoo.ngo/v1/sentinel/logs` — Lists a project's captured error events, newest first, optionally narrowed to those whose message or exception text contains a search string.
...
```bash
curl -sS "https://api.zoo.ngo/v1/sentinel/logs" \

...
Read-only Zoo capability derived from the `todo` OpenAPI product. Base URL `https://api.zoo.ngo`.
...
- `GET https://api.zoo.ngo/v1/todo/projects` — Returns the boards of your org — the places your work actually is.
- `GET https://api.zoo.ngo/v1/todo/projects/{key}` — Returns one board of your org by its key — the repository name.
- `GET https://api.zoo.ngo/v1/todo/projects/{key}/issues` — Returns a board's issues — work items with their column, priority, assignee, labels and schedule.
- `GET https://api.zoo.ngo/v1/todo/projects/{key}/issues/{num}` — Returns ONE work item in full — its description included.
...
- You need a different `todo` capability — that product's skills are listed at `https://api.zoo.ngo/.well-known/agent-skills/_todo/index.json`.

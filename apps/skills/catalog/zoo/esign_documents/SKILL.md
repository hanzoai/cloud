...
- `GET https://api.zoo.ngo/v1/esign/documents` — Returns your org's documents, newest first.
- `GET https://api.zoo.ngo/v1/esign/documents/{id}` — Returns one document with its recipients and field layout.
- `GET https://api.zoo.ngo/v1/esign/documents/{id}/audit` — Returns the document's full audit trail, oldest first.
- `GET https://api.zoo.ngo/v1/esign/documents/{id}/download` — Returns the document — the sealed PDF once it is complete.
...
```bash
curl -sS "https://api.zoo.ngo/v1/esign/documents" \
...
- You need a different `esign` capability — that product's skills are listed at `https://api.zoo.ngo/.well-known/agent-skills/_esign/index.json`.

...
- `GET https://api.zoo.ngo/v1/admin/orgs` — Lists the tenant directory one row per org, sorted by slug: member count and the org's month-to-date spend and credit balance, read live from IAM and commerce.
...
```bash
curl -sS "https://api.zoo.ngo/v1/admin/orgs" \

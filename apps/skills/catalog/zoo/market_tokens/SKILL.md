...
- `GET https://api.zoo.ngo/v1/market/tokens` — Answers the tokens one chain's indexer has seen, with the decimals a caller needs to read any amount of one correctly.
...
```bash
curl -sS "https://api.zoo.ngo/v1/market/tokens"
```

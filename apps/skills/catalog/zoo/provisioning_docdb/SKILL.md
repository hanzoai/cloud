...
version: "8.0.0"
description: "Read provisioning docdb: ListDocDB lists the caller org's Zoo DocDB document databases., GetDocDB returns one Zoo DocDB database's metadata.."
---

# Zoo · PROVISIONING · docdb
...
- `GET https://api.zoo.ngo/v1/provisioning/docdb` — ListDocDB lists the caller org's Zoo DocDB document databases.
- `GET https://api.zoo.ngo/v1/provisioning/docdb/{name}` — GetDocDB returns one Zoo DocDB database's metadata.
...
```bash
curl -sS "https://api.zoo.ngo/v1/provisioning/docdb" \

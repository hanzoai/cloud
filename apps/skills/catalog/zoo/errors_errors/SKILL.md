...
Read-only Zoo capability derived from the `errors` OpenAPI product. Base URL `https://api.zoo.ngo`.
...
- `GET https://api.zoo.ngo/v1/errors` — Errors returns the caller org's most recently captured errors, newest first.
...
```bash
curl -sS "https://api.zoo.ngo/v1/errors" \

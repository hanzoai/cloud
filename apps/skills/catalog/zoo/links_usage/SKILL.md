...
name: links_usage
version: "8.0.0"
description: "Read links usage: Shows one provider account's own usage dashboard., Breaks down what the gateway routed through each of your accounts., Shows plan consumption and Zoo spend side by side.."
---

# Zoo · LINKS · usage

Read-only Zoo capability derived from the `links` OpenAPI service. Base URL `https://api.zoo.ngo`.
...
- `GET https://api.zoo.ngo/v1/links/usage` — Shows one provider account's own usage dashboard.
- `GET https://api.zoo.ngo/v1/links/usage/accounts` — Breaks down what the gateway routed through each of your accounts.
- `GET https://api.zoo.ngo/v1/links/usage/summary` — Shows plan consumption and Zoo spend side by side.
...
```bash
curl -sS "https://api.zoo.ngo/v1/links/usage" \

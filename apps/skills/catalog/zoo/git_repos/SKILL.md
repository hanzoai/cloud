...
- `GET https://api.zoo.ngo/v1/git/repos` — Returns the repos in the caller's scope, most recently updated first.
- `GET https://api.zoo.ngo/v1/git/repos/{name}` — Returns one repo with its live ref state: every branch name and the resolved HEAD commit.
...
```bash
curl -sS "https://api.zoo.ngo/v1/git/repos" \

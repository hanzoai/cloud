...
- `GET https://api.zoo.ngo/v1/iam/permissions` — Returns the permissions in one organization, newest first — each one a grant saying which people or roles may do what, and to which resources.
- `GET https://api.zoo.ngo/v1/iam/permissions/{owner}/{name}` — Returns one permission: who it grants to, what it allows, and the resources it covers.
...
```bash
curl -sS "https://api.zoo.ngo/v1/iam/permissions" \

...
Read-only Lux capability derived from the `compute` OpenAPI service. Base URL `https://api.lux.network`.
...
- `GET https://api.lux.network/v1/compute/bots` — Returns the caller org's bot machines — the kind=bot machines — each joined with the agent binding that says which cloud Agent it runs.
- `GET https://api.lux.network/v1/compute/bots/{id}` — Returns one of the caller org's bot machines with its agent binding.
...
```bash
curl -sS "https://api.lux.network/v1/compute/bots" \

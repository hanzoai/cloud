...
- `GET https://api.lux.network/v1/ai/finetune/hf/datasets` — Proxies a HuggingFace dataset search (dataset picker).
- `GET https://api.lux.network/v1/ai/finetune/hf/models` — Proxies a HuggingFace model search (base-model picker).
- `GET https://api.lux.network/v1/ai/finetune/hf/repo` — Returns a repo's detail (files, gated/private state).
- `GET https://api.lux.network/v1/ai/finetune/job` — Returns one job with refreshed live status.
- `GET https://api.lux.network/v1/ai/finetune/jobs` — Returns the org's jobs, refreshing live status for active ones.
- `GET https://api.lux.network/v1/ai/finetune/presets` — Returns the new-job catalog plus, when a selection is passed (?baseModel&method&task&preset[&datasetExamples]), the recommended config so the console can render "Recommended" as a one-click, ready-to-run default.
...
```bash
curl -sS "https://api.lux.network/v1/ai/finetune/hf/datasets" \

...
- `GET https://api.lux.network/v1/o11y/product/metrics` — Returns one product's RED series — request rate, errors, p50 and p95 latency — for the caller's org, plus that org's LLM usage rollup over the same window.
...
```bash
curl -sS "https://api.lux.network/v1/o11y/product/metrics" \

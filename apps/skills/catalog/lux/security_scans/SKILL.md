...
- `GET https://api.lux.network/v1/security/scans` — Is the org's scan history, newest first, each as the same summary the submission answered — files read, findings fired, tally by severity.
- `GET https://api.lux.network/v1/security/scans/{id}` — Returns one scan together with every finding on it, so the detail view is one round-trip rather than a list call per scan.
...
```bash
curl -sS "https://api.lux.network/v1/security/scans" \

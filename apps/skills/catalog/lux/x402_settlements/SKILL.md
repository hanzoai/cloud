...
- `GET https://api.lux.network/v1/x402/settlements/{id}` — Settlement reads one x402 payment receipt by id.
...
```bash
curl -sS "https://api.lux.network/v1/x402/settlements/{id}" \
...
- You need a different `x402` capability — that product's skills are listed at `https://api.lux.network/.well-known/agent-skills/_x402/index.json`.

...
- `GET https://api.lux.network/v1/dns/zones` — List your org's DNS zones
- `GET https://api.lux.network/v1/dns/zones/{zone}` — Read one DNS zone
- `GET https://api.lux.network/v1/dns/zones/{zone}/records` — List a zone's DNS records
- `GET https://api.lux.network/v1/dns/zones/{zone}/records/{record}` — Read one DNS record
...
```bash
curl -sS "https://api.lux.network/v1/dns/zones" \
...
- You need a different `dns` capability — that product's skills are listed at `https://api.lux.network/.well-known/agent-skills/_dns/index.json`.

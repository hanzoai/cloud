...
- `GET https://api.lux.network/v1/network` — Returns the caller's org overlay network on the Zero Trust fabric.
- `GET https://api.lux.network/v1/network/{id}` — Returns one overlay network by id, scoped to the caller's org.
...
```bash
curl -sS "https://api.lux.network/v1/network" \

...
Read-only Lux capability derived from the `todo` OpenAPI service. Base URL `https://api.lux.network`.
...
- `GET https://api.lux.network/v1/todo/projects` — Returns the boards of your org — the places your work actually is.
- `GET https://api.lux.network/v1/todo/projects/{key}` — Returns one board of your org by its key — the repository name.
- `GET https://api.lux.network/v1/todo/projects/{key}/issues` — Returns a board's issues — work items with their column, priority, assignee, labels and schedule.
- `GET https://api.lux.network/v1/todo/projects/{key}/issues/{num}` — Returns ONE work item in full — its description included.
...
```bash
curl -sS "https://api.lux.network/v1/todo/projects"
```

...
- `GET https://api.lux.network/v1/o11y/errortracking/issues` — Lists the caller's org's grouped error issues (by fingerprint) with status, level, counts and first/last-seen.
- `GET https://api.lux.network/v1/o11y/errortracking/issues/{id}` — Returns one grouped issue with its latest occurrence sample.
...
```bash
curl -sS "https://api.lux.network/v1/o11y/errortracking/issues" \

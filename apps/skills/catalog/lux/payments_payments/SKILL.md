...
# Lux · PAYMENTS · payments

Read-only Lux capability derived from the `payments` OpenAPI service. Base URL `https://api.lux.network`.
...
- `GET https://api.lux.network/v1/payments/{id}` — Read one settled payment by its id
...
```bash
curl -sS "https://api.lux.network/v1/payments/{id}" \

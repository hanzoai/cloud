...
- `GET https://api.lux.network/v1/research/artifacts` — Returns the caller org's research-diary feed newest-first — the snapshots and reports tied to its runs, as metadata and content addresses; the bytes themselves are fetched by hash.
- `GET https://api.lux.network/v1/research/artifacts/{sha256}` — Fetch one recorded artifact's bytes by its content hash.
...
```bash
curl -sS "https://api.lux.network/v1/research/artifacts" \

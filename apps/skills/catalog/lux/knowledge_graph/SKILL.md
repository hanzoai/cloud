...
- `GET https://api.lux.network/v1/knowledge/graph` — Returns the caller org's knowledge as a node/edge graph shaped for a force-directed renderer: pages, memories and synced sources as nodes; the page parent tree, the wikilinks between pages, and each source's connector provenance as edges.
...
```bash
curl -sS "https://api.lux.network/v1/knowledge/graph" \

...
- `GET https://api.lux.network/v1/audit` — List reads the caller's OWN org audit trail, newest first, with the total the filter matched so a console can page it.
...
```bash
curl -sS "https://api.lux.network/v1/audit" \
...
- You need a different `audit` capability — that product's skills are listed at `https://api.lux.network/.well-known/agent-skills/_audit/index.json`.

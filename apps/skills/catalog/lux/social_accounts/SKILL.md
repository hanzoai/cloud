...
- `GET https://api.lux.network/v1/social/accounts` — Returns the org's connected accounts — each one's id, network, handle, status and timestamps, most-recently-updated first.
- `GET https://api.lux.network/v1/social/accounts/{id}` — Returns one of the org's connected accounts by id — its network, handle, status and timestamps — or 404.
...
```bash
curl -sS "https://api.lux.network/v1/social/accounts" \

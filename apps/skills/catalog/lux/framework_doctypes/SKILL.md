...
- `GET https://api.lux.network/v1/framework/doctypes` — Returns every DocType defined in the caller's org.
- `GET https://api.lux.network/v1/framework/doctypes/{name}` — Returns one DocType definition — its fields, naming rule, permissions and lifecycle flags.
...
```bash
curl -sS "https://api.lux.network/v1/framework/doctypes" \

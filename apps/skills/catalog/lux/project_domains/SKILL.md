...
- `GET https://api.lux.network/v1/project/{slug}/domains` — Returns every custom hostname this site holds: the live ones, plus any pending claim with the DNS records it still owes.
...
```bash
curl -sS "https://api.lux.network/v1/project/{slug}/domains" \

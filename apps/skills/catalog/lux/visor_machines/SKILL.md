...
- `GET https://api.lux.network/v1/visor/machines` — Returns every machine the caller's org has — Visor's registry, the live DigitalOcean droplets and the DOKS worker nodes (deduped into one union), plus the BYO machines that dialed in via `hanzo link` (provider "byo").
- `GET https://api.lux.network/v1/visor/machines/agents` — Returns every agent↔machine binding in the caller's org — which machines are running which cloud Agent, with vm's own reconciled status.
- `GET https://api.lux.network/v1/visor/machines/{id}` — Returns one of the caller org's machines by its org-scoped name.
- `GET https://api.lux.network/v1/visor/machines/{id}/agent` — Returns the agent binding of one of the caller org's machines, or 404 when the machine runs no bot runtime.
...
```bash
curl -sS "https://api.lux.network/v1/visor/machines" \

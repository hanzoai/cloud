...
- `GET https://api.lux.network/v1/captable/rounds` — Returns the caller org's fundraising rounds, newest first.
- `GET https://api.lux.network/v1/captable/rounds/{id}` — Returns one of the caller org's fundraising rounds together with every investment written into it, oldest first.
...
```bash
curl -sS "https://api.lux.network/v1/captable/rounds" \

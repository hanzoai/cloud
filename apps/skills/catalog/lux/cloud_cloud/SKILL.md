...
- `GET https://api.lux.network/v1/cloud` — Returns the clouds this deployment can link and what linking each one needs — the DigitalOcean token, the AWS role and external id, the GCP credential JSON, the Azure app — plus whether the provider can be linked without storing any long-lived secret.
...
```bash
curl -sS "https://api.lux.network/v1/cloud"
```

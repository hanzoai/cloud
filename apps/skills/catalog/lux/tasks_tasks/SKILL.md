...
Read-only Lux capability derived from the `tasks` OpenAPI product. Base URL `https://api.lux.network`.
...
- You need a different `tasks` capability — that product's skills are listed at `https://api.lux.network/.well-known/agent-skills/_tasks/index.json`.

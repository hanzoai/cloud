...
- `GET https://api.lux.network/v1/iam/applications` — Returns the applications in one organization, newest first — each product or site your people sign in to, with the sign-in methods and redirect URIs it allows.
- `GET https://api.lux.network/v1/iam/applications/{owner}/{name}` — Returns one application: its sign-in methods, its allowed redirect URIs and the client credentials your integration authenticates with.
...
```bash
curl -sS "https://api.lux.network/v1/iam/applications" \

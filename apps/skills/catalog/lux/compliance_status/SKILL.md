...
version: "8.0.0"
description: "Read compliance status: Status is the org's honest posture read: the wired provider and the per-status tally of its verifications.."
---

# Lux · COMPLIANCE · status
...
- `GET https://api.lux.network/v1/compliance/status` — Status is the org's honest posture read: the wired provider and the per-status tally of its verifications.

## Response

- `/v1/compliance/status` → `statusView` object with fields: `disclaimer`, `provider`, `verifications`.

## Example

```bash
curl -sS "https://api.lux.network/v1/compliance/status" \

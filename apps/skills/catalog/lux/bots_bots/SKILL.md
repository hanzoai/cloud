...
- `GET https://api.lux.network/v1/bots` — List returns the caller org's live bot runs, read from the bot runtime and projected into the console contract with each run's live session URL derived here.
...
```bash
curl -sS "https://api.lux.network/v1/bots" \

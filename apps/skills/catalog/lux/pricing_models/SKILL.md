...
name: pricing_models
version: "8.0.0"
description: "Read pricing models: Returns the whole model catalog — every model the gateway serves, Zen and third-party alike — filtered to what the caller's org may see.."
---
...
- `GET https://api.lux.network/v1/pricing/models` — Returns the whole model catalog — every model the gateway serves, Zen and third-party alike — filtered to what the caller's org may see.

## Response

- `/v1/pricing/models` → `pricingModelList` object with fields: `models`, `total`, `updated`.

## Example

```bash
curl -sS "https://api.lux.network/v1/pricing/models" \

...
- `GET https://api.lux.network/v1/campaign/{id}/metrics` — Returns a campaign's results over a window: the analytics funnel (impressions, clicks, conversions, revenue, visitors), the spend each channel's connector reports, and the derived growth KPIs — CTR, CVR, CAC and ROAS.
...
```bash
curl -sS "https://api.lux.network/v1/campaign/{id}/metrics" \

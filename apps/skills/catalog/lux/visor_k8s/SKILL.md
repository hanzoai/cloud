...
- `GET https://api.lux.network/v1/visor/k8s/clusters` — Lists the org's DOKS clusters (Visor, house account) folded with the org's BYO clusters — ONE fleet cluster view under the unified k8s noun.
- `GET https://api.lux.network/v1/visor/k8s/clusters/{id}` — Returns one cluster's detail: node pools + worker nodes.
- `GET https://api.lux.network/v1/visor/k8s/nodes` — Returns every DOKS worker node in the org's clusters as a machine — the SAME set the fleet folds in (managedMachines), exposed directly under the k8s noun.
...
```bash
curl -sS "https://api.lux.network/v1/visor/k8s/clusters" \

...
- `GET https://api.lux.network/v1/benchmark/catalog` — Is the canonical public benchmarks this arena runs — the id, title, axis, item count and upstream source of each, with native marking the ones the standardized harness runs today; the rest are registered and adapter-pending.
...
```bash
curl -sS "https://api.lux.network/v1/benchmark/catalog" \

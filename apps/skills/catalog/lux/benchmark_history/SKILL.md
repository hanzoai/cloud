...
- `GET https://api.lux.network/v1/benchmark/history` — Returns each model's measured score per run over time, oldest first, with the change between runs.
...
```bash
curl -sS "https://api.lux.network/v1/benchmark/history" \

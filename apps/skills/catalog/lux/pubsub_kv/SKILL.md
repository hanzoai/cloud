...
- `GET https://api.lux.network/v1/pubsub/kv/{bucket}/{key}` — Get returns one key's current value and revision.
- `GET https://api.lux.network/v1/pubsub/kv/{bucket}/{key}/history` — History returns one key's retained revisions, oldest first — every put and every delete marker up to the bucket's History depth.
...
```bash
curl -sS "https://api.lux.network/v1/pubsub/kv/{bucket}/{key}" \

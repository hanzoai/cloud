...
- `GET https://api.lux.network/v1/sentinel/traces` — Lists the traces a project's captured errors reference, each with how many errors landed on it, when they started and stopped, and the latest message seen — the entry point for "which requests are failing".
- `GET https://api.lux.network/v1/sentinel/traces/{id}` — Returns one trace's captured errors for a project — every error event that carried the trace id, in the order the events plane holds them.
...
```bash
curl -sS "https://api.lux.network/v1/sentinel/traces" \

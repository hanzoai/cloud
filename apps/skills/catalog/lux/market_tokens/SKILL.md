...
name: market_tokens
version: "8.0.0"
description: "Read market tokens: Answers the tokens one chain's indexer has seen, with the decimals a caller needs to read any amount of one correctly.."
---
...
- `GET https://api.lux.network/v1/market/tokens` — Answers the tokens one chain's indexer has seen, with the decimals a caller needs to read any amount of one correctly.
...
- `/v1/market/tokens` → `Tokens` object with fields: `chain`, `reach`, `tokens`.

## Example

```bash
curl -sS "https://api.lux.network/v1/market/tokens"
```

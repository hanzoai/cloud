...
- `GET https://api.lux.network/v1/tools/catalog` — Lists the MCP servers the public registries publish, as we hold them: our canonical copy of registry.modelcontextprotocol.io, plus what we decided about each entry.
- `GET https://api.lux.network/v1/tools/catalog/{id}` — Returns one catalog entry in full: the publisher's description, its repository and site, every package form with the runtime that launches it, and every hosted endpoint.
...
```bash
curl -sS "https://api.lux.network/v1/tools/catalog" \

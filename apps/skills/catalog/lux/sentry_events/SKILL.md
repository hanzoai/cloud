...
- `GET https://api.lux.network/v1/sentry/events/{id}` — Returns one captured error event of a project, by its id.
...
```bash
curl -sS "https://api.lux.network/v1/sentry/events/{id}"
```

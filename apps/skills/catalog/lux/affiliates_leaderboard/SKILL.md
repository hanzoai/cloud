...
- `GET https://api.lux.network/v1/affiliates/leaderboard` — Answers the top affiliates by lifetime accrued commission, shown by OPT-IN HANDLE with aggregate figures only, plus the caller's own exact rank.
...
```bash
curl -sS "https://api.lux.network/v1/affiliates/leaderboard" \

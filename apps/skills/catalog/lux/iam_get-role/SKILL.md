...
- `GET https://api.lux.network/v1/iam/get-role` — Reads one record — the older spelling of the single reads on the REST surface, over the same data and the same permissions.
...
```bash
curl -sS "https://api.lux.network/v1/iam/get-role" \

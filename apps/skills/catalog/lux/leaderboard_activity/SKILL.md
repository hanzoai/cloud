...
- `GET https://api.lux.network/v1/leaderboard/activity` — Activity returns the per-day usage series for ONE authorized subject — the points a contribution heatmap and a timeline are drawn from, gap-filled so every day in the range is present.
...
```bash
curl -sS "https://api.lux.network/v1/leaderboard/activity" \

...
- `GET https://api.lux.network/v1/ad/campaigns` — Returns the caller org's ad campaigns, most recently updated first, optionally narrowed to one lifecycle status.
- `GET https://api.lux.network/v1/ad/campaigns/{id}` — Returns one of the caller org's campaigns.
...
```bash
curl -sS "https://api.lux.network/v1/ad/campaigns" \

...
- `GET https://api.lux.network/v1/mq/streams` — Returns the org's streams, name-ordered, with their live state.
- `GET https://api.lux.network/v1/mq/streams/{name}` — Returns one stream's configuration and live state.
- `GET https://api.lux.network/v1/mq/streams/{name}/messages` — Reads stored messages without a consumer: by sequence, by newest on a subject, or walking a subject forward from a sequence.
- `GET https://api.lux.network/v1/mq/streams/{stream}/consumers` — Returns a stream's consumers, name-ordered, with delivery state.
- `GET https://api.lux.network/v1/mq/streams/{stream}/consumers/{name}` — Returns one consumer's configuration and delivery state.
...
```bash
curl -sS "https://api.lux.network/v1/mq/streams"
```

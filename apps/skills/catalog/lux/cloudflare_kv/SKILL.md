...
- `GET https://api.lux.network/v1/cloudflare/kv/namespaces` — KVNamespaceList lists the Workers KV namespaces on the org's Cloudflare account.
- `GET https://api.lux.network/v1/cloudflare/kv/namespaces/{namespace}/values/{key}` — Read a Workers KV value as its stored bytes
...
```bash
curl -sS "https://api.lux.network/v1/cloudflare/kv/namespaces" \

...
- `GET https://api.lux.network/v1/billing/recharge` — Reads the caller's auto-reload rule: top the balance up by `amountCents` whenever it falls below `thresholdCents`, charging the card on file off-session.
...
```bash
curl -sS "https://api.lux.network/v1/billing/recharge" \

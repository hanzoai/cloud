...
- `GET https://api.lux.network/v1/search` — Lists the caller org's search indexes.
- `GET https://api.lux.network/v1/search/{name}` — Returns one search index's metadata.
...
```bash
curl -sS "https://api.lux.network/v1/search" \

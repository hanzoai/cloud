...
- `GET https://api.lux.network/v1/settings/{product}` — Reads the caller org's configuration for one product, with every secret field MASKED — only the names of the set secrets come back, never their values, which live in KMS.
...
```bash
curl -sS "https://api.lux.network/v1/settings/{product}" \
...
- You need a different `settings` capability — that product's skills are listed at `https://api.lux.network/.well-known/agent-skills/_settings/index.json`.

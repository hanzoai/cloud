...
Read-only Lux capability derived from the `sentinel` OpenAPI product. Base URL `https://api.lux.network`.
...
- `GET https://api.lux.network/v1/sentinel/issues` — Lists the caller's org's grouped error issues, optionally narrowed to one project and one time window, and filtered by status, level, environment, service, a free-text query and a sort.
- `GET https://api.lux.network/v1/sentinel/issues/{id}` — Returns one grouped issue of the caller's org with its latest occurrence sample.
- `GET https://api.lux.network/v1/sentinel/issues/{id}/events` — Lists one issue's captured occurrences, scoped to a project — a project is an isolation unit, so the caller declares which project's occurrences to read.
...
```bash
curl -sS "https://api.lux.network/v1/sentinel/issues" \

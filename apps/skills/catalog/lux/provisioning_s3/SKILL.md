...
- `GET https://api.lux.network/v1/provisioning/s3` — Lists the caller org's object-storage buckets.
- `GET https://api.lux.network/v1/provisioning/s3/{name}` — Returns one bucket's metadata.
...
```bash
curl -sS "https://api.lux.network/v1/provisioning/s3" \

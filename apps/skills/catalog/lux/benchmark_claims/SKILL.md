...
version: "8.0.0"
description: "Read benchmark claims: Lists the effective published claims: what the leaderboard will use for each (benchmark, model) after the seed, the import and any stored correction are layered.."
---

# Lux · BENCHMARK · claims
...
- `GET https://api.lux.network/v1/benchmark/claims` — Lists the effective published claims: what the leaderboard will use for each (benchmark, model) after the seed, the import and any stored correction are layered.
...
- `/v1/benchmark/claims` → `claimsOut` object with fields: `data`, `total`.

## Example

```bash
curl -sS "https://api.lux.network/v1/benchmark/claims" \
...
- You need a different `benchmark` capability — that product's skills are listed at `https://api.lux.network/.well-known/agent-skills/_benchmark/index.json`.

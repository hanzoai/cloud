...
name: pricing_tariff
version: "8.0.0"
description: "Read pricing tariff: Returns the platform's rate card: what a bill is made of, what each part costs, and the completion windows a request may ask for.."
---
...
- `GET https://api.lux.network/v1/pricing/tariff` — Returns the platform's rate card: what a bill is made of, what each part costs, and the completion windows a request may ask for.

## Response

- `/v1/pricing/tariff` → `Tariff` object with fields: `components`, `rates`, `unit`, `windows`.

## Example

```bash
curl -sS "https://api.lux.network/v1/pricing/tariff" \

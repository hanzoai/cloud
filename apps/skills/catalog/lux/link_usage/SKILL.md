...
name: link_usage
version: "8.0.0"
description: "Read link usage: Shows one provider account's own usage dashboard., Breaks down what the gateway routed through each of your accounts., Shows plan consumption and Lux spend side by side.."
---

# Lux · LINK · usage

Read-only Lux capability derived from the `link` OpenAPI product. Base URL `https://api.lux.network`.
...
- `GET https://api.lux.network/v1/link/usage` — Shows one provider account's own usage dashboard.
- `GET https://api.lux.network/v1/link/usage/accounts` — Breaks down what the gateway routed through each of your accounts.
- `GET https://api.lux.network/v1/link/usage/summary` — Shows plan consumption and Lux spend side by side.
...
```bash
curl -sS "https://api.lux.network/v1/link/usage" \
...
- You need a different `link` capability — that product's skills are listed at `https://api.lux.network/.well-known/agent-skills/_link/index.json`.

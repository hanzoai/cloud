...
name: admin_waitlist
version: "8.0.0"
description: "Read admin waitlist: Reads one waitlist's leaderboard from the Lux waitlist engine — position, points and referral standing per entry — proxied server-authed with the engine secret, never a client credential.."
---
...
- `GET https://api.lux.network/v1/admin/waitlist` — Reads one waitlist's leaderboard from the Lux waitlist engine — position, points and referral standing per entry — proxied server-authed with the engine secret, never a client credential.
...
```bash
curl -sS "https://api.lux.network/v1/admin/waitlist" \

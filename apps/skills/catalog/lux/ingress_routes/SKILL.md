...
- `GET https://api.lux.network/v1/ingress/routes` — Returns every routing rule the caller's org has configured, ordered by id.
- `GET https://api.lux.network/v1/ingress/routes/{id}` — Returns one of the caller org's routing rules by id.
...
```bash
curl -sS "https://api.lux.network/v1/ingress/routes" \

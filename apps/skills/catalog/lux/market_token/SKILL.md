...
- `GET https://api.lux.network/v1/market/token` — Answers one token's daily history — open, high, low, close, price and volume per UTC day, oldest first.
...
```bash
curl -sS "https://api.lux.network/v1/market/token"
```

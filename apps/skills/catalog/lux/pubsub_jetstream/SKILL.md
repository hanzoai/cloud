...
# Lux · PUBSUB · jetstream

Read-only Lux capability derived from the `pubsub` OpenAPI service. Base URL `https://api.lux.network`.
...
- `GET https://api.lux.network/v1/pubsub/jetstream/streams` — Returns the org's streams, sorted by name.
- `GET https://api.lux.network/v1/pubsub/jetstream/streams/{stream}` — Returns one stream of the caller's org — its configuration and its live state (messages, bytes, sequence range, consumer count).
- `GET https://api.lux.network/v1/pubsub/jetstream/streams/{stream}/consumers` — Returns one stream's consumers, sorted by name.
- `GET https://api.lux.network/v1/pubsub/jetstream/streams/{stream}/consumers/{name}` — Returns one consumer of one org stream — its configuration and its cursor: delivered and acknowledged sequences, pending and redelivered counts.
...
```bash
curl -sS "https://api.lux.network/v1/pubsub/jetstream/streams" \

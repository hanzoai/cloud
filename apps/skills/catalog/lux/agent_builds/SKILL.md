...
- `GET https://api.lux.network/v1/agent/builds` — Returns the public index of every published build, most recently updated first, so a gallery can link straight to the story behind each product.
...
```bash
curl -sS "https://api.lux.network/v1/agent/builds" \

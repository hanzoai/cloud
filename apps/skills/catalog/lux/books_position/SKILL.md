...
- `GET https://api.lux.network/v1/books/position` — Returns the org's Balance Sheet as of `to` (empty = all time), with the Assets == Liabilities + Equity equation proof.
...
```bash
curl -sS "https://api.lux.network/v1/books/position"
```

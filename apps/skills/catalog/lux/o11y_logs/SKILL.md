...
- `GET https://api.lux.network/v1/o11y/logs` — Returns the most recent log records in the query window, newest first — each record an open object carrying its nanosecond timestamp and whatever fields the record was ingested with.
- `GET https://api.lux.network/v1/o11y/logs/aggregate` — Returns the logs aggregate buckets for the query window.
- `GET https://api.lux.network/v1/o11y/logs/fields` — Returns the log field catalog: the fields already selected as indexed columns, and the interesting ones seen in the data that could be.
- `GET https://api.lux.network/v1/o11y/logs/livetail` — Follow log records as they arrive
- `GET https://api.lux.network/v1/o11y/logs/pipelines/{version}` — Returns the caller's org's log parsing pipelines at one config version — "latest" for the newest — along with that version's deployment record and the recent version history.
- `GET https://api.lux.network/v1/o11y/logs/promote_paths` — Lists the log body paths already promoted or indexed, with the indexes each carries.
...
```bash
curl -sS "https://api.lux.network/v1/o11y/logs" \

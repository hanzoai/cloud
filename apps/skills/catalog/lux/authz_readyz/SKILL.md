...
- `GET https://api.lux.network/v1/authz/readyz` — Readiness of the policy engine
...
```bash
curl -sS "https://api.lux.network/v1/authz/readyz" \
...
- You need a different `authz` capability — that product's skills are listed at `https://api.lux.network/.well-known/agent-skills/_authz/index.json`.

...
- `GET https://api.lux.network/v1/platform/sites` — Returns every project your org owns.
- `GET https://api.lux.network/v1/platform/sites/{slug}` — Returns one project of yours by slug — its settings, its live URL and the deployment currently serving it.
- `GET https://api.lux.network/v1/platform/sites/{slug}/deployments` — Returns a project's deploy history, newest version first.
- `GET https://api.lux.network/v1/platform/sites/{slug}/deployments/{id}` — Returns one deployment of a project by id.
- `GET https://api.lux.network/v1/platform/sites/{slug}/domains` — Returns every custom hostname this site holds: the live ones, plus any pending claim with the DNS records it still owes.
- `GET https://api.lux.network/v1/platform/sites/{slug}/releases` — Returns a site's releases newest-first, marking the active one — the rollback menu.
...
```bash
curl -sS "https://api.lux.network/v1/platform/sites"
```

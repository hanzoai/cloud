...
- `GET https://api.lux.network/v1/o11y/users` — Lists the caller's org members.
- `GET https://api.lux.network/v1/o11y/users/me` — Returns the calling user together with every role they hold.
- `GET https://api.lux.network/v1/o11y/users/me/dashboards` — Is dashboardListV2 personalized for the calling user: each dashboard carries the caller's pinned state, and pinned dashboards float to the top of the requested ordering.
- `GET https://api.lux.network/v1/o11y/users/{id}` — Returns one org member together with every role they hold, by user id.
- `GET https://api.lux.network/v1/o11y/users/{id}/reset_password_tokens` — Returns the reset-password token a user already has; absent one, the answer is a not-found rather than a fresh token.
- `GET https://api.lux.network/v1/o11y/users/{id}/roles` — Returns every role one org member holds, by user id.
...
```bash
curl -sS "https://api.lux.network/v1/o11y/users" \

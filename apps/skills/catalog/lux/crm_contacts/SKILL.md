...
- `GET https://api.lux.network/v1/crm/contacts` — Returns the caller org's contacts, most recently updated first.
- `GET https://api.lux.network/v1/crm/contacts/{id}` — Returns one of the caller org's contacts.
...
```bash
curl -sS "https://api.lux.network/v1/crm/contacts"
```

...
version: "8.0.0"
description: "Read provisioning datastore: Lists the caller org's Lux Datastore warehouses., Returns one Lux Datastore warehouse's metadata.."
---

# Lux · PROVISIONING · datastore
...
- `GET https://api.lux.network/v1/provisioning/datastore` — Lists the caller org's Lux Datastore warehouses.
- `GET https://api.lux.network/v1/provisioning/datastore/{name}` — Returns one Lux Datastore warehouse's metadata.
...
```bash
curl -sS "https://api.lux.network/v1/provisioning/datastore" \

...
- `GET https://api.lux.network/v1/prompts/metrics` — Metrics returns real per-prompt statistics for the caller's org: how many versions each prompt has, which one is current, and when it was created and last changed.
...
```bash
curl -sS "https://api.lux.network/v1/prompts/metrics" \

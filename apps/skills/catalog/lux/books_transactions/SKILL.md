...
- `GET https://api.lux.network/v1/books/transactions` — Returns the org's booked ledger as a single-line register, newest first: one row per voucher, with its date, description, vendor, category, source and amount in exact cents.
...
```bash
curl -sS "https://api.lux.network/v1/books/transactions" \

...
Read-only Lux capability derived from the `translate` OpenAPI product. Base URL `https://api.lux.network`.
...
- You need a different `translate` capability — that product's skills are listed at `https://api.lux.network/.well-known/agent-skills/_translate/index.json`.

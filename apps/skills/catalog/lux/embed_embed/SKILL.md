...
Read-only Lux capability derived from the `embed` OpenAPI service. Base URL `https://api.lux.network`.
...
- `GET https://api.lux.network/v1/embed` — Reports whether one of this brand's shared embedded apps (cms, erp, help) may be framed by the caller and is actually running, so a console module can choose between the embed and the provision panel.
...
```bash
curl -sS "https://api.lux.network/v1/embed" \

...
- `GET https://api.lux.network/v1/channels` — Reports every chat channel this org can send through, and whether it can send through it right now.

## Response

- `/v1/channels` → `chatChannels` object with fields: `channels`.

## Example

```bash
curl -sS "https://api.lux.network/v1/channels" \

...
name: market_pools
version: "8.0.0"
description: "Read market pools: Answers the automated market makers on one chain: their two tokens, their fee tier, and what has moved through each.."
---
...
- `GET https://api.lux.network/v1/market/pools` — Answers the automated market makers on one chain: their two tokens, their fee tier, and what has moved through each.
...
- `/v1/market/pools` → `Pools` object with fields: `chain`, `pools`, `reach`.

## Example

```bash
curl -sS "https://api.lux.network/v1/market/pools"
```

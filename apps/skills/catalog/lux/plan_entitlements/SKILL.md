...
- `GET https://api.lux.network/v1/plan/entitlements/{id}` — Returns what one plan GRANTS and not what it costs: the canonical namespaced entitlement block and the flat license-feature list derived from it.
...
```bash
curl -sS "https://api.lux.network/v1/plan/entitlements/{id}" \

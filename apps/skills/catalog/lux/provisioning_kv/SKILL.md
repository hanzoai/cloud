...
version: "8.0.0"
description: "Read provisioning kv: ListKV lists the caller org's Lux KV stores., GetKV returns one Lux KV store's metadata.."
---
...
- `GET https://api.lux.network/v1/provisioning/kv` — ListKV lists the caller org's Lux KV stores.
- `GET https://api.lux.network/v1/provisioning/kv/{name}` — GetKV returns one Lux KV store's metadata.
...
```bash
curl -sS "https://api.lux.network/v1/provisioning/kv" \

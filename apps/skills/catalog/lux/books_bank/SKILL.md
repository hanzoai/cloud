...
- `GET https://api.lux.network/v1/books/bank/transactions` — Returns the org's normalized bank transactions, newest first — every row the import and connector paths have ingested, with its amount in exact cents, its direction, and whether it has been matched to a voucher yet.
- `GET https://api.lux.network/v1/books/bank/unreconciled` — Returns the org's unmatched bank inflows and their open clarifying questions — the queue a human answers so an unexplained deposit is never guessed into revenue.
...
```bash
curl -sS "https://api.lux.network/v1/books/bank/transactions" \

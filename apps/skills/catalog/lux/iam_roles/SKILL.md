...
- `GET https://api.lux.network/v1/iam/roles` — Returns your organization's roles, newest first — each a named group of people that permissions are granted to.
- `GET https://api.lux.network/v1/iam/roles/{owner}/{name}` — Returns one role: who is in it, and the roles it includes.
...
- `/v1/iam/roles/{owner}/{name}` → `iam.Role` object with fields: `createdAt`, `createdTime`, `deleted`, `description`, `displayName`, `domains`, `id`, `isEnabled`, `name`, `owner`, `roles`, `teams`.

## Example

```bash
curl -sS "https://api.lux.network/v1/iam/roles" \

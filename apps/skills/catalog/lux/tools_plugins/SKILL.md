...
- `GET https://api.lux.network/v1/tools/plugins` — Reports what this deployment actually mounted: every subsystem the composition root declared and whether it is switched on.
- `GET https://api.lux.network/v1/tools/plugins/authored` — Lists the plugins the caller's org BUILT, newest first, each with the TypeScript as authored.
...
```bash
curl -sS "https://api.lux.network/v1/tools/plugins" \

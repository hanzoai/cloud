...
- `GET https://api.lux.network/v1/framework/{doctype}` — Returns the caller org's documents of one DocType, filtered, ordered and projected by the query.
- `GET https://api.lux.network/v1/framework/{doctype}/{name}` — Returns one document by name, with Password fields redacted.
...
```bash
curl -sS "https://api.lux.network/v1/framework/{doctype}" \

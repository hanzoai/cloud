...
func init() {
	...
	zip.Describe("GET /v1/sync", zip.Doc{
		...
		Fields: map[string]string{
			...
			"endpointView.locator":   "Locator addresses the thing INSIDE that provider, in the provider's own terms —\nan https clone URL for a hosted forge, a bare repository name for hanzo-git. It\nnever carries a credential.",
			"endpointView.provider":  "Provider is the concrete integration: \"github\", \"gitlab\" or \"hanzo-git\".",
			"syncList.data":          "Data is the org's syncs, each with its endpoints, policy and last-synced time.",
			"syncView.actor":         "Actor is the identity a reconcile writes AS. It is the loop guard: a change\nthis identity made is one we already have, so it is not synced back.",
			"syncView.createdAt":     "CreatedAt is when the link was first declared, RFC3339 in UTC.",
			"syncView.direction":     "Direction is which way work flows: \"both\", \"pull\" (target ← source), \"push\"\n(source → target), or \"off\" — which keeps the link declared and moves nothing.",
			"syncView.id":            "ID is the link's handle, derived from its source and target — which is what\nmakes re-declaring the same pair an update rather than a duplicate.",
			"syncView.kind":          "Kind is what is being synced. \"git\" today; the field exists so a storage or\ndatabase link is a value here rather than a second route family.",
			"syncView.source":        "Source is the side read FROM on a pull.",
			"syncView.target":        "Target is the side written TO on a push.",
			"syncView.trigger":       "Trigger is what starts a reconcile: \"webhook\" (the provider tells us),\n\"poll\" (we ask on a schedule), or \"manual\" (only an explicit call).",

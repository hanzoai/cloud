...
// adapter plugin/campaigns/seams.go registers (campaigns.Plan → ads.PaidPlan → LaunchPaid)
// driven through the campaigns.Channel interface, so the whole chain
...
func paidChannelForTest() campaigns.Channel {
	return campaigns.NewChannel(campaigns.KindPaid,
		func(ctx context.Context, org string, p campaigns.Plan) (campaigns.Ref, error) {
			...
			return campaigns.Ref{Platform: r.Platform, Account: r.Account, ExternalID: r.ExternalID, Status: r.Status, Detail: r.Detail}, err
		},
		func(ctx context.Context, org string, ref campaigns.Ref) (int64, error) {
			return PaidSpend(ctx, org, PaidRef{Platform: ref.Platform, Account: ref.Account, ExternalID: ref.ExternalID})
		},
		func(ctx context.Context, org string, ref campaigns.Ref) error {
...
func TestCampaignPaidChannel_EndToEnd(t *testing.T) {
	...
	if paid.Kind() != campaigns.KindPaid {
		t.Fatalf("channel kind want paid, got %q", paid.Kind())
	}

	ref, err := paid.Launch(context.Background(), "acme", campaigns.Plan{
...
func TestCampaignPaidChannel_ConnectorDisabledBlocksSend(t *testing.T) {
	...
	paid := paidChannelForTest()
	_, err := paid.Launch(context.Background(), "acme", campaigns.Plan{

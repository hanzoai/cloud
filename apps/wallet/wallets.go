...
//	POST /v1/wallet            {accountId,name,custody,tier,chain}  -> create wallet (Provision)
//	GET  /v1/wallet                                                -> list MY wallets
//	GET  /v1/wallet/:id                                            -> get one (404 if not my org)
//	POST /v1/wallet/:id/keys                                       -> rotate key material
//	POST /v1/wallet/:id/sign   {message?|digest?}                  -> sign (digest=hex 32B, else Keccak256(message))
//
...
func Use(app cloud.Router, deps cloud.Deps) error {
	...
	s := &cloud.Service[state]{Base: cloud.NewBase(deps, "wallet"), State: state{
		store:          st,
		custody:        custody,
		defaultCustody: def,
		audit:          deps.Audit,
...
//
// Static /v1/wallet/accounts routes register BEFORE the /v1/wallet/:id param
// route so the static segment wins. The collection root is declared on the /v1
// PARENT with a non-empty leaf — `zip.Post(g, "", …)` would name /v1/wallet/, a
...
func routes(app cloud.Router, s *cloud.Service[state]) error {
	...
	zip.Post(v1, "/wallet", o.createWallet)
	zip.Get(v1, "/wallet", o.listWallets)

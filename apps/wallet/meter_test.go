...
func billed(t *testing.T, l *planetest.Ledger, kind Kind) (*zip.App, *ring) {
	...
	s.Bill = cloud.NewMeter(cloud.Deps{Metering: l.Client(t)}, "wallets")
	return app, r
}
...
func create(t *testing.T, app *zip.App, org string) (int, []byte) {
	...
	return req(t, app, http.MethodPost, "/v1/wallet", org, map[string]any{
...
func TestRingSignBillsSeparately(t *testing.T) {
	...
	if code, b := req(t, app, http.MethodPost, "/v1/wallet/"+w.ID+"/sign", "acme",

...
func mkAccount(t *testing.T, app *zip.App, org string) string {
	t.Helper()
	code, body := req(t, app, http.MethodPost, "/v1/wallet/accounts", org, map[string]any{"name": "ops"})
...
func mkWallet(t *testing.T, app *zip.App, org, acctID, custody, tier, chain string) Wallet {
	t.Helper()
	code, body := req(t, app, http.MethodPost, "/v1/wallet", org, map[string]any{
...
func signOnce(t *testing.T, app *zip.App, org, walletID string, digest []byte) []byte {
	t.Helper()
	code, body := req(t, app, http.MethodPost, "/v1/wallet/"+walletID+"/sign", org,
...
func TestKMSSingleSigEndToEnd(t *testing.T) {
	...
	code, body := req(t, app, http.MethodPost, "/v1/wallet/"+w.ID+"/keys", "acme", nil)
	if code != http.StatusOK {
...
func TestPerTenantIsolation(t *testing.T) {
	...
	if code, _ := req(t, app, http.MethodGet, "/v1/wallet/"+wa.ID, "orgb", nil); code != http.StatusNotFound {
		t.Fatalf("cross-tenant GET = %d, want 404", code)
	}
	// Org B cannot SIGN with org A's wallet.
	if code, _ := req(t, app, http.MethodPost, "/v1/wallet/"+wa.ID+"/sign", "orgb",
	...
	_, body := req(t, app, http.MethodGet, "/v1/wallet", "orgb", nil)
	var list struct {
	...
	for _, tc := range []struct{ method, path string }{
		...
		{http.MethodGet, "/v1/wallet/accounts"},
		{http.MethodPost, "/v1/wallet/accounts"},
		{http.MethodGet, "/v1/wallet/" + wa.ID},
		{http.MethodPost, "/v1/wallet/" + wa.ID + "/sign"},
	} {

...
func init() {
	...
	zip.Describe("GET /v1/links/usage", zip.Doc{
		...
		Fields: map[string]string{
			...
			"dashResp.account":             "Account is the account the series narrows to, when one was named.",
			"dashResp.available":           "Available reports whether the warehouse answered; false is an honest \"we\nhave no data\", NOT zero usage.",
			"dashResp.current":             "Current is the live state of each lane — the dash headline.",
			"dashResp.from":                "From and To are the resolved [from, to) window, RFC 3339 UTC.",
			"dashResp.provider":            "Provider is the provider whose meter answered.",
			"dashResp.range":               "Range is the resolved period label.",
			"dashResp.scope":               "Scope is always \"user\": the caller's own linked accounts.",
			"dashResp.source":              "Source is always \"account\": the provider's own meter, not a Hanzo charge.",
			"dashResp.windows":             "Windows is every window instance in range, newest first.",
			"sampleView.account":           "Account is the provider-side account the sample belongs to.",
			"sampleView.cachedInputTokens": "CachedInputTokens is the window's cached-prompt-token count.",
			"sampleView.confidence":        "Confidence says whether the counters that remain mean anything, as the\nmeter graded itself.",
			"sampleView.costCents":         "CostCents is the window's spend in cents, as the provider's meter states it.",
			"sampleView.costLimitCents":    "CostLimitCents is the window's spend cap in cents, when the meter knows one.",
			"sampleView.currency":          "Currency is the ISO currency the cost fields are stated in.",
			"sampleView.inputTokens":       "InputTokens is the window's prompt-token count.",
			"sampleView.lane":              "Lane names the meter's own lane label for this measurement.",
			"sampleView.machine":           "Machine is the machine the collector observed the account on.",
			"sampleView.outputTokens":      "OutputTokens is the window's completion-token count.",
			"sampleView.plan":              "Plan is the provider plan label the account is on.",
			"sampleView.requests":          "Requests is the window's request count.",
			"sampleView.resetsAt":          "ResetsAt is when the window resets, RFC 3339 UTC.",
			"sampleView.synthetic":         "Synthetic marks a sample the collector derived rather than observed.",
			"sampleView.totalTokens":       "TotalTokens is the window's total token count.",
			"sampleView.usedPct":           "UsedPct is how much of the window's allowance is consumed, 0..100.",
			"sampleView.window":            "Window is the window class: 6h, day, week or month.",
			"sampleView.windowMinutes":     "WindowMinutes is the window's length as the meter reported it.",
			"sampleView.windowStart":       "WindowStart is when the measured window opened, RFC 3339 UTC.",
		},
	})
	...
	zip.Describe("GET /v1/links/usage/summary", zip.Doc{
		...
		Fields: map[string]string{
			...
			"summaryResp.account":   "Account and Hanzo report each ledger's own availability, so a partial\nwarehouse never fabricates the other half.",
			"summaryResp.from":      "From and To are the one [from, to) window BOTH halves resolved, RFC 3339 UTC.",
	...
	zip.Describe("POST /v1/links", zip.Doc{
		...
		Fields: map[string]string{
			...
			"linkView.usage":       "Usage is the last good usage snapshot, clamped and re-serialized to known\nfields at ingest.",
			"linkView.user":        "User is the owning subject — the validated caller who registered the link.",
			"registerReq.account":  "Account is the provider-side account identifier.",
			"registerReq.host":     "Host is the machine's human hostname label.",
			"registerReq.kind":     "Kind decides how the account's inference BILLS and defaults to\nsubscription: a subscription account bills the user's own monthly plan and\nis metered here for visibility only, while an apikey account bills through\ncommerce on the gateway path.",
			"registerReq.machine":  "Machine is the stable machine identifier. Required, length-bounded.",
			"registerReq.os":       "OS is the machine's operating system label.",
			"registerReq.plan":     "Plan is the provider plan label (e.g. \"Claude Max\").",
			"registerReq.provider": "Provider is the AI provider the account belongs to. Required, length-bounded.",
			"registerReq.usage":    "Usage is an optional usage snapshot, clamped and re-serialized to known\nfields; omitting it keeps the last good one.",
		},
	})
	...
	zip.Describe("POST /v1/links/usage", zip.Doc{
		...
		Fields: map[string]string{
			...
			"linkView.usage":              "Usage is the last good usage snapshot, clamped and re-serialized to known\nfields at ingest.",
			"linkView.user":               "User is the owning subject — the validated caller who registered the link.",
			"reportReq.samples":           "Samples is the batch form, up to 256 samples; leave it empty to send one\nsample inline on the same fields.",
			"reportResp.accepted":         "Accepted is how many samples this report landed.",
			"reportResp.links":            "Links is the link row each distinct (machine, provider, account) in the\nbatch refreshed.",
			"reportResp.stored":           "Stored reports whether history was durably written; false means the\nwarehouse was unavailable and only the link rows were refreshed.",
			"sampleReq.account":           "Account is the provider-side account the sample belongs to.",
			"sampleReq.cachedInputTokens": "CachedInputTokens is the window's cached-prompt-token count.",
			"sampleReq.confidence":        "Confidence says how real the counters are, as the meter graded itself.",
			"sampleReq.costCents":         "CostCents is the window's spend in cents, as the provider's meter states it.",
			"sampleReq.costLimitCents":    "CostLimitCents is the window's spend cap in cents, when the meter knows one.",
			"sampleReq.currency":          "Currency is the ISO currency the cost fields are stated in.",
			"sampleReq.inputTokens":       "InputTokens is the window's prompt-token count.",
			"sampleReq.kind":              "Kind is subscription or apikey; anything else is refused.",
			"sampleReq.lane":              "Lane names the meter's own lane label for this measurement.",
			"sampleReq.machine":           "Machine is the machine the collector observed the account on. Required.",
			"sampleReq.outputTokens":      "OutputTokens is the window's completion-token count.",
			"sampleReq.plan":              "Plan is the provider plan label the account is on.",
			"sampleReq.provider":          "Provider is the AI provider whose meter reported this sample. Required.",
			"sampleReq.requests":          "Requests is the window's request count.",
			"sampleReq.resetsAt":          "ResetsAt is when the window resets, RFC 3339, bounded.",
			"sampleReq.synthetic":         "Synthetic marks a sample the collector derived rather than observed.",
			"sampleReq.totalTokens":       "TotalTokens is the window's total token count.",
			"sampleReq.usedPct":           "UsedPct is how much of the window's allowance is consumed, clamped 0..100.",
			"sampleReq.window":            "Window is the window class, one of 6h, day, week, month; anything else is\nrefused rather than silently reclassified.",
			"sampleReq.windowMinutes":     "WindowMinutes is the window's length as the meter reported it.",
			"sampleReq.windowStart":       "WindowStart is when the measured window opened, RFC 3339, bounded to a\nsane interval around now.",
		},
	})
}

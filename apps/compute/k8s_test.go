...
func mountK8s(t *testing.T, f *k8sFake) *zip.App {
	...
	t.Setenv("COMPUTE_CLIENT_ID", "")     // force the bearer-forward path (fake ignores auth)
...
// GET /v1/compute/k8s/clusters is org-scoped: cloud forwards the validated org, and maps
// Visor clusters to the managed clusterView. No org → 403 (never reaches Visor).
func TestK8sClustersListTenantScoped(t *testing.T) {
	f := &k8sFake{}
	app := mountK8s(t, f)

	if code, _ := reqK8s(t, app, http.MethodGet, "/v1/compute/k8s/clusters", "", false, nil); code != http.StatusForbidden {
		t.Fatalf("no-org list want 403, got %d", code)
	}

	code, body := reqK8s(t, app, http.MethodGet, "/v1/compute/k8s/clusters", "acme", false, nil)
...
// GET /v1/compute/k8s/clusters/:id returns detail: node pools (poolId mapped from Visor's
// id) with the derived node count, and the worker nodes as machineViews.
func TestK8sClusterDetail(t *testing.T) {
	f := &k8sFake{}
	app := mountK8s(t, f)

	code, body := reqK8s(t, app, http.MethodGet, "/v1/compute/k8s/clusters/cl-1", "acme", false, nil)
	if code != http.StatusOK {
...
// GET /v1/compute/k8s/nodes returns every worker node as a machineView, org-scoped.
func TestK8sNodesTenantScoped(t *testing.T) {
	f := &k8sFake{}
	app := mountK8s(t, f)

	if code, _ := reqK8s(t, app, http.MethodGet, "/v1/compute/k8s/nodes", "", false, nil); code != http.StatusForbidden {
		t.Fatalf("no-org nodes want 403, got %d", code)
	}
	code, body := reqK8s(t, app, http.MethodGet, "/v1/compute/k8s/nodes", "acme", false, nil)
	if code != http.StatusOK {
...
// POST /v1/compute/k8s/clusters is ADMIN-GATED: a non-admin is refused BEFORE the request
...
func TestK8sCreateClusterAdminGated(t *testing.T) {
	...
	if code, _ := reqK8s(t, app, http.MethodPost, "/v1/compute/k8s/clusters", "", false, valid); code != http.StatusForbidden {
		t.Fatalf("no-org create want 403, got %d", code)
	}
	// Validated but NON-admin → 403, and the mutation never reached Visor.
	if code, _ := reqK8s(t, app, http.MethodPost, "/v1/compute/k8s/clusters", "acme", false, valid); code != http.StatusForbidden {
	...
	code, body := reqK8s(t, app, http.MethodPost, "/v1/compute/k8s/clusters", "acme", true, valid)
	...
	if code, _ := reqK8s(t, app, http.MethodPost, "/v1/compute/k8s/clusters", "acme", true,
		map[string]any{"name": "x", "region": "sfo3"}); code != http.StatusBadRequest {
		t.Fatalf("admin create without pool size want 400, got %d", code)
	}
}

// DELETE /v1/compute/k8s/clusters/:id is ADMIN-GATED, like create.
func TestK8sDeleteClusterAdminGated(t *testing.T) {
	f := &k8sFake{}
	app := mountK8s(t, f)

	if code, _ := reqK8s(t, app, http.MethodDelete, "/v1/compute/k8s/clusters/cl-1", "", false, nil); code != http.StatusForbidden {
		t.Fatalf("no-org delete want 403, got %d", code)
	}
	if code, _ := reqK8s(t, app, http.MethodDelete, "/v1/compute/k8s/clusters/cl-1", "acme", false, nil); code != http.StatusForbidden {
	...
	if code, _ := reqK8s(t, app, http.MethodDelete, "/v1/compute/k8s/clusters/cl-1", "acme", true, nil); code != http.StatusNoContent {

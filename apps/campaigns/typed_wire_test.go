...
func TestTypedOpsPreserveTheCampaignWire(t *testing.T) {
	...
	code, raw := doJSON(t, app, http.MethodPost, "/v1/campaigns", org, map[string]any{
	...
	t.Run("the list keeps its {data: …} envelope and binds ?status/?limit", func(t *testing.T) {
		code, raw := send(t, app, http.MethodGet, "/v1/campaigns", org, "", "")
		...
		code, raw = send(t, app, http.MethodGet, "/v1/campaigns?status=live", org, "", "")
		if code != http.StatusOK {
		...
		if code, _ := send(t, app, http.MethodGet, "/v1/campaigns?limit=abc", org, "", ""); code != http.StatusOK {
	...
	t.Run("the three writes still REQUIRE a JSON body", func(t *testing.T) {
		...
		for _, tc := range []struct{ name, method, path, ctype, body string }{
			{"create, no body", http.MethodPost, "/v1/campaigns", "", ""},
			{"create, json ctype, no body", http.MethodPost, "/v1/campaigns", "application/json", ""},
			{"update, no body", http.MethodPut, "/v1/campaigns/" + id, "", ""},
			{"update, text ctype", http.MethodPut, "/v1/campaigns/" + id, "text/plain", `{"name":"X"}`},
			{"addChannel, no body", http.MethodPost, "/v1/campaigns/" + id + "/channels", "", ""},
		} {
	...
	t.Run("metrics binds its window off the query string", func(t *testing.T) {
		code, raw := send(t, app, http.MethodGet, "/v1/campaigns/"+id+"/metrics?range=7d", org, "", "")
		if code != http.StatusOK {
		...
		_, raw = send(t, app, http.MethodGet, "/v1/campaigns/"+id+"/metrics?range=nope", org, "", "")
		_ = json.Unmarshal(raw, &m)
		if m["range"] != "30d" {
	...
	t.Run("removing a channel answers the campaign; deleting it answers 204", func(t *testing.T) {
		code, raw := send(t, app, http.MethodDelete, "/v1/campaigns/"+id+"/channels/paid", org, "", "")
		if code != http.StatusOK {
		...
		if code, _ := send(t, app, http.MethodDelete, "/v1/campaigns/"+id+"/channels/paid", org, "", ""); code != http.StatusNotFound {
			t.Errorf("removing it twice = %d, want 404", code)
		}

		code, raw = send(t, app, http.MethodDelete, "/v1/campaigns/"+id, org, "", "")
		...
		if code, _ := send(t, app, http.MethodDelete, "/v1/campaigns/"+id, org, "", ""); code != http.StatusNotFound {
...
func TestCampaignTenancyIsNeverACallerField(t *testing.T) {
	...
	code, raw := doJSON(t, app, http.MethodPost, "/v1/campaigns", "acme", map[string]any{"name": "Secret"})
	if code != http.StatusCreated {
	...
	for _, tc := range []struct{ method, path string }{
		{http.MethodGet, "/v1/campaigns/" + id},
		{http.MethodPost, "/v1/campaigns/" + id + "/launch"},
		{http.MethodPost, "/v1/campaigns/" + id + "/pause"},
		{http.MethodGet, "/v1/campaigns/" + id + "/metrics"},
		{http.MethodDelete, "/v1/campaigns/" + id + "/channels/paid"},
	} {
	...
	if code, _ := send(t, app, http.MethodDelete, "/v1/campaigns/"+id, "other", "", ""); code != http.StatusNotFound {
		t.Errorf("cross-org DELETE = %d, want 404", code)
	}

	for _, tc := range []struct{ method, path string }{
		{http.MethodGet, "/v1/campaigns"},
		{http.MethodGet, "/v1/campaigns/summary"},
		{http.MethodPost, "/v1/campaigns"},
		{http.MethodGet, "/v1/campaigns/" + id},
		{http.MethodPut, "/v1/campaigns/" + id},
		{http.MethodDelete, "/v1/campaigns/" + id},
		{http.MethodPost, "/v1/campaigns/" + id + "/launch"},
		{http.MethodPost, "/v1/campaigns/" + id + "/pause"},
		{http.MethodGet, "/v1/campaigns/" + id + "/metrics"},
		{http.MethodPost, "/v1/campaigns/" + id + "/channels"},
		{http.MethodDelete, "/v1/campaigns/" + id + "/channels/paid"},
	} {
...
func campaignRoutes(t *testing.T) (served map[string]bool, typed map[string]string) {
	...
	resetChannels()
	app := mountWire(t)
	doc, err := openapi.Spec(app, openapi.Info{Title: "campaigns", Version: "v1"})
	if err != nil {
		t.Fatalf("spec: %v", err)
	}
	...
	ours := func(p string) bool {
		return strings.HasPrefix(p, "/v1/campaigns") && p != "/v1/campaigns/health"
	}
...
func TestLaunchAndPauseIgnoreTheirBody(t *testing.T) {
	...
	for _, path := range []string{"/v1/campaigns/" + id + "/launch", "/v1/campaigns/" + id + "/pause"} {
...
var untypedByDesign = map[string]string{
	"POST /v1/campaigns/{id}/launch": "has never read a request body — the id comes from the URL and " +
	...
	"POST /v1/campaigns/{id}/pause": "same: a body-less POST that zip cannot declare as body-less.",
}
...
func TestEveryPublishedFieldIsDescribed(t *testing.T) {
	resetChannels()
	doc, err := openapi.Spec(mountWire(t), openapi.Info{Title: "campaigns", Version: "v1"})
	if err != nil {
		t.Fatalf("spec: %v", err)
	}

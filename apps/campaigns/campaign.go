...
// DO with it. /v1/campaigns is a CONSUMER of connectors — the role HIP-0126 gives
// Flows — never a second credential path. "campaigns" used to be braided across
...
//
//	GET    /v1/campaigns/summary               per-org roll-up + wired channels
//	GET    /v1/campaigns                        list campaigns (?status=)   -> {data:[…]}
//	POST   /v1/campaigns                        create a campaign (draft)    -> Campaign (201)
//	GET    /v1/campaigns/:id                    campaign detail              -> Campaign
//	PUT    /v1/campaigns/:id                    update a draft campaign      -> Campaign
//	DELETE /v1/campaigns/:id                    delete a campaign
//	POST   /v1/campaigns/:id/launch             fan out to channels          -> Campaign
//	POST   /v1/campaigns/:id/pause              pause every live channel     -> Campaign
//	GET    /v1/campaigns/:id/metrics            analytics results + spend    -> Metrics
//	POST   /v1/campaigns/:id/channels           add a channel                -> Campaign
//	DELETE /v1/campaigns/:id/channels/:kind     remove a channel             -> Campaign
//
// serve.go auto-registers GET /v1/campaigns/health (no OwnsHealth here).
package campaigns
...
func Mount(app cloud.Router, deps cloud.Deps) error {
	...
	b := cloud.NewBase(deps, "campaigns")
...
func routes(app cloud.Router, zapp *zip.App, s *cloud.Service[state]) {
	...
	// on the group with an empty leaf: joining "/v1/campaigns" with "" yields
	// "/v1/campaigns/", a DIFFERENT path from the one these two have always served.
	zip.Get(zapp, "/v1/campaigns", o.list)
	zip.Post(zapp, "/v1/campaigns", o.create, zip.WithStatus(http.StatusCreated))

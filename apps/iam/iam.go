...
//
// It closes the CONNECTION, not the orm.DB. The ORM borrows a handle it did not
// open, so its Close is a no-op and closing it would leave the file open with the
// process gone — which on a keyed store is how a WAL is left behind unmerged.
func Shutdown() error {
	conn := embeddedConn

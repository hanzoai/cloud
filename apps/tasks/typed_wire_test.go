...
// the exact route at /v1/tasks IS NEVER ENTERED. The greedy sibling at
...
//
// The redirect at /v1/tasks is net/http's subtree rule applied to a pattern, and
...
func TestTheEngineDoesNotServeTheBareNoun(t *testing.T) {
	...
	for _, m := range []string{http.MethodGet, http.MethodPost, http.MethodPut, http.MethodPatch, http.MethodDelete} {
		...
		if rec.Code != http.StatusNotFound {
			t.Errorf("engine HTTPHandler %s /v1/tasks = %d, want 404 — the engine now serves the "+
	...
	if rec.Code != http.StatusTemporaryRedirect {
		t.Errorf("cloud's own mux GET /v1/tasks = %d, want 307 — the pattern the redirect is "+
...
// remainder, so /v1/tasks/* covers /v1/tasks and WINS there in either
...
func TestTheGreedyRouteClaimsTheBareNoun(t *testing.T) {
	...
	for _, c := range []struct {
		...
		if got := who(c.build); got != "greedy" {
			t.Errorf("%s: GET /v1/tasks reached %q, want greedy — the exact route is reachable "+
				"now, so /v1/tasks can carry a handler of its own and the refusal in "+
	...
	if got := who(func(a *zip.App) {
		...
		t.Errorf("with a :param sibling GET /v1/tasks reached %q, want exact — the control for "+
			"this being about greediness rather than precedence has changed", got)
	}
}

// TestTheBareNounAnswersEveryByte pins the WHOLE answer at /v1/tasks on every
...
func TestTheBareNounAnswersEveryByte(t *testing.T) {
	...
	for _, c := range []struct{ method, ct, body string }{
		...
		if resp.StatusCode != http.StatusTemporaryRedirect {
			t.Errorf("%s /v1/tasks = %d, want 307", c.method, resp.StatusCode)
		}
		if loc := resp.Header.Get("Location"); loc != subtree {
			t.Errorf("%s /v1/tasks Location = %q, want %q", c.method, loc, subtree)
		}
		if ct := resp.Header.Get("Content-Type"); ct != c.ct {
			t.Errorf("%s /v1/tasks Content-Type = %q, want %q", c.method, ct, c.ct)
		}
		if string(got) != c.body {
			t.Errorf("%s /v1/tasks body = %q, want %q", c.method, got, c.body)
		}
	}
}
...
func TestTheBareNounFailsSoftWithNoEngine(t *testing.T) {
	...
	if resp.StatusCode != http.StatusServiceUnavailable {
		t.Fatalf("engine-nil GET /v1/tasks = %d, want 503", resp.StatusCode)
	}

...
//
// The Hanzo starter-kit gallery at /v1/template, in TWO layers that never mix:
//
...
//
//	GET    /v1/template          public catalog + (validated caller) that org's own -> {data:[StarterKit]}
//	GET    /v1/template/:slug    one kit: the caller org's own, else public         -> StarterKit
//	POST   /v1/template          publish a kit PRIVATE to the caller's org          -> 201 StarterKit
//	PUT    /v1/template/:slug    replace the caller org's own kit                   -> StarterKit
//	DELETE /v1/template/:slug    delete the caller org's own kit                    -> 204
//
...
// because the collection root IS /v1/template: declaring it as the empty leaf
// of a Group("/v1/template") names /v1/template/ — a path this API has never
...
func routes(app cloud.Router, zapp *zip.App, s *cloud.Service[state]) {
	...
	// could only hang on a /v1/template node, and every op below registers on
	// zapp, the root app, so that node would carry middleware over an empty subtree
	// and zip refuses to compose it.

	o := ops{s: s}
	zip.Get(zapp, "/v1/template", o.browse)
	zip.Post(zapp, "/v1/template", o.publish, zip.WithStatus(http.StatusCreated))
	zip.Get(zapp, "/v1/template/:slug", o.get)
	zip.Put(zapp, "/v1/template/:slug", o.replace)
	zip.Delete(zapp, "/v1/template/:slug", o.remove)
}

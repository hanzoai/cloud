...
func TestPrivateTemplateIsOrgOnly(t *testing.T) {
	...
	if code, body := do(t, app, http.MethodPost, "/v1/template", "acme", tpl); code != http.StatusCreated {
		t.Fatalf("publish want 201, got %d (%s)", code, body)
	}

	// Owner sees it, stamped with the SERVER's org.
	code, body := do(t, app, http.MethodGet, "/v1/template", "acme", nil)
	...
	if code, _ := do(t, app, http.MethodGet, "/v1/template/acme-internal", "acme", nil); code != http.StatusOK {
		t.Fatalf("acme get own want 200, got %d", code)
	}

	// Another org: not in the browse list, and not addressable by slug.
	code, body = do(t, app, http.MethodGet, "/v1/template", "globex", nil)
	if code != http.StatusOK {
	...
	if code, _ := do(t, app, http.MethodGet, "/v1/template/acme-internal", "globex", nil); code != http.StatusNotFound {
	...
	// and nothing else.
	code, body = do(t, app, http.MethodGet, "/v1/template", "", nil)
	if code != http.StatusOK {
	...
	if code, _ := do(t, app, http.MethodGet, "/v1/template/acme-internal", "", nil); code != http.StatusNotFound {
...
func TestWritesBindOrg(t *testing.T) {
	...
	code, body := do(t, app, http.MethodPost, "/v1/template", "acme", tpl)
	...
	if code, _ := do(t, app, http.MethodPost, "/v1/template", "", tpl); code != http.StatusForbidden {
		t.Fatalf("anonymous publish want 403, got %d", code)
	}
	if code, _ := do(t, app, http.MethodPut, "/v1/template/widget", "globex",
		StarterKit{Title: "Hijacked"}); code != http.StatusNotFound {
		t.Fatalf("cross-org edit want 404, got %d", code)
	}
	if code, _ := do(t, app, http.MethodDelete, "/v1/template/widget", "globex", nil); code != http.StatusNotFound {
		t.Fatalf("cross-org delete want 404, got %d", code)
	}
	// The owner's own edit + delete still work, and the row is really gone.
	if code, _ := do(t, app, http.MethodPut, "/v1/template/widget", "acme",
		StarterKit{Title: "Widget Kit v2"}); code != http.StatusOK {
		t.Fatalf("owner edit want 200, got %d", code)
	}
	if code, _ := do(t, app, http.MethodDelete, "/v1/template/widget", "acme", nil); code != http.StatusNoContent {
		t.Fatalf("owner delete want 204, got %d", code)
	}
	if code, _ := do(t, app, http.MethodGet, "/v1/template/widget", "acme", nil); code != http.StatusNotFound {
...
func TestSlugStaysSingleValued(t *testing.T) {
	...
	if code, _ := do(t, app, http.MethodPost, "/v1/template", "acme",
	...
	if code, _ := do(t, app, http.MethodPost, "/v1/template", "globex",
		StarterKit{Slug: "dup", Title: "Globex's own"}); code != http.StatusCreated {
		t.Fatalf("another org's same slug want 201, got %d", code)
	}
	if code, body := do(t, app, http.MethodGet, "/v1/template/dup", "globex", nil); code != http.StatusOK {

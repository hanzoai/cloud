...
// untypedByDesign is the CLOSED list of evals operations that are NOT typed ops.
...
// TestEveryRouteIsTypedOrNamed fails when an evals operation is neither a typed
// op nor named above — so the next route added here is typed by default.
func TestEveryRouteIsTypedOrNamed(t *testing.T) {
	served, typed, _ := opsUnderTest(t)
	if len(served) != 16 {
		t.Errorf("evals serves %d operations, expected 16 — update this gate deliberately", len(served))
	}
	...
	for key := range untypedByDesign {
		if !served[key] {
			t.Errorf("untypedByDesign names %q, which evals no longer serves", key)
		}
	}
}
...
func TestEveryTypedOpIsDescribed(t *testing.T) {
	...
	if len(typed) == 0 {
		t.Fatal("no typed evals ops in the registry at all")
	}
	for key, desc := range typed {
		if strings.TrimSpace(desc) == "" {
			t.Errorf("%s has no description — run: go generate -run zipdoc ./apps/eval/...", key)
		}
	}
}
...
func TestEveryPublishedFieldIsDescribed(t *testing.T) {
	...
	if len(schemas) == 0 {
		t.Fatal("no evals schemas in the typed registry at all")
	}
	var bare []string

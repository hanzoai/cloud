...
func TestNodesListIsOrgScoped(t *testing.T) {
	...
	for _, org := range []string{"acme", "globex"} {
		code, body := call(t, app, http.MethodGet, "/v1/node", org, nil)
	...
	code, body := call(t, app, http.MethodGet, "/v1/node", "initech", nil)
...
func TestNodeRoutesRefuseAnUnvalidatedCaller(t *testing.T) {
	...
	if code, _ := call(t, app, http.MethodPost, "/v1/node/n1/invoke", "", map[string]any{"command": "canvas.snapshot"}); code != http.StatusForbidden {
...
func TestInvokeReachesTheNodeAndPassesItsAnswerBack(t *testing.T) {
	...
	code, body := call(t, app, http.MethodPost, "/v1/node/laptop-1/invoke", "acme",
...
func TestForeignNodeAnswersLikeAnAbsentOne(t *testing.T) {
	...
	foreignCode, foreignBody := call(t, app, http.MethodPost, "/v1/node/laptop-1/invoke", "acme", snapshot)
	absentCode, absentBody := call(t, app, http.MethodPost, "/v1/node/no-such-machine/invoke", "acme", snapshot)
	...
	if code, _ := call(t, app, http.MethodGet, "/v1/node", "acme", nil); code != http.StatusOK {
...
func TestInvokeIsPolicyChecked(t *testing.T) {
	...
	code, body := call(t, app, http.MethodPost, "/v1/node/n1/invoke", "acme",
...
// The surface is /v1/node/*. Never /api/, and nothing outside this capability's
...
func TestRoutesLiveUnderTheirOwnNoun(t *testing.T) {
	...
	for _, r := range app.Fiber().GetRoutes() {
		...
		if !strings.HasPrefix(r.Path, "/v1/node") {
			continue
		}
		if r.Method == "HEAD" {

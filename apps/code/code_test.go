...
func TestPrincipalGateForbidden(t *testing.T) {
	...
	cases := []struct {
		...
		{http.MethodPost, "/v1/code/index", indexIn{Repo: "r", Files: []fileInput{{Path: "a.go", Content: "package a"}}}},
	}
	for _, c := range cases {
...
func TestIndexSearchContextAsk(t *testing.T) {
	...
	status, b := doAuth(t, app, http.MethodPost, "/v1/code/context", "acme",
		contextIn{Query: "how does Hello greet", BudgetTokens: 2000, Repo: "svc"})
...
func TestIncrementalAndPrune(t *testing.T) {
	...
	body := indexIn{Repo: "svc", Prune: true, Files: []fileInput{{Path: "greeter.go", Content: goFixture}}}

...
// automation_trigger.go is the outbound client from the inbound-event surfaces
...
// custody — a cycle), so the composition root (apps.wire_clients) injects
// automations.Deliver here as a primitive-typed func. Until it does — and whenever the
// automations subsystem is disabled — the client is nil and every fire is a safe no-op.
...
// fireAutomation is the wired client (nil until apps.wire_clients sets it).
var fireAutomation TriggerFunc

// SetAutomationTrigger wires the automations Deliver client. Called once at the
...
// a nil client (automations disabled) or an empty org is a no-op, and a dispatch error

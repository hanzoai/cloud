...
func TestMetaAuthorizeURL(t *testing.T) {
	raw, err := metaAuthorize(OAuthConfig{ClientID: "app-123"}, "https://api.hanzo.ai/v1/integration/meta_ads/callback", "st8")
	if err != nil {
...
func TestMetaExchangeSealsLongLivedToken(t *testing.T) {
	...
	res, err := metaExchange(context.Background(), OAuthConfig{ClientID: "app", ClientSecret: "secret"},
		"https://api.hanzo.ai/v1/integration/meta_ads/callback", "authcode")
	if err != nil {
...
func TestMetaStateCannotBeReplayedAtGoogleAds(t *testing.T) {
	...
	res := cfReq(t, app, http.MethodPost, "/v1/integration/meta_ads/connect", "acme", true, nil)
	...
	cb := req(t, app, http.MethodGet,
		"/v1/integration/google_ads/callback?state="+url.QueryEscape(metaState)+"&code=x", "", nil)
...
func TestMetaConnectRequiresOrgAdmin(t *testing.T) {
	...
	t.Setenv(metaAppSecretEnv, "app-secret")
	app := newApp(t, newKMS(t))
	res := cfReq(t, app, http.MethodPost, "/v1/integration/meta_ads/connect", "acme", false, nil)

...
func connectSlack(t *testing.T, app *zip.App, org, code string) httpResult {
	t.Helper()
	res := req(t, app, http.MethodPost, "/v1/integration/slack/connect", org, nil)
	...
	return req(t, app, http.MethodGet,
		"/v1/integration/slack/callback?code="+url.QueryEscape(code)+"&state="+url.QueryEscape(state), "", nil)
}
...
func TestIntegrationsListShape(t *testing.T) {
	...
	if r := req(t, app, http.MethodGet, "/v1/integration", "", nil); r.Code != http.StatusForbidden {
	...
	if r := req(t, app, http.MethodGet, "/v1/integration/nope", "acme", nil); r.Code != http.StatusNotFound {
...
func TestIntegrationsConnectUnconfigured503(t *testing.T) {
	...
	app := newApp(t, newKMS(t))
	if r := req(t, app, http.MethodPost, "/v1/integration/github/connect", "acme", nil); r.Code != http.StatusServiceUnavailable {
...
func TestIntegrationsConnectNoPrincipal403(t *testing.T) {
	slackConfiguredEnv(t)
	app := newApp(t, newKMS(t))
	if r := req(t, app, http.MethodPost, "/v1/integration/slack/connect", "", nil); r.Code != http.StatusForbidden {
...
func TestIntegrationsConnectKMSNotReady503(t *testing.T) {
	...
	app := newApp(t, nil)
	if r := req(t, app, http.MethodPost, "/v1/integration/slack/connect", "acme", nil); r.Code != http.StatusServiceUnavailable {
...
func TestIntegrationsCallbackReplayRejected(t *testing.T) {
	...
	res := req(t, app, http.MethodPost, "/v1/integration/slack/connect", "acme", nil)
	if res.Code != http.StatusOK {
	...
	state := u.Query().Get("state")
	cbPath := "/v1/integration/slack/callback?code=goodcode&state=" + url.QueryEscape(state)
...
func TestIntegrationsCallbackTamperedStateRejected(t *testing.T) {
	...
	r := req(t, app, http.MethodGet, "/v1/integration/slack/callback?code=goodcode&state=forged.deadbeef", "", nil)
	if r.Code != http.StatusFound || !strings.Contains(r.Location, "error=slack") {
...
func TestIntegrationsDisconnect(t *testing.T) {
	...
	r := req(t, app, http.MethodPost, "/v1/integration/slack/disconnect", "acme", nil)
	if r.Code != http.StatusOK {
	...
	// Idempotent.
	if r := req(t, app, http.MethodPost, "/v1/integration/slack/disconnect", "acme", nil); r.Code != http.StatusOK {
...
func TestIntegrationsOrgIsolation(t *testing.T) {
	...
	r := req(t, app, http.MethodGet, "/v1/integration", "globex", nil)
	var out struct {
		Providers []providerView `json:"providers"`
	}
	_ = json.Unmarshal(r.Body, &out)
...
func TestIntegrationsDisconnectNoPrincipal403(t *testing.T) {
	...
	rq := httptest.NewRequest(http.MethodPost, "/v1/integration/slack/disconnect", nil)
...
func TestIntegrationsGithubCallbackFailsClosed(t *testing.T) {
	...
	r := req(t, app, http.MethodGet,
		"/v1/integration/github/callback?code=inst_123&state="+url.QueryEscape(state), "", nil)
...
func TestIntegrationsCallbackOversizedCodeRejected(t *testing.T) {
	...
	cb := req(t, app, http.MethodGet,
		"/v1/integration/slack/callback?code="+huge+"&state="+url.QueryEscape(state), "", nil)

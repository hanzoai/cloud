...
// Cloud binary — the /v1/integration surface (Slack today; GitHub scaffolded;
...
// never a new route.
//
// Surface (subsystem name "integrations", prefix /v1/integration):
//
//	GET    /v1/integration                      list providers + this org's status  -> {providers:[...]}
//	GET    /v1/integration/:provider            one provider (404 unknown id)        -> Provider
//	POST   /v1/integration/:provider/connect    begin OAuth (org-authed)             -> {authorizeUrl} | 503 | 403
//	GET    /v1/integration/:provider/callback   PUBLIC, state-authed                  -> 302 to console
//	POST   /v1/integration/:provider/disconnect revoke + forget (org-authed)         -> {disconnected:true}
//
...
const (
	// userScope marks a Provider on the per-user /v1/integration/connectors plane. User-scoped
	// providers are invisible on /v1/integration; their rows are keyed
...
type Provider struct {
	...
	RedirectPath string   // OAuth redirect path; MUST equal /v1/integration/{id}/callback
	...
	// Scope selects the custody plane: "" = org-scoped /v1/integration (default);
	// userScope = per-user /v1/integration/connectors (rows keyed (org,user,provider,label),
...
// annotated with this org's connection. User-plane providers (/v1/integration/connectors) are
// not in it.
type listOut struct {
...
// Mount wires /v1/integration/* onto app. Complex flavour: it publishes the
...
func Use(app cloud.Router, deps cloud.Deps) error {
	...
	// OAuth/config surface (nothing on /v1/integration/connectors calls them) and at least one
...
func init() {
	// ── install entry point ──────────────────────────────────────────────────
	openapi.Describe("/v1/integration/slack/install", http.MethodGet,
	...
	openapi.Describe("/v1/integration/slack/events", http.MethodPost,
	...
	openapi.Describe("/v1/integration/slack/commands", http.MethodPost,
	...
	openapi.Describe("/v1/integration/discord/interactions", http.MethodPost,
		...
		"The Interactions Endpoint URL for the Discord app. It answers Discord's PING with "+
			...
			"clean."+vendorCall)
	openapi.Describe("/v1/integration/teams/events", http.MethodPost,
	...
	openapi.Describe("/v1/integration/whatsapp/webhook", http.MethodGet,
	...
	openapi.Describe("/v1/integration/whatsapp/webhook", http.MethodPost,
	...
	openapi.Describe("/v1/integration/telegram/webhook", http.MethodPost,
	...
	openapi.Describe("/v1/integration/linear/webhook", http.MethodPost,
		"Linear webhook",
		"The address Linear delivers Issue and Comment events to. An issue event is "+
			...
			"secret — sealed at /v1/integration/linear/claim — verifies the HMAC over the raw body, "+
			"so the tenant is the organization the signature proves, never a header."+
			vendorCall)
	openapi.Describe("/v1/integration/github/webhook", http.MethodPost,
		"GitHub App webhook",
		"The address the GitHub App delivers events to. A push is handed to the repository "+
			...
			"times it is edited, closed or reopened. A repository event that puts a repo "+
			"INTO the granted set (created, transferred, unarchived) raises one todo "+
			"offering to import it; accepting means POSTing that repo to /v1/integration/github/import. It never imports on its own.\n\n"+
	...
	openapi.Describe("/v1/integration/slack/link", http.MethodGet,
	...
	openapi.Describe("/v1/integration/slack/link/slack", http.MethodGet,
	...
	openapi.Describe("/v1/integration/slack/link/callback", http.MethodGet,
	...
	openapi.Describe("/v1/integration/discord/link", http.MethodGet,
		...
		"The entry point behind the connect prompt Hanzo shows in a Discord server. It "+
			...
			"more."+linkFlow)
	openapi.Describe("/v1/integration/discord/link/discord", http.MethodGet,
	...
	openapi.Describe("/v1/integration/discord/link/callback", http.MethodGet,
	...
	openapi.Describe("/v1/integration/teams/link", http.MethodGet,
		...
		"The entry point behind the connect prompt Hanzo shows in Teams. It starts a link "+
			...
			linkFlow)
	openapi.Describe("/v1/integration/teams/link/aad", http.MethodGet,
	...
	openapi.Describe("/v1/integration/teams/link/callback", http.MethodGet,
		...
		"The final leg: it binds the verified directory identity to the Hanzo account that "+
			...
			linkFlow)
	openapi.Describe("/v1/integration/telegram/link", http.MethodGet,
	...
	openapi.Describe("/v1/integration/telegram/link/auth", http.MethodGet,
	...
	openapi.Describe("/v1/integration/telegram/link/callback", http.MethodGet,
	...
	openapi.Describe("/v1/integration/:provider/callback", http.MethodGet,
...
func routes(app cloud.Router, zapp *zip.App, s *cloud.Service[state]) {
	...
	zip.Get(zapp, "/v1/integration", o.list)
	...
	app.Get("/v1/integration/slack/install", cloud.Terminal(cloud.Handle(s, slackInstall)))
	app.Post("/v1/integration/slack/events", cloud.Terminal(cloud.Handle(s, slackEvents)))
	app.Post("/v1/integration/slack/commands", cloud.Terminal(cloud.Handle(s, slackCommands)))
	...
	// /:provider wildcards.
	zip.Post(zapp, "/v1/integration/slack/join", o.slackJoin)
	...
	// /v1/integration/<vendor>/<the vendor's own noun>, GitHub's being "webhook".
	...
	zip.Get(zapp, "/v1/integration/github/installations", o.githubInstallations)
	// Bind installations the App already holds to the org the caller acts in.
	zip.Post(zapp, "/v1/integration/github/claim", o.githubClaim)
	zip.Get(zapp, "/v1/integration/github/repos", o.githubRepos)
	...
	zip.Post(zapp, "/v1/integration/github/search", o.githubSearch)
	zip.Post(zapp, "/v1/integration/github/fork", o.githubFork)
	...
	zip.Post(zapp, "/v1/integration/github/repos/import", o.githubImport, zip.WithStatus(http.StatusAccepted))
	...
	zip.Post(zapp, "/v1/integration/github/issues/backfill", o.githubIssuesBackfill)
	...
	zip.Post(zapp, "/v1/integration/linear/claim", o.linearClaim)
	zip.Post(zapp, "/v1/integration/linear/comments", o.linearComment, zip.WithStatus(http.StatusCreated))
	zip.Post(zapp, "/v1/integration/linear/issues/backfill", o.linearIssuesBackfill)
	...
	zip.Get(zapp, "/v1/integration/github/repos/:repo/pages", o.githubPagesGet)
	zip.Post(zapp, "/v1/integration/github/repos/:repo/pages", o.githubPagesEnable)
	zip.Put(zapp, "/v1/integration/github/repos/:repo/pages", o.githubPagesUpdate)
	zip.Delete(zapp, "/v1/integration/github/repos/:repo/pages", o.githubPagesDisable)
	// 202: the build is queued at GitHub, not completed here.
	zip.Post(zapp, "/v1/integration/github/repos/:repo/pages/builds", o.githubPagesBuild, zip.WithStatus(http.StatusAccepted))
	...
	app.Post("/v1/integration/discord/interactions", cloud.Terminal(cloud.Handle(s, discordInteractions)))
	app.Get("/v1/integration/discord/link", cloud.Handle(s, discordLink))
	app.Get("/v1/integration/discord/link/discord", cloud.Handle(s, discordLinkDiscord))
	app.Get("/v1/integration/discord/link/callback", cloud.Handle(s, discordLinkCallback))
	app.Post("/v1/integration/teams/events", cloud.Terminal(cloud.Handle(s, teamsEvents)))
	app.Get("/v1/integration/teams/link", cloud.Handle(s, teamsLink))
	app.Get("/v1/integration/teams/link/aad", cloud.Handle(s, teamsLinkAAD))
	app.Get("/v1/integration/teams/link/callback", cloud.Handle(s, teamsLinkCallback))
	zip.Post(zapp, "/v1/integration/telegram/connect", o.telegramConnect)
	app.Post("/v1/integration/telegram/webhook", cloud.Terminal(cloud.Handle(s, telegramWebhook)))
	...
	app.Get("/v1/integration/whatsapp/webhook", cloud.Handle(s, whatsappVerify))
	app.Post("/v1/integration/whatsapp/webhook", cloud.Terminal(cloud.Handle(s, whatsappWebhook)))
	app.Get("/v1/integration/telegram/link", cloud.Handle(s, telegramLink))
	app.Get("/v1/integration/telegram/link/auth", cloud.Handle(s, telegramLinkAuth))
	app.Get("/v1/integration/telegram/link/callback", cloud.Handle(s, telegramLinkCallback))
	...
	zip.Get(zapp, "/v1/integration/gitlab/projects", o.gitlabProjects)
	// Per-USER connector plane (/v1/integration/connectors — connectors.go).
	// BEFORE the /:provider wildcards, for the reason gitlab/projects is: the
	// bare GET /v1/integration/:provider below matches one segment, and a
	...
	zip.Get(zapp, "/v1/integration/:provider", o.get)
	zip.Post(zapp, "/v1/integration/:provider/connect", o.connect)
	...
	// OAuth callback.
	app.Get("/v1/integration/:provider/callback", cloud.Handle(s, callback))
	zip.Post(zapp, "/v1/integration/:provider/disconnect", o.disconnect)
	// apikey connectors: re-verify a stored credential live (`hanzo connector verify`).
	zip.Post(zapp, "/v1/integration/:provider/verify", o.verifyConn)
}
...
// surface (/v1/integration) never resolves a user-scoped provider and vice
...
func callbackPath(provider string) string { return "/v1/integration/" + provider + "/callback" }

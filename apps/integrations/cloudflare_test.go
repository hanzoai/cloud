...
func cfConnect(t *testing.T, app *zip.App, org string) httpResult {
	t.Helper()
	return cfReq(t, app, http.MethodPost, "/v1/integration/cloudflare/connect", org, true,
...
func TestCloudflareConnectRequiresOrgAdmin(t *testing.T) {
	...
	res := cfReq(t, app, http.MethodPost, "/v1/integration/cloudflare/connect", "acme", false,
		map[string]any{"token": cfTestToken})
...
func TestCloudflareConnectRequiresPrincipal(t *testing.T) {
	...
	res := cfReq(t, app, http.MethodPost, "/v1/integration/cloudflare/connect", "", false,
		map[string]any{"token": cfTestToken})
	if res.Code != http.StatusForbidden {
...
func TestCloudflareConnectEmptyTokenRejected(t *testing.T) {
	newCFMock(t, &cfMock{status: "active"})
	app := newApp(t, newKMS(t))
	res := cfReq(t, app, http.MethodPost, "/v1/integration/cloudflare/connect", "acme", true,
...
func TestCloudflareTenantIsolation(t *testing.T) {
	...
	rb := cfReq(t, app, http.MethodGet, "/v1/integration/cloudflare", "orgb", false, nil)
	...
	if r := cfReq(t, app, http.MethodPost, "/v1/integration/cloudflare/verify", "orgb", false, nil); r.Code != http.StatusNotFound {
		t.Fatalf("orgb verify want 404, got %d (%s)", r.Code, r.Body)
	}
	// B's disconnect is idempotent-OK but MUST NOT remove A's secret or row.
	if r := cfReq(t, app, http.MethodPost, "/v1/integration/cloudflare/disconnect", "orgb", true, nil); r.Code != http.StatusOK {
...
func TestCloudflareVerifyEndpoint(t *testing.T) {
	...
	r := cfReq(t, app, http.MethodPost, "/v1/integration/cloudflare/verify", "acme", false, nil)
...
func TestCloudflareDisconnectRemovesTokenAndRow(t *testing.T) {
	...
	if r := cfReq(t, app, http.MethodPost, "/v1/integration/cloudflare/disconnect", "acme", true, nil); r.Code != http.StatusOK {
...
func TestCloudflareTokenNeverLogged(t *testing.T) {
	...
	if r := cfReq(t, app, http.MethodPost, "/v1/integration/cloudflare/connect", "acme", true,
	...
	_ = cfReq(t, app, http.MethodPost, "/v1/integration/cloudflare/connect", "beta", true,
		map[string]any{"token": cfTestToken})
...
func cfConnectNoToken(t *testing.T, app *zip.App, org string) httpResult {
	t.Helper()
	return cfReq(t, app, http.MethodPost, "/v1/integration/cloudflare/connect", org, true, map[string]any{})
}
...
func TestCloudflareOAuthConnectReturnsAuthorizeURLWhenConfigured(t *testing.T) {
	...
	if !strings.HasSuffix(q.Get("redirect_uri"), "/v1/integration/cloudflare/callback") {
...
func TestCloudflareOAuthCallbackSealsSameKMSCoordinate(t *testing.T) {
	...
	cb := "/v1/integration/cloudflare/callback?code=cf-oauth-code-xyz&state=" + url.QueryEscape(state)
...
func TestCloudflareOAuthCallbackFailClosedOnExchangeError(t *testing.T) {
	...
	cb := "/v1/integration/cloudflare/callback?code=bad&state=" + url.QueryEscape(u.Query().Get("state"))

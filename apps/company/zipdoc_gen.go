...
func init() {
	zip.Describe("GET /v1/company", zip.Doc{
		Description: "Get returns the caller org's formation and the stages reachable from it.\nAn org that has not begun a formation is not found.",
		Fields: map[string]string{
			"Filing.status":                 "manual | submitted | filed | rejected",
			"Formation.alreadyIncorporated": "SKIP path.",
			"Formation.documentIds":         "dataroom doc ids of generated formation docs",
			"Formation.importedDocs":        "dataroom doc ids ingested from Drive",
			"Founder.decidedBy":             "who settled a terminal KYC status: the provider name, or a reviewer's user id",
			"Founder.kycRef":                "idv session reference",
			"Genesis.root":                  "0x… keccak root of the founding allocation",
			"Genesis.status":                "pending | anchored",
			"Genesis.txHash":                "L1 transaction hash (empty until anchored)",
	...
	zip.Describe("POST /v1/company/fundraise/round", zip.Doc{
		...
		Fields: map[string]string{
			"RoundInput.roundType": "PRICED | SAFE | CONVERTIBLE_NOTE",

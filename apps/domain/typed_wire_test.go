...
// untypedByDesign is the CLOSED list of domain operations that are NOT typed ops.
...
func opsUnderTest(t *testing.T) (served map[string]bool, typed map[string]string, schemas map[string]any) {
	...
	doc, err := openapi.Spec(app, openapi.Info{Title: "domain", Version: "v1"})
	if err != nil {
		t.Fatalf("spec: %v", err)
	}
	...
	ours := func(p string) bool { return strings.HasPrefix(p, "/v1/domain") }
...
// TestEveryRouteIsTypedOrNamed fails when an domain operation is neither a typed
// op nor named above — so the next route added here is typed by default.
func TestEveryRouteIsTypedOrNamed(t *testing.T) {
	served, typed, _ := opsUnderTest(t)
	if len(served) != 7 {
		t.Errorf("domain serves %d operations, expected 7 — update this gate deliberately", len(served))
	}
	...
	for key := range untypedByDesign {
		if !served[key] {
			t.Errorf("untypedByDesign names %q, which domain no longer serves", key)
		}
	}
}
...
func TestEveryTypedOpIsDescribed(t *testing.T) {
	...
	if len(typed) == 0 {
		t.Fatal("no typed domain ops in the registry at all")
	}
	for key, desc := range typed {
		if strings.TrimSpace(desc) == "" {
			t.Errorf("%s has no description — run: go generate -run zipdoc ./apps/domain/...", key)
		}
	}
}
...
func TestEveryPublishedFieldIsDescribed(t *testing.T) {
	...
	if len(schemas) == 0 {
		t.Fatal("no domain schemas in the typed registry at all")
	}
	var bare []string

...
import (
	...
	"github.com/hanzoai/cloud/apps/knowledge/evernote"
	"github.com/hanzoai/cloud/apps/knowledge/notion"
	"github.com/hanzoai/cloud/apps/knowledge/obsidian"
	"github.com/hanzoai/cloud/apps/knowledge/roam"
	"github.com/hanzoai/cloud/apps/knowledge/vault"
...
func init() {
	openapi.Describe("/v1/kb/import", http.MethodPost,
		...
		"Ingests an uploaded export as a tree of kb-page documents with its link structure "+
			...
			"Scoped to the caller's validated org; `?project=` narrows every imported page to one "+
			"project. No validated principal is 403, and an org that has not installed the kb "+
			"module is refused with the install call to make first. The bounds are 64 MB per "+
			"upload, 5000 pages and 8 MB per archive entry: pages past the five-thousandth are "+
			"dropped and a larger entry is truncated at its bound, and a page the store rejects "+
			"is skipped — so the answer's `imported` count is what was actually filed, not what "+
			"was sent.")
}

...
// ledgercoreCredit implements commerce's creditledger.CreditLedger over cloud's
// native finance ledger — the SAME per-org account (finance.Current()) the AI
// spend-gate reads and the edge meter debits. Injected at mountCommerce
// (EmbedConfig.Ledger), it makes commerce's POST /v1/billing/credit mint into the
// ONE ledger: a granted credit is immediately visible to the gate (one ledger, no
// split). This is the cloud half of the one-ledger seam — commerce defines the
// interface, cloud implements it once here, the compiler enforces the match.
//
...
type ledgercoreCredit struct{}

// compile-time proof the adapter satisfies commerce's exported seam.
var _ creditledger.CreditLedger = ledgercoreCredit{}
...
func (ledgercoreCredit) Credit(ctx context.Context, in creditledger.CreditInput) (string, int64, error) {
	fin := finance.Current()
	if fin == nil {
...
func (ledgercoreCredit) Balance(ctx context.Context, org, currency string) (int64, error) {
	fin := finance.Current()
	if fin == nil {

...
func entitlementOps(t *testing.T) (served map[string]bool, typed map[string]string) {
	...
	doc, err := openapi.Spec(app, openapi.Info{Title: "entitlement", Version: "v1"})
	if err != nil {
		t.Fatalf("spec: %v", err)
	}
...
func TestTheOrgComesFromTheURLNotTheBody(t *testing.T) {
	...
	code, body := send(t, app, orgMember("POST", "/v1/entitlement/orgs/acme", "acme",
	...
	if code, _ := send(t, app, orgMember("GET", "/v1/entitlement/orgs/victim", "acme", nil)); code != http.StatusForbidden {

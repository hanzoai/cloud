...
// base). It is the money-client stand-in that lets the tests PROVE commission accrues
...
func applyAndApprove(t *testing.T, app *zip.App, s *cloud.Service[state], org, requestedCode, approveCode string) (id, code string) {
	t.Helper()
	code2, body := req(t, app, http.MethodPost, "/v1/affiliate/apply", org, false, map[string]any{"requestedCode": requestedCode})
	...
	st, ab := req(t, app, http.MethodPost, "/v1/admin/affiliate/"+ar.ID+"/approve", "admin", true, map[string]any{"code": approveCode})
	if st != http.StatusOK {
...
func TestApplyIdempotent(t *testing.T) {
	...
	if code, _ := req(t, app, http.MethodPost, "/v1/affiliate/apply", "", false, map[string]any{}); code != http.StatusForbidden {
		t.Fatalf("no-principal apply want 403, got %d", code)
	}
	// Malformed vanity code → 400.
	if code, _ := req(t, app, http.MethodPost, "/v1/affiliate/apply", "orgA", false, map[string]any{"requestedCode": "x"}); code != http.StatusBadRequest {
		t.Fatalf("bad vanity code want 400, got %d", code)
	}
	// First apply → 201 applied, default rate.
	code, body := req(t, app, http.MethodPost, "/v1/affiliate/apply", "orgA", false, map[string]any{"requestedCode": "acme"})
	...
	code, body = req(t, app, http.MethodPost, "/v1/affiliate/apply", "orgA", false, map[string]any{"requestedCode": "different"})
...
func TestApproveMintsCodeAndVanityUniqueness(t *testing.T) {
	...
	_, bBody := req(t, app, http.MethodPost, "/v1/affiliate/apply", "orgB", false, map[string]any{"requestedCode": "launch"})
	var b struct{ ID string }
	_ = json.Unmarshal(bBody, &b)
	if st, body := req(t, app, http.MethodPost, "/v1/admin/affiliate/"+b.ID+"/approve", "admin", true, nil); st != http.StatusConflict {
	...
	if st, body := req(t, app, http.MethodPost, "/v1/admin/affiliate/"+b.ID+"/approve", "admin", true, map[string]any{"code": "launch-b"}); st != http.StatusOK {
	...
	if st, _ := req(t, app, http.MethodPost, "/v1/admin/affiliate/aff_missing/approve", "admin", true, nil); st != http.StatusNotFound {
...
func TestAttributeSelfUnknownAndIdempotent(t *testing.T) {
	...
	req(t, app, http.MethodPost, "/v1/affiliate/apply", "orgPending", false, map[string]any{"requestedCode": "pending1"})
	if code, _ := req(t, app, http.MethodPost, "/v1/affiliate/attribute", "orgX", false, map[string]any{"code": "pending1"}); code != http.StatusNotFound {
	...
	// No principal → 403.
	if code, _ := req(t, app, http.MethodPost, "/v1/affiliate/attribute", "", false, map[string]any{"code": codeA}); code != http.StatusForbidden {
		t.Fatalf("no-principal attribute want 403, got %d", code)
	}
	// Unknown code → 404.
	if code, _ := req(t, app, http.MethodPost, "/v1/affiliate/attribute", "orgB", false, map[string]any{"code": "nope-nope"}); code != http.StatusNotFound {
		t.Fatalf("unknown-code attribute want 404, got %d", code)
	}
	// Self-attribution (orgA uses its OWN code) → 400.
	if code, _ := req(t, app, http.MethodPost, "/v1/affiliate/attribute", "orgA", false, map[string]any{"code": codeA}); code != http.StatusBadRequest {
		t.Fatalf("self-attribution want 400, got %d", code)
	}
	// orgB attributes to orgA → 201.
	if code, body := req(t, app, http.MethodPost, "/v1/affiliate/attribute", "orgB", false, map[string]any{"code": codeA}); code != http.StatusCreated {
		t.Fatalf("first attribute want 201, got %d (%s)", code, body)
	}
	// Re-attribute (same code) → 200, not created (idempotent, first-touch).
	code, body := req(t, app, http.MethodPost, "/v1/affiliate/attribute", "orgB", false, map[string]any{"code": codeA})
	if code != http.StatusOK {
	...
	req(t, app, http.MethodPost, "/v1/affiliate/attribute", "orgB", false, map[string]any{"code": codeC})
...
func TestSweepAccruesSpendTimesRateIdempotent(t *testing.T) {
	...
	if code, _ := req(t, app, http.MethodPost, "/v1/affiliate/attribute", "orgB", false, map[string]any{"code": codeA}); code != http.StatusCreated {
		t.Fatalf("attribute want 201, got %d", code)
	}

	// No spend yet: a sweep accrues NOTHING.
	code, body := req(t, app, http.MethodPost, "/v1/admin/affiliate/sweep", "admin", true, nil)
	if code != http.StatusOK {
	...
	req(t, app, http.MethodPost, "/v1/admin/affiliate/sweep", "admin", true, nil)
...
// here: GET /v1/affiliate is a PURE READ. Only the admin POST accrues.
func TestAffiliateReadGrantsNothing(t *testing.T) {
	app, s, fc := mount(t)
	_, codeA := applyAndApprove(t, app, s, "orgA", "acme", "")
	req(t, app, http.MethodPost, "/v1/affiliate/attribute", "orgB", false, map[string]any{"code": codeA})
	fc.setSpend("orgB", 5000) // $50 → 40% margin $20 → 20% share = 400c ($4)

	code, body := req(t, app, http.MethodGet, "/v1/affiliate", "orgA", false, nil)
	if code != http.StatusOK {
		t.Fatalf("GET /v1/affiliate want 200, got %d (%s)", code, body)
	}
	var v struct {
...
func TestPayoutIsRecordOnlyAndPendingGuard(t *testing.T) {
	...
	if st, _ := req(t, app, http.MethodPost, "/v1/admin/affiliate/"+idA+"/payout", "orgA", false, map[string]any{"amountCents": 100, "method": "credits"}); st != http.StatusForbidden {
		t.Fatalf("non-admin payout want 403, got %d", st)
	}
	// Over-pending → 400 (2000 available, ask 3000).
	if st, _ := req(t, app, http.MethodPost, "/v1/admin/affiliate/"+idA+"/payout", "admin", true, map[string]any{"amountCents": 3000, "method": "credits"}); st != http.StatusBadRequest {
		t.Fatalf("over-pending payout want 400, got %d", st)
	}

	// Credits payout of 1200c → RECORDED, paid moves, wallet untouched.
	st, body := req(t, app, http.MethodPost, "/v1/admin/affiliate/"+idA+"/payout", "admin", true, map[string]any{"amountCents": 1200, "method": "credits", "reference": "ledger-1"})
	if st != http.StatusOK {
	...
	st, body = req(t, app, http.MethodPost, "/v1/admin/affiliate/"+idA+"/payout", "admin", true, map[string]any{"amountCents": 800, "method": "wire", "reference": "wire-xyz"})
	if st != http.StatusOK {
	...
	if st, _ := req(t, app, http.MethodPost, "/v1/admin/affiliate/"+idA+"/payout", "admin", true, map[string]any{"amountCents": 1, "method": "credits"}); st != http.StatusBadRequest {
		t.Fatalf("drained payout want 400, got %d", st)
	}
}

// TestAdminGateAndDirectory: every /v1/admin/affiliate* route is SuperAdmin
...
func TestAdminGateAndDirectory(t *testing.T) {
	...
	for _, p := range []string{"/v1/admin/affiliate", "/v1/admin/affiliate/sweep",
		"/v1/admin/affiliate/" + idA + "/approve", "/v1/admin/affiliate/" + idA + "/suspend",
		"/v1/admin/affiliate/" + idA + "/payout"} {
		method := http.MethodPost
		if p == "/v1/admin/affiliate" {
	...
	code, body := req(t, app, http.MethodGet, "/v1/admin/affiliate", "admin", true, nil)
	if code != http.StatusOK {
	...
	if st, _ := req(t, app, http.MethodPost, "/v1/admin/affiliate/"+idA+"/suspend", "admin", true, nil); st != http.StatusOK {
		t.Fatalf("suspend want 200, got %d", st)
	}
	if code, _ := req(t, app, http.MethodPost, "/v1/affiliate/attribute", "orgD", false, map[string]any{"code": codeA}); code != http.StatusNotFound {
...
func attributeOK(t *testing.T, app *zip.App, org, code string) {
	t.Helper()
	if st, body := req(t, app, http.MethodPost, "/v1/affiliate/attribute", org, false, map[string]any{"code": code}); st/100 != 2 {
...
func TestMultiLevelUplineWalk(t *testing.T) {
	...
	if st, body := req(t, app, http.MethodPost, "/v1/admin/affiliate/sweep", "admin", true, nil); st != http.StatusOK {
...
func TestCycleRejection(t *testing.T) {
	...
	if st, _ := req(t, app, http.MethodPost, "/v1/affiliate/attribute", "orgA", false, map[string]any{"code": codeB}); st != http.StatusBadRequest {
		t.Fatalf("direct cycle want 400, got %d", st)
	}
	// Long cycle: orgA referred by orgC would close A→B→C→A (C's upline reaches A).
	if st, _ := req(t, app, http.MethodPost, "/v1/affiliate/attribute", "orgA", false, map[string]any{"code": codeC}); st != http.StatusBadRequest {
...
func TestReferredByImmutableOrg(t *testing.T) {
	...
	st, body := req(t, app, http.MethodPost, "/v1/affiliate/attribute", "orgX", false, map[string]any{"code": codeB})
	if st != http.StatusOK {
...
// TestAffiliatesMeSurface proves GET /v1/affiliate/me returns the caller's code,
...
func TestAffiliatesMeSurface(t *testing.T) {
	...
	code, body := req(t, app, http.MethodGet, "/v1/affiliate/me", "orgZ", false, nil)
	if code != http.StatusOK {
	...
	code, body = req(t, app, http.MethodGet, "/v1/affiliate/me", "orgA", false, nil)
	if code != http.StatusOK {
...
func TestAdminReferralsAnalytics(t *testing.T) {
	...
	if st, _ := req(t, app, http.MethodGet, "/v1/admin/affiliate/referrals", "orgA", false, nil); st != http.StatusForbidden {
		t.Fatalf("non-admin /v1/admin/affiliate/referrals want 403, got %d", st)
	}

	code, body := req(t, app, http.MethodGet, "/v1/admin/affiliate/referrals", "admin", true, nil)
	if code != http.StatusOK {
...
func TestUse(t *testing.T) {
	...
	r := httptest.NewRequest(http.MethodGet, "/v1/affiliate", nil)
	...
	if resp.StatusCode != http.StatusForbidden {
		t.Fatalf("mounted GET /v1/affiliate (no principal) want 403, got %d", resp.StatusCode)
	}
}

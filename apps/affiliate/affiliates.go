...
// The loop, end to end:
//
//  1. An org APPLIES to be an affiliate (POST /v1/affiliate/apply), optionally
//     requesting a vanity code. Staff APPROVE it (POST /v1/admin/affiliate/:id/
...
//  2. A new org signs up via the link → the console posts POST /v1/affiliate/
//     attribute with the code → we record referred_org↔affiliate (first-touch,
//     one per referred org, self-attribution blocked).
//  3. The ACCRUAL SWEEP (POST /v1/admin/affiliate/sweep, SuperAdmin; also lazy
...
//  4. Staff RECORD a payout of accrued commission (POST /v1/admin/affiliate/:id/payout,
...
// Surface:
//
//	GET  /v1/affiliate                        (org)          my status, code, link, referred count, accrued/pending/paid, payouts
//	POST /v1/affiliate/apply                  (org)          apply to the program (optional vanity code)
//	POST /v1/affiliate/attribute              (org=referred) record attribution from an ?aff code
//	GET  /v1/admin/affiliate                  (SuperAdmin) every affiliate + a summary
//	POST /v1/admin/affiliate/:id/approve      (SuperAdmin) approve + mint the code
//	POST /v1/admin/affiliate/:id/suspend      (SuperAdmin) suspend
//	POST /v1/admin/affiliate/:id/payout       (SuperAdmin) RECORD a payout (record-only; a human settles it)
//	POST /v1/admin/affiliate/sweep            (SuperAdmin) accrue commission for every referred org this period
//	GET  /v1/admin/affiliate/referrals        (SuperAdmin) cross-tenant referral board over this app's attribution spine
//
// serve.go auto-registers GET /v1/affiliate/health.
package affiliate
...
import (
	...
	"github.com/hanzoai/cloud/apps/author"
...
func routes(app cloud.Router, s *cloud.Service[state]) {
	o := ops{s: s}
	r := cloud.ZipApp(app)
	zip.Get(r, "/v1/affiliate", o.standing)
	zip.Get(r, "/v1/affiliate/me", o.self)
	// Self-service dashboard reads/writes (all org-scoped to the caller's own affiliate).
	zip.Get(r, "/v1/affiliate/me/earnings", o.earnings)
	zip.Get(r, "/v1/affiliate/me/links", o.links)
	zip.Post(r, "/v1/affiliate/me/links", o.mintLink, zip.WithStatus(http.StatusCreated))
	zip.Post(r, "/v1/affiliate/me/handle", o.setHandle)
	...
	zip.Post(r, "/v1/affiliate/apply", o.apply, zip.WithStatus(http.StatusOK, http.StatusCreated))
	zip.Post(r, "/v1/affiliate/attribute", o.attribute, zip.WithStatus(http.StatusOK, http.StatusCreated))
	...
	zip.Post(r, "/v1/affiliate/click", o.click)
	...
	zip.Get(r, "/v1/affiliate/leaderboard", o.board)
	...
	// genuinely serves at /v1/admin/referral/*.
...
// applyRequest is the POST /v1/affiliate/apply body: an optional requested vanity
...
// attributeRequest is the POST /v1/affiliate/attribute body: the affiliate's code
...
// /v1/affiliate surface stays bare JSON (read via the /cloud proxy + restGet).
...
// directoryOut is the enveloped GET /v1/admin/affiliate answer.
...
// referralsOut is the enveloped GET /v1/admin/affiliate/referrals answer.
...
// approval is the POST /v1/admin/affiliate/:id/approve input: the affiliate
...
// disbursal is the POST /v1/admin/affiliate/:id/payout input.
...
// payoutOut is the enveloped POST /v1/admin/affiliate/:id/payout answer.
...
// accrualsOut is the enveloped POST /v1/admin/affiliate/sweep answer.
...
func (o ops) adminSweep(ctx context.Context, _ *cloud.Unit) (*accrualsOut, error) {
	...
	for _, src := range sources {
		...
		r, rerr := author.AccrueForOrg(ctx, src, spend, period, now)
		if rerr != nil {
			royaltyErrs++

...
// set-once/immutable), which is also the idempotency key for POST /v1/affiliate/
...
// affiliate); every /v1/affiliate read is scoped by the caller's org server-side.
type Store struct {
	db *sql.DB
}
...
// ── analytics (the /v1/admin/affiliate/referrals cross-tenant board) ─────────────────────

...
func TestProfitShareMarginInvariant(t *testing.T) {
	...
	if st, body := req(t, app, http.MethodPost, "/v1/admin/affiliate/"+idC+"/rate", "admin", true, map[string]any{"rateBps": maxL1RateBps(ctx)}); st != http.StatusOK {
		t.Fatalf("set L1 rate to cap want 200, got %d (%s)", st, body)
	}

	const spend = 10000
	fc.setSpend("orgD", spend)
	if st, _ := req(t, app, http.MethodPost, "/v1/admin/affiliate/sweep", "admin", true, nil); st != http.StatusOK {
...
// TestSetRateGateAndCap proves POST /v1/admin/affiliate/:id/rate is SuperAdmin-gated,
...
func TestSetRateGateAndCap(t *testing.T) {
	...
	// Non-admin → 403.
	if st, _ := req(t, app, http.MethodPost, "/v1/admin/affiliate/"+idA+"/rate", "orgA", false, map[string]any{"rateBps": 1000}); st != http.StatusForbidden {
		t.Fatalf("non-admin set-rate want 403, got %d", st)
	}
	// Over the cap → 400 (would let the schedule exceed the margin).
	if st, _ := req(t, app, http.MethodPost, "/v1/admin/affiliate/"+idA+"/rate", "admin", true, map[string]any{"rateBps": maxL1RateBps(ctx) + 1}); st != http.StatusBadRequest {
		t.Fatalf("over-cap set-rate want 400, got %d", st)
	}
	// Negative → 400.
	if st, _ := req(t, app, http.MethodPost, "/v1/admin/affiliate/"+idA+"/rate", "admin", true, map[string]any{"rateBps": -1}); st != http.StatusBadRequest {
		t.Fatalf("negative set-rate want 400, got %d", st)
	}
	// Missing affiliate → 404.
	if st, _ := req(t, app, http.MethodPost, "/v1/admin/affiliate/aff_missing/rate", "admin", true, map[string]any{"rateBps": 1000}); st != http.StatusNotFound {
		t.Fatalf("missing set-rate want 404, got %d", st)
	}
	// Valid at the cap → 200, the rate is persisted.
	if st, body := req(t, app, http.MethodPost, "/v1/admin/affiliate/"+idA+"/rate", "admin", true, map[string]any{"rateBps": maxL1RateBps(ctx)}); st != http.StatusOK {
		t.Fatalf("cap set-rate want 200, got %d (%s)", st, body)
	}
	a, _ := s.State.store.GetByID(ctx, idA)
	if a.RateBps != maxL1RateBps(ctx) {
		t.Fatalf("rate = %d, want %d", a.RateBps, maxL1RateBps(ctx))
	}
	...
	if a2.AccruedCents != share(10000, maxL1RateBps(ctx)) {
		t.Fatalf("accrued at cap rate = %d, want %d", a2.AccruedCents, share(10000, maxL1RateBps(ctx)))
	}
}

// linkRow is one link row from GET /v1/affiliate/me/links.
type linkRow struct {
...
func getLinks(t *testing.T, app *zip.App, org string) linksResp {
	t.Helper()
	st, body := req(t, app, http.MethodGet, "/v1/affiliate/me/links", org, false, nil)
	if st != http.StatusOK {
...
func TestLinksLifecycle(t *testing.T) {
	...
	st, body := req(t, app, http.MethodPost, "/v1/affiliate/me/links", "orgA", false, map[string]any{"label": "twitter"})
	...
	if st, _ := req(t, app, http.MethodPost, "/v1/affiliate/click", "", false, map[string]any{"code": newCode}); st != http.StatusOK {
	...
	if st, _ := req(t, app, http.MethodPost, "/v1/affiliate/me/links", "orgA", false, map[string]any{"code": codeA}); st != http.StatusConflict {
		t.Fatalf("dup code want 409, got %d", st)
	}
	if st, _ := req(t, app, http.MethodPost, "/v1/affiliate/me/links", "orgA", false, map[string]any{"code": "x"}); st != http.StatusBadRequest {
		t.Fatalf("bad code want 400, got %d", st)
	}

	// A not-yet-approved affiliate can't create links.
	req(t, app, http.MethodPost, "/v1/affiliate/apply", "orgP", false, map[string]any{})
	if st, _ := req(t, app, http.MethodPost, "/v1/affiliate/me/links", "orgP", false, map[string]any{"label": "x"}); st != http.StatusBadRequest {
	...
	for {
		...
		if _, err := s.State.store.CreateLink(ctx, mint.ID("aln"), idA, code, "fill", time.Now().Unix()); err != nil {
			t.Fatalf("fill link: %v", err)
		}
	}
	if st, _ := req(t, app, http.MethodPost, "/v1/affiliate/me/links", "orgA", false, map[string]any{"label": "over"}); st != http.StatusBadRequest {
...
func TestLinkCodeResolvesAttributionAndSuspend(t *testing.T) {
	...
	st, body := req(t, app, http.MethodPost, "/v1/affiliate/me/links", "orgA", false, map[string]any{"label": "x"})
	if st != http.StatusCreated {
	...
	req(t, app, http.MethodPost, "/v1/admin/affiliate/"+idA+"/suspend", "admin", true, nil)
	if st, _ := req(t, app, http.MethodPost, "/v1/affiliate/attribute", "orgD", false, map[string]any{"code": newCode}); st != http.StatusNotFound {
...
func TestEarningsAndCrossAffiliateIsolation(t *testing.T) {
	...
	// No principal → 403.
	if st, _ := req(t, app, http.MethodGet, "/v1/affiliate/me/earnings", "", false, nil); st != http.StatusForbidden {
		t.Fatalf("no-principal earnings want 403, got %d", st)
	}

	// A's earnings: byReferredOrg has X only; the raw org id "orgY" NEVER appears.
	st, body := req(t, app, http.MethodGet, "/v1/affiliate/me/earnings", "orgA", false, nil)
	if st != http.StatusOK {
	...
	_, body = req(t, app, http.MethodGet, "/v1/affiliate/me/earnings", "orgB", false, nil)
	...
	_, body = req(t, app, http.MethodGet, "/v1/affiliate/me/earnings", "orgZ", false, nil)
	var nz struct {
		IsAffiliate bool `json:"isAffiliate"`
	}
...
func TestLinksCrossAffiliateIsolation(t *testing.T) {
	...
	// A mints a link.
	st, body := req(t, app, http.MethodPost, "/v1/affiliate/me/links", "orgA", false, map[string]any{"label": "a-secret"})
	if st != http.StatusCreated {
...
func TestAccrualConvergesAtMaxRate(t *testing.T) {
	...
	if st, _ := req(t, app, http.MethodPost, "/v1/admin/affiliate/"+idC+"/rate", "admin", true, map[string]any{"rateBps": maxL1RateBps(ctx)}); st != http.StatusOK {
...
func TestLeaderboardPrivacy(t *testing.T) {
	...
	if st, _ := req(t, app, http.MethodPost, "/v1/affiliate/me/handle", "orgA", false, map[string]any{"handle": "alice"}); st != http.StatusOK {
		t.Fatalf("A set handle want 200, got %d", st)
	}
	req(t, app, http.MethodPost, "/v1/affiliate/me/handle", "orgB", false, map[string]any{"handle": "bob"})

	// No principal → 403.
	if st, _ := req(t, app, http.MethodGet, "/v1/affiliate/leaderboard", "", false, nil); st != http.StatusForbidden {
		t.Fatalf("no-principal leaderboard want 403, got %d", st)
	}

	// A views: leaders = [alice#1, bob#2]; C not listed. NO org identity anywhere.
	st, body := req(t, app, http.MethodGet, "/v1/affiliate/leaderboard", "orgA", false, nil)
	if st != http.StatusOK {
	...
	_, body = req(t, app, http.MethodGet, "/v1/affiliate/leaderboard", "orgC", false, nil)
	lb = decodeLeaderboard(t, body)
	if len(lb.Leaders) != 2 {
	...
	_, body = req(t, app, http.MethodGet, "/v1/affiliate/leaderboard", "orgZ", false, nil)
	lb = decodeLeaderboard(t, body)
	if lb.You != nil {

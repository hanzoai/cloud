...
// /v1/pref surface behind the user menu on every Hanzo surface (console, insights,
...
//
//	GET   /v1/pref   the caller's own document          -> prefsView
//	PATCH /v1/pref   shallow key-wise merge into it     -> prefsView
//
...
func init() {
	openapi.Describe("/v1/pref", http.MethodPatch,
...
// /v1/pref group: joinPath normalises "" to "/", so the group form named
// /v1/pref/ — a path this API has never served — in the document, the
...
func routes(app cloud.Router, s *service) {
	...
	// that it installed for itself could only hang on a /v1/pref node, and both
	...
	zip.Get(cloud.ZipApp(app), "/v1/pref", prefsOps{s: s}.getPrefs)
	...
	app.Patch("/v1/pref", s.patchPrefs)
}

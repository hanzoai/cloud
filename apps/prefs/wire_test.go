...
// TestPrefsAnswerAtBothPathForms — both verbs are declared at /v1/prefs, with no
// trailing slash, and fiber's non-strict routing means /v1/prefs/ still reaches
...
func TestPrefsAnswerAtBothPathForms(t *testing.T) {
	app := mountPrefs(t)
	for _, p := range []string{"/v1/prefs", "/v1/prefs/"} {
...
func TestReadFailsClosedWithoutAValidatedPrincipal(t *testing.T) {
	app := mountPrefs(t)
	if code, _ := as(t, app, http.MethodGet, "/v1/prefs", "", "acme", nil); code != http.StatusForbidden {
		t.Errorf("unauth GET want 403, got %d", code)
	}
	if code, _ := as(t, app, http.MethodPatch, "/v1/prefs", "", "acme", []byte(`{"a":1}`)); code != http.StatusForbidden {
...
func TestEmptyDocumentIsASuccess(t *testing.T) {
	app := mountPrefs(t)
	code, body := as(t, app, http.MethodGet, "/v1/prefs", "alice", "acme", nil)
...
func TestIsolationIsTheQUALIFIEDSubject(t *testing.T) {
	app := mountPrefs(t)
	if code, body := as(t, app, http.MethodPatch, "/v1/prefs", "z", "hanzo", []byte(`{"theme":"dark"}`)); code != http.StatusOK {
		t.Fatalf("hanzo/z write: %d %s", code, body)
	}
	code, body := as(t, app, http.MethodGet, "/v1/prefs", "z", "admin", nil)
	if code != http.StatusOK {
...
func TestPatchStillMerges(t *testing.T) {
	app := mountPrefs(t)
	as(t, app, http.MethodPatch, "/v1/prefs", "alice", "acme", []byte(`{"theme":"dark"}`))
	as(t, app, http.MethodPatch, "/v1/prefs", "alice", "acme", []byte(`{"density":"compact"}`))

	_, body := as(t, app, http.MethodGet, "/v1/prefs", "alice", "acme", nil)
	var got prefsView
	...
	as(t, app, http.MethodPatch, "/v1/prefs", "alice", "acme", []byte(`{"theme":null}`))
	_, body = as(t, app, http.MethodGet, "/v1/prefs", "alice", "acme", nil)
...
func TestPatchKeepsTheWireThatKeepsItUntyped(t *testing.T) {
	...
	if code, _ := as(t, app, http.MethodPatch, "/v1/prefs", "alice", "acme", []byte(big)); code != http.StatusRequestEntityTooLarge {
	...
	if code, _ := as(t, app, http.MethodPatch, "/v1/prefs", "alice", "acme", []byte(``)); code != http.StatusBadRequest {
		t.Errorf("empty patch body want 400, got %d", code)
	}
	if code, _ := as(t, app, http.MethodPatch, "/v1/prefs", "alice", "acme", []byte(`null`)); code != http.StatusBadRequest {
		t.Errorf("null patch body want 400, got %d", code)
	}
	if code, _ := as(t, app, http.MethodPatch, "/v1/prefs", "alice", "acme", []byte(`[1,2]`)); code != http.StatusBadRequest {
	...
	if code, body := as(t, app, http.MethodPatch, "/v1/prefs", "alice", "acme",
	...
	if len(many) > maxKeys {
		if code, _ := as(t, app, http.MethodPatch, "/v1/prefs", "alice", "acme", b); code != http.StatusRequestEntityTooLarge {

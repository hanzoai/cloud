...
func projections(t *testing.T) (served map[string]bool, typed map[string]*openapi.Operation) {
	...
	doc, err := openapi.Spec(app, openapi.Info{Title: "datasets", Version: "v1"})
	if err != nil {
		t.Fatalf("spec: %v", err)
	}
...
func TestTheSurfaceIsExactlyWhatItSays(t *testing.T) {
	want := []string{
		...
		"GET /v1/datasets/{name}/export",
		"GET /v1/datasets/{name}/lineage",
		"POST /v1/datasets",
		"POST /v1/datasets/{name}/materialize",
	}
...
func TestTheManifestNamesWhatTheRouterServes(t *testing.T) {
	prefixes := manifest.PrefixesFor("datasets")
	if len(prefixes) == 0 {

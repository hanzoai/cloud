...
// /v1/datasets that no other app claims; zip refuses two owners for one
...
//	meter+gate   cloud.NewResourceMeter(deps, "datasets"); Gate before the one
//	             priced op and Meter after it.
//	logs         cloud.NewBase(deps, "datasets") gives the scoped logger.
...
func Mount(app cloud.Router, deps cloud.Deps) error {
	...
	base := cloud.NewBase(deps, "datasets")
	p := &plane{
		store: warehouse{},
		brand: deps.Brand,
		bill:  base.Bill,
...
func mount(p *plane, app cloud.Router) error {
	...
	// the published address exactly `/v1/datasets` — a group root composes to
	// `/v1/datasets/`, and a trailing slash in the document is a trailing slash
	...
	// /v1/datasets and nowhere else; grouping at /v1/risk for a shorter leaf
	...
	zip.Get(z, "/v1/datasets/:name/lineage", o.lineage,
		...
		zip.WithTags("datasets"))
	zip.Get(z, "/v1/datasets/:name/export", o.export,
...
func (p *plane) charge(ctx context.Context, c caller, cost int64) error {
	err := p.bill.Gate(ctx, c.ledger, c.project, c.validated, "datasets", cost)
	if err == nil {
		return nil
	}

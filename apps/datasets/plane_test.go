...
func exercise(t *testing.T) ([]call, *fake) {
	...
	if code, body := do(t, app, http.MethodDelete, "/v1/datasets/d", "one", nil); code != 200 {
...
func TestADatasetNameCannotCarrySyntax(t *testing.T) {
	...
	if code, body := do(t, app, http.MethodPost, "/v1/datasets", "one", in); code != http.StatusOK {
		t.Fatalf("a mixed-case name was refused: %d (%s)", code, body)
	}
	if code, body := do(t, app, http.MethodGet, "/v1/datasets/mixedcase", "one", nil); code != http.StatusOK {

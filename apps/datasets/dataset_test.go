...
func newPlaneBilled(f *fake, m meter) *plane {
	base := cloud.NewBase(cloud.Deps{Brand: brand}, "datasets")
...
func settled(t *testing.T, app *zip.App, org, name string) riskDataset {
	...
	for {
		code, body := do(t, app, http.MethodGet, "/v1/datasets/"+name, org, nil)
...
func build(t *testing.T, app *zip.App, org string, in riskDatasetSpec) riskDataset {
	t.Helper()
	if code, body := do(t, app, http.MethodPost, "/v1/datasets", org, in); code != http.StatusOK {
		t.Fatalf("declare: %d (%s)", code, body)
	}
	if code, body := do(t, app, http.MethodPost, "/v1/datasets/"+in.Name+"/materialize", org, nil); code != http.StatusAccepted {
...
func TestForeignOrgSeesNothing(t *testing.T) {
	...
	for _, org := range []string{"acme", "globex"} {
		code, body := do(t, app, http.MethodGet, "/v1/datasets", org, nil)
		if code != http.StatusOK {
	...
	code, body := do(t, app, http.MethodGet, "/v1/datasets/shared/export?limit=1000", "acme", nil)
	if code != http.StatusOK {
	...
	if code, body := do(t, app, http.MethodDelete, "/v1/datasets/shared", "acme", nil); code != http.StatusOK {
	...
	if code, _ := do(t, app, http.MethodGet, "/v1/datasets/shared/export", "acme", nil); code != http.StatusConflict {
		t.Fatalf("acme exported a disposed dataset: %d", code)
	}
	code, body = do(t, app, http.MethodGet, "/v1/datasets/shared/export?limit=1000", "globex", nil)
	if code != http.StatusOK {
...
func TestNoPrincipalReachesNothing(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{http.MethodPost, "/v1/datasets", declared("x")},
		{http.MethodGet, "/v1/datasets", nil},
		{http.MethodGet, "/v1/datasets/x", nil},
		{http.MethodPost, "/v1/datasets/x/materialize", nil},
		{http.MethodGet, "/v1/datasets/x/lineage", nil},
		{http.MethodGet, "/v1/datasets/x/export", nil},
		{http.MethodDelete, "/v1/datasets/x", nil},
	} {
...
func TestTheAnonymousLaneIsNotATenant(t *testing.T) {
	...
	code, body := do(t, app, http.MethodGet, "/v1/datasets", tenant.Public, nil)
	if code != http.StatusForbidden {
...
func TestAPublishedVersionCannotBeMutated(t *testing.T) {
	...
	code, body := do(t, app, http.MethodPost, "/v1/datasets/d/materialize", "one", nil)
...
func TestAnIncompleteAttemptIsNeverReadable(t *testing.T) {
	...
	if code, body := do(t, app, http.MethodPost, "/v1/datasets", "one", declared("d")); code != http.StatusOK {
		t.Fatalf("declare: %d (%s)", code, body)
	}
	...
	if code, body := do(t, app, http.MethodGet, "/v1/datasets/d/export", "one", nil); code != http.StatusConflict {
...
func TestAnUnreachableStoreRefusesRatherThanAnswersEmpty(t *testing.T) {
	f := &fake{down: true}
	app := mountHTTP(t, newPlane(f))
	code, body := do(t, app, http.MethodGet, "/v1/datasets", "one", nil)
...
func TestTheStoresOwnWordsNeverReachTheCaller(t *testing.T) {
	...
	for _, tc := range []struct{ method, path string }{
		...
		{http.MethodGet, "/v1/datasets/d"},
		{http.MethodGet, "/v1/datasets/d/export"},
		{http.MethodGet, "/v1/datasets/d/lineage"},
		{http.MethodDelete, "/v1/datasets/d"},
		{http.MethodPost, "/v1/datasets/d/materialize"},
	} {
	...
	code, body := do(t, app, http.MethodPost, "/v1/datasets", "one", declared("d"))
...
func TestARowFromAnotherTenantIsRefusedNotFiltered(t *testing.T) {
	...
	code, body := do(t, app, http.MethodGet, "/v1/datasets", "acme", nil)
	if code == http.StatusOK {
...
func TestAWindowYoungerThanItsHorizonIsRefusedAtTheDoor(t *testing.T) {
	...
	code, body := do(t, app, http.MethodPost, "/v1/datasets", "one", in)
...
func TestTheProcessIsBoundedAcrossTenantsToo(t *testing.T) {
	...
	surface(f, "late", 3, 3)
	if code, body := do(t, app, http.MethodPost, "/v1/datasets", "late", declared("d")); code != http.StatusOK {
		t.Fatalf("declare: %d (%s)", code, body)
	}
	code, body := do(t, app, http.MethodPost, "/v1/datasets/d/materialize", "late", nil)
	if code != http.StatusServiceUnavailable {
	...
	if code, body := do(t, app, http.MethodPost, "/v1/datasets/d/materialize", "late", nil); code != http.StatusAccepted {
...
func TestADeadStoreIsNeverReportedAsTheCallersMistake(t *testing.T) {
	...
	if code, body := do(t, app, http.MethodGet, "/v1/datasets/d/export?split=holdout", "one", nil); code != http.StatusBadRequest {
	...
	f.down = true
	f.mu.Unlock()
	code, body := do(t, app, http.MethodGet, "/v1/datasets/d/export?split=train", "one", nil)
	if code != http.StatusServiceUnavailable {
...
func TestLineageIsAdmittedLikeTheScanItIs(t *testing.T) {
	...
	f.forget()
	code, body := do(t, app, http.MethodGet, "/v1/datasets/d/lineage", "one", nil)
	if code != http.StatusConflict {
...
func TestTheBillersOwnWordsNeverReachTheCaller(t *testing.T) {
	...
	for _, probe := range []struct {
		...
		{"materialize", http.MethodPost, "/v1/datasets/d/materialize", nil},
		{"create", http.MethodPost, "/v1/datasets", declared("other")},
	} {
...
func TestAVersionNumberIsNeverReusedAcrossADisposal(t *testing.T) {
	...
	if code, body := do(t, app, http.MethodDelete, "/v1/datasets/orders", "one", nil); code != http.StatusOK {
		t.Fatalf("dispose: %d (%s)", code, body)
	}
	...
	if code, body := do(t, app, http.MethodGet, "/v1/datasets/orders/export", "one", nil); code != http.StatusConflict {
...
func TestATokenFromAnotherBrandIsNotThisBrandsTenant(t *testing.T) {
	...
	if code, body := vouched(t, app, http.MethodGet, "/v1/datasets", "one", brand, nil); code != http.StatusOK {
	...
	f.forget()
	code, body := vouched(t, app, http.MethodGet, "/v1/datasets", "one", "lux", nil)
	if code != http.StatusForbidden {
	...
	if code, body := vouched(t, app, http.MethodPost, "/v1/datasets", "one", "lux", declared("d")); code != http.StatusForbidden {
...
func exported(t *testing.T, app *zip.App, org, name string) []riskDatasetRow {
	t.Helper()
	code, body := do(t, app, http.MethodGet, "/v1/datasets/"+name+"/export?limit=5000", org, nil)
	if code != http.StatusOK {
...
func lineageOf(t *testing.T, app *zip.App, org, name string, version int) riskLineage {
	...
	code, body := do(t, app, http.MethodGet,
		fmt.Sprintf("/v1/datasets/%s/lineage?version=%d", name, version), org, nil)
	if code != http.StatusOK {

...
func TestTheOrgComesFromTheURLNotTheBody(t *testing.T) {
	...
	code, body := send(t, app, orgMember("POST", "/v1/entitlements/orgs/acme", "acme",
	...
	if code, _ := send(t, app, orgMember("GET", "/v1/entitlements/orgs/victim", "acme", nil)); code != http.StatusForbidden {

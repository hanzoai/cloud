...
func registerTargetHTTP(t *testing.T, app *zip.App, org string, body map[string]any) targetView {
	t.Helper()
	code, b := do(t, app, http.MethodPost, "/v1/agent/targets", org, body)
...
func TestHTTPTargetsIsolationAndCRUD(t *testing.T) {
	...
	_, b := do(t, app, http.MethodGet, "/v1/agent/targets", "acme", nil)
	var al targetsResp
	mustJSON(t, b, &al)
	if len(al.Targets) != 1 || al.Targets[0].ID != tv.ID {
		t.Fatalf("acme list want its target, got %+v", al.Targets)
	}
	_, b = do(t, app, http.MethodGet, "/v1/agent/targets", "evil", nil)
	var el targetsResp
	mustJSON(t, b, &el)
	if len(el.Targets) != 0 {
	...
	if code, _ := do(t, app, http.MethodGet, "/v1/agent/targets/"+tv.ID, "evil", nil); code != http.StatusNotFound {
		t.Fatalf("evil GET acme target want 404, got %d", code)
	}
	if code, _ := do(t, app, http.MethodPatch, "/v1/agent/targets/"+tv.ID, "evil", map[string]any{"status": TargetOffline}); code != http.StatusNotFound {
		t.Fatalf("evil PATCH acme target want 404, got %d", code)
	}
	if code, _ := do(t, app, http.MethodDelete, "/v1/agent/targets/"+tv.ID, "evil", nil); code != http.StatusNotFound {
		t.Fatalf("evil DELETE acme target want 404, got %d", code)
	}
	// The target survived evil's attempts.
	if code, _ := do(t, app, http.MethodGet, "/v1/agent/targets/"+tv.ID, "acme", nil); code != http.StatusOK {
		t.Fatalf("acme target must survive, got %d", code)
	}

	// Anonymous (X-Org-Id without a validated principal) is refused everywhere.
	if code, _ := doNoUser(t, app, http.MethodGet, "/v1/agent/targets", "acme", nil); code != http.StatusForbidden {
		t.Fatalf("anon list want 403, got %d", code)
	}
	if code, _ := doNoUser(t, app, http.MethodPost, "/v1/agent/targets", "acme", map[string]any{"label": "x"}); code != http.StatusForbidden {
		t.Fatalf("anon register want 403, got %d", code)
	}

	// Validation: bad kind / bad status / missing label → 400.
	if code, _ := do(t, app, http.MethodPost, "/v1/agent/targets", "acme", map[string]any{"label": "x", "kind": "toaster"}); code != http.StatusBadRequest {
		t.Fatalf("bad kind want 400, got %d", code)
	}
	if code, _ := do(t, app, http.MethodPost, "/v1/agent/targets", "acme", map[string]any{"label": "x", "status": "melting"}); code != http.StatusBadRequest {
		t.Fatalf("bad status want 400, got %d", code)
	}
	if code, _ := do(t, app, http.MethodPost, "/v1/agent/targets", "acme", map[string]any{"label": "  "}); code != http.StatusBadRequest {
		t.Fatalf("blank label want 400, got %d", code)
	}

	// Patch mutates; delete removes.
	code, b := do(t, app, http.MethodPatch, "/v1/agent/targets/"+tv.ID, "acme", map[string]any{"status": TargetOffline, "capacity": "8 vCPU / 32G"})
	...
	if code, _ := do(t, app, http.MethodDelete, "/v1/agent/targets/"+tv.ID, "acme", nil); code != http.StatusOK {
		t.Fatalf("delete want 200, got %d", code)
	}
	if code, _ := do(t, app, http.MethodGet, "/v1/agent/targets/"+tv.ID, "acme", nil); code != http.StatusNotFound {
...
func TestHTTPSessionTargetAssociation(t *testing.T) {
	...
	// A bogus target → 400.
	if code, _ := do(t, app, http.MethodPost, "/v1/agent/sessions", "acme", map[string]any{"agent": "dev", "target": "does-not-exist"}); code != http.StatusBadRequest {
	...
	if code, _ := do(t, app, http.MethodPost, "/v1/agent/sessions", "acme", map[string]any{"agent": "dev", "target": evilTgt.ID}); code != http.StatusBadRequest {
	...
	code, b := do(t, app, http.MethodPatch, "/v1/agent/sessions/"+plain.ID, "acme", map[string]any{"target": tgt.ID})
	if code != http.StatusOK {
	...
	code, b = do(t, app, http.MethodPatch, "/v1/agent/sessions/"+plain.ID, "acme", map[string]any{"target": ""})
	if code != http.StatusOK {
	...
	if code, _ := do(t, app, http.MethodPatch, "/v1/agent/sessions/"+plain.ID, "acme", map[string]any{"target": evilTgt.ID}); code != http.StatusBadRequest {
	...
	_, b = do(t, app, http.MethodGet, "/v1/agent/targets/"+tgt.ID, "acme", nil)
	var td targetView
	mustJSON(t, b, &td)
...
func TestHTTPSessionContextAndLastEvent(t *testing.T) {
	...
	if code, _ := do(t, app, http.MethodPost, "/v1/agent/sessions/"+sv.ID+"/events", "acme",

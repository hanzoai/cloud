...
func init() {
	...
	zip.Describe("github.com/hanzoai/cloud/apps/agents GET /v1/agent", zip.Doc{
		...
		Fields: map[string]string{
			...
			"agentView.consumed_micro_usd": "ConsumedMicroUSD is what has been spent in the current period, in micro-USD.\nIt is settled from what the gateway reported, not from the quote.",
			...
			"agentView.max_task_micro_usd": "MaxTaskMicroUSD is the ceiling for a single run, in micro-USD. A session\ncannot exceed it even when the period cap still has room, so one runaway\ntask cannot consume a month.",
			...
			"agentView.period":             "Period is the window the cap resets on: day, week or month.",
	...
	zip.Describe("github.com/hanzoai/cloud/apps/agents GET /v1/agent/:ref", zip.Doc{
		...
		Fields: map[string]string{
			...
			"agentRunView.micro_usd":        "MicroUSD is what the run spent, integer micro-USD. Absent when nothing was metered.",
	...
	zip.Describe("github.com/hanzoai/cloud/apps/agents GET /v1/agent/:ref/spend", zip.Doc{
		Description: "Answers what one of your org's agents has spent, in integer micro-USD.\n\nIt answers the agent's budget — `cap_micro_usd` per `period`,\n`max_task_micro_usd` per run — with what the current period has consumed,\nwhat remains, and `by_component`: the spend attributed to `model` (every\ncompletion the agent bought), `computer` (the runtime it was resident for)\nand `tool`. A component with no spend is absent, not zero. Every amount is an\ninteger number of micro-USD (1,000,000 = $1); 11902000 is $11.902. Pass\n`by=component` to ask for the breakdown by name — it is the one grouping, and\nthe default.",
		Fields: map[string]string{
			"spendQuery.by":                 "By groups the answer: \"component\" is the only grouping today.",
			"spendQuery.ref":                "Ref is the agent's public id or its org-unique name.",
			"spendView.by_component":        "ByComponent holds only the components with spend. Absent, not zero.",
			"spendView.cap_micro_usd":       "CapMicroUSD is the total this agent may spend within one Period, as an\ninteger number of micro-USD (1,000,000 = $1). It is required at creation: a\ncap of zero would mean no limit and no per-agent spend record at all.",
			"spendView.consumed_micro_usd":  "ConsumedMicroUSD is what has been spent in the current period, in micro-USD.\nIt is settled from what the gateway reported, not from the quote.",
			"spendView.max_task_micro_usd":  "MaxTaskMicroUSD is the ceiling for a single run, in micro-USD. A session\ncannot exceed it even when the period cap still has room, so one runaway\ntask cannot consume a month.",
			"spendView.period":              "Period is the window the cap resets on: day, week or month.",
			"spendView.period_started_at":   "PeriodStartedAt is when the current period began, as a Unix second.",
			"spendView.ref":                 "Ref names the agent this spend belongs to.",
			"spendView.remaining_micro_usd": "RemainingMicroUSD is what the cap still allows this period, in micro-USD.",
		},
	})
	...
	zip.Describe("github.com/hanzoai/cloud/apps/agents PATCH /v1/agent/:ref", zip.Doc{
		...
		Fields: map[string]string{
			...
			"updateAgentIn.cap_micro_usd":      "The budget, any part of it. Changing the period opens a new window.",
			...
			"updateAgentIn.max_task_micro_usd": "MaxTaskMicroUSD is the ceiling for a single run, in micro-USD. A session\ncannot exceed it even when the period cap still has room, so one runaway\ntask cannot consume a month.",
			...
			"updateAgentIn.period":             "Period is the window the cap resets on: day, week or month.",
	...
	zip.Describe("github.com/hanzoai/cloud/apps/agents POST /v1/agent", zip.Doc{
		...
		Fields: map[string]string{
			...
			"createAgentIn.max_task_micro_usd": "MaxTaskMicroUSD is the ceiling for a single run, in micro-USD. A session\ncannot exceed it even when the period cap still has room, so one runaway\ntask cannot consume a month.",
			...
			"createAgentIn.period":             "Period is the window the cap resets on: day, week or month.",
	...
	zip.Describe("github.com/hanzoai/cloud/apps/agents POST /v1/agent/sessions", zip.Doc{
		...
		Fields: map[string]string{
			...
			"registerReq.budget_micro_usd": "BudgetMicroUSD caps what this session may spend, integer micro-USD. Zero\nmeans no cap; a cap can be raised or removed later, never added.",
	...
	zip.Describe("github.com/hanzoai/cloud/apps/agents POST /v1/agent/sessions/:id/budget", zip.Doc{
		Description: "Sets, raises, or removes a session's cap.\n\n  - a replacement must be strictly greater than what the session has consumed\n  - removal is one-way: a session whose cap was removed cannot take one again,\n    and a session created without one cannot be given one\n  - raising or removing the cap resumes work that paused at it",
		Fields: map[string]string{
			"sessionBudgetIn.budget_micro_usd":     "BudgetMicroUSD is the new cap, or null to remove the cap for good.",
			"sessionBudgetIn.id":                   "ID is the session, from the path.",
			"sessionBudgetView.budget_micro_usd":   "BudgetMicroUSD is this session's own ceiling in micro-USD, beyond the agent's.\nA replacement must strictly exceed what the session has already consumed, and\nremoving it is one-way.",
			"sessionBudgetView.budget_removed":     "BudgetRemoved says the session's own cap was taken off. It cannot be put back.",
			"sessionBudgetView.consumed_micro_usd": "ConsumedMicroUSD is what has been spent in the current period, in micro-USD.\nIt is settled from what the gateway reported, not from the quote.",
			"sessionBudgetView.id":                 "ID names the session.",
			"sessionBudgetView.status":             "Status is the session's state: running, paused at its cap, or done.",
		},
	})

...
func (s *Store) copyOrgTo(ctx context.Context, org string, dst *Store) error {
	...
	copies := []struct {
		...
		{"agents",
			...
			`INSERT OR IGNORE INTO agents (` + agentCols + `) VALUES (` + placeholders(agentCols) + `)`},
		{"runs",
			`SELECT ` + runCols + ` FROM agent_runs WHERE org=?`,
			`INSERT OR IGNORE INTO agent_runs (` + runCols + `) VALUES (` + placeholders(runCols) + `)`},
		{"sessions",
		...
		{"events",
			...
			`INSERT OR IGNORE INTO agent_session_events (` + eventCols + `) VALUES (` + placeholders(eventCols) + `)`},
		{"targets",
			`SELECT ` + targetCols + ` FROM agent_targets WHERE org=?`,
			`INSERT OR IGNORE INTO agent_targets (` + targetCols + `) VALUES (` + placeholders(targetCols) + `)`},
		{"claim keys",

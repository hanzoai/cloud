...
// Routes registers the member routes this package owns outright. The collection
// routes (GET and POST /v1/sandboxes) and the member GET/DELETE are shared with
// the compute surface and dispatch through the exported handlers below, because
// one resource may not be registered twice.

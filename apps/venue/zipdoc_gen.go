...
func init() {
	...
	zip.Describe("GET /v1/cloud/accounts", zip.Doc{
		...
		Fields: map[string]string{
			"accountView.account":    "Account is the provider's human label for it, e.g. the DigitalOcean team\nemail.",
			"accountView.clusters":   "Clusters is the fleet names this account currently owns. Unlinking detaches\nexactly these and nothing else.",
			"accountView.externalId": "ExternalID is the provider's own identifier for the account — a\nDigitalOcean account uuid, an AWS account id, a GCP project.",
			"accountView.label":      "Label is the org-chosen name for this account within the provider, which is\nhow a second account at the same provider is addressed. Defaults to\n\"default\".",
			"accountView.linkedAt":   "LinkedAt is when the account was first linked, RFC3339 UTC. Re-linking the\nsame label keeps the original.",
			"accountView.project":    "Project is the fleet shard the account's clusters were folded into,\nrecorded at link time so a later sync or unlink acts on the same shard.",
			"accountView.provider":   "Provider is the cloud the account belongs to: digitalocean, aws, gcp or\nazure.",
			"accountView.syncedAt":   "SyncedAt is when it was last discovered, RFC3339 UTC.",
			"accountsView.accounts":  "Accounts is every account this org has linked, across all providers. Empty\nwhen it has linked none.",
		},
	})

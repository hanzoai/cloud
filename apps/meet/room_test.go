...
// roomID is a room's own id within a workspace. It carries a hyphen and, in the
...
func askCall(t *testing.T, app *zip.App, workspace, room, bearer string) (int, string) {
	t.Helper()
	req := httptest.NewRequest(http.MethodGet,
		"/v1/meet/call?workspace="+workspace+"&room="+room, nil)
	if bearer != "" {
...
// PARSES back to the same workspace.
//
// It matters because the parse is an authorization input. workspace() picks the
// segment the caller's membership is checked against, so a composition that did not
// round-trip would check the caller against a workspace that is not the room's — and
...
func TestRoomNameRoundTripsThroughTheMembershipParse(t *testing.T) {
	for _, id := range []string{roomID, "dm-1", "a_b_c", "", "with spaces", "ünïcode"} {
		if got := workspace(roomName(workspaceA, id)); got != workspaceA {
			t.Errorf("workspace(roomName(%q, %q)) = %q, want %q", workspaceA, id, got, workspaceA)
		}
	}
}

// TestSplitsRefusesAWorkspaceThatWouldNotParseBack pins the refusal that MAKES the
// round trip above total rather than usually-true.
//
// A workspace carrying the separator parses back as a PREFIX of itself, so the
// membership question would be asked about a different workspace than the one named
// — "a_b" checked as "a" — and a member of the prefix would be seated in a room of a
// workspace they are not in. The controls are the reason this is a refusal and not a
// fold: folding the character maps two workspaces onto one media room, which is the
// same defect wearing a repair.
func TestSplitsRefusesAWorkspaceThatWouldNotParseBack(t *testing.T) {
	for _, bad := range []string{"a_b", "_", "_leading", "trailing_", ""} {
		if splits(bad) {
			t.Errorf("splits(%q) = true, but it does not round-trip", bad)
		}
	}
	for _, ok := range []string{workspaceA, "plain", "with-hyphens"} {
		if !splits(ok) {
...
// TestCallResolvesARoomToItsMediaName is the integration: a member of the workspace
...
func TestCallResolvesARoomToItsMediaName(t *testing.T) {
	...
	code, body := askCall(t, app, workspaceA, roomID, access(t, "person-42"))
	if code != http.StatusOK {
		t.Fatalf("got %d (%s), want 200", code, body)
	}
	got := decodeCall(t, body)
	if want := workspaceA + "_" + roomID; got.Name != want {
...
// who could join it. Without this the operation is a workspace-membership oracle for
...
func TestCallRefusesANonMember(t *testing.T) {
	app := mountWith(t, keyFileWith(t, keyBody("devkey", "devsecret")),
		holds(map[string]string{workspaceA: token.RoleMember}))

	other := "22222222-2222-4222-8222-222222222222"
	code, body := askCall(t, app, other, roomID, access(t, "person-42"))
	if code != http.StatusUnauthorized {
		t.Fatalf("got %d (%s), want 401 for a workspace the caller is not in", code, body)
	}
...
func TestCallRefusesAnAnonymousCaller(t *testing.T) {
	app := mount(t, "devkey", "devsecret")
	if code, body := askCall(t, app, workspaceA, roomID, ""); code != http.StatusUnauthorized {
...
// A workspace carrying the separator is the one way a caller could smuggle a
// composed name in through the workspace half, and it is refused with a 400 rather
// than parsed.
func TestCallRefusesAComposedName(t *testing.T) {
	app := mount(t, "devkey", "devsecret")
	code, body := askCall(t, app, workspaceA+"_"+roomID, roomID, access(t, "person-42"))

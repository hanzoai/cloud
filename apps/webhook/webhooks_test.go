...
func TestRegistryCRUDAndSecret(t *testing.T) {
	...
	// create
	code, body := do(t, app, http.MethodPost, "/v1/webhook", "acme", map[string]any{
	...
	code, body = do(t, app, http.MethodGet, "/v1/webhook/"+created.ID, "acme", nil)
	...
	code, body = do(t, app, http.MethodGet, "/v1/webhook", "acme", nil)
	if code != http.StatusOK {
	...
	code, body = do(t, app, http.MethodPut, "/v1/webhook/"+created.ID, "acme", map[string]any{
	...
	// delete
	code, _ = do(t, app, http.MethodDelete, "/v1/webhook/"+created.ID, "acme", nil)
	if code != http.StatusNoContent {
		t.Fatalf("delete want 204, got %d", code)
	}
	code, _ = do(t, app, http.MethodGet, "/v1/webhook/"+created.ID, "acme", nil)
...
func TestRegistryAuth(t *testing.T) {
	...
	if code, _ := do(t, app, http.MethodPost, "/v1/webhook", "acme", map[string]any{"url": "http://x.test/h"}); code != http.StatusBadRequest {
		t.Fatalf("http:// url want 400, got %d", code)
	}
	// missing url rejected
	if code, _ := do(t, app, http.MethodPost, "/v1/webhook", "acme", map[string]any{"events": []string{"a.b"}}); code != http.StatusBadRequest {
		t.Fatalf("missing url want 400, got %d", code)
	}
	// bad status rejected
	if code, _ := do(t, app, http.MethodPost, "/v1/webhook", "acme", map[string]any{"url": "https://x.test/h", "status": "paused"}); code != http.StatusBadRequest {
...
func TestRegistryOrgIsolation(t *testing.T) {
	...
	code, body := do(t, app, http.MethodPost, "/v1/webhook", "borg", map[string]any{
	...
	if code, _ := do(t, app, http.MethodGet, "/v1/webhook/"+bEndpoint.ID, "acme", nil); code != http.StatusNotFound {
		t.Fatalf("A GET B's endpoint want 404, got %d", code)
	}
	if code, _ := do(t, app, http.MethodPut, "/v1/webhook/"+bEndpoint.ID, "acme", map[string]any{"url": "https://evil.test/h"}); code != http.StatusNotFound {
		t.Fatalf("A PUT B's endpoint want 404, got %d", code)
	}
	if code, _ := do(t, app, http.MethodDelete, "/v1/webhook/"+bEndpoint.ID, "acme", nil); code != http.StatusNotFound {
		t.Fatalf("A DELETE B's endpoint want 404, got %d", code)
	}

	// B still has its endpoint intact.
	code, body = do(t, app, http.MethodGet, "/v1/webhook/"+bEndpoint.ID, "borg", nil)
	if code != http.StatusOK {
...
func createEP(t *testing.T, app *zip.App, org string, in map[string]any) Endpoint {
	t.Helper()
	code, body := do(t, app, http.MethodPost, "/v1/webhook", org, in)
	if code != http.StatusCreated {
		t.Fatalf("create want 201, got %d (%s)", code, body)
	}
	var e Endpoint
...
func getEP(t *testing.T, app *zip.App, org, id string) Endpoint {
	t.Helper()
	code, body := do(t, app, http.MethodGet, "/v1/webhook/"+id, org, nil)
	if code != http.StatusOK {
...
func getDeliveries(t *testing.T, app *zip.App, org, id, query string) []DeliveryRow {
	t.Helper()
	code, body := do(t, app, http.MethodGet, "/v1/webhook/"+id+"/deliveries"+query, org, nil)
	if code != http.StatusOK {
...
func TestDeliveriesListFilterAndLimit(t *testing.T) {
	...
	if code, _ := do(t, app, http.MethodGet, "/v1/webhook/"+ep.ID+"/deliveries", "borg", nil); code != http.StatusNotFound {
		t.Fatalf("cross-org deliveries want 404, got %d", code)
	}
	// unknown id → 404.
	if code, _ := do(t, app, http.MethodGet, "/v1/webhook/wh_missing/deliveries", "acme", nil); code != http.StatusNotFound {
...
func TestTestSendDeliversAndRecords(t *testing.T) {
	...
	code, body := do(t, app, http.MethodPost, "/v1/webhook/"+ep.ID+"/test", "acme", nil)
	if code != http.StatusOK {
...
func TestRotateSecretInvalidatesOld(t *testing.T) {
	...
	if code, _ := do(t, app, http.MethodPost, "/v1/webhook/"+ep.ID+"/test", "acme", nil); code != http.StatusOK {
	...
	code, body := do(t, app, http.MethodPost, "/v1/webhook/"+ep.ID+"/secret", "acme", nil)
	if code != http.StatusOK {

...
// webhook surface is /v1/webhook, top-level, never under /v1/billing, and it fans out
// EVERY platform event, not just commerce's.
//
// TWO ORTHOGONAL HALVES.
//
//   - REGISTRY (api.go, store.go) — /v1/webhook CRUD. An org manages ONLY its own
...
func Use(app cloud.Router, deps cloud.Deps) error {
	...
	// /v1/webhook fully serving while the consumer retries in the background.
	st.disp.start()

	b.Log.Info("webhooks mounted", "prefix", "/v1/webhook")
	return nil
}
...
// routes registers the /v1/webhook CRUD surface as TYPED ops: one registry
...
// "", …)` names /v1/webhook/ — with a trailing slash — and op.Path is the
// identity every projection keys on. That is what the untyped registration did,
// and the published subset carried /v1/webhook/ for a collection every caller
...
func routes(app cloud.Router, s *cloud.Service[*state]) error {
	...
	o := ops{s: s}
	zip.Get(zapp, "/v1/webhook", o.listEndpoints)
	zip.Post(zapp, "/v1/webhook", o.createEndpoint, zip.WithStatus(http.StatusCreated))

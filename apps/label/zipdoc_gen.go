...
func init() {
	zip.Describe("GET /v1/risk/labels", zip.Doc{
		...
		Fields: map[string]string{
			"riskLabelRecord.by":       "By is the identity that asserted, stamped server-side at the write.",
			"riskLabelRecord.knowable": "Knowable is when THIS PLANE could first have answered with the assertion:\nthe later of Seen and the server clock at the write, derived server-side.\nIt is the instant the leakage guard compares, so it is published beside the\nclaim it was derived from — an answer whose rule nobody can see is one\nnobody can check.",
			"riskLabelRecord.seen":     "Seen is when the FILER said the assertion became knowable. It is\nprovenance: it is recorded and published, and it decides nothing.",
			"riskLabelRecord.wrote":    "Wrote is the server clock at the write. It is the only time on the record\nthe tenant did not supply, and it is what retention measures against.",
			"riskLabelsIn.from":        "From and To bound the EVENT time, half-open, RFC 3339.",
			"riskLabelsIn.kind":        "Kind and Subject narrow to one entity.",
			"riskLabelsIn.limit":       "Limit caps the page. Out of range takes the plane's own bound.",
			"riskLabelsIn.source":      "Source narrows to one asserter — the read that answers \"what has commerce\ntold us\", separately from \"what has an analyst told us\".",
			"riskLabelsOut.count":      "Count is how many this page holds. It is not a total: a total over an\nunbounded append-only log is a full scan of a single-writer file.",
			"riskLabelsOut.labels":     "Labels is the page, newest event first.",
		},
	})
	zip.Describe("GET /v1/risk/labels/coverage", zip.Doc{
	...
	zip.Describe("GET /v1/risk/labels/vocabulary", zip.Doc{
		...
		Fields: map[string]string{
			"riskLabelVocabulary.kinds":      "Kinds, Dispositions and Sources are the closed vocabularies. A value\noutside them is refused at the door.",
			"riskLabelVocabulary.precedence": "Precedence is the sources in the order that resolves a conflict, strongest\nfirst. It is DERIVED from the same declaration the resolver reads, so the\npublished order is the enforced order and cannot drift from it.",
			"riskLabelVocabulary.retention":  "Retention is the platform floor in days: no tenant may dispose of a label\nyounger than this, because a label can be the input to an adverse action.",
	...
	zip.Describe("POST /v1/risk/labels", zip.Doc{
	...
	zip.Describe("POST /v1/risk/labels/hold", zip.Doc{
	...
	zip.Describe("POST /v1/risk/labels/resolve", zip.Doc{

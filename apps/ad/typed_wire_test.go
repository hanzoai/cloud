...
// This file is the MEASUREMENT that typing /v1/ad did not move its wire, and
...
var untypedByDesign = map[string]string{
	"POST /v1/ad/campaigns/{id}/launch": "launch is deliberately BODY-TOLERANT. It reads its " +
...
func adOps(t *testing.T) (served map[string]bool, typed map[string]*openapi.Operation) {
	t.Helper()
	app := mountApp(t)
	doc, err := openapi.Spec(app, openapi.Info{Title: "ad", Version: "v1"})
	if err != nil {
		t.Fatalf("spec: %v", err)
	}
	...
	ours := func(p string) bool { return strings.HasPrefix(p, "/v1/ad/") }
...
func TestLaunchStillIgnoresAMalformedBody(t *testing.T) {
	app := mountApp(t)
	code, body := do(t, app, http.MethodPost, "/v1/ad/campaigns", "acme",
	...
	code, body = do(t, app, http.MethodPost, "/v1/ad/campaigns/"+created.ID+"/launch", "acme",
...
func TestTheQueryStringCannotRedirectAWrite(t *testing.T) {
	app := mountApp(t)
	code, body := do(t, app, http.MethodPost, "/v1/ad/campaigns?name=HIJACK&status=active&budget=999", "acme",
...
func TestThePathIsTheAddressingAuthority(t *testing.T) {
	app := mountApp(t)
	_, body := do(t, app, http.MethodPost, "/v1/ad/campaigns", "acme", []byte(`{"name":"Mine","platform":"meta"}`))
	var mine AdCampaign
	if err := json.Unmarshal(body, &mine); err != nil {
		t.Fatalf("decode: %v", err)
	}
	_, body = do(t, app, http.MethodPost, "/v1/ad/campaigns", "beta", []byte(`{"name":"Theirs","platform":"meta"}`))
	...
	code, body := do(t, app, http.MethodPut, "/v1/ad/campaigns/"+mine.ID, "acme",
	...
	code, body = do(t, app, http.MethodGet, "/v1/ad/campaigns/"+theirs.ID, "beta", nil)
	if code != http.StatusOK {
...
func TestFailsClosedWithoutAValidatedPrincipal(t *testing.T) {
	...
	for _, r := range []struct{ method, path string }{
		{http.MethodGet, "/v1/ad/summary"},
		{http.MethodGet, "/v1/ad/campaigns"},
		{http.MethodPost, "/v1/ad/campaigns"},
		{http.MethodGet, "/v1/ad/campaigns/camp_x"},
		{http.MethodPut, "/v1/ad/campaigns/camp_x"},
		{http.MethodDelete, "/v1/ad/campaigns/camp_x"},
	} {
		var body []byte
...
func TestListAndSummaryEnvelopes(t *testing.T) {
	app := mountApp(t)
	code, body := do(t, app, http.MethodGet, "/v1/ad/campaigns", "acme", nil)
	if code != http.StatusOK {
	...
	code, body = do(t, app, http.MethodGet, "/v1/ad/summary", "acme", nil)
	if code != http.StatusOK {
...
func TestDeleteAnswers204WithNoBody(t *testing.T) {
	app := mountApp(t)
	_, body := do(t, app, http.MethodPost, "/v1/ad/campaigns", "acme", []byte(`{"name":"Gone","platform":"x"}`))
	var c AdCampaign
	if err := json.Unmarshal(body, &c); err != nil {
		t.Fatalf("decode: %v", err)
	}
	code, got := do(t, app, http.MethodDelete, "/v1/ad/campaigns/"+c.ID, "acme", nil)
	...
	if code, got := do(t, app, http.MethodDelete, "/v1/ad/campaigns/"+c.ID, "acme", nil); code != http.StatusNotFound {
...
func TestEveryPublishedFieldIsDescribed(t *testing.T) {
	doc, err := openapi.Spec(mountApp(t), openapi.Info{Title: "ad", Version: "v1"})
	if err != nil {
		t.Fatalf("spec: %v", err)
	}

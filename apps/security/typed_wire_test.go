...
// untypedByDesign is the CLOSED list of security operations that are NOT typed ops.
...
func opsUnderTest(t *testing.T) (served map[string]bool, typed map[string]string, schemas map[string]any) {
	t.Helper()
	app := mountApp(t)
	doc, err := openapi.Spec(app, openapi.Info{Title: "security", Version: "v1"})
	if err != nil {
		t.Fatalf("spec: %v", err)
	}
	...
	ours := func(p string) bool { return strings.HasPrefix(p, "/v1/security") }
...
// TestEveryRouteIsTypedOrNamed fails when an security operation is neither a typed
// op nor named above — so the next route added here is typed by default.
func TestEveryRouteIsTypedOrNamed(t *testing.T) {
	served, typed, _ := opsUnderTest(t)
	if len(served) != 7 {
		t.Errorf("security serves %d operations, expected 7 — update this gate deliberately", len(served))
	}
	...
	for key := range untypedByDesign {
		if !served[key] {
			t.Errorf("untypedByDesign names %q, which security no longer serves", key)
		}
	}
}
...
func TestEveryTypedOpIsDescribed(t *testing.T) {
	...
	if len(typed) == 0 {
		t.Fatal("no typed security ops in the registry at all")
	}
	for key, desc := range typed {
		if strings.TrimSpace(desc) == "" {
			t.Errorf("%s has no description — run: go generate -run zipdoc ./apps/security/...", key)
		}
	}
}
...
func TestEveryPublishedFieldIsDescribed(t *testing.T) {
	...
	if len(schemas) == 0 {
		t.Fatal("no security schemas in the typed registry at all")
	}
	var bare []string
